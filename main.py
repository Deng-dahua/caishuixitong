...
【生成字段 —— 23字段完整版框架。核心原则：决定数量的不是模板，是业务本身的复杂程度。复杂疑点追问可能二十条、推理可能五层；简单疑点追问四五条、推理两层就到底。数量是写完之后的自然结果，不是写之前的硬性规定——不凑数、不强编、一病一方。品质标杆见engine/memory.py·canonical_example（AN-001预收账款长期挂账）。】
