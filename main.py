...
【生成字段 —— 23字段完整版框架。穿透追问穷举到无话可问，推理链推到逻辑尽头。数量是写完后的自然结果，不是写之前的硬性指标。品质标杆见engine/memory.py·canonical_example（AN-001预收账款长期挂账·13条追问穷举·4层推理到底·4种合法解释已穷举）。】

- direction: 推理到逻辑尽头。从现象出发，每层追问"为什么"，直到无法再追问。最后一层落地定性。三层结束条件: 定性落地/证据尽头/逻辑闭环。每层格式【推理第N层：XX法则】依赖证据：XX → 结论：XX。层数自然呈现，复杂多简单少。
- drill_questions: 穷举到无话可问。三组递进(事实→证据→逻辑)，每组追问到穷尽。有效追问标准: 指向新信息/可验证事实/与潜台词对应。三组结束条件: 事实穷尽/证据穷尽/逻辑穷尽。格式 Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。

。
- phenomena: 典型表现枚举(非穷举)，每种格式: 表现+典型行业/场景。至少5种+兜底条款+排除条件。排除条件: 什么情况下类似表现不属于本异常。
- drill_questions: 至少3组(事实/证据/逻辑)，每组≥2条，总计≥6条。格式 Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。确实简单可减至4条并注明原因。三组递进排列。
- focus: 策略层——舞弊手法预判。格式: 手法名称: 操作方式→识别要点。用①②③④标注。与drill_questions分工: focus=策略层(预判)，drill=执行层(提问)。
- normal_reason: 至少4种情形。格式 {情形}——需提供{具体证据}，证据可核验。确无合法情形时如实写0-3种并注明原因。降低下限避免注水凑数。
- determination: 三路径对应证据链三档定级——无法证明→线索(单源)→定性存疑不入结论；部分证明→强证据(2源)→定性涉嫌；完整证明→铁证(≥3源)→定性认定。每路径定义进入条件。
- risk_table: 覆盖实际涉及税种/维度，不设数量下限。格式 税种:具体风险描述。只列实际涉及的。部分疑点只影响单一税种（如印花税漏缴），强制多税种会注水。
- evidence: 四层框架+金额分级+优先级。大额(>10万)分AB场景(自提vs直运)；中额(1-10万)≥2维度；小额(<1万)抽样。每层标注优先级(必须/应当/可以)。排雷层与normal_reason联动。
- action: 至少5步，每步标注动作类型(现场核查/纸面比对/外调走访/联网核查)+预期产出。纯申报类异常允许注明现场核查不可行并补充替代手段。
...
【禁止行为】禁止凑数凑字。追问穷举到无话可问，推理到逻辑尽头。数量是自然结果，不是硬性指标。一病一方。
