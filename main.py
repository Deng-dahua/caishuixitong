...
def _domain_document_completeness(docs_list, bank_txs, sal_invs, pur_invs, salaries, social_security, vouchers, inventory,
    ...
    MISSING_DEFS = [
        ("contract", "合同文件缺失", "高风险", 9,
         lambda: f"缺少合同文件：{len(set(str(i.get('buyer',''))[:15] for i in sal_invs if i.get('buyer'))) if sal_invs else 0}个销项客户+{len(set(str(i.get('seller',''))[:15] for i in pur_invs if i.get('seller'))) if pur_invs else 0}个供应商。"
                f"合同是证明交易真实性的第一份书面证据。按供应商性质可分为三类："
                f"① 主营业务供应商（纺织原料/加工/成品等）——必须签合同，交易金额大、频次高，无合同则无法验证交易真实性；"
                f"② 重大支出供应商（单笔或累计>5万元的非日常类）——必须签合同；"
                f"③ 日常费用供应商（加油/酒店/餐饮/税务）——以发票为凭证即可，不需合同。"
                f"被查单位缺失合同的影响集中在第一类和第二类。四流合一缺了合同流，印花税的计税依据也缺失。",
         lambda: f"缺少合同文件——需区分供应商性质：主营业务（纺织原料/加工/成品等）和重大支出必须有合同，日常费用（加油/酒店/餐饮）以发票为凭证即可。"
                f"\n\n① 稽查逐笔质疑主营业务和重大交易无合同的合理性；② 无合同→印花税漏缴→补税+罚款；③ 大额采购无合同→虚开发票嫌疑。注意：日常费用类不需要合同。",
         lambda: "《税收征收管理法》第五十四条；《印花税法》关于应税合同的规定。",
         "① 为主营业务供应商补签购销合同；② 为单笔>5万元的非日常交易补签合同；③ 合同必须明确：双方名称、金额、货物内容、履行期限、违约责任；④ 日常费用类（加油/酒店/餐饮）以发票为凭证即可，不需补签；⑤ 按合同金额补缴印花税。"),
...
def _domain_multi_source_cross(bank_txs, sal_invs, pur_invs, salaries, social_security, vouchers, inventory, db, company_id):
    ...
    if bank_txs and pur_invs and inventory:
        ...
        pay_no_inv = []
        for name, amt in sorted(bank_payees.items(), key=lambda x: -x[1])[:10]:
            matched = any(name[:6] in s for s in inv_sellers)
            if not matched and amt > 5000:
                pay_no_inv.append(f"{name}({amt:,.0f}元)")
        inv_no_pay = []
        for name, amt in sorted(inv_sellers.items(), key=lambda x: -x[1])[:10]:
        ...
        if pay_no_inv:
            findings.append({
                "type": "付款无进项发票（三源交叉）",
                "level": "高风险", "score": 9,
                "detail": f"银行流水中向{len(pay_no_inv)}个供应商付款但无对应进项发票：{'、'.join(pay_no_inv[:3])}等。",
                "description": f"结合银行流水支出、进项发票、存货入库三源交叉比对发现：银行账户向以下供应商支付了货款，但进项发票中未找到对应供应商的开票记录：{'、'.join(pay_no_inv[:5])}。这意味着企业付了款却没有取得发票，存在以下可能：供应商未开票或延迟开票、账外采购、或以采购名义转移资金。",
        ...
        if inv_no_pay:
            findings.append({
                "type": "进项发票无付款记录（三源交叉）",
                "level": "中风险", "score": 7,
                "detail": f"{len(inv_no_pay)}个供应商开具进项发票但银行无付款记录：{'、'.join(inv_no_pay[:3])}等。",
                "description": f"交叉比对发现：以下供应商开具了进项发票，但在银行流水中未找到对应的付款记录：{'、'.join(inv_no_pay[:5])}。这意味着取得了发票但没有付款记录，存在以下可能：以现金方式付款、通过其他账户付款、发票为虚开、或款项尚未支付（挂账）。",
    ...
    if pur_invs:
        ...
        if abnormal_names:
            findings.append({
                "type": "供应商名称异常",
                "level": "中风险", "score": 6,
                "detail": f"发现{len(abnormal_names)}个名称异常的供应商：{'、'.join(abnormal_names[:5])}。",
                "description": f"进项发票供应商中存在{len(abnormal_names)}个名称异常的情况：{'、'.join(abnormal_names[:5])}。供应商名称过短或包含特殊字符可能意味着：发票信息填写不规范、供应商未进行工商登记、或使用了虚假名称开票。",
...
def _domain_triangle_invoice_inventory_payment(pur_invs, inventory, bank_txs):
    ...
    if amt_mismatch > 5:
        ...
        findings.append({
            ...
            "description": f"将进项发票的销方名称与银行付款的对方户名进行双向比对。注意：实际经营中发票与付款天然不是一一对应关系——一次付款可能对应多张发票（合并付款），一张发票可能对应多次付款（分期付款），还存在先付款后到票（预付账款）或先到票后付款（应付账款）的跨期情形。因此，发票名称与付款记录不匹配，不等于交易不真实。\n\n"
                + f"经比对，被查单位{len(pur_invs)}张进项发票中，{amt_mismatch}张（{amt_mismatch/len(pur_invs)*100:.0f}%）的销方名称在当前银行付款记录中找不到名称匹配的付款。涉及采购金额{total_unmatched:,.0f}元，占进项采购总额{pct:.0f}%。\n\n"
                + f"这{amt_mismatch}张\u201c未匹配\u201d发票，可能属于以下情况：\n"
                + f"· 自然跨期：发票已开但付款发生在分析期间之外——或者发票是期末开的（跨至下期付款），或者付款在上期已发生（票后到）。这是最常见也最合理的情况，需要通过拉长银行流水期间或核对应付账款明细来验证。\n"
                + f"· 合并/拆单付款：一张付款凭证对应多张发票，或一张发票对应多笔付款——名称能对上但金额对不上，同样会被系统判定为匹配成功，此类记录已在匹配范围内。\n"
                + f"· 预付账款：付款在先、发票在后——付款时供应商还未开票，发票是在后续期间取得。如果{amt_mismatch}张发票中有属于此类的，对应的付款记录应出现在银行流水的前期（而非当期）。\n"
                + f"· 非对公支付：通过现金、微信、支付宝或个人账户付款——商业上可能属实，但无法在对公账户银行流水中体现，进项税额抵扣在稽查中面临被否定的风险。\n"
                + f"· 赊购未付：货已到、票已开，但货款确实尚未支付——需要应付账款明细账支撑。\n"
                + f"· 虚开发票：没有真实交易，只走票不走钱——这是最需要排除的情况，但也是占比最低的一种。\n\n"
                + f"综合判断：{amt_mismatch}张发票涉及金额{total_unmatched:,.0f}元（占进项{pct:.0f}%），需要逐笔核实属于上述哪种情况。重点是排除最后一种（虚开），同时确认前几种情况有对应的会计记录（应付账款/预付账款明细）佐证。涉及的主要供应商及金额：{examples}等。",
            "how_found": f"查阅被查单位提供的进项发票（共{len(pur_invs)}张）和银行账户流水。将进项发票销方名称与银行流水付款对方户名逐条比对，发现{amt_mismatch}张发票的供应商名称在当前银行付款记录中无法匹配。",
            "tax_impact": f"① 若为自然跨期或预付账款——风险较低，需补充前后期间银行流水或应付/预付账款明细佐证；② 若非对公支付——进项税额抵扣可能被否定，需提供第三方交易记录证明资金流真实性；③ 若确属赊购未付——核对进项税额转出金额；④ 若被认定为虚开发票→刑事责任（《刑法》第205条）+行政罚款+纳税信用降级。注意：发票与付款天然不是1:1关系，未匹配不等于虚开。",
            "policy_ref": "《发票管理办法》第二十二条（禁止虚开）；《国家税务总局关于加强增值税征收管理若干问题的通知》（三流一致要求）；《刑法》第二百零五条（虚开增值税专用发票罪）",
            "suggestion": f"要求被查单位逐笔说明{amt_mismatch}张发票的货款情况：①若已付款但银行流水未覆盖——补充提供前期银行流水或付款凭证；②若为预付账款——提供前期付款记录和预付账款明细账；③若为分期/合并付款——提供付款计划和对账明细；④若通过现金/第三方支付——提供交易记录和收款收据（需供应商签章）；⑤若确属未付款——核对应付账款明细，必要时进项税额转出。无法说明的按虚开发票立案。",
            "category": "三角验证",
            "rule_id": 1502,
            "source_chain": "资金流-发票收付款匹配",
        })
