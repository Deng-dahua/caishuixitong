...
【生成字段 —— 23字段完整版框架。每个字段的下限为最低要求，超过下限以异常点实际出发；达不到下限如实注明原因】
- direction: 推理链【推理第一层：XX】→...至少4层，每层50-80字，末层落地定性。复杂可5-6层，简单如实写并注明。
- phenomena: 异常定义+5种常见表现(①②③④⑤)+适用范围兜底。
- drill_questions: 至少8条分3组，每组标注组名。格式 Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。角度少时如实写并注明。
- focus: 最常见的舞弊手法，用①②③④逐条标注。
- normal_reason: 至少4种情形。格式 {情形}——需提供{具体证据}，证据可核验。确无合法情形时如实写0-3种并注明原因。降低下限避免注水凑数。
- determination: 三条推理路径——无法证明→定性+后果；部分证明→分类处理；完整证明→排除风险。结尾附应对总原则。
- risk_table: 覆盖实际涉及税种/维度，不设数量下限。格式 税种:具体风险描述。只列实际涉及的。部分疑点只影响单一税种（如印花税漏缴），强制多税种会注水。
- evidence: 四层框架——货物流+合同资金流+业务合理性+排雷。大额业务区分AB场景(自提vs直运)。
- action: 至少5步，必须含现场核查(实地查看/调取记录/外调走访)，不能全是纸面比对。
- suggestion: 明确查实后的处置方式(补税/罚款/移送)，分税种说明。
- threshold: 量化阈值+前置条件(如销售额门槛)。二元异常写=是即触发。预警等级黄/橙/红。
- remedy: 三阶段——自查(稽查前)→应对(稽查中·含话术策略)→制度(长期)。

【禁止行为】禁止凑数凑字。达不到下限就注明原因。一病一方。
