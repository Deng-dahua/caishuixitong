...
【生成字段 —— 框架指导，非硬编码模板。根据该异常点实际特征灵活决定深度和广度】
- direction: 推理链，从表面现象逐层深入到法律定性。以说清楚为优先级，不以层数为目标。复杂异常点可能5-6层，简单异常点2层也可能足够。
- phenomena: 异常现象描述+该异常点的具体表现。以准确描述异常特征为核心，不追求字数。
- drill_questions: 分2-3组追问，每组2-4条。每条"Q:问题→潜台词:稽查真实意图。A:假设回答"。追问数量=该异常点的稽查角度数量。必须穷举所有逃税可能但不凑数。
- focus: 稽查重点指向，列出该异常点最需要关注的几个方向。
- normal_reason: 列举真实存在的合法合规解释。如果该异常点几乎没有正常情形（如虚开发票），写0-1种即可并注明"极少有合法情形"。不强编不存在的正常情形。
- determination: 1-3条定性路径，路径数量=该异常点可能的定性方向数量。必须区分"疑点→核查→定性"的递进逻辑。
- risk_table: 只列该异常点实际涉及的税种。每个税种的描述必须具体到"此异常在此税种下如何被处理"。
- action: 可执行的核查步骤，步骤数目和内容取决于该异常点的核查复杂度。
- evidence: 具体到文件名层级（如"购销合同的价格条款"），不同异常点证据组合不同。
- threshold: 基于税法或行业数据的量化触发条件。二元异常写"=是即触发"，连续异常给具体门槛。
- remedy: 三阶段(自查→应对→制度)，内容取决于异常点性质（轻微/严重/系统性）。

【禁止行为】禁止为了凑字数、凑层数、凑条数而生成空洞内容。宁可一条精准的追问，不要三条废话追问。
