...
【生成字段 —— 23字段完整版框架。穿透追问穷举至稽查终点（证实违法或排除违法），问题间环环相扣、因果递进。推理链推到定性落地或排除风险为止。品质标杆见engine/memory.py·canonical_example（AN-001预收账款长期挂账·追问环环相扣致稽查终点·推理因果递进到底）。】

- direction: 推理至稽查终点——证实违法或排除违法。层与层之间因果递进、环环相扣。推理在两种终点之一停止:①证实违法(落地定性，指向铁证);②排除违法(确认无违规)。每层格式【推理第N层：XX法则】依赖证据：XX → 结论：XX。
- drill_questions: 穷举至稽查终点——问题间环环相扣，每个答案自然引出下一个问题。追问在证实违法或排除违法时停止。有效追问标准: 答案引出下一个问题/可验证/推动稽查进程。格式 Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。
...
【禁止行为】禁止凑数凑字。追问穷举至稽查终点（证实或排除），问题环环相扣、因果递进。一病一方。
