...
async def smart_update_rules(request: Request):
    ...
    summary_lines = [
        ...
        "1.【法规名称准确性】引用法条名称是否与现行法律完全一致——严禁编造或使用AI生成的不存在法条（如《增值税法》尚未通过时应引用《增值税暂行条例》；《个人所得税法》已施行但引用时应确认条款号与现行版本一致）",
        "2.【法规时效性】引用的法规是否现行有效——已被废止/修订/替代的法规必须移除或更新；标注（试行）的文件需核实是否已转正或废止；引用财税发文（如财税[2003]158号）需确认未被后续文件替代",
        "3.【刑事标准准确性】刑事责任移送门槛的量化标准是否正确——虚开专票罪看「税款数额」而非「发票金额」、入罪门槛以最新司法解释（法释〔2024〕4号）为准：税款>=10万元或造成损失>=5万元。偷税移送标准：逃税>=10万元且占应纳税额10%以上",
        "4.【税法例外条款】是否遗漏法定例外情形——如代垫运费不属于价外费用、买方自提不产生卖方运费义务、善意取得虚开发票的例外处理等。不能把「一般规则」当成「唯一规则」",
        "5.【定性逻辑严谨性】定性表述不能绝对化——疑点≠结论。应区分「高疑点信号→需进一步核查取证→综合判断定性」的递进逻辑，不能直接从「无运输费」跳到「定性为无货虚开」",
        "6.【关联法规完整性】同一涉税事项涉及的多部法规是否都有引用——如虚开发票同时涉及《发票管理办法》（行政处罚）和《刑法》第205条（刑事责任），两者缺一不可",
        "—— 内容质量要求 ——",
        "7.【业务场景完整性】normal_reason是否穷举了所有合法合规的「无异常」情形——每种情形必须附带对应的核心证据清单",
        "8.【证据链可操作性】evidence是否包含取证可落地的具体文件名称——不能只有泛泛的「合同」「发票」，需具体到「购销合同的价格条款」「买方出具并盖章的货物自提证明」",
        "9.【追问的攻击性】drill_questions是否直击要害——追问必须带「→潜台词:」标注稽查的真实意图，不能只是温和的Q&A",
        "10.【量化阈值】threshold是否有具体的量化触发条件——必须有前置条件（如销售额>50万元）+ 计算公式 + 判定阈值，不能空泛",
        "—— 盲区发现 ——",
        "11.【政策废止与新颁】近年来废止或新颁布的税收法律、行政法规、部门规章，识别规则库中引用已废止政策的情况",
        "12.【新型业态演变】直播带货、跨境电商、灵活用工、SaaS/共享经济等新业态——现有规则是否覆盖了这些业态特有的风险点",
        "13.【舞弊手法演进】新增的偷逃税手法（如利用数电发票跨省虚开、AI生成虚假合同等）是否已有对应检测规则",
        "14.【跨税种联动】增值税×所得税、个税×社保、发票×资金流等跨税种勾稽规则是否完备",
        "15.【征管手段升级】金税四期/数电发票全面推行后——新增了哪些数据比对能力、应补充哪些规则利用新数据源（如银行流水自动比对、社保基数与工资比对等）",
        "请返回JSON格式：{\"new_rules\":[{新增的规则,含item/category/level/detail/suggestion/policy_ref}],\"modify\":[{修改建议,含id/原item/新item/原因}],\"delete\":[{删除建议,含id/item/原因}],\"summary\":\"一句话概述本次更新要点，突出政策合规问题\"}",
    ]
