...
【生成字段 —— JSON格式】
- direction: 4层推理链【推理第一层】→【推理第四层】，每层80-120字，总计300-500字
- phenomena: 异常现象描述+常见表现(①②③④⑤)，200-300字
- drill_questions: 8条追问分3组，每条必须带"→潜台词:"和"A:"，总计500-700字
- focus: 稽查重点指向(①②③④⑤)，100-150字
- normal_reason: 5-6种正常合规情形，每种带"（需提供XX证据）"
- determination: 2条推理路径（路径一/路径二），每条含定性结论和处理
- risk_table: 4-6行风险表，格式"税种:描述→处理"
- action: ①-⑤共5步稽查动作
- evidence: 证据清单，用·分隔各项
- threshold: 量化触发条件+预警等级(黄/橙/红)
...
async def smart_update_rules(request: Request):
    ...
    summary_lines = [
        ...
        "—— 内容质量要求 ——",
        "7.【业务场景完整性】normal_reason是否穷举了所有合法合规的「无异常」情形——每种须附带核心证据清单",
        "8.【证据链可操作性】evidence是否包含取证可落地的具体文件名称——不能只有泛泛的「合同」，须具体到「购销合同的价格条款」「买方盖章的自提证明」",
        "9.【追问的攻击性】drill_questions是否直击要害——追问必须带「→潜台词:」标注稽查真实意图，不能只是温和Q&A",
        "10.【量化阈值】threshold是否有具体量化条件——必须前置条件（如销售额>50万）+计算公式+判定阈值，不能空泛",
        "—— 盲区发现（覆盖规则库内容缺口）——",
