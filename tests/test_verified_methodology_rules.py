...
class CoverageGovernanceTests(unittest.TestCase):
    def test_coverage_report_uses_canonical_and_reviewed_assets(self):
        ...
        self.assertEqual(inventory["rules"], 200)
        self.assertEqual(inventory["industry_scenarios"], 111)
        self.assertGreaterEqual(len(inventory["clue_depths"]), 4)
        self.assertGreaterEqual(max(inventory["clue_depths"]), 7)
        self.assertEqual(inventory["validation_depths"], [2, 3, 4, 5, 6])
        self.assertEqual(inventory["domain_collaboration_depths"], [1, 2, 3, 4, 5])
