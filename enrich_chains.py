...
PREFIX_TO_TOPIC = {
    '发票': '发票税务合规', '发票深度': '发票深度税务合规',
    '增值税': '增值税税务合规', '企业所得税': '企业所得税税务合规',
    '个税': '个人所得税税务合规', '个税深度': '个税深度税务合规',
    '成本': '成本费用税务合规', '成本费用配比': '成本费用配比',
    '收入合规': '收入合规税务合规', '隐匿收入': '隐匿收入税务合规',
    '虚开': '虚开发票税务合规', '出口退税': '出口退税税务合规',
    '跨境': '跨境税务税务合规', '跨境电商': '跨境电商税务合规',
    '行业': '行业专项税务合规', '经营实质': '经营实质税务合规',
    '经营': '经营合规税务合规', '经营深度': '经营深度税务合规',
    '资金流': '资金流税务合规', '资产负债往来': '资产负债往来',
    '税务合规': '税务合规技术', '风控': '风险控制',
    '风险': '风险排查', '金融': '金融税务税务合规',
    '财产税': '财产税税务合规', '申报': '申报合规税务合规',
    '社保': '社保税务合规', '国际': '国际税务税务合规',
    '转让定价': '转让定价税务合规', '股权资本': '股权资本税务合规',
    '知识产权': '知识产权税务合规', '政府补贴': '政府补贴税务合规',
    '数字经济': '数字经济税务合规', '海关': '海关税务税务合规',
    '供应链': '供应链核查', '慈善非营利': '慈善非营利税务合规',
    '融资租赁': '融资租赁税务合规', '破产重组': '破产重组税务合规',
    '资料完备': '资料完备检查', '进销存': '进销存税务合规',
    '税务': '税务合规',
}
...
def generate_description(chain):
    ...
    if isinstance(ip, list) and len(ip) > 0:
        ...
        # 无detail → 用步骤名合成
        if step_summaries:
            return f"税务合规线索链【{name}】，覆盖{'、'.join(step_summaries[:5])}等{'等多个' if len(step_summaries)>5 else ''}风险点。通过多维度数据比对识别异常，确保税务合规全覆盖。"
        
        return f"税务合规线索链【{name}】，共{len(ip)}个调查步骤，通过多源数据交叉验证识别税务风险。"
    ...
    if isinstance(steps, list) and len(steps) > 0:
        ...
        if actions:
            return f"税务合规线索链【{name}】，按步骤执行：{'；'.join(actions[:5])}{'...' if len(actions)>5 else ''}，通过逐层筛查锁定风险点。全行业适用，不依赖特定行业词库。"
        return f"税务合规线索链【{name}】，共{len(steps)}个步骤，通过逐层筛查锁定风险点。全行业适用。"
    
    # 4) investigation_path 为字符串
    if isinstance(ip, str) and ip:
        return f"税务合规线索链【{name}】。调查路径：{ip[:200]}"
    
    # 5) 兜底
    return f"税务合规线索链【{name}】，通过多源数据交叉验证识别税务风险。"
