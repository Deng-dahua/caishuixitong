...
def red_team_falsification(all_findings, pipeline_log=None):
    """红队证伪：三个攻击维度验证每条高风险发现的可靠性"""
    results = {"total": len(all_findings), "falsified": 0, "passed": 0, "details": []}
    ...
    for f in high_findings:
        ...
        defeated = 0
        for h in hypotheses:
            if _attack_with_evidence(h, detail, all_findings):
                defeated += 1
...
def blind_destruction_test(all_findings, pipeline_log=None):
    ...
    import random
    results = {"tested": 0, "collapsed": 0, "stable": 0, "details": []}
    ...
    for f in sample:
        ...
        if len(detail) < 100:
            ...
            results["stable"] += 1
            f["_blind_test"] = "稳定: 证据链鲁棒性强"

    if pipeline_log is not None:
        pipeline_log.append(
            f"[自省层·盲测] {results['tested']}条抽样 {results['stable']}稳定 {results['collapsed']}崩塌"
        )
    return results
