    ...
    if personal:
        ...
        if pct > 30:
            ...
            "how_found": f"通道1(发票): 从{len(sal_invs)}张销项发票中筛选购方名称为'个人'的{len(personal)}张({p_total:,.2f}元)，占全部销项{pct:.2f}%。通道2(银行): 验证银行流水中是否有对应的个人付款记录，双通道交叉确认后输出结论。",
                ...
                "tax_impact": "若被认定为异常开票行为，可能面临发票协查、纳税评估甚至税务税务合规。情节严重的可能被认定为虚开发票。",
...
def _domain_contract_comparison(db, company_id, sal_invs, pur_invs):
    ...
    if no_ct and len(no_ct) >= 2:
        ...
        "how_found": "從销项发票中提取所有购方名称，与合同档案中的甲方/乙方名称进行模糊匹配，找出有发票但无合同的客户。",
            ...
            "description": f"贵公司共有{len(buyers)}个销项发票客户，但仅有{coverage}个客户能找到对应的合同，{len(no_ct)}个客户的交易缺少合同支撑。合同是证明交易真实性的核心证据，也是税务税务合规中判断'四流合一'（合同流、资金流、发票流、货物流）的首要环节。大量交易无合同，一旦被税务合规将难以证明交易的真实性和合理性。",
    ...
    if total_sales > 0 and total_purchases > 0:
        ...
        if purchase_ratio > T.ratios.double_ratio:
            findings.append({"type": "购销弹性严重失衡", "level": "高风险", "score": 9,
                ...
                "tax_impact": "此指标是税务税务合规最高优先级重点关注项。差额部分将被推定为隐匿收入或虚增进项，面临补税+罚款+滞纳金。",
...
def _domain_document_completeness(docs_list, bank_txs, sal_invs, pur_invs, salaries, social_security, vouchers, inventory,
    ...
    MISSING_DEFS = [
        ...
        ("bank", "银行流水缺失", "高风险", 10,
         ...
         lambda: "银行流水是税务合规的第一个调取对象（《税务税务合规工作规程》明确规定）。缺失意味着：(1)税务合规无法验证企业全部银行账户的资金进出是否均已入账；(2)无法检测是否存在资金回流（供应商付款后资金回流至法人/股东/关联方个人账户）；(3)无法核实是否存在账外经营账户（未向税务机关报告的银行账户）。税务合规会直接要求限期提供，逾期不提供触发核定征收。",
         lambda: "缺失银行流水→税务合规无法验证收入完整性+无法检测资金回流→税务机关从金税系统/第三方数据（电力/海关/上下游企业）倒推核定收入→核定结果远超企业实际→补税+0.5-5倍罚款+滞纳金。",
         lambda: "《税收征收管理法》第三十五条（核定征收）、第五十四条；《税务税务合规工作规程》第二十二条（检查取证）。",
        ...
        ("voucher", "记账凭证缺失", "高风险", 8,
         ...
         lambda: "记账凭证是追溯账务处理全过程的原始依据。缺失=税务合规无法核查：(1)会计分录的借贷方向、科目运用、金额是否正确；(2)每笔记账是否附有合法有效的原始凭证（发票/合同/银行回单/入库单等）；(3)收入确认、成本结转、费用归集的时点和金额是否符合会计准则。根据《税务税务合规工作规程》，企业有义务提供完整的会计凭证，缺失即构成资料提供不全。",
         lambda: "缺失凭证→税务合规无法追溯分录准确性/科目运用/原始凭证匹配→会计账簿视为不健全→依据《税收征收管理法》第三十五条核定征收（税务机关有权按核定利润率/核定应纳税额的方式确定应纳税额，结果通常远超企业实际税负）。",
         lambda: "《税收征收管理法》第三十五条（核定征收）、第五十四条、第五十六条；《税务税务合规工作规程》关于资料提供义务的规定。",
        ...
        ("social_security", "社保明细缺失", "中风险", 6,
         ...
         lambda: "缺失社保明细→无法验证社保缴费基数与工资表的一致性→金税四期人社税务数据联动后差异自动预警→税务合规局收到独立推送→社保稽核+税务税务合规联动→补缴社保+滞纳金+罚款。",
    ...
    if missing_count > 0:
        ...
        findings.append({
            ...
            "policy_ref": "《税收征收管理法》第五十四条、第五十六条（资料提供义务及罚则）；《税务税务合规工作规程》第二十二条（检查取证）。",
...
def _domain_advanced_rules(bank_txs, sal_invs, pur_invs, salaries, social_security, vouchers, inventory):
    ...
    if len(round_txs) >= 3:
        findings.append({
            ...
            "tax_impact": "整数交易易引发反洗钱监测和税务税务合规关注，可能被认定为无真实商业背景的资金往来。",
    ...
    if total_expense > 0 and expense_to_sellers / total_expense < 0.3:
        findings.append({
            ...
            "description": f"银行流水中总共支出{total_expense:,.2f}元，但只有{expense_to_sellers:,.2f}元（{expense_to_sellers/total_expense*100:.2f}%）能匹配到进项发票上的销方名称。\n\n"
                "需要区分：剩余资金是正常费用支出（工资/租金/税费等）还是无法解释的资金流动。如果大量资金流出无法通过进项发票/费用票据/工资表/资产凭证等资料解释，则构成资金去向不明——这是税务税务合规的核心关注点。",
    ...
    if geo_disjoint and not has_transport and (remote_sellers >= 3 or remote_buyers >= 3):
        ...
        findings.append({
            ...
            "tax_impact": (
                ...
                "这是整个税务税务合规报告中最核心的发现——因为它不是一个点的问题，而是整个经营链条在物理层面无法成立。"
            ),
            "policy_ref": (
...
RULE_DATA_REQUIREMENTS = {
    ...
    237: ("税务税务合规应对预案文件", "无法验证税务合规应对预案的完备性"),
...
def _domain_rule_coverage(all_findings, bank_txs, sal_invs, pur_invs, vouchers, salaries, social_security, inventory, docs):
    ...
    if missing_data:
        ...
        findings.append({
            ...
            "policy_ref": "《税务税务合规工作规程》关于企业提供完整经营资料的义务。",
...
def _check_accounting_system_gap(invoices, bank_txs, vouchers):
    ...
    if (has_inv or has_bank) and not has_voucher:
        ...
        findings.append({
            ...
            "description": (
                "序时账（记账凭证）是税务税务合规的核心资料，缺少凭证将导致以下风险无法排除：\n"
            ...
            "policy_ref": "《税收征收管理法》第十九条、第二十四条；《会计法》第九条；《税务税务合规工作规程》",
            "suggestion": (
...
MISSING_CONSEQUENCE_TRIGGER = {
    "bank": {
        ...
        "law": "《税收征收管理法》第三十五条（核定征收）、第五十四条；《税务税务合规工作规程》第二十二条（检查取证）",
    ...
    "voucher": {
        ...
        "law": "《税收征收管理法》第三十五条、第五十四条、第五十六条；《税务税务合规工作规程》",
...
def _check_conclusion_consistency(all_findings):
    ...
    for rule in CONTRADICTION_RULES:
        ...
        contradictions.append({
            ...
            "policy_ref": "《税务税务合规工作规程》关于证据链完整性、一致性的要求",
            "category": "综合定性·结论自洽",
