...
__all__ = [
    ...
    "_domain_bom_verify",
    "_domain_warehouse_capacity",
    "_domain_transport_necessity",
    ...
    if inventory:
        ...
        if imbalances:
            ...
            findings.append({"type": "进销存数量勾稽不平衡", "level": "高风险", "score": 7,
                ...
                "description": f"进销存台账的数量勾稽关系为「期初库存 + 本期入库 − 本期出库 = 期末库存」。逐行核对发现{len(imbalances)}条品项不满足该恒等式，差额异常。勾稽不平衡意味着：① 账实不符（实际存货与账面不一致，可能已销售未入账或虚增库存）；② 单据缺失或串户（出库/入库漏记）；③ 单位换算或盘点错误。这是风险检查判定存货真实性的第一道闸门。",
...
def _domain_bom_verify(bom_data, inventory, pur_invs, sal_invs):
    ...
    这是生产型企业风险检查的核心验证：用BOM表的标准配方，验证企业的
    ...
    if processing and all_products:
        ...
        findings.append({"type": "委外加工链条待BOM核验", "level": "中风险", "score": 6,
            ...
            "description": f"企业存在委托加工业务（加工费合计{total_amt:,.0f}元）。税务风险检查委外加工的核心证据链：① 委托加工合同；② 发出原料清单（应与BOM原料一致）；③ 收回成品数量（应与BOM产出匹配）；④ 加工费定价依据（加工费率合理性）。" + (f"跨地区委托加工（{cross_str}）运输与税务风险更高，须重点核实发出原料与收回成品是否形成完整闭环，防范虚增加工成本或接受虚开发票。" if cross else "本地加工亦须核实合同与收回数量。"),
...
def _domain_warehouse_capacity(inventory, bank_txs=None, sal_invs=None, pur_invs=None, target_industry="", warehouse_contracts=None):
    ...
    # 金额缺失时，用「总金额/总数量 × 期末数量」粗估期末货值（仅用于面积量级推算）
    if total_end_val <= 0 and total_qty > 0 and total_amt > 0:
        unit = total_amt / total_qty
        total_end_val = unit * total_end_qty

    if total_end_val <= 0 and total_end_qty <= 0:
        return findings
    ...
    if contracts and required_area > 0:
        ...
        if contracts_no_area:
            findings.append({"type": "仓库租赁合同缺面积条款", "level": "中风险", "score": 6,
                ...
                "description": "仓库租赁合同未载明仓储面积，属于关键条款缺失：既无法验证存货是否有真实存放空间，也无法核验租金单价（元/㎡/年）的合理性。风险检查实践中，面积条款缺失的仓租合同常被用于虚增租赁费用列支。",
    ...
    MARKET_RENT = 280.0
    affordable_area = annual_rent / MARKET_RENT if annual_rent > 0 else 0.0

    if annual_rent > 0 and required_area > 0 and affordable_area > 0:
        if required_area > affordable_area * 1.5:
            findings.append({"type": "仓储容量不匹配-存货放不下", "level": "高风险", "score": 8,
                "how_found": f"期末存货约需{required_area:,.0f}㎡仓储空间（按货值{total_end_val:,.0f}元÷行业约2万元/㎡承载估算）；而期间仓储租金{annual_rent:,.0f}元仅能支撑{affordable_area:,.0f}㎡（÷市场约280元/㎡/年）。所需面积为可承载面积的{required_area/affordable_area:.1f}倍。",
                "detail": f"所需面积约{required_area:,.0f}㎡，可承载面积约{affordable_area:,.0f}㎡，缺口{required_area-affordable_area:,.0f}㎡。",
                "description": f"企业声明的期末存货（货值{total_end_val:,.0f}元）按行业仓储承载密度测算约需{required_area:,.0f}平方米，但其支付的仓储租金（{annual_rent:,.0f}元）折算仅能租用约{affordable_area:,.0f}平方米。所需面积远超可承载面积，说明现有仓储空间在物理上放不下账面存货。",
                "tax_impact": "存货放不下→账实不符：要么存货数据虚增（虚增成本/虚抵进项），要么真实存货在账外（已销售未入账未开票）。无论哪种均为重大涉税风险，税务机关可据此核定。",
                "suggestion": "① 提供仓库租赁合同并明确仓库面积、坐落、仓储品类；② 提供仓库平面图与最大堆码容量；③ 出具期末存货盘点表（实存数量/金额）；④ 若存货真实，解释仓储面积缺口来源（外租仓库须补租赁合同与发票）。",
                "category": "域6 存货"})
        elif required_area > affordable_area:
            findings.append({"type": "仓储容量偏紧-需核实", "level": "中风险", "score": 5,
                "how_found": f"所需面积约{required_area:,.0f}㎡，可承载面积约{affordable_area:,.0f}㎡，租金支撑略显不足。",
                "detail": f"所需{required_area:,.0f}㎡ vs 可承载{affordable_area:,.0f}㎡。",
                "description": f"账面存货所需仓储面积（{required_area:,.0f}㎡）略高于租金可支撑面积（{affordable_area:,.0f}㎡），需核实是否存在未计租金的外租仓库或关联方免费提供仓储。",
                "tax_impact": "关联方免费提供仓储→视同销售/关联交易定价问题；外租未计→租赁费列支不完整。",
                "suggestion": "提供全部仓库租赁合同（含面积）与租赁发票，澄清面积缺口。",
                "category": "域6 存货"})
        else:
            findings.append({"type": "仓储容量匹配-经营真实", "level": "低风险", "score": 2,
                "how_found": f"所需面积约{required_area:,.0f}㎡，可承载面积约{affordable_area:,.0f}㎡，租金支撑充足。",
                "detail": f"仓储费与库存规模基本匹配（可承载{affordable_area:,.0f}㎡ ≥ 所需{required_area:,.0f}㎡）。",
                "description": f"企业支付的仓储租金折算面积（{affordable_area:,.0f}㎡）能够容纳账面存货所需空间（{required_area:,.0f}㎡），存货物理存放具备合理性基础。",
                "category": "域6 存货"})
    else:
        # 无租金数据：无法做面积比对，给出待核项
        findings.append({"type": "仓储面积待核-缺租赁合同", "level": "中风险", "score": 5,
            "how_found": f"期末存货约需{required_area:,.0f}㎡仓储空间（按货值{total_end_val:,.0f}元估算），但银行流水未检出仓储/仓库租赁支出，亦未上传仓库租赁合同。",
            "detail": f"所需面积约{required_area:,.0f}㎡，无租金/合同数据可比对。",
            "description": f"系统按行业承载密度测算，期末存货约需{required_area:,.0f}平方米仓储空间。但既无仓储费用支出记录，也未提供仓库租赁合同，无法验证存货是否具备真实的物理存放空间——这是风险检查判定存货真实性的关键一环。",
            "tax_impact": "无仓储费+无租赁合同而有大额存货→税务机关直接推定账实不符：已销售未入账（隐匿收入）或存货虚构（虚增成本）。",
            "suggestion": "① 上传仓库租赁合同，合同须明确载明：仓库面积、坐落位置、仓储品类、租赁期限、租金及支付方式；② 提供仓储费/物业费/仓库水电费发票；③ 提供期末存货盘点表与仓库平面图。",
            "category": "域6 存货"})
    ...
    if contracts:
        findings.append({"type": "仓库租赁合同内容审查要点", "level": "信息", "score": 1,
            ...
            "description": "税务风险检查对仓库租赁合同的审查要点：① 面积是否与存货规模匹配；② 坐落是否与企业经营地/供应商/客户地理一致；③ 仓储品类是否涵盖实际存货；④ 租金是否与当地市场（约280元/㎡/年）相符。合同缺失面积条款或面积明显不足，均构成证据瑕疵。",
            "category": "域6 存货"})
    else:
        findings.append({"type": "仓库租赁合同内容审查要点", "level": "信息", "score": 1,
...
def _domain_transport_necessity(bank_txs, sal_invs, pur_invs, target_industry="", transport_contracts=None):
    ...
    if not has_goods or goods_value <= 0:
        return findings

    # 跨省判定：出现≥2个不同省份/城市即视为跨地区经营
    cross = len(cities) >= 2

    exp_low = goods_value * 0.03
    exp_high = goods_value * 0.08
    ...
    if actual_transport <= 0:
        if tc and contract_freight > 0:
            ...
            findings.append({"type": "到货价含运-零运费有条款解释仍需物流单据", "level": "中风险", "score": 6,
                ...
                "description": f"运输合同采用到货价（含运）定价、运费由购方承担并计入货物采购成本，这为账面无独立运输费提供了合同解释。但注意两点风险检查要点：① 到货价下运费已隐含在采购成本中，须防范以虚高「含运价」虚增采购成本、多抵进项；② 定价方式不能免除货物流举证——新疆等地跨省运输仍须提供物流运单、签收记录等物证，否则货物流链条依然断裂。",
            ...
            findings.append({"type": "无运输费记录-货物流存疑", "level": "中风险", "score": 6,
                "how_found": f"货值{goods_value:,.0f}元，但未检出任何运输/物流类支出。",
                "detail": f"实际运输费0元，货值{goods_value:,.0f}元。",
        ...
        findings.append({"type": "运输费占比严重偏低", "level": "高风险", "score": 8,
            "how_found": f"实际运输费{actual_transport:,.0f}元，仅占货值{ratio*100:.2f}%（行业合理3%~8%，即{exp_low:,.0f}~{exp_high:,.0f}元）。",
            "detail": f"运输费占比{ratio*100:.2f}%，远低于3%下限。",
            "description": f"实际运输费{actual_transport:,.0f}元，与货值{goods_value:,.0f}元相比占比仅{ratio*100:.2f}%，显著低于制造业/贸易业3%~8%的合理区间。运输费严重不足，货物流规模与购销规模不匹配。",
            "tax_impact": "运输费与货值严重不匹配→货物流物证不足→成本真实性存疑→税前扣除与进项抵扣面临调整。",
            "suggestion": "① 解释运输费偏低原因（自提/供应商包邮须合同佐证）；② 补提供运输单据与运费发票；③ 无法佐证→相关成本与进项转出。",
            "category": "域4 成本费用"})
    elif ratio < 0.03:
        findings.append({"type": "运输费占比偏低", "level": "中风险", "score": 5,
            "how_found": f"实际运输费{actual_transport:,.0f}元，占货值{ratio*100:.2f}%（下限3%）。",
            "detail": f"运输费占比{ratio*100:.2f}%，接近下限。",
            "description": f"运输费占货值{ratio*100:.2f}%，略低于行业合理区间下限（3%）。需说明运输组织方式（如主要自提、近距离供货）。",
            "tax_impact": "占比偏低需合理解释，否则成本真实性存疑。",
            "suggestion": "提供运输合同与运费发票，说明运输组织方式。",
            "category": "域4 成本费用"})
    else:
        findings.append({"type": "运输费与货值匹配", "level": "低风险", "score": 2,
            "how_found": f"实际运输费{actual_transport:,.0f}元，占货值{ratio*100:.2f}%（行业3%~8%）。",
            "detail": f"运输费占比{ratio*100:.2f}%，处于合理区间。",
            "description": f"运输费{actual_transport:,.0f}元与货值{goods_value:,.0f}元配比合理（{ratio*100:.2f}%），货物流物证链基本完整。",
            "category": "域4 成本费用"})
    ...
    if tc:
        findings.append({"type": "运输合同运费条款审查要点", "level": "信息", "score": 1,
            ...
            "description": "税务风险检查对运输合同/条款的审查：① 运费由谁承担（到货价含在料价、出厂价企业自付、运费到付）；② 若供应商包邮（到货价），须提供合同运费条款及供应商运费发票，否则本企业成本不得扣除；③ 运费发票品名/金额须与购销规模匹配。无法提供运输证明→成本费用不得税前扣除。",
            "category": "域4 成本费用"})
    else:
        findings.append({"type": "运输合同运费条款审查要点", "level": "信息", "score": 1,
...
def _domain_salary_ss_hf_compare(salaries, social_security):
    ...
    if uninsured:
        ...
        findings.append({
            ...
            "description": f"发现{len(uninsured_persons)}名员工（共{len(uninsured)}组『姓名+月份』配对项，即每位员工每个月的工资与社保逐一比对中出现的『有工资无社保』情形）有工资发放记录但在社保缴纳名单中未找到对应月份记录。未依法参保不仅面临社保稽核与行政处罚（责令补缴+滞纳金+罚款），还会触发金税四期人社-税务数据比对预警，并牵连工资薪金企业所得税税前扣除的真实性。",
...
def _domain_contract_comparison(db, company_id, sal_invs, pur_invs):
    ...
    if no_ct and len(no_ct) >= 2:
        ...
        "how_found": "從销项发票中提取所有购方名称，与合同档案中的甲方/乙方名称进行模糊匹配，找出有发票但无合同的客户。",
            ...
            "description": f"贵公司共有{len(buyers)}个销项发票客户，但仅有{coverage}个客户能找到对应的合同，{len(no_ct)}个客户的交易缺少合同支撑。合同是证明交易真实性的核心证据，也是税务合规中判断'四流合一'（合同流、资金流、发票流、货物流）的首要环节。大量交易无合同，一旦被税务合规将难以证明交易的真实性和合理性。",
            "tax_impact": "缺少合同支撑的交易，税务机关可能要求企业补充提供其他交易真实性证据。如无法提供，将面临进项税额不予抵扣、成本不予税前扣除、甚至被认定为虚开发票的严重后果。此外，合同是印花税的计税依据，无合同也意味着印花税可能存在漏缴。",
            "policy_ref": "《民法典》关于合同订立的规定；《印花税法》关于应税合同的规定；国家税务总局关于'四流合一'的税务合规要求。",
...
def _domain_business_substance(db, company_id, sal_invs, pur_invs, bank_txs, salaries):
    """域12: 经营实质深度税务合规 — 多角度、多维度、多样化手段（ctx增强版）"""
    findings = []
    ...
    if not has_fixed_asset and total_sales > T.amount_thresholds.large_transaction:
        findings.append({"type": "无固定资产购置记录", "level": "中风险", "score": 5,
            ...
            "tax_impact": "固定资产缺失削弱经营真实性的证明力，税务合规中会被作为'空壳经营'的辅助证据。",
    ...
    if anomaly_count >= 2:
        findings.append({"type": "经营实质综合预警", "level": "高风险", "score": 10,
            ...
            "description": f"综合以上{len(findings)}项经营实质检测维度，共有{anomaly_count}项触发高风险预警。多维度、多角度的检测相互印证，表明企业经营实质存在系统性疑点，强烈建议进行全面自查和规范整改。税务机关在税务合规中会综合运用这些指标来评估企业的经营真实性和纳税遵从度。",
            "tax_impact": "多项经营实质指标同时异常，将触发税务机关的重点关注和全面税务合规，企业面临较大的补税和处罚风险。",
...
def _domain_invoice_deep(invoices):
    ...
    if total > 0 and len(sensitive) / total > T.ratios.material_deviation:
        ...
        "how_found": "扫描进项发票的货物名称，检测是否包含咨询、服务费、技术、设计、广告、推广、策划等关键词。计算服务类发票占比，超过30%触发预警。",
            ...
            "tax_impact": "若无法证明服务交易的真实性（无服务合同、无成果交付、无付款记录），相关进项税额将被要求转出，已计入成本费用的支出也将被纳税调增。情节严重的可能被移送税务合规。",
...
def _domain_document_completeness(docs_list, bank_txs, sal_invs, pur_invs, salaries, social_security, vouchers, inventory,
    ...
    """评估提交资料的完整度，逐项量化缺失资料的税务合规风险和牵连影响
    税务合规必查14类资料：银行流水/销项发票/进项发票/记账凭证/工资表/社保明细/进销存台账/
    ...
    if total_parsed_docs == 0 and docs_list:
        findings.append({
            ...
            "tax_impact": "资料无法解析意味着无法进行风险分析。但请注意：这些资料在企业手中是完整的，只是导出格式不兼容——税务合规时可直接提供原始格式，不存在真实缺失。",
    ...
    ALL_CATEGORIES = [
        ("bank", "银行流水", "验证资金全链路，税务合规第一调取对象。缺失→无法验证收入完整性+无法检测资金回流→税务机关从金税系统/第三方数据倒推核定收入→结果远超企业实际"),
        ("sales_invoice", "销项发票", "验证开票收入与申报收入匹配。缺失→税务合规直接从金税系统调取开票数据与银行流水比对→收款大于开票金额→推定为隐匿收入→补税+0.5-5倍罚款"),
        ("purchase_invoice", "进项发票", "验证成本真实性+进项税额抵扣合法性。缺失→税务合规逐一核验全部进项税额抵扣凭证→异常发票做进项转出+补税+滞纳金"),
        ...
        ("social_security", "社保明细", "核实用工合规性+缴费基数真实性。缺失→无法验证社保缴费基数与工资表的一致性→金税四期人社税务数据联动后会直接推送到税务合规局，形成独立案件"),
        ...
        ("financial", "资产负债表+利润表", "验证企业财务状况与申报数据的匹配性。缺失→无法比对报表收入与申报收入/开票收入→三源比对失效→隐匿收入/虚列成本无法被系统发现但税务合规可现场调取"),
    ...
    MISSING_DEFS = [
        ("contract", "合同文件缺失", "高风险", 9,
         ...
         lambda: f"缺少合同文件——需按业务性质四层判断：{len(mc_list)}家主营业务采购/交易额{must_total:,.2f}元必须有合同，{len(sc_list)}家重要费用/交易额{should_total:,.2f}元应签合同，{len(ms_list)}家日常消费类以发票为凭证即可。①税务合规逐笔质疑{len(mc_list)+len(sc_list)}笔无合同交易的商业合理性；②无合同→印花税漏缴(约{stamp_tax_est:,.2f}元)；③大额无合同→虚开发票嫌疑。",
         lambda: f"缺失合同→四流合一断裂→{must_total+should_total:,.2f}元交易无合同支撑→税务合规可逐笔质疑交易真实性→虚开发票嫌疑→补税+罚款+滞纳金；印花税计税依据缺失→漏缴约{stamp_tax_est:,.2f}元。",
        ...
        ("bank", "银行流水缺失", "高风险", 10,
         lambda: "缺少银行流水——税务合规第一调取对象，验证资金全链路的原始证据缺失",
         lambda: "银行流水是税务合规的第一个调取对象（《税务合规工作规程》明确规定）。缺失意味着：(1)税务合规无法验证企业全部银行账户的资金进出是否均已入账；(2)无法检测是否存在资金回流（供应商付款后资金回流至法人/股东/关联方个人账户）；(3)无法核实是否存在账外经营账户（未向税务机关报告的银行账户）。税务合规会直接要求限期提供，逾期不提供触发核定征收。",
         lambda: "缺失银行流水→税务合规无法验证收入完整性+无法检测资金回流→税务机关从金税系统/第三方数据（电力/海关/上下游企业）倒推核定收入→核定结果远超企业实际→补税+0.5-5倍罚款+滞纳金。",
         lambda: "《税收征收管理法》第三十五条（核定征收）、第五十四条；《税务合规工作规程》第二十二条（检查取证）。",
         "① 整理全部对公账户银行流水（含已注销账户），覆盖税务合规所属期全部月份；② 法人、主要股东、财务负责人个人账户中与经营相关的流水也应整理备查。"),
        
        ("sales_invoice", "销项发票缺失", "高风险", 9,
         lambda: "缺少销项发票——无法验证企业实际开票收入与申报收入的匹配",
         lambda: "销项发票是验证收入规模的核心资料。缺失=税务合规无法：(1)比对申报收入vs实际开票收入是否一致；(2)比对开票客户vs银行回款客户是否一致（是否存在未开票即收款）；(3)核实是否存在应开未开发票的隐匿收入。金税四期已实现全国发票数据集中，税务合规可直接从金税系统调取企业的全部开票记录。",
         lambda: "缺失销项发票→税务合规直接从金税系统调取开票数据+银行流水→银行收款金额大于开票金额的部分→推定为隐匿未开票收入→补缴增值税+企业所得税+0.5-5倍罚款+滞纳金。",
        ...
        ("purchase_invoice", "进项发票缺失", "高风险", 9,
         ...
         lambda: "进项发票是验证成本真实性、进项税额抵扣合法性的核心资料。缺失=税务合规无法：(1)验证已抵扣的进项税额对应的发票是否真实、是否属于可抵扣范围；(2)验证供应商是否真实经营（是否存在开票后走逃/注销）；(3)比对采购发票vs存货入库vs银行付款的三流一致性。金税四期已对异常抵扣凭证实现自动预警。",
         lambda: "缺失进项发票→税务合规逐一核验全部进项税额抵扣凭证→异常发票（走逃/失控/虚开/品名不符）做进项税额转出→补缴增值税+滞纳金；同时对应的成本不得税前扣除→补缴企业所得税。",
        ...
        ("voucher", "记账凭证缺失", "高风险", 8,
         ...
         lambda: "记账凭证是追溯账务处理全过程的原始依据。缺失=税务合规无法核查：(1)会计分录的借贷方向、科目运用、金额是否正确；(2)每笔记账是否附有合法有效的原始凭证（发票/合同/银行回单/入库单等）；(3)收入确认、成本结转、费用归集的时点和金额是否符合会计准则。根据《税务合规工作规程》，企业有义务提供完整的会计凭证，缺失即构成资料提供不全。",
         lambda: "缺失凭证→税务合规无法追溯分录准确性/科目运用/原始凭证匹配→会计账簿视为不健全→依据《税收征收管理法》第三十五条核定征收（税务机关有权按核定利润率/核定应纳税额的方式确定应纳税额，结果通常远超企业实际税负）。",
         lambda: "《税收征收管理法》第三十五条（核定征收）、第五十四条、第五十六条；《税务合规工作规程》关于资料提供义务的规定。",
         "① 确保完整的记账凭证（序时账）随时可调取；② 每张凭证必须包含：日期、凭证号、摘要、会计科目、借贷金额、附件张数；③ 凭证所附原始凭证（发票/合同/银行回单/入库单等）齐全且一一对应。"),
        
        ("trial_balance", "科目余额表缺失", "中风险", 7,
         lambda: "缺少科目余额表——无法验证总账与明细账的一致性，税务合规必查基础档案缺失",
         lambda: "科目余额表是连接总账与明细账的桥梁，也是编制财务报表的基础，属于税务合规必查基础档案。缺失=税务合规无法核实：(1)各科目期初期末余额是否衔接（是否存在凭空增减）；(2)各科目本期发生额是否与凭证汇总一致（是否存在账外调整）；(3)重点科目（应收账款/应付账款/存货/收入/成本）的余额是否合理。",
        ...
        ("salary", "工资表缺失", "中风险", 6,
         ...
         lambda: "工资是企业所得税前扣除的大项，也是个税代扣代缴的基础资料。缺失=税务合规无法核实：(1)税前扣除的工资费用是否真实（是否存在虚列人头/虚增工资金额）；(2)个税是否足额代扣代缴（实发工资vs申报工资是否一致）；(3)工资表人数vs社保参保人数是否匹配。",
        ...
        ("social_security", "社保明细缺失", "中风险", 6,
         ...
         lambda: "社保明细是验证企业用工合规性的核心资料。缺失=无法核实：(1)是否全员参保（是否存在只发工资不缴社保的'隐形用工'）；(2)社保缴费基数是否与实际工资一致（低基数参保差额=少缴社保+少扣个税）。金税四期已将人社数据与税务数据打通，缴费基数与申报工资的差异自动推送至税务合规局。",
         lambda: "缺失社保明细→无法验证社保缴费基数与工资表的一致性→金税四期人社税务数据联动后差异自动预警→税务合规局收到独立推送→社保稽核+税务合规联动→补缴社保+滞纳金+罚款。",
        ...
        ("inventory", "进销存台账缺失", "中风险", 5,
         ...
         lambda: "进销存台账是验证存货真实性和购销匹配的基础档案。缺失=税务合规无法判断：(1)账面库存是否真实存在（是否存在已销售未出库/已报废未处理/虚假入库）；(2)采购量+期初库存-销售量=期末库存，三者逻辑是否自洽；(3)是否存在账外存货（仓库有货但账面无记录）。",
         lambda: "缺失进销存→税务合规进行实地盘点→账实不符的部分→推定为已销售未入账→补缴增值税+企业所得税；存货账实严重不符→认定为账外经营/虚增成本→核定征收。",
        ...
        ("financial", "财务报表缺失", "中风险", 7,
         ...
         lambda: "资产负债表和利润表是企业财务状况的核心文件。缺失=税务合规无法验证：(1)申报收入与报表收入是否一致（是否存在两套账）；(2)资产规模与经营规模是否匹配（小微企业报表显示数千万资产→异常）；(3)往来科目余额是否异常（大额其他应收款/应付款可能隐藏资金抽逃或账外经营）。",
         lambda: "缺失财务报表→无法比对报表收入与申报收入/开票收入→三源比对失效→隐匿收入/虚列成本无法被系统自动发现→但税务合规可现场调取原始账簿逐一核实→查出的问题更严重。",
        ...
        ("vat", "增值税申报表缺失", "中风险", 6,
         ...
         lambda: "缺失增值税申报表→无法确认企业是否足额申报→税务合规直接从金税系统调取申报记录+发票数据比对→未申报或少申报的部分→补缴增值税+滞纳金+0.5-5倍罚款。",
        ...
        ("cit", "企业所得税申报表缺失", "中风险", 6,
         ...
         lambda: "缺失所得税申报表→无法核实所得税汇算清缴的准确性→税务合规直接调取金税系统申报记录+凭证数据比对→少缴的部分→补缴企业所得税+滞纳金+罚款。",
        ...
        ("other_tax", "其他税种申报表缺失", "低风险", 3,
         ...
         lambda: "印花税、城建税、教育费附加、房产税、土地使用税等小税种虽然单笔金额不大，但在税务合规中常常成为突破口——因为企业容易忽视而导致漏缴，税务合规一旦查到就是板上钉钉的违法事实。缺失=无法核实：(1)印花税是否按购销合同/借款合同/账簿/证照足额缴纳；(2)城建税及教育费附加是否按实际缴纳的增值税额正确计算；(3)房产税/土地使用税是否按房产原值/土地面积足额申报。",
         lambda: "缺失小税种申报→税务合规逐项核验→漏缴部分→补缴税款+每日万分之五滞纳金+0.5-5倍罚款→虽然单项金额不大，但多项累积+滞纳金滚存后数字可观，且容易成为税务合规深挖其他问题的'突破口'。",
    ...
    for key, ftype, level, score, detail_fn, desc_fn, impact_fn, policy, suggestion in MISSING_DEFS:
        if key not in doc_types_present:
            findings.append({
                ...
                "detail": detail_fn(),
                "description": desc_fn(),
                "how_found": f"逐一检测了14类税务合规必查资料的提交状态，{ftype.replace('缺失','')}类资料未提交",
    ...
    if missing_count > 0:
        ...
        findings.append({
            ...
            "score": total_score,
            "detail": f"经审核全部{total_categories}类税务合规必查资料：已提交{len(present_names)}类（{'、'.join(present_names)}），缺失{missing_count}类：{missing_detail}。",
            "description": f"经审核本次提交的全部资料，共计{total_categories}类税务合规必查资料，已覆盖{len(present_names)}类（{'、'.join(present_names)}），缺失{missing_count}类（{missing_detail}）。\n\n根据《税务合规工作规程》第二十二条，被查单位应在税务合规通知下达后按税务机关要求及时、完整提交相关资料。每缺一类资料，对应分析域无法执行，税务合规结论的完整性和精确性将受实质性影响。已提交资料对应的分析域均已执行完毕，缺失资料的后果逐一列明于下方证据材料。",
            "how_found": f"逐一检测{total_categories}类税务合规必查资料的提交状态——从文件解析结果的数据类型和文件名称判定。",
            "tax_impact": "税务合规通知下达后无法在限期内提供完整资料→面临罚款（单位最高5万元）→税务机关从其他数据源（金税系统、银行流水、第三方信息）倒推核定应纳税额→核定结果通常高于企业实际→补税+滞纳金+罚款。",
            "policy_ref": "《税收征收管理法》第五十四条、第五十六条（资料提供义务及罚则）；《税务合规工作规程》第二十二条（检查取证）。",
            "suggestion": f"补充缺失的{missing_count}类资料。按照金税四期税务合规必查清单，企业应确保以下{total_categories}类资料随时可调取、完整、规范：" + "、".join([f"{name}" for _, name, _ in ALL_CATEGORIES]) + "。",
            "items": missing_items,
            "category": "域14 资料完备度"
        })
    else:
        findings.append({
            "type": "资料完备度综合评估",
            "level": "低风险", "score": 2,
            "detail": f"已提交全部{total_categories}类税务合规必查资料：{'、'.join(present_names)}。",
            "description": f"本次分析覆盖了全部{total_categories}类税务合规必查核心资料，资料完整度高，能够支撑全面的涉税风险分析和税务合规应对。",
            "how_found": f"逐一检测{total_categories}类税务合规必查资料的提交状态，全部检测通过。",
            "category": "域14 资料完备度"
        })
    ...
    if payment_no_inv:
        ...
        findings.append({
            ...
            "description": (
                ...
                f"⚠ 税务合规关键判断：如果这些付款方是企业而非个人、金额非整数、摘要含'货款''项目款'等经营关键词 → 高度嫌疑为隐匿收入。"
            ),
            "how_found": (
    ...
    if party_mismatch:
        ...
        findings.append({
            ...
            "description": (
                ...
                f"税务合规会追问：B为什么替A付钱？A和B什么关系？是否有真实的货物交付？"
            ),
...
def _domain_advanced_rules(bank_txs, sal_invs, pur_invs, salaries, social_security, vouchers, inventory):
    ...
    if len(round_txs) >= 3:
        findings.append({
            ...
            "tax_impact": "整数交易易引发反洗钱监测和税务合规关注，可能被认定为无真实商业背景的资金往来。",
    ...
    if sal_invs:
        ...
        if sal_invs and no_remark / len(sal_invs) > T.ratios.half:
            findings.append({
                ...
                "description": f"有{no_remark}张销项发票未填写备注栏信息。根据规定，建筑服务、运输服务、不动产租赁、销售不动产等特定业务的发票备注栏属于必填项目。大量空白备注栏可能意味着未按规定填写发票信息，虽然不直接构成偷逃税，但在税务合规中会被认定为发票管理不规范。",
    ...
    if vr_invoiced > 0 and inv_total > 0:
        ...
        if gap / max(vr_invoiced, 1) > T.ratios.minor_deviation:
            findings.append({
                ...
                "tax_impact": "凭证与发票金额不一致，说明会计核算与税务申报之间存在脱节，税务合规时会深究每一笔差异的来源和性质。",
...
def _four_step_audit_framework(all_findings, bank_txs, invoices, target_entity):
    """四步税务合规分析法：将核心发现按 detect→verify→diagnose→report 统一处理"""
    ...
    if total_expense > 0 and expense_to_sellers / total_expense < 0.3:
        findings.append({
            ...
            "description": f"银行流水中总共支出{total_expense:,.2f}元，但只有{expense_to_sellers:,.2f}元（{expense_to_sellers/total_expense*100:.2f}%）能匹配到进项发票上的销方名称。\n\n"
                "需要区分：剩余资金是正常费用支出（工资/租金/税费等）还是无法解释的资金流动。如果大量资金流出无法通过进项发票/费用票据/工资表/资产凭证等资料解释，则构成资金去向不明——这是税务合规的核心关注点。",
...
def _domain_triangle_invoice_inventory_payment(pur_invs, inventory, bank_txs):
    ...
    # ── 税务合规方法论④-D：进项发票与银行付款匹配（主营成本识别驱动）──
    ...
    if reimb_count > 0:
        findings.append({
            ...
            "description": (
                f"我在做进项发票与银行付款匹配之前，先对{len(pur_invs)}张进项发票按品名做了三层分类——这是真实税务合规的必要步骤。\n\n"
    ...
    if geo_disjoint and not has_transport and (remote_sellers >= 3 or remote_buyers >= 3):
        ...
        findings.append({
            ...
            "description": (
                ...
                f"换一个税务合规员拿同样资料，同样会得出这个结论——因为三组地址互不重叠+零运输成本是无法解释的客观事实。"
            ),
            "how_found": (
            ...
            "tax_impact": (
                ...
                "这是整个税务合规报告中最核心的发现——因为它不是一个点的问题，而是整个经营链条在物理层面无法成立。"
            ),
            "policy_ref": (
    ...
    if sal_total > 0 and pur_total > 0:
        ...
        if io_ratio > high:
            ...
            findings.append({
                ...
                "suggestion": f"税务合规方向：1)银行流水收款与销项发票逐月比对→找出收款>开票的月份，追查未开票收入；2)大额供应商穿透→核实是否为空壳公司、是否存在资金回流；3)存货盘点→核实库存商品是否与进销存逻辑一致；4)若进销比偏高是因为库存积压，要求企业提供存货盘点表佐证。",
                "category": "行业对标"
            })
...
RULE_DATA_REQUIREMENTS = {
    ...
    237: ("税务合规应对预案文件", "无法验证税务合规应对预案的完备性"),
    241: ("行业税务合规重点指引对照", "无法判断贵司行业是否列入年度税务合规重点"),
...
def _domain_rule_coverage(all_findings, bank_txs, sal_invs, pur_invs, vouchers, salaries, social_security, inventory, docs):
    ...
    if missing_data:
        ...
        findings.append({
            ...
            "policy_ref": "《税务合规工作规程》关于企业提供完整经营资料的义务。",
...
def _check_accounting_system_gap(invoices, bank_txs, vouchers):
    ...
    if (has_inv or has_bank) and not has_voucher:
        ...
        findings.append({
            ...
            "description": (
                "序时账（记账凭证）是税务合规的核心资料，缺少凭证将导致以下风险无法排除：\n"
            ...
            "policy_ref": "《税收征收管理法》第十九条、第二十四条；《会计法》第九条；《税务合规工作规程》",
            "suggestion": (
...
# ═══════════════════════════════════════════════════════════
# 税务合规重点：现实中不管score多少，这些风险类型就是税务合规必查项
...
def _fix_level_by_audit_priority(ftype, current_level):
    """税务合规重点发现强制等级——不根据score计算，根据审计实务的必查优先级"""
...
MISSING_CONSEQUENCE_TRIGGER = {
    "bank": {
        ...
        "consequence": "缺失银行流水→税务合规无法验证收入完整性+无法检测资金回流→税务机关从金税系统/第三方数据（电力/海关/上下游企业）倒推核定收入→核定结果远超企业实际→补税+0.5-5倍罚款+滞纳金",
        "law": "《税收征收管理法》第三十五条（核定征收）、第五十四条；《税务合规工作规程》第二十二条（检查取证）",
        "action": "整理全部对公账户银行流水（含已注销账户），覆盖税务合规所属期全部月份；法人、主要股东、财务负责人个人账户中与经营相关的流水也应整理备查"
    },
    "sales_invoice": {
        "risk": "销项发票缺失→无法验证申报收入→隐匿收入推定风险",
        "level": "高风险", "priority": "P0",
        "consequence": "缺失销项发票→税务合规直接从金税系统调取开票数据+银行流水→银行收款>开票金额→推定为隐匿未开票收入→补缴增值税+企业所得税+0.5-5倍罚款+滞纳金",
    ...
    "purchase_invoice": {
        ...
        "level": "高风险", "priority": "P0",
        "consequence": "缺失进项发票→税务合规逐一核验全部进项税额抵扣凭证→异常发票（走逃/失控/虚开/品名不符）做进项税额转出→补缴增值税+滞纳金；对应成本不得税前扣除→补缴企业所得税",
    ...
    "voucher": {
        ...
        "level": "高风险", "priority": "P0",
        "consequence": "缺失凭证→税务合规无法追溯分录准确性/科目运用/原始凭证匹配→会计账簿视为不健全→依据《税收征收管理法》第三十五条核定征收（税务机关有权按核定利润率/核定应纳税额的方式确定应纳税额，结果通常远超企业实际税负）",
        "law": "《税收征收管理法》第三十五条、第五十四条、第五十六条；《税务合规工作规程》",
    ...
    "social_security": {
        ...
        "level": "中风险", "priority": "P1",
        "consequence": "缺失社保明细→无法验证社保缴费基数与工资表的一致性→金税四期人社税务数据联动后会直接推送到税务合规局，形成独立案件",
    ...
    "contract": {
        ...
        "level": "高风险", "priority": "P0",
        "consequence": "缺失合同→四流合一断裂→大额交易无合同支撑→税务合规可逐笔质疑交易真实性→虚开发票嫌疑→补税+罚款+滞纳金；印花税计税依据缺失",
    ...
    "financial": {
        ...
        "level": "中风险", "priority": "P1",
        "consequence": "缺失资产负债表+利润表→无法比对报表收入与申报收入/开票收入→三源比对失效→隐匿收入/虚列成本无法被系统发现但税务合规可现场调取",
        "law": "《税收征收管理法》第二十五条（纳税申报）；《企业会计准则第30号——财务报表列报》",
        "action": "补充完整的资产负债表和利润表（按税务合规所属期逐月/逐年）；确保财务报表收入与增值税申报表、企业所得税申报表的收入一致"
    },
    "vat": {
        ...
        "action": "补充税务合规所属期全部增值税申报表（含主表+附表）；逐月与销项发票汇总数、进项发票汇总数比对一致"
    },
    "cit": {
        ...
        "action": "补充税务合规所属期全部企业所得税申报表（含主表+附表）；确保收入、成本、费用与财务报表一致"
    },
    "ind_tax": {
        ...
        "action": "补充税务合规所属期全部个人所得税申报表；与工资表的个税代扣金额逐人逐月比对一致"
    },
    "other_tax": {
        "risk": "小税种申报缺失→多税种叠加风险",
        "level": "低风险", "priority": "P2",
        "consequence": "缺失小税种申报→税务合规逐项核验→漏缴部分→补缴税款+每日万分之五滞纳金+0.5-5倍罚款→多项累积+滞纳金滚存后数字可观",
...
def _check_conclusion_consistency(all_findings):
    ...
    for rule in CONTRADICTION_RULES:
        ...
        contradictions.append({
            ...
            "policy_ref": "《税务合规工作规程》关于证据链完整性、一致性的要求",
            "category": "综合定性·结论自洽",
...
EARLY_WARNING_ESCALATION = [
    {
        ...
        "forward_projection": (
            ...
            "→一旦被税务合规发现，不仅补税+罚款，还可能触发持续经营能力质疑→被列为风险纳税人"
        ),
        "checklist": (
    ...
    },
    {
        ...
        "forward_projection": (
            ...
            "→税务合规升级为联合执法（税务+公安+市监）"
        ),
        "checklist": (
            "①核查集中供应商/客户是否与企业在股权/人员上关联；"
            "②评估如核心交易对手退出后的替代方案；"
            "③保留完整的交易档案（合同+发票+物流+付款）以备税务合规。"
        ),
        "timeframe": "未来3-6个月",
        "level": "高风险",
    },
...
# 企业基本画像：有企业名称就出
_block("entity_profile", "税务合规对象基本情况",
...
def _build_report_blocks(result, company_id):
    ...
    # ── 结论和签字永远在最后 ──
    p += 1
    blocks.append({"type": "conclusion", "title": "税务合规结论与建议", "priority": p, "data": {
    ...
    try:
        ...
        if missing_docs:
            findings.append({
                ...
                "description": f"出口退税必须留存备查的单证包括：报关单（出口货物报关单）、出口发票、收汇核销单（或涉外收入申报表）、购货合同、运输单据。当前缺失{'、'.join(missing_docs)}，在税务风险检查中可能被认定为单证不齐，不得办理退税。",
