...
COMMON_FACT_CONTRACTS = {
    "COMMON-REVENUE-RECONCILIATION": {
        ...
        "rule_ids": {"VR001", "VR002", "VR057", "VR058", "VR059", "VR061"},
    ...
    "COMMON-COST-FUND-EVIDENCE": {
        "name": "主营业务成本资金证据链核验",
        "rule_ids": {"VR060"},
        "target_fact": "主营业务成本是否都有真实的资金支付印证：对公流水匹配为强证据；个人垫付后报销发票流与资金流分离；无任何支付记录指向挂账未付、虚开发票或发票未入账。",
        "lead": "采购与抵扣域",
        "supporting": ["进项发票明细", "银行付款流水", "供应商对账单", "应付账款账龄表", "员工报销凭证"],
        "opposing": ["批量汇总付款", "第三方平台代付", "员工垫付后凭票报销", "挂账后分期付款", "现金小额零星采购"],
        "steps": ["识别主营业务成本发票", "按供应商与金额匹配银行付款流水", "区分对公支付、个人垫付与无支付记录", "对无资金印证的采购逐笔核验付款去向"],
    },
    "COMMON-INCOME-COMPLETENESS": {
        "name": "收入完整性核验（预收挂账/价外费用/利息/补贴/资产处置）",
        "rule_ids": {"VR063", "VR064", "VR068", "VR069", "VR070"},
        "target_fact": "预收账款长期挂账、价外费用、利息收入、财政补贴、固定资产处置等收入是否完整入账申报。",
        "lead": "收入与申报协同域",
        "supporting": ["科目余额表", "序时账", "银行流水", "合同", "纳税申报表"],
        "opposing": ["合同分期发货", "退还保证金", "代收代付", "不征税收入文件", "借款归还"],
        "steps": ["提取预收账款余额", "识别摘要关键词收入", "核对入账科目与申报", "逐笔核验收入完整性"],
    },
    "COMMON-WITHHOLDING-COMPLIANCE": {
        "name": "个人所得税扣缴合规核验（个人劳务报酬）",
        "rule_ids": {"VR065"},
        "target_fact": "向个人支付劳务报酬、稿酬、佣金等应税所得是否履行代扣代缴个税义务。",
        "lead": "人员薪酬与扣缴域",
        "supporting": ["银行流水", "劳务合同", "代开发票", "个税扣缴申报表"],
        "opposing": ["员工工资已扣缴", "股东分红已代扣", "已取得代开发票并代扣", "报销款项"],
        "steps": ["识别公转私劳务性质付款", "核对个税扣缴申报", "核验合同发票一致性", "计算应扣未扣税额"],
    },
    "COMMON-INVOICE-AUTHENTICITY": {
        "name": "发票真实性核验（上游异常开票/折扣异常）",
        "rule_ids": {"VR066", "VR067"},
        "target_fact": "进项发票上游顶额集中开票特征与销项折扣开具方式是否符合税法规定。",
        "lead": "发票真实性域",
        "supporting": ["进项发票明细", "销项发票明细", "合同", "物流单据", "付款流水"],
        "opposing": ["大宗贸易集中采购", "集团集中开票", "同票注明折扣", "真实返利协议"],
        "steps": ["统计供应商开票特征", "识别顶额集中开票", "核验折扣开具方式", "穿透上游税务状态"],
    },
    "COMMON-PROVISIONAL-COST-LOOP": {
        "name": "暂估成本·其他应付款·公转私闭环核验",
        "rule_ids": {"VR062"},
        "target_fact": "账面主营业务成本超出进项发票支持的部分是否为暂估成本，暂估成本是否挂其他应付款，公账向个人账户付款是否用于冲销挂账——虚构成本与公转私资金循环。",
        "lead": "成本费用与扣除域",
        "supporting": ["科目余额表", "序时账", "进项发票明细", "银行付款流水", "暂估入库与期后发票"],
        "opposing": ["期后取得发票冲回", "真实劳务外包", "个体户采购代开发票", "往来借款归还", "真实个人服务合同"],
        "steps": ["汇总账面主营成本与发票口径成本", "计算暂估成本差额", "提取其他应付款余额与发生额", "汇总公账向个人账户付款", "对暂估、挂账、公转私三者金额勾稽"],
    },
...
RULE_CONCEPTS = {
    ...
    "VR055": ("工资", "平均工资", "拆分发放", "个税", "税率跳档", "薪酬结构"),
    "VR056": ("公私混同", "个人账户", "发薪", "工资", "代发", "个税扣缴"),
    "VR057": ("第三方平台", "收款", "对公归集", "资金监管", "平台账户", "归集比"),
    "VR058": ("个人码", "个人账户", "收款码", "账外收入", "监管盲区", "归集"),
    "VR059": ("现金交易", "现金收款", "库存现金", "现金日记账", "账外", "取现"),
    "VR060": ("主营业务成本", "资金证据", "对公流水", "个人垫付", "挂账", "支付印证", "虚开发票"),
    "VR061": ("主营业务收入", "收款印证", "银行流水", "平台收款", "应收账款", "回款", "虚构收入"),
    "VR062": ("暂估成本", "其他应付款", "公转私", "虚构成本", "账票缺口", "无票成本", "个人付款"),
    "VR063": ("预收账款", "合同负债", "长期挂账", "收入延迟", "纳税义务时间", "不结转收入"),
    "VR064": ("价外费用", "违约金", "赔偿金", "滞纳金", "零星收入", "未开票"),
    "VR065": ("劳务报酬", "稿酬", "佣金", "代扣个税", "扣缴义务", "居间费"),
    "VR066": ("异常凭证", "顶额开票", "集中开票", "走逃", "上游虚开", "版别限额"),
    "VR067": ("商业折扣", "折让", "折扣开票", "负数发票", "折扣计税"),
    "VR068": ("利息收入", "结息", "财务费用", "存款利息", "未申报"),
    "VR069": ("财政补贴", "政府补助", "扶持资金", "不征税收入", "专项拨款"),
    "VR070": ("固定资产处置", "设备处置", "车辆处置", "变卖", "处置收入"),
}
