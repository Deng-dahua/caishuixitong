...
TAX_BURDEN_RULES = {
    ...
    # ═══════════════════════════════════════════════════════
        "rule_precise_writing": {
            ...
            "23_fields": {
                ...
                "deep": {
                    ...
                    "fields": {
                        ...
                        "evidence": {
                            "name": "证据清单",
                            "format": "四层框架: 货物流+合同资金流+业务合理性+排雷。交易金额分级: 大额(>10万)分AB场景(自提vs直运); 中额(1-10万)至少取2个维度证据; 小额(<1万)抽样核验。每层证据标注优先级: 必须获取/应当获取/可以获取。首层命名后必须在同一层内出现优先级标注——光有名字（如'第一层·货物流'）没有必须获取/应当获取/可以获取标注的不合格。排雷层与⑯联动——⑯列出合法情形，排雷层验证这些情形的证据是否真实存在。",
            ...
            "summary": "疑点稽查编制标准——从四条标杆规则（#1借贷不平/#2凭证量异常/#1716运输费缺失/#1717预收挂账）反复打磨后归纳的顶级编制体系。通用指导每一条疑点实现'要么铁证如山，要么自证清白'的二元判定。编制逻辑：①判断攻击角度（商业逻辑/物理事实/数据穿透）→②按攻击角度选择标杆规则的结构和攻击力→③逐字段对照顶级精写十条编写→④自检：存疑能否升级为铁证？企业能否按预设的逃生舱自证排除？答案全是否→重写。23字段是维度框架，四条铁律是灵魂，三条攻击角度是起点，十项自检是终点。融合方法详见 gold_standard_decoded 段。",
            "canonical_example": {
                "id": 1813,
                "item": "预收账款长期挂账不转收入",
                "quality_notes": {
