    ...
    # ═══════════════════════════════════════════════════════
        "rule_precise_writing": {
            ...
            "gold_standard_decoded": {
                "description": "顶级精写十条——从四条标杆规则（#1借贷不平/#2凭证量异常/#1716运输费缺失/#1717预收挂账）的反复打磨中提炼的设计哲学。任意一条规则满足此十条即为顶级精写。",
                "rules": [
                    {"id":"GS-1","name":"矛盾一句话","criterion":"数据矛盾点必须能用'X vs Y——一句话说清哪里对不上'表述。一句话说不清=疑点不成立。标杆: #1717'已收款+未申报'——五个字钉死。"},
                    {"id":"GS-2","name":"推理至法条","criterion":"推理链每层标注依赖证据，最后一层必须落地到具体法条编号。标杆: #1第四层'资金流交叉验证→账外循环锁定→偷税(征管法§63)'——从数据异常推到法律定性。"},
                    {"id":"GS-3","name":"追问堵死物理极限","criterion":"穿透追问必须包含物理常识类问题，用不可辩驳的事实堵死辩解空间。标杆: #1716 Q3'工厂有没有能吊装这个吨位设备的行车'——装车能力是物理事实，没有就是没有。"},
                    {"id":"GS-4","name":"三路径各有进入条件","criterion":"定性路径三条必须各自定义明确的进入条件（单源/2源/≥3源），不得笼统写'根据证据情况分别处理'。标杆: 四条全部做到了每路径写清触发来源和证据数量。"},
                    {"id":"GS-5","name":"证据四层命名贴合","criterion":"证据首层命名必须贴合疑点实际类型——实物交易用货物流、账务断裂用账实核对层、纯申报用申报数据层、成本费用用费用真实性层、关联交易用关联关系与定价层。标杆: #2'费用真实性层'命名完全贴合'凭证量异常'的疑点性质。"},
                    {"id":"GS-6","name":"税负能算出具体数字","criterion":"tax_impact和suggestion必须给出具体补税计算公式和金额区间，使稽查人员拿规则即可测算。标杆: #1717'增值税=挂账余额÷(1+税率)×税率'——可直接代入计算。"},
                    {"id":"GS-7","name":"触发含三色预警","criterion":"触发指标除量化阈值+行业差异+四维前置条件外，必须含黄/橙/红三色预警等级，按严重程度分档。标杆: #1717'黄(账龄365-540天)→橙(540-730天)→红(>730天)'。"},
                    {"id":"GS-8","name":"正常解释有最常见标注","criterion":"合法解释中必须标注'最常见解释'作为优先核验素材。稽查人员到此先攻这条解释。"},
                    {"id":"GS-9","name":"核查动作含现场核查","criterion":"稽查动作不少于3步，必须包含至少1项现场核查。标杆: #1716含'装卸能力实测'(现场核查)和'物流公司走访'(外调走访)——物理事实只能现场验证。"},
                    {"id":"GS-10","name":"适用条件五维度","criterion":"适用条件须覆盖行业限制+纳税人资质+规模门槛+时间条件+金额门槛五个维度。"}
                ],
                "usage": "每轮精写完成后对照十条逐项自检。十条全满=顶级精写。发现缺项即补，不凑数不注水。"
            },
            "execution_guide": {
