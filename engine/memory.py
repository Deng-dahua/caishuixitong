...
# ═══ 分行业经营要素清单 ═══
# 背景：原逻辑对全行业套用同一套六要素（租赁/水电/物业/通信/物流/办公），会把
# "制造业没有生产能耗"这种致命缺失，与"服务业没有办公用品"同等看待，导致真正
# 指向空壳/账外经营的缺失被稀释。
# 真实经营的制造业必然发生：厂房场地、生产能耗、直接材料、运输物流、生产人员、生产设备。
# 先落地制造业（用户指定深耕），其余行业沿用通用六要素兜底，后续按同一模板扩展。
INDUSTRY_ELEMENT_PROFILES = {
    "制造业": {
        "required": ["场地", "能源", "直接材料", "运输物流", "人员", "设备"],
        "expected": ["包装", "仓储", "维修"],
        "optional": ["外协加工", "研发", "模具"],
        "note": "真实生产的制造业必然发生：厂房场地、生产能耗、直接材料、运输物流、生产人员、生产设备",
    },
}

# 要素识别关键词（通用 + 制造业专属）
# 重要：一律使用双字及以上词组。若用单字"电"/"水"，会误命中"电动牙刷""电子商务"
# 等品名，把没有能耗的企业误判为有能耗（假阴性），使要素缺失检测彻底失效。
ELEMENT_KEYWORDS = {
    "场地": ["租金", "租赁", "房租", "厂房", "场地", "物业"],
    "能源": ["电费", "水费", "用电", "供电", "自来水", "燃气", "天然气", "蒸汽", "能源", "电力", "热力", "燃煤", "燃油"],
    "直接材料": ["材料", "原料", "主料", "辅料", "钢材", "钢板", "铝材", "铜材", "塑料", "塑胶", "化工", "毛坯", "配件", "零件", "线材", "管材", "五金"],
    "运输物流": ["运输", "物流", "货运", "运费", "快递", "配送", "装卸", "搬运", "快运", "承运"],
    "人员": ["工资", "薪酬", "社保", "公积金", "劳务费", "人工费"],
    "设备": ["设备", "机器", "机床", "机械", "生产线", "注塑机", "冲压机", "模具", "融资租赁", "折旧"],
    "包装": ["包装", "包装物", "纸箱", "纸盒", "泡沫", "托盘", "缠绕膜"],
    "仓储": ["仓储", "仓库", "保管费", "堆存", "库房"],
    "外协加工": ["加工费", "委外", "外协", "受托加工", "来料加工"],
}
...
TAX_BURDEN_RULES = {
    ...
    "monitor_point_taxonomy": {
        ...
        "principle": "不是用规则去套企业，而是让数据自己说话，让矛盾自己暴露。企业上传一次资料，系统跑完全部规则，触发疑点按风险等级排序，经线索链与证据链并行配合（串行调查路径+并行交叉验证）、分析链跨域串联推理，四层数据平权汇入风险检查报告。",
        "categories": [
        ...
        "maintenance": "新增规则必须赋 monitor_category（13类之一）。智能更新/自动发现规则写入时同步打标；类别名称禁止自创。疑点=可检测的数据异常信号；定性标准/税务处理指引/政策适用等知识型内容禁止入疑点库，统一存 static/audit_knowledge.json 风险检查知识库。",
    },
