...
TAX_BURDEN_RULES = {
    ...
    # ═══════════════════════════════════════════════════════
        "rule_precise_writing": {
            ...
            "summary": "疑点稽查编制标准——通用指导每一条疑点实现'要么铁证如山，要么自证清白'的二元判定。23字段是维度框架，四条铁律是灵魂（铁证如山/自证清白/宁存疑不错杀/孤证不立），顶级精写十条是方法论，三条攻击角度是起点（商业逻辑/物理事实/数据穿透）。死磕最后的稽查结果——存疑不是终点，要么升级为铁证，要么被排除。",
            "canonical_example": {
                "id": 1813,
                "item": "预收账款长期挂账不转收入",
                "quality_notes": {
