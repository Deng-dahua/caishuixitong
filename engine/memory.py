...
═════ 推理系统仪表盘（2026-06-30 补录）═════
  ...
  【#2 规则库】1717条税务合规指令按分类浏览/搜索/详情查看
...
═════ 系统文件关联清单（2026-06-30）═════
  ...
  static/tax_risk_rules_local_export.json（1717条税务合规指令）
...
TAX_BURDEN_RULES = {
    ...
    # 权威源：1717条规则全部带 monitor_category 字段，取值必须来自以下13类。
    ...
    # ═══════════════════════════════════════════════════════
        "rule_precise_writing": {
            ...
            "rule_index": {
                "description": "30条顶级精写规则全景索引——按攻击角度分类，每条标注矛盾核心、追问数量、标杆特征，供编制时选型参照。",
                "total": 30,
                ...
                "cross_reference": "所有30条规则均按本标准（preamble哲学+attack_angles选型+23_fields逐字段+checklist十三勾验收）编制。每条规则的详细内容见 static/tax_risk_rules_local_export.json。这30条既是本标准的产出物，也是标准有效性的验证集——如果一条新规则无法达到同等深度，说明标准本身还需要完善。"
            },
