    ...
    # ═══════════════════════════════════════════════════════
        "rule_precise_writing": {
            ...
            "repealed_law_watch": {
                ...
                "repealed": [
                    {"废止法规": "《中华人民共和国增值税暂行条例》及其实施细则", "废止日期": "2026-01-01", "替代法规": "《中华人民共和国增值税法》（主席令第41号，2026-01-01施行）及《增值税法实施条例》（国务院令第826号）", "条文映射": "暂行条例第2条纳税人→增值税法第2条；第2条税率→第9条；第4条应纳税额→第14条；第6条销售额/价外费用→第17条；第19条纳税义务发生时间→第28条"},
            ...
            "23_fields": {
                ...
                "deep": {
                    ...
                    "fields": {
                        ...
                        "drill_questions": {
                            "name": "穿透追问",
                            "format": "穷举至稽查终点——问题间环环相扣、因果递进。三组递进方向(事实→证据→逻辑)为追问框架，每组穷举至无法提出新的有效追问。每组攻击要点: 事实层——交易六要素全覆盖（谁/什么/什么时候/在哪/怎么办的/谁参与的），任一空白=追问未穷举; 证据层——四流全追问（合同流/货物流/资金流/发票流），每流必须追到具体证据类型; 逻辑层——所有合理商业解释全排除（合同特殊条款/行业惯例/交易对手特殊情况/关联方关系/不可抗力），无法提出新解释=穷举完成。必须包含至少2条物理常识类追问——用不可辩驳的物理事实堵死辩解空间（如'工厂有没有能吊装这个吨位设备的行车''仓库能不能放得下全部挂账对应货物''仓储费为什么没有同步增加'）。追问在两种终点之一停止：①证实违法（铁证闭环）②排除违法（自证清白）。格式: Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。追问末尾必须包含穷举完成判定——'事实层六要素全覆盖/证据层四流全追问/逻辑层所有解释全排除→三问全答是→穷举完成'。",
                            "flexible": "追问数量由三维覆盖度决定，不设上下限: 事实层（交易六要素全覆盖，至少5问）+ 证据层（四流全追问，至少4问）+ 逻辑层（所有解释全排除，至少3问）。含至少2条物理类追问。数量是穷举完后的自然结果。标杆参照: #1716(15问含3条物理追问)、#1717(15问含2条物理追问)、#9(16问含全渠道收款追问)。",
            ...
            "summary": "疑点稽查编制标准——从17条深度精写规则（#1-#15、#1716、#1717）反复打磨后归纳的顶级编制体系。通用指导每一条疑点实现'要么铁证如山，要么自证清白'的二元判定。编制逻辑：①判断攻击角度（商业逻辑/物理事实/数据穿透）→②按攻击角度选择标杆规则的结构和攻击力→③逐字段对照顶级精写十条编写→④自检：存疑能否升级为铁证？企业能否按预设的逃生舱自证排除？答案全是否→重写。23字段是维度框架，四条铁律是灵魂，三条攻击角度是起点，十项自检是终点。每个字段的format描述已融入17条深度精写中提炼的全部模式（物理追问/首层命名/三色预警/物理核查优先/穷举标注/常见解释标注等）。",
            "canonical_example": {
                "id": 1813,
                "item": "预收账款长期挂账不转收入",
                "quality_notes": {
            ...
            "gold_standard_decoded": {
                ...
                "construction_methodology": {
                    ...
                    "drill_questions": {
                        ...
                        "physical_block": "必须包含至少2条物理常识类问题——用不可辩驳的事实堵死辩解空间。如#1717吸收的Q14'仓库能不能放得下'、Q15'仓储费为什么没有同步增加'。",
            ...
            "rule_index": {
                "description": "2条顶级精写规则全景索引——按攻击角度分类，每条标注矛盾核心、追问数量、标杆特征，供编制时选型参照。",
                "total": 30,
                "type_A_business_logic": {
                    "count": 5,
                    "desc": "A类（商业逻辑不可辩驳）——矛盾本身违反正常市场主体的经济理性，企业无法用'行业惯例'解释，开案成本最低，两证即可定案。",
                    "rules": [
                        {"id": 1717, "item": "预收账款挂账未申报", "score": 8, "contradiction": "已收款+未申报——五个字钉死", "drill_questions": "15问含仓库容积+仓储费物理追问", "signature": "所有标杆中矛盾最尖锐、辩解空间最小的规则。Q10'客户为什么愿意把钱放你这一年多'直接戳穿。推理5层到征管法第63条。"},
                ...
                "cross_reference": "所有2条规则均按本标准（preamble哲学+attack_angles选型+23_fields逐字段+checklist十三勾验收）编制。每条规则的详细内容见 static/tax_risk_rules_local_export.json。这2条既是本标准的产出物，也是标准有效性的验证集——如果一条新规则无法达到同等深度，说明标准本身还需要完善。"
            },
            ...
            "standard_upgrade_criteria": {
                ...
                "discovery_process": {
                    ...
                    "batch_size_note": "N=5为初始值。精写经验积累后，可由发现标准自身调整（如改为'每精写12条审一次'或'发现缺口即审'）。此调整记录写入发现标准的version_history。"
                },
                "cross_batch_dedup": {
                    ...
                    "two_layer_aggregation": {
                        "layer_1": {
                            "name": "批内交叉比较",
                            "description": "当前批次内部（如#6~#10共5条）各自捕获的缺口放在一起，看哪些缺口是同一种类型被多条规则触发",
                            "rules": [
                ...
                "self_upgrade": {
                    ...
                    "version_history": [
                        {"version": "v1.0", "date": "2026-07-22", "trigger": "精写#1~#5过程中发现现有前端面板的简版升级判定规则（单一攻击角度维度）无法捕获GS项/字段格式/特殊类型等缺口，系统性缺陷暴露——需建立完整的多维发现标准并将其纳入引擎记忆", "changes": "新建standard_upgrade_criteria章节：7个判定维度(DIM-1~7)、6步判定流程、4条自举规则(SU-1~4)；首次定义了'发现标准'为独立的元标准章节，与精写编制标准并行演进"}
                    ]
                },
                "usage_example": {
                    ...
                    "conclusion": "精写5条规则→触发DIM-1/2/3/4/5共5项缺口→5项中全部为共性缺口→触发精写编制标准升级。同时DIM-7未触发新盲区→发现标准v1.0暂不升级。"
                }
            },
            "execution_guide": {
