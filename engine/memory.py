        ...
        "rule_precise_writing": {
            ...
            "23_fields": {
                "basic": {
                    ...
                    "fields": {
                        "id": {"name": "异常编号", "format": "{类型前缀}-{三位序号}，类型: AN=隐匿收入/VC=虚列成本/VI=虚开发票/ST=少缴税款/OT=其他", "engine_exec": "发现溯源锚点。一键分析中 finding._rule_id 以此关联规则与发现；跨域线索链/证据链/分析链的 rule_refs 以此串联规则；删改规则时引擎按 id 同步全部链引用，防幽灵引用。"},
                        ...
                        "applicable_condition": {"name": "适用条件", "format": "五维度结构化: 行业限制+纳税人资质+规模门槛+时间条件+金额门槛。非全部必填，以实际为准", "engine_exec": "触发前置闸门（一键分析执行）。threshold扫描前引擎解析五维度中当前可判定的维度：行业限制与企业行业（加权投票识别结果）比对，限定行业不符直接拦截不触发，防止跨行业误报；资质/规模/时间/金额维度随对应数据源接入逐步启用硬校验。"}
                    }
                },
                "source": {
                    "count": 2,
                ...
                "deep": {
                    ...
                    "fields": {
                        ...
                        "phenomena": {
                            ...
                            "engine_exec": "铁律0载体（一键分析执行）。引擎提取「数据矛盾点：」首句注入 finding._contradiction，报告直接展示该发现的数据矛盾本质；全文参与语义匹配辅助命中。"
                        },
                        "focus": {
                            "name": "稽查重点",
                        ...
                        "normal_reason": {
                            ...
                            "engine_exec": "逃生舱执行（一键分析执行）。引擎将合法解释摘要注入 finding._escape_hatch，报告提示稽查员先验证排除项后再定性，防误伤合规企业——本体论三问之③的运行时落地。"
                        },
                        "determination": {
                            "name": "定性路径",
                            ...
                            "engine_exec": "定性分级标尺（一键分析执行）。引擎按发现的独立来源域数量自动映射三档定级写入 finding._evidence_grade：1域=线索/2域=强证据/≥3域=铁证，与三路径定级一一对应；阈值以下处理分支约束合规提示不入稽查程序。"
                        },
                        "risk_table": {
                            "name": "风险表格",
            ...
            "execution_guide": {
                ...
                "purpose": "精写编制说明（v3配套执行指引）。不重复定义23字段格式和内容，解决编写者在实际精写过程中反复出现的执行偏差问题。标准管「写什么」，本说明管「怎么写才不会写错」。本说明每一条防错都对应引擎校验点（三处校验器同源：命令行自检validate_rule_v3.py、验证API、启动加载校验），写错即被拦截——说明即校验契约，不是纸面建议。",
                "id_prefix_map": {
                ...
                "common_errors": [
                    ...
                    {"id": 3, "error": "level写法不在受控词表", "correct": "现行受控词表：极高风险/高风险/中风险/低风险/良好（自动发现规则另有信息档）——校验器与全库引擎逻辑均按此词表执行，禁止其他变体", "field": "④", "engine_check": "启动加载校验VALID_LEVELS词表；score-level锚点一致性检查（8+分须高/极高，6-7分须中/高）；重罪词条目强制≥高风险"},
                    {"id": 4, "error": "追问数量未达穷举就提交", "correct": "用穷举完成判定标准三问自检，全部答'是'才提交", "field": "⑬", "engine_check": "穿透追问格式校验：Q{N}/→潜台词/A三件套齐全、事实→证据→逻辑三组顺序正确、至少3问"},
                    {"id": 5, "error": "推理链未标注依赖证据", "correct": "每层必须写'依赖证据:XX'，漏标=引擎无法验证", "field": "⑫", "engine_check": "校验direction每层含「依赖证据」标注+复杂度标记+推理层格式，缺一报错"},
                    {"id": 6, "error": "正常解释证据要求写'提供相关证明'", "correct": "禁止笼统，必须具体到文件类型", "field": "⑯", "engine_check": "校验normal_reason至少4种、每种含「——需提供」+具体文件类型、附穷举说明；引擎将其注入finding._escape_hatch供稽查员验证排除"},
                    ...
                    {"id": 9, "error": "定性路径只有一条", "correct": "三路径必须全写(无法证明→线索/部分证明→强证据/完整证明→铁证)", "field": "⑰", "engine_check": "校验determination三路径（线索/强证据/铁证）+应对总原则齐全；引擎按独立来源域数为每条发现执行三档定级(_evidence_grade)"},
                    ...
                    {"id": 11, "error": "定性路径只有三路径，没有写阈值以下怎么处理", "correct": "有量化阈值(㉑)的规则必须补全'阈值以下处理'分支——不生成正式发现但需以合规提示形式记录，不影响合规度评分", "field": "⑰", "engine_check": "校验有量化阈值的规则determination必须含「阈值以下处理」分支；引擎按该分支将未达门槛情形记为合规提示不入稽查程序"},
                    ...
                    {"id": 13, "error": "正常解释没有标注'最常见解释'", "correct": "4种合法解释中必须有一项标注'最常见解释'作为红队攻击一优先素材——全员漏标率超过95%", "field": "⑯", "engine_check": "校验normal_reason中是否存在「最常见解释」字样——缺失则报错；引擎将此标记注入finding以提示稽查员优先验证该解释"}
                ],
                "scoring_anchors": {
                    "description": "风险评分锚点（⑤score）。引擎执行：score-level锚点一致性硬校验——8分以上必须高/极高风险、6-7分必须中/高风险；重罪词条目（骗税/伪造/虚开/两套账/洗钱等19词）强制≥8分且≥高风险；threshold扫描生成的疑点提示score上限5分待人工核实后按锚点升级",
                    "levels": [
                ...
                "impact_levels": {
                    "description": "影响程度判定锚点（⑱risk_table）。先锁定核心影响，再判断必然联动的次要影响，最后列出可能的间接影响。不凑数不强编。。引擎执行：校验risk_table必须含「核心」影响标注；附加税费标「间接」直接报错（必然联动应标次要）；报告补税测算按核心/次要税种范围计算",
                    "核心": "直接导致补税或触犯法条构成违法",
                    "次要": "随核心影响必然联动，非独立存在",
                    "间接": "可能存在但非必然，需额外证据支持"
                },
                "evidence_priority": {
                    "description": "证据优先级（⑲evidence）。引擎执行：校验evidence必须三级优先级（必须获取/应当获取/可以获取）齐全+金额分级（大额/中额/小额）；「必须获取」缺失对应发现只能停留线索档，不得升强证据/铁证",
                    "必须获取": "四流合一直接证据，缺1项闭环不成立→入线索等级",
                    "应当获取": "业务合理性佐证，增强但非必需",
                    "可以获取": "排雷辅助证据，补强或排除作用，不获取不影响闭环"
                },
                "evidence_layer_naming": {
                    "description": "第一层命名指引（⑲evidence）。不强制套用「货物流」，准确描述核心验证内容。引擎执行：命名贴合软校验——监控维度属申报流监控/账表质量与勾稽/社保与个税交叉/税务合规与程序的疑点，evidence首层若仍写「货物流」即报错（无实物流转的疑点套用货物流=证据框架错位）",
                    "映射": {
                        "实物交易类": "货物流",
                        "账务断裂类": "账实核对层",
                        "纯申报类": "申报数据层",
                ...
                "quality_benchmarks": {
                    "description": "品质标杆对照：不是比数量，是比每个字段是否达到同等穷举深度。引擎执行：标杆条目与库内实时一致（数字id现行体系），验证API可逐条复检36项",
                ...
                "submission_checklist": {
                    "description": "提交前自检（6组23项·每项均有引擎校验点，三处校验器同源执行）",
