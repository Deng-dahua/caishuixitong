...
═════ 系统核心能力宣言与角色边界 ═════
  ...
  系统负责：账务处理/跨域协商/审核闭环/过滤器/数据自检/行业认知/调度/知识库/法律推理
  智哥负责：编码态度/质量流程/验证自查/输出自检/框架搭建（不把业务逻辑硬编码）

══════════════════════════════════════════════════════════════
  ═══ 规则篇 —— 系统的硬逻辑规范，不可违反 ═══
  以下章节定义系统在各环节必须遵守的规则和判定标准。
  每条规则在代码中有对应的实现或检测机制，不是纸上条文。
══════════════════════════════════════════════════════════════

═════ 行业推断铁律（写入系统记忆中，随记忆系统永续生效）═════
  行业推断唯一依据 = 销项发票品名
  不参考进项发票品名
...
═════ 系统税务合规判定规则（2026-06-28 老邓亲授，写入系统记忆）═════
...
【规则九：文件类型识别体系（13类）】
  系统必须通过四步推理识别文件类型，不得仅靠文件名或单一关键词：
...
【规则十五：报告纯净度】
  ...
  - 系统自诊/修正记录（属于系统内部工作日志）
  代码: static/js/tax-doc-analysis.js 文本清理逻辑

【规则十六：审查驳回学习闭环】
  税务合规员通过审查面板驳回某条发现 → 系统记录驳回（finding_type + action:dismiss）
...
【系统铁律九：全行业适用】
  所有分析逻辑、规则系统、行业对标代码必须面向全行业各企业通用。
...
【规则二十二：第四章税务合规结论详细化】
  ...
  ⑤ 推理系统综合结论卡片（_phase4_synthesis存在时渲染）
  每段基于实际数据动态生成，禁止固定模板
...
═════ 模块联动关系矩阵（2026-06-29 新增）═════
  ...
  【数据类联动】
  方法论数量 → 手册/分析链/系统仪表盘/AGI/质量保障/core.js
  规则数量 → 税务合规指令/系统仪表盘/AGI/memory.py
  域函数数量 → 域分析/分析链/系统仪表盘
...
═════ 四阶段推理管线（2026-06-30 补录）═════
  系统核心推理系统采用四阶段递进推理架构，数据在四个阶段之间单向流动。
...
═════ 推理系统仪表盘（2026-06-30 补录）═════
  系统的实时状态监控中心，6个标签页覆盖系统全部运行维度。
...
═════ 前端页面体系（2026-06-30 补录）═════
  ...
  合同管理 / 档案管理 / 税务申报 / 资料风险分析 / 推理系统仪表盘 / 税务AGI
...
═════ 规则编号对照表（2026-06-30）═════
  ...
  【系统铁律 — 规则篇（本文档，约束系统硬逻辑）】
  系统铁律一：科目name → 写入前查DB（第6章）
  系统铁律二：三号合并 → 禁止逐条for调用（第6章）
  系统铁律三：审计铁律 → audit.py 7项全过（第6章）
  系统铁律四：ref_id去重 → 精确匹配禁模糊（第6章）
  系统铁律五：普票税额并入成本 → 普票不拆税额（第6章）
  系统铁律六：7分类禁止兜底 → 不在7分类返回None（第6章）
  系统铁律七：规则=代码 → 记忆与实现必须一致（第7章）
  系统铁律八：代码即承诺 → 声称的功能必须代码存在（第7章）
  系统铁律九：全行业适用 → 禁止行业特化硬编码（第7章）
  系统铁律十：主动关联更新 → 一处过时全项目同步（第7章）
  系统铁律十一：方法论先行 → 功能必须先有方法论定义（第7章）
  系统铁律十二：跨模块内容一致性 → 同一内容多模块出现必须一致（第8章·新增）
...
# ═══════════════════════════════════════════════════════════════
# 系统学习要点（2026-06-30 老邓亲授）：
# 同一个品名关键词，在不同企业类型、不同用途下，抵扣结果完全不同。
# 系统必须做"企业画像 → 品名 → 用途"三要素综合判定，而非简单关键词匹配。
# 
# 例如：
...
#
# 同样的品名"酒"，四种完全不同的税务处理！
# 系统必须学会：先看企业是谁，再看用来干什么，最后做判定。
VAT_CONTEXTUAL_REVERSAL_OVERRIDES = {
    "description": "关键词触发了进项税额转出嫌疑，但以下上下文可豁免——系统的'二次思考'机制",
            ...
            "exempt_when": [
                {
                    ...
                    "reason": "金融企业的融资成本属于正常经营支出（但贷款服务本身的进项仍不可抵扣——这是硬性规定。此处指金融企业作为借款方支付给其他金融机构的利息，该利息支出的进项税额按现行税法仍不可抵扣。系统保留此条作为特殊情况记录待税法修正。）",
                    "enterprise_types": ["银行", "金融", "保险", "证券"],
                    "account_keywords": [],
                    "note": "现行税法下贷款服务进项一律不可抵扣，此为硬性规定，不支持任何豁免。系统记录此条作为与用户的讨论备忘。",
    ...
    "engine_learning_note": (
        ...
        "排除误判后给出终审结论。"
        "系统记录了老邓2026-06-30的教导：同一个'酒'字，在餐饮店是调料，在酒厂是原料，在化工厂是燃料，在贸易公司才可能是招待费。"
    ),
}
...
TYC_INTEGRATION = {
    ...
    "usage_in_system": {
        "追问时自动触发": "用户问'查一下这家供应商'→ AGI系统调用TianyanchaClient.check_company()",
...
TAX_BURDEN_RULES = {
    ...
    "note": (
        "以上三原则写入系统记忆后，任何涉及税负计算的代码模块必须遵守。"
    ...
    "monitor_point_taxonomy": {
        ...
        "principle": "不是用规则去套企业，而是让数据自己说话，让矛盾自己暴露。企业上传一次资料，系统跑完全部规则，触发疑点按风险等级排序，经线索链与证据链并行配合（串行调查路径+并行交叉验证）、分析链跨域串联推理，四层数据平权汇入稽查报告。",
        "categories": [
            {"name": "资金流监控", "count": 70, "logic": "银行流水是外部基准，比对申报、发票、账载，揭示隐匿收入、账外循环。私户收款/资金回流/公转私/现金异常/账外支付。"},
            {"name": "发票流监控", "count": 158, "logic": "进销匹配、品名关联、开票行为、发票合规。品名背离/数量不匹配/顶额开票/作废红冲/连号/夜间开票/滞留票。"},
            {"name": "申报流监控", "count": 429, "logic": "增值税、所得税、财报三表比对，税种间勾稽。两税收入不一致/零申报/税负率偏低/留抵异常/预缴不足。含66行业税负率区间规则。"},
            {"name": "社保与个税交叉", "count": 178, "logic": "工资、社保、个税三数勾稽。三源不一致/有薪无保/全员零个税/劳务报酬/股东借款视同分红/年终奖拆分。"},
            {"name": "经营实质穿透", "count": 194, "logic": "用物理世界数据（电、水、场地、人工、产能）验证账面真实性。电耗产量/仓储库存/运费发货/设备产能/物耗能耗产能三线归一。"},
            {"name": "关联交易与利益输送", "count": 53, "logic": "穿透隐藏关联关系，检验交易独立公允性。六员重叠/转移定价/资本弱化/无息拆借/购销闭环。"},
            {"name": "虚开发票专项", "count": 88, "logic": "资金回流、四流不一、闭环开票、短期激增。最高风险级：变名/挂靠/富余票/走逃失联/空壳开票。"},
            {"name": "财产行为税监控", "count": 99, "logic": "房产、土地、印花、城建、环保等小税种与主税、资产变动联动比对。有房无税/合同未贴花/附加税不匹配/土增清算。"},
            {"name": "出口退税监控", "count": 45, "logic": "报关、发票、收汇三单匹配，产能与出口规模匹配。假报出口/循环出口/假自营真代理/留抵退税异常。"},
            {"name": "行业专项监控", "count": 192, "logic": "制造/建筑/房地产/商贸/餐饮/电商/医药/教育/灵活用工/再生资源等行业特定风险。"},
            {"name": "外部数据比对", "count": 24, "logic": "工商、银行、海关、电力、人社等第三方数据与申报数据比对。"},
            {"name": "账表质量与勾稽", "count": 134, "logic": "报表自身平衡、账表一致、跨期逻辑合理。恒等式断裂/科目勾稽/往来挂账/折旧摊销/凭证质量。"},
            {"name": "税务合规与程序", "count": 53, "logic": "备案、申报期限、优惠资格、历史处罚、合同管理、发票程序等合规性监控。"},
        ],
        ...
        "maintenance": "新增规则必须赋 monitor_category（13类之一）。智能更新/自动发现规则写入时同步打标；类别名称禁止自创。疑点=可检测的数据异常信号；定性标准/税务处理指引/政策适用等知识型内容禁止入疑点库，统一存 static/audit_knowledge.json 稽查知识库。",
    },

    # ═══════════════════════════════════════════════════════
        "rule_precise_writing": {
            "version": "2026-07-18 · 23字段修订版 + 穷举完成判定标准 + 疑点本体论",
            "principle": "精写标准=23字段完整框架 + 分级深度  + 证据映射。每个字段的下限为最低要求，以异常点实际出发；确实达不到下限如实注明原因。框架是地图，不是尺子。",
            # ═══ 疑点本体论（老邓 2026-07-18 确立·位阶高于一切编制规则）═══
            "doubt_ontology": {
                "title": "税务疑点只有一个来源——数据矛盾",
                "core": "不是法律条文，不是会计知识，不是处理标准。就是数据矛盾。'该企业可能存在偷税风险'不可查；'银行账户全年收款5000万但申报收入只有3000万'立刻可查——后者才是疑点。写疑点第一步：你手里有哪些数据？这些数据之间应该是什么关系？关系不对，就是疑点。",
                "three_types": [
                    "①应该相等的，但不等了——资产负债表左右不等 / 两税申报收入不一致 / 工资社保个税三源不一致（勾稽断裂类）",
                    "②应该有的，但没有——有销售无运费 / 有房产无房产税 / 有员工无社保（缺失类）",
                    "③不应该这样的，但这样了——购钢材开咨询票 / 新办3个月开5000万 / 凌晨3点开票（模式异常类。注：'应该'的基准来自行业基准库与行为统计规律，基准本身也是数据，故本类仍是数据矛盾）",
                ],
                "writing_steps": [
                    "第一步·定义数据矛盾点：一句话说清哪两套数据对不上（'银行收款总额 vs 增值税申报收入'√，'成本费用不合规'×）。一句话说不清，疑点不成立。",
                    "第二步·穷举合理解释：给企业定罪前穷尽合法商业解释，每种解释规定可排除它的具体证据（股东注资→打款备注+增资协议；借款→合同+利息记录）。企业拿不出证据，矛盾才升级为风险。",
                    "第三步·构建定性路径：铁证（多源互证+无法自证→认定+补税罚款）/ 强证据（部分印证→涉嫌+续查）/ 线索（小额偶发→存疑不入结论）。",
                ],
                "three_checks": [
                    "①它能被系统自动发现吗？——系统无法从数据扫出的异常不是有效规则（'可能存在风险'×，'连续3个月进项大于销项且无库存增加'√）",
                    "②它有明确的证据路径吗？——触发后稽查员必须知道查哪些单据、调哪些流水、问哪些人，否则是空谈",
                    "③它有合法的逃生舱吗？——企业能用证据排除疑点；不能被排除的规则逻辑有漏洞，会误伤合规企业",
                ],
                "writing_order": "先写四流矛盾（钱=银行流水 vs 票=发票 vs 账=申报表 vs 实=经营数据），再写税种与税种间矛盾，再写财务与实物矛盾，最后写行业特有矛盾。",
                "boundary": "疑点库不是法规汇编，不是会计教材，是一张数据矛盾关系网。立案标准、偷税与少缴的区分等属于规则查实之后用的知识，存 static/audit_knowledge.json，永不入疑点库。",
                "template_mapping": "快速起草模板8段与23字段映射：现象描述(数据矛盾点+典型表现+排除条件)→⑭phenomena；推理链→⑫direction；稽查重点→⑮focus；正常解释→⑯normal_reason；定性路径→⑰determination；证据清单→⑲evidence；触发指标→㉑threshold；稽查处理→㉒suggestion。模板起草，精写补全其余字段。",
            },
            "iron_rules": [
                "【铁律0·数据矛盾唯一来源】疑点只来自数据矛盾——应该相等的不等了/应该有的没有/不应该这样的但这样了。写不出'X vs Y哪里对不上'的条目不是疑点。",
            ...
            "exhaustion_criteria": {
                ...
                "drill_questions_done": {
                    ...
                    "最终判定": "当规则编写者对以下三问全部回答'否'时追问穷举完成：①还有没有尚未覆盖的交易要素(谁/什么/什么时候/在哪里/怎么做/谁参与的)？②四流中还有没有尚未追问到对应证据的环节？③对方还可能提出什么我没问到的合理商业解释？全部答'否'，追问数量即最终数量——是11720条还是1720条都是正确答案。"
                },
                "direction_done": {
            ...
            "repealed_law_watch": {
                ...
                "repealed": [
                    {"废止法规": "《中华人民共和国增值税暂行条例》及其实施细则", "废止日期": "2026-01-01", "替代法规": "《中华人民共和国增值税法》（主席令第41号，2026-01-01施行）及《增值税法实施条例》（国务院令第826号）", "条文映射": "暂行条例第1720条纳税人→增值税法第1720条；第1720条税率→第1720条；第1720条应纳税额→第11720条；第1720条销售额/价外费用→第11720条；第11720条纳税义务发生时间→第21720条"},
                    {"废止法规": "《中华人民共和国营业税暂行条例》", "废止日期": "2016-05-01", "替代法规": "全面营改增，相关业务改征增值税，依《增值税法》", "条文映射": "营业税应税项目→增值税应税交易，不再有营业税"},
                    {"更新法规": "《中华人民共和国会计法》", "现行版本": "2024年修正（2024-07-01施行）", "注意": "引用须标注2024修正版；第1720条新增'任何单位不得以虚假的经济业务事项或者资料进行会计核算'"},
                ],
                ...
                "checker_program": "engine/law_validity_checker.py —— 可运行的法律时效性核查程序（老邓2026-07-13确立）。理念：不把法条写死当永久真理，引用现行有效法律并由程序动态核查+自动处理，无'待人工核验'出口。维护CURRENT_VALID_LAWS(现行有效清单)+REPEALED_LAWS(已废止清单)；提供 check_policy_ref(text)逐条核查、scan_rules(rules)全库扫描、auto_process(rules)系统自动校验+自动处理(替换废止法条+自动核验补标注)。已接入 audit_consistency.py：--fix 时系统自动处理，引用废止法自动替换、缺核验的自动核验补标注。法律变动时只更新两张清单+重跑，无需逐条改死代码。"
            },
            "preamble": {
                "title": "疑点稽查编制总纲——要么铁证如山，要么自证清白",
                "core_philosophy": "本标准的终极目标不是让系统多发现几个疑点，而是让每一条疑点都经得起稽查实战的终极检验。编制疑点不是写'可能有问题的清单'——而是写'要么你拿出证据证明自己清白，要么我拿出铁证证明你有问题'的二元判定规程。每一条规则写完后，必须能回答：如果最终查无实据，是规则本身的问题还是被查企业的问题？如果是规则的逻辑有漏洞、追问不够狠、证据链不够紧——那这条规则就要重写。",
                "four_iron_principles": [
                    {"name":"铁证如山","desc":"完整证据闭环（≥3独立来源+无反向证据+三性全通过）→认定。不可辩驳——不是'我们认为'，是'任何理性第三人拿到同样证据只能得出同一结论'。"},
                    {"name":"自证清白","desc":"企业能用规则中预设的合法解释和证据要求，逐项自证排除疑点。规则必须给出逃生舱——不是放水，是确保每一枪都打在真正有问题的人身上。"},
                    {"name":"宁存疑不错杀","desc":"证据不达标→存疑不入结论。永远不要把'存疑'升格为'认定'。存疑不是失败，是严谨。"},
                    {"name":"孤证不立","desc":"单一数据源永远不能形成闭环。银行流水+发票+合同+申报——至少两个独立来源的证据指向同一事实，闭环才成立。"}
                ],
                "binary_outcome": "每条规则的终极产出必须是二元判定：要么铁证如山（完整闭环→认定→补税+罚款+移送），要么自证清白（企业穷举排除全部合法解释→疑点消除）。中间态'存疑'是中途站，不是终点站——存疑要么升级为铁证，要么被排除，不能永远存疑。"
            },
            "23_fields": {
                "basic": {
                    "count": 9,
                    ...
                    "fields": {
                        "id": {"name": "异常编号", "format": "{类型前缀}-{三位序号}，类型: AN=隐匿收入/VC=虚列成本/VI=虚开发票/ST=少缴税款/OT=其他", "engine_exec": "发现溯源锚点。一键分析中 finding._rule_id 以此关联规则与发现；跨域线索链/证据链/分析链的 rule_refs 以此串联规则；删改规则时系统按 id 同步全部链引用，防幽灵引用。"},
                        "item": {"name": "异常名称", "format": "受控词表统一命名，同义不同名禁止", "engine_exec": "规则匹配主键。一键分析三级消费：①发现type与item精确匹配→注入深度字段；②精确失败走bigram语义匹配（阈值0.55宁缺勿滥）；③threshold主动扫描按item防重复生成。受身份锁保护，任何自动流程不得改名。"},
                        "category": {"name": "所属类别", "format": "五类之一: 隐匿收入/虚列成本/虚开发票/少缴税款/其他违规", "engine_exec": "定性分类载体。报告按类别汇总违规性质，LLM推理上下文携带。受身份锁保护。"},
                        "level": {"name": "风险等级", "format": "极高/高/中/低/良好。与合规度对应: 极高<40分/高40-60/中60-80/低80-90/良好>90", "engine_exec": "发现风险等级来源。threshold扫描触发的发现按「疑点非结论」铁律统一降为中风险待人工核实；校验器强制重罪条目≥高风险、score-level锚点一致。"},
                        "score": {"name": "风险评分", "format": "1-10分。锚点: 10=系统性造假/金额巨大/主观故意; 5=中等风险/需补充证据; 1=低风险/小额/偶发", "engine_exec": "发现评分与排序依据。报告按分值降序展示；threshold扫描注入的发现score上限5分（金额维度已验证、业务属性未核实）；8分以上进入高风险清单。"},
                        "check_frequency": {"name": "稽查频率", "format": "高频=每户必查; 中频=行业匹配时查; 低频=特定条件触发时查", "engine_exec": "扫描调度档位（一键分析执行）。高频/中频规则参与threshold常规扫描（触发置信度门槛0.85）；低频=特定条件触发时查——仅在结构化条件强触发（置信度≥0.95）时生成发现，防低频规则全量扫描误报。"},
                        "policy_ref": {"name": "法律依据", "format": "引用【现行有效】的法律法规——统一冠以'现行有效的《XX法》(版本)第X条'表述，保留具体条号以保证稽查精确性。法律不写死、不当永久真理：每条引用必须过 engine/law_validity_checker 法律时效性核查程序验证，policy_ref末尾强制附'法规现行性核验：YYYY-MM-DD'。法律废止/更替时，只更新 repealed_law_watch 与 law_validity_checker 清单并重跑核查，即可标出全库需更新的规则。", "engine_exec": "发现的法律依据，直接注入发现与报告。law_validity_checker接入审计：自动核验法规现行性、废止法规自动替换、强制核验日期。"},
                        "tax_impact": {"name": "税务影响", "format": "区分最低影响与典型影响。格式: 税种(影响程度:核心/次要/间接，最低补税X万，典型补税Y万)。多个税种分别列明。须给出具体补税计算公式，使稽查人员拿到即可测算——如'增值税=挂账余额÷(1+税率)×税率；企业所得税=挂账余额×应税利润率×25%'。禁止笼统写'补税XX税'不给金额。", "engine_exec": "报告税负测算与处置建议段落的数据源，随发现注入。"},
                        "applicable_condition": {"name": "适用条件", "format": "五维度结构化: 行业限制+纳税人资质+规模门槛+时间条件+金额门槛。非全部必填，以实际为准", "engine_exec": "触发前置闸门（一键分析执行）。threshold扫描前系统解析五维度中当前可判定的维度：行业限制与企业行业（加权投票识别结果）比对，限定行业不符直接拦截不触发，防止跨行业误报；资质/规模/时间/金额维度随对应数据源接入逐步启用硬校验。"}
                    }
                },
                "source": {
                    "count": 2,
                    ...
                    "fields": {
                        "source": {"name": "来源", "format": "空=人工精写 / 系统发现 / LLM生成", "engine_exec": "来源标记与身份锁保护对象。自动发现规则须经人工确认转正；报告区分人工精写/系统发现/LLM生成三种来源的可信度。"},
                        "auto_type": {"name": "自动发现类型", "format": "行业基准校准/购销倒挂/毛利为负/缺失数据/综合异常/未知模式", "engine_exec": "自动发现分类标记。自动规则面板分组展示与转正审核参考；通病类信号已在生成端黑名单拦截。"}
                    }
                },
                "deep": {
                    "count": 12,
                    ...
                    "fields": {
                        "direction": {
                            "name": "推理链",
                            "format": "推理至稽查终点——从数据矛盾出发，每一层追问'为什么会出现这个矛盾'，层与层之间因果递进、环环相扣。根据攻击角度选择推理深度: A类(商业逻辑不可辩驳)→推理链可精简至2-3层，因矛盾本身已接近自证; B类(物理事实不可辩驳)→推理链加物理验证层，用测量替代推理; C类(数据异常需穿透)→推理链至少4层，含追溯断裂时点+科目穿透定位两层核心技术。所有类型必须包含'追溯首次异常时点'层（定位异常起点，排除系统性故障解释）。最后一层必须落地到具体法条编号。层数是因果链条的自然长度。每层格式: 【推理第N层: XX法则】依赖证据: XX → 结论: XX。",
                            "flexible": "三种结束条件(满足任一): ①定性落地——最后一层已明确指向证实违法或排除违法; ②证据尽头——下一层需要的证据无法获取，标注断点; ③逻辑闭环——最后一层回到第一层前提，无新因果环节。",
                            "engine_exec": "推理骨架（一键分析执行）。命中后注入 finding._rule_direction，作为LLM推理上下文与报告「为何成为疑点」段落的推理链来源；跨域分析链的推理路径与其衔接。"
                        },
                        "drill_questions": {
                            "name": "穿透追问",
                            "format": "穷举至稽查终点——问题间环环相扣、因果递进。三组递进方向(事实→证据→逻辑)为追问框架，每组穷举至无法提出新的有效追问。每组攻击要点: 事实层——交易六要素全覆盖（谁/什么/什么时候/在哪/怎么办的/谁参与的），任一空白=追问未穷举; 证据层——四流全追问（合同流/货物流/资金流/发票流），每流必须追到具体证据类型; 逻辑层——所有合理商业解释全排除（合同特殊条款/行业惯例/交易对手特殊情况/关联方关系/不可抗力），无法提出新解释=穷举完成。必须包含至少1720条物理常识类追问——用不可辩驳的物理事实堵死辩解空间（如'工厂有没有能吊装这个吨位设备的行车''仓库能不能放得下全部挂账对应货物''仓储费为什么没有同步增加'）。追问在两种终点之一停止：①证实违法（铁证闭环）②排除违法（自证清白）。格式: Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。追问末尾必须包含穷举完成判定——'事实层六要素全覆盖/证据层四流全追问/逻辑层所有解释全排除→三问全答是→穷举完成'。",
                            "flexible": "追问数量由三维覆盖度决定，不设上下限: 事实层（交易六要素全覆盖，至少5问）+ 证据层（四流全追问，至少4问）+ 逻辑层（所有解释全排除，至少3问）。含至少1720条物理类追问。数量是穷举完后的自然结果。标杆参照: #1716(15问含1720条物理追问)、#1717(15问含1720条物理追问)、#9(16问含全渠道收款追问)。",
                            "engine_exec": "稽查执行层素材。命中后随规则上下文注入LLM与报告稽查建议段，为现场讯问提供环环相扣的追问路径。"
                        },
                        "phenomena": {
                            "name": "现象描述",
                            "format": "典型表现枚举(非穷举)，每种格式: 表现描述+数据特征(百分比/绝对值)。兜底条款: '其他同类或类似表现'。增加排除条件: 什么情况下的类似表现不属于本异常，须明确排除边界。数据矛盾点须用一句话说清两组数据的对比关系——'X vs Y——哪里对不上'，如'预收账款账面余额 vs 增值税申报收入——已收款+未申报'，一句话说不清的疑点不成立。每个典型表现须含具体数据特征（如'>资产总额0.5%且绝对额>5万元'），使系统可据此扫描。",
                            "flexible": "至少5种典型表现。无法枚举到5种的注明原因。",
                            "engine_exec": "铁律0载体（一键分析执行）。系统提取「数据矛盾点：」首句注入 finding._contradiction，报告直接展示该发现的数据矛盾本质；全文参与语义匹配辅助命中。"
                        },
                        "focus": {
                            "name": "稽查重点",
                            "format": "策略层——舞弊手法预判，不直接用于提问。格式: 舞弊手法名称: 具体操作方式 → 识别要点。用①②③④逐条标注。与⑬分工: ⑮是策略层(预判)，⑬是执行层(将预判转化为讯问问题)。",
                            "flexible": "至少①②③，多则不限",
                            "engine_exec": "策略层预判。命中后注入LLM上下文，指导报告舞弊手法提示段；与drill_questions策略→执行分工。"
                        },
                        "normal_reason": {
                            "name": "正常业务解释",
                            "format": "穷举全部真实存在的合法情形。格式 {情形}——需提供{具体证据(文件类型)}。禁止使用'提供相关证明'等笼统表述，必须具体到文件类型（如'销售合同原件''银行付款凭证''客户书面确认函'）。必须标注'最常见解释'作为稽查人员优先核验素材——所有合法解释中至少有一项标注（最常见）。穷举完毕后标注'穷举说明：已穷举全部合法情形'。确无合法情形时如实列明0-3种并注明原因（如'该异常违背正常商业逻辑，无法提出合法解释'），禁止编造。",
                            "flexible": "数量=真实存在的合法情形数量，下限0上限穷举完毕。穷举判定标准(五个自问): 合同特殊条款/行业惯例/交易对手特殊情况/税收政策特殊规定/不可抗力——五问全答'否'=穷举完成。≥5种时需自问该异常是否真构成异常。标杆: #2写了5种、#1717从4种吸收到5种。",
                            "engine_exec": "逃生舱执行（一键分析执行）。系统将合法解释摘要注入 finding._escape_hatch，报告提示稽查员先验证排除项后再定性，防误伤合规企业——本体论三问之③的运行时落地。"
                        },
                        "determination": {
                            "name": "定性路径",
                            "format": "必须为平铺文本段落，严禁JSON dict/键值对格式。三路径必须对应证据链三档定级: ①无法证明→线索等级(单源数据触发)→定性'存疑，建议补充调查'+不入正式结论; ②部分证明→强证据(2独立来源验证)→定性'涉嫌XX'+可入正式结论但标注'证据部分闭环'; ③完整证明→铁证(≥3独立来源闭环)→定性'认定XX'+入正式结论可作处罚依据。每路径定义进入条件。设置了量化阈值(㉑)的规则，必须增加'阈值以下处理'分支——门槛未达到时以合规提示形式记录，不入稽查程序、不影响合规度评分。结尾附应对总原则。",
                            "flexible": "终极目标: 要么铁证如山（完整闭环→认定→补税罚款移送），要么自证清白（企业按预设逃生舱逐项排除→疑点消除）。中间态'存疑'是中途站不是终点站——存疑必须能升级为铁证或降级为排除，不能永远存疑。应对总原则: 能完整证明走铁证路径——锁定核心矛盾+追踪资金去向+固定闭环证据; 能部分证明走强证据路径——锁定差额归属+补独立来源证据后重判; 无法证明入线索池不入正式结论。有量化阈值的必须增加阈值以下处理分支。避免'一触发就定性'和'不触发就无视'两个极端。",
                            "engine_exec": "定性分级标尺（一键分析执行）。系统按发现的独立来源域数量自动映射三档定级写入 finding._evidence_grade：1域=线索/2域=强证据/≥3域=铁证，与三路径定级一一对应；阈值以下处理分支约束合规提示不入稽查程序。"
                        },
                        "risk_table": {
                            "name": "风险表格",
                            ...
                            "engine_exec": "报告跨税种影响段落数据源，核心/次要/间接标注决定补税测算的税种范围。"
                        },
                        "evidence": {
                            "name": "证据清单",
                            "format": "四层框架: 货物流+合同资金流+业务合理性+排雷。交易金额分级: 大额(>10万)分AB场景(自提vs直运); 中额(1-10万)至少取2个维度证据; 小额(<1万)抽样核验。每层证据标注优先级: 必须获取/应当获取/可以获取。首层命名必须贴合疑点实际类型——禁止全部套用'货物流': 实物交易类→货物流; 账务断裂类→账实核对层; 纯申报类→申报数据层; 成本费用类→费用真实性层; 关联交易类→关联关系与定价层; 发票类→发票与货物流; 资金流类→资金流与收款方核对层。排雷层与⑯联动——⑯列出合法情形，排雷层验证这些情形的证据是否真实存在。",
                            "flexible": "每层至少1项'必须获取'。首层名根据疑点类型选择——滥用'货物流'直接报错。大额(>10万)四层全取+AB必分; 中额(1-10万)至少取一二层+排雷层; 小额(<1万)抽样核验。标杆: #2'费用真实性层'(凭证量异常)、#1716'货物流与财务流核对层'(运输费缺失)、#1'账实核对层'(借贷不平)。",
                            "engine_exec": "取证指引。四层框架与必须/应当/可以优先级随发现注入报告稽查建议，决定证据调取顺序与金额分级核验深度。"
                        },
                        "threshold": {
                            "name": "触发指标",
                            "format": "必须有量化阈值+前置条件(行业+资质+数据+时间四维度)+三色预警等级(黄/橙/红)。阈值增加行业差异: 通用阈值+行业调整值(如建筑730天/商贸90天/服务180天/制造540天)。二元异常写'=是即触发'。预警等级按严重程度升档: 黄=轻微超出阈值/橙=明显超出/红=严重超出且伴随其他异常信号。",
                            "flexible": "前置条件四个维度各自独立，全满足才触发。预警等级三色必含——黄/橙/红逐级升档，不能笼统写'超过一定期限'。标杆: #1717'黄(账龄365-540天)→橙(540-730天)→红(>730天)'同时含四个行业调整值；#1'黄(差额5-10万)→橙(10-100万)→红(>100万)'。",
                            "engine_exec": "主动扫描执行核心（一键分析执行）。rule_consumer 结构化解析量化条件后对账扫描（银行流水/发票/工资/社保/凭证），条件满足即主动生成发现——触发门槛按check_frequency分档（常规0.85/低频0.95）；无法结构化解析的threshold降级为匹配辅助文本，不阻断分析。"
                        },
                        "action": {
                            "name": "稽查动作",
                            "format": "从纸面比对到现场核查穷举全部核查步骤，物理验证类核查优先于纸面比对——能用测量验证的不用文件验证。每步格式: 动作类型(现场核查/纸面比对/外调走访/联网核查)+具体操作+预期产出。现场核查须写清具体操作（如'实地测量仓库可用容积'而非笼统的'现场核查'）。确不存在现场核查可能性的(如纯申报类异常)，注明原因后补充替代性核查手段(交叉比对第三方数据/函调/联网核查)，不强制虚构现场核查。",
                            "flexible": "下限至少3步含1项现场核查，上限穷举完毕。物理验证优于纸面比对——B类(物理事实)攻击角度的规则，核查动作中物理验证类至少占2步。标杆: #1716含'装卸能力实测'(现场核查)和'物流公司走访'(外调走访)共5步；#1717吸收后加到7步含'仓库容积实测'+'仓储费用穿透'。",
                            "engine_exec": "核查方案来源。纸面比对→现场核查步骤随发现注入报告核查方案段。"
                        },
                        "suggestion": {
                            "name": "稽查处理",
                            "format": "稽查局视角。固定格式: 定性→补税(分税种+金额+具体计算公式)→滞纳金(日万分之五+起止日期)→罚款(征管法条款+区间)→移送标准(金额/情节门槛)。与⑰定性路径结论一致。须包含补税测算段落，给出各税种的具体计算公式，使金额可直接复核——如'增值税=隐匿收入差额×适用税率；企业所得税=差额×25%；滞纳金=应补税额×日万分之五×滞纳天数'。禁止笼统写'补缴相关税款'。",
                            "flexible": "无",
                            "engine_exec": "稽查处置建议，直接进入发现与报告处置段落（稽查局视角）。"
                        },
                        "remedy": {
                            "name": "整改建议",
                            ...
                            "engine_exec": "整改建议，进入报告企业视角段落，与suggestion分视角呈现。"
                        }
                    }
                }
            },
            "summary": "疑点稽查编制标准——从11720条深度精写规则（#1-#15、#1716、#1717）反复打磨后归纳的顶级编制体系。通用指导每一条疑点实现'要么铁证如山，要么自证清白'的二元判定。编制逻辑：①判断攻击角度（商业逻辑/物理事实/数据穿透）→②按攻击角度选择标杆规则的结构和攻击力→③逐字段对照顶级精写十条编写→④自检：存疑能否升级为铁证？企业能否按预设的逃生舱自证排除？答案全是否→重写。23字段是维度框架，四条铁律是灵魂，三条攻击角度是起点，十项自检是终点。每个字段的format描述已融入11720条深度精写中提炼的全部模式（物理追问/首层命名/三色预警/物理核查优先/穷举标注/常见解释标注等）。",
            "canonical_example": {
                "id": 1813,
                "item": "预收账款长期挂账不转收入",
                "quality_notes": {
                    "drill_questions": "11720条，三组递进（事实Q1-Q5/证据Q6-Q9/逻辑Q10-Q13），每组穷举至无新的有效追问。最后自检：事实六要素全覆盖、证据四流全追问、逻辑三个合理解释全排除 → 穷举完成。",
                    "direction": "4层，自然结果而非硬性目标。第一层→账龄异常识别，第四层→隐匿收入定性。中间无跳步（第二层纳税义务判定、第三层未申报事实固定）。每层标注依赖证据，可被系统自动校验。",
            ...
            "gold_standard_decoded": {
                "description": "顶级精写方法论——从七条标杆规则（#1借贷不平/#2凭证量异常/#1716运输费缺失/#1717预收挂账/#1718资金闭环回流/#1719社保人数缺口/#1720月末集中开票）的反复打磨中提炼的编制体系。v6升级: 新增时序异常型维度——从静态数据比较升级到动态行为节奏分析。",
                "attack_angles": {
                    "description": "疑点的六种攻击角度。编制疑点的第一步是判断它属于哪种攻击角度——这决定了推理链的穿透深度、追问的攻击力和辩解空间的大小。",
                    "type_A": {"name":"商业逻辑不可辩驳","desc":"矛盾本身违反正常市场主体的经济理性。企业无法用'行业惯例'解释。典型: #1717预收挂账——500万放在别人账户一年多不催，任何正常企业都不会这样做。开案成本最低，两证即可定案。","marker":"大额资金无偿存放、长期挂账不处理、单方受益无对价"},
                    "type_B": {"name":"物理事实不可辩驳","desc":"矛盾违反物理常识。不需要推理，只需要测量。典型: #1716运输费缺失——三吨设备不可能自己飞到客户处。追问用物理堵死辩解空间（装车能力实测、仓库容积测算）。","marker":"重量/体积与实际不符、能耗与产量不一致、仓储容量装不下存货"},
                    "type_C": {"name":"数据异常需穿透","desc":"矛盾是数据偏差而非必然违规。需要逐层排除合法解释后才能定性。典型: #1借贷不平——可能是系统切换/会计差错/账外循环，需要科目穿透+资金流交叉验证才能定。","marker":"比率偏离基准、凭证量异常、税负率偏差"},
                    "type_D": {"name":"关系网络不可辩驳","desc":"矛盾不在一对数值之间，而在多个节点之间的环形关系。单节点视角看不出任何问题——进销票合规、资金有进有出、合同齐全——但把所有节点连成交易网络图后，环形闭环暴露。典型: #1718资金回流闭环——A付给B→B转给C→C转回A实控人个人卡。必须构建网络图谱才能发现。","marker":"六员重叠、地址电话重叠、资金闭环回流、购销闭环、网络图谱、隐性关联"},
                    "type_E": {"name":"法律义务真空","desc":"矛盾不是两个数对不上，而是一个数应该有但根本不存在。A的存在（银行代发工资=有员工）必然触发B的法定义务（有员工=必须参保），但B=0。不是偏差，是法律义务的完全缺失。典型: #1719社保人数缺口——代发100人但参保0人或严重不足。追问必须逐人排除五种合法免参保情形，拿不出证据的一律推定为违法。","marker":"有A无B、应有未有、法定义务缺失、零值异常、系统性地规避"},
                    "type_F": {"name":"时序节奏异常","desc":"矛盾不在数值本身，而在数值发生的时间节奏。数值比对看不出问题（金额正确、品名合规、四流齐全），但时间分布统计暴露了人为操纵的痕迹。正常企业的开票行为应均匀分布在每个工作日——若月末最后3天集中开出全月40%以上的发票，这种时间节奏极度异常。典型: #1720月末集中开票——27天开了60%、3天开了40%。追问必须用客户下单记录、红冲关联、物流时间三个维度交叉验证。","marker":"月末突击、时间分布不均匀、集中度指数、红冲关联、进销时间差、非工作时间操作"}
                },
                "construction_methodology": {
                    "description": "从七条标杆提炼的逐字段编制方法论。每个字段说明为何这样写、标杆案例、常见错误。",
                    "phenomena": {
                        "principle": "矛盾一句话说清。用'X vs Y——哪里对不上'格式，一句话说不清=疑点不成立。",
                        "golden_example": "#1717'预收账款账面余额 vs 增值税申报收入——已收款+未申报'（五个字钉死）和#1716升级版'有销售+有收款+无运费=物理上不可能'",
                        "each_manifestation": "每种典型表现必须含数据特征（百分比/绝对值），使系统可据此扫描。格式: 表现描述+数据特征。如'差额>资产总额0.5%且绝对额>5万元'。",
                        "exclusion": "排除条件必须写清边界——什么情况不适用。如'软件升级导致的短期格式异常（应在1-2个月内修复）不适用'。",
                        "anti_pattern": "禁止: '该企业可能存在偷税风险'——主观推测不可查。反例: '成本费用不合规'——不是数据矛盾。"
                    },
                    "direction": {
                        "principle": "推理链每层标注依赖证据，最后一层必须落地到具体法条编号。层数=因果链条的自然长度，不凑数。",
                        "golden_example": "#1第四层'资金流交叉验证→账外循环锁定→偷税(征管法§63)'——从数据异常推到法律定性。每层: 【推理第N层: XX法则】依赖证据: XX → 结论: XX。",
                        "trace_back": "从#1学: 加入'追溯断裂时点'层——定位首次出现异常的时间点。若与系统切换/人员变动重合，两套账嫌疑剧增。从#1717学: 加入'追溯首次挂账时点'层——判断是否为体外资金注入通道。",
                        "anti_pattern": "禁止: 推理链不到法条就结束。禁止: 各层之间无因果递进关系。"
                    },
                    "drill_questions": {
                        "principle": "三组递进（事实→证据→逻辑），每组穷举至无法提出新的有效追问。追问攻击力来自物理常识和商业逻辑的双重堵死。",
                        "golden_example": "#1716 Q3'工厂有没有能吊装这个吨位设备的行车'——装车能力是物理事实，没有就是没有。#1717 Q10'客户为什么愿意把钱放你这一年多'——大额资金无偿存放违背商业理性。",
                        "physical_block": "必须包含至少1720条物理常识类问题——用不可辩驳的事实堵死辩解空间。如#1717吸收的Q14'仓库能不能放得下'、Q15'仓储费为什么没有同步增加'。",
                        "end_verification": "必须包含追问完成判定——事实层六要素全覆盖、证据层四流全追问、逻辑层所有合法解释全排除。三问全答是→穷举完成。",
                        "anti_pattern": "禁止: 追问只有'请提供XX'缺乏攻击性。禁止: 没有物理验证类问题。"
                    },
                    "determination": {
                        "principle": "三路径（无法证明→线索/部分证明→强证据/完整证明→铁证），每条路径必须定义明确的进入条件（单源/2源/≥3源闭环）。结尾附应对总原则。有量化阈值的必须增加'阈值以下处理'分支。",
                        "golden_example": "#1717'路径一: 仅账龄数据（单源）→存疑不入结论。路径二: 账龄+银行收款（2源）→涉嫌，标注部分闭环。路径三: 账龄+收款+客户外调确认（≥3源）→认定隐匿收入'。",
                        "anti_pattern": "禁止: 笼统写'根据证据情况分别处理'不写进入条件。禁止: 使用JSON dict/键值对格式。"
                    },
                    "evidence": {
                        "principle": "四层框架 + 每层必须获取/应当获取/可以获取三级优先级 + 大额/中额/小额金额分级。首层命名必须贴合疑点实际类型。",
                    ...
                    "threshold": {
                        ...
                        "anti_pattern": "禁止: 笼统写'超过一定期限'不给具体数值。"
                    },
                    "tax_impact_and_suggestion": {
                        "principle": "税负必须能算出具体数字。tax_impact给出金额区间（最低X万/典型Y万），suggestion给出具体计算公式。",
                        "golden_example": "#1717'增值税=挂账余额÷(1+税率)×税率；企业所得税=挂账余额×应税利润率×25%'。tax_impact'最低约3万（挂账10万级），典型65-390万（挂账200-500万级）'。",
                        "anti_pattern": "禁止: 笼统写'补缴相关税款'。禁止: 不给金额区间。"
                    },
                    "normal_reason": {
                        "principle": "穷举全部合法情形，每种格式'{情形}——需提供:{具体文件类型}'。必须标注'最常见解释'。5种以上需自问该异常是否真构成异常。",
                        "golden_example": "#2写了5种合法解释（四条最多），#1717吸收了#2的模式也做到5种。每种解释对应具体证据，如'客户要求延迟交货且有书面确认——需提供:延迟交货书面请求函+仓储费支付记录+仓储合同（最常见）'。",
                        "anti_pattern": "禁止: 写'提供相关证明'笼统表述。禁止: 不标注最常见解释。"
                    },
                    "action": {
                        "principle": "不少于3步且含1项现场核查。每步: 动作类型+具体操作+预期产出。物理验证类优于纸面比对类。",
                        "golden_example": "#1716'装卸能力实测'（现场核查）和'物流公司走访'（外调走访）。#1717吸收后加'仓库容积实测'和'仓储费用穿透'。",
                        "anti_pattern": "禁止: 只有纸面比对没有现场核查。禁止: 写'现场核查'但不写具体操作。"
                    },
                    "missing_type": {
                        "principle": "缺失型疑点的矛盾不在数值偏离，而在法律义务的完全真空。有A→必须B但B=0。编制时不能只写'X vs Y不匹配'，必须写明'A存在→触发法定义务B→但B=0'的因果链。证据层首层命名为'基数锁定层'（先锚定A确实存在），第二层为'缺口归因层'（逐人逐项排除合法理由）。",
                        "golden_example": "#1719'代发工资人数 vs 社保参保人数——有工资=有员工=必须参保=但参保为零或严重不足'。代发人数-参保人数=非法用工疑似人数。每个缺口人员=每天产生万分之五滞纳金。",
                        "five_legal_exemptions": "缺失型必须穷举所有合法免履行义务的情形，且每种情形必须逐人提供证据。如#1719的五种合法免参保：①劳务派遣（派遣公司缴纳）②实习生（学校在籍）③退休返聘（已领养老金）④非全日制（<4h/天+<24h/周）⑤独立劳务（无隶属管理）。每种需逐人证据。",
                        "double_check": "缺失型必须做双线并查——不仅有A无B（人数缺口），还要核查有A但B缩水（基数缺口）。如#1719同时查参保人数缺口+参保基数缺口（按最低基数而非实际工资）。",
                        "anti_pattern": "禁止: 用'A!=B'格式描述缺失型矛盾。禁止: 不穷举合法免履行义务的情形。禁止: 不做双线并查（只查人数不查基数）。"
                    },
                    "temporal_type": {
                        "principle": "时序异常型疑点的矛盾不在数值而在时间节奏。正常业务行为应呈均匀分布，但如果某时段（月末/季末/年末）行为集中度异常偏高（如月末3天开票>40%），则暴露了人为操纵的时间模式。编制时必须将时间维度从'辅助信息'升级为'核心矛盾'——写明'A的分布不均匀，集中度指数为X'，而非笼统说'A有异常'。",
                        "golden_example": "#1720'开票时间分布 vs 均匀分布预期——27天开了60%、3天开了40%。集中度指数=月末3天/全月=40%。红冲关联度=月末开票后次月红冲/月末开票总额=22%。进销时间差=进项集中在月初1-5日+销项集中在月末28-31日。'",
                        "three_cross_checks": "时序异常型必须做三重交叉验证：①客户下单时间验证（企业说客户月末下单→查客户的实际下单时间）；②红冲关联检测（月末开票后次月是否被大量红冲→开了又冲=心虚）；③物流与收款时间核验（开票日期vs出库日期vs收款日期→时间背离=人为操纵）。",
                        "time_indices": "三个核心量化指标：①集中度指数=目标时段金额/全周期总金额；②红冲关联率=目标时段开票后下一周期红冲金额/目标时段开票总额；③进销时间差=销项集中时段与进项集中时段的日期偏移。",
                        "anti_pattern": "禁止: 只写'开票时间异常'不给集中度指数。禁止: 不做客户下单时间交叉验证。禁止: 不查红冲关联和进销时间差。"
                    },
                    "network_graph": {
                        "principle": "涉及多主体交易的疑点，现象不能只写数值比对立，必须构建交易网络图谱——标注节点（A/B/C等主体）、边（资金流向/发票流向）、重叠标记（六员/地址/电话）。单节点视角看不出问题的，网络视角下环形矛盾暴露。",
                        "golden_example": "#1718'进销交易网络图——A从B购入→A向C售出→货款A付给B→B转给C→C转回A实控人。这不是两个数值不相等，而是一张图上的环形闭环。'",
                        "four_dimensions": "关系穿透四维度: ①六员重叠（法人/股东/财务/办税/领票/联系人≥2人重叠）；②地址电话重叠（注册地址/经营地址/电话≥2项相同）；③资金闭环回流（A付款→B→中间方→A实控人个人卡，回流金额≥90%）；④购销闭环（进B销C、进C销B或A→B→C→D→A环形）。",
                        "drill_three_groups": "追问分三组: 第一组关系穿透（揭开面纱——查工商/地址/人员/开户行）；第二组资金追踪（回流闭环——付款→收款→转出→回流）；第三组交易实质（戳穿纸面——供货能力/物流/时间间隔/价差/质检）。",
                        "anti_pattern": "禁止: 用'A≠B'格式描述网络型矛盾。禁止: 追问只有纸面比对没有网络图谱视角。禁止: 正常解释不覆盖中间商模式的合法商业场景。"
                    }
                },
                "cross_pollination_method": {
                    "description": "五条标杆的互相学习记录。编制任何规则时，先判断攻击角度（A/B/C/D），然后找到对应类型的标杆规则，从中借结构、借攻击力、借格式。",
                    "matrix": {
                        "from_1717": ["矛盾短句钉死技术", "税负公式具体化", "三色预警等级设计", "行业调整值精细化"],
                        "from_1716": ["物理常识类追问攻击", "物理验证类核查动作", "追问结束条件结构化验证"],
                        "from_1": ["推理链追溯断裂时点", "科目穿透定位技术", "舞弊手法预判详细度"],
                        "from_2": ["证据四层命名贴合度", "正常解释穷举充分性", "话术策略对主观故意的测试"],
                        "from_1718": ["网络图谱构建技术", "六员重叠交叉比对", "资金闭环路径追踪", "隐性关联分层穿透", "交易实质验证方法论"],
                        "from_1719": ["缺失因果链表述", "合法免履行义务穷举", "双线并查技术（人数+基数）", "逐人归因证据清单", "违法后果定量算账"],
                        "from_1720": ["时间分布统计技术", "集中度指数量化", "红冲关联检测", "三重交叉验证（下单+红冲+物流）", "进销时间差分析"]
                    },
                    "usage": "写完后对照上述矩阵，检查是否缺少某条标杆的技术特征。缺什么补什么——不是增加数量，是增加攻击角度。"
                },
                "checklist": [
                    {"id":"GS-1","name":"矛盾一句话","criterion":"数据矛盾点必须能用'X vs Y——一句话说清哪里对不上'表述。一句话说不清=疑点不成立。"},
                    {"id":"GS-2","name":"推理至法条","criterion":"推理链每层标注依赖证据，最后一层落地到具体法条编号和条款。"},
                    {"id":"GS-3","name":"追问堵死物理极限","criterion":"穿透追问必须包含物理常识类问题，用不可辩驳的事实堵死辩解空间。"},
                    {"id":"GS-4","name":"三路径各有进入条件","criterion":"定性路径三条必须各自定义明确的进入条件（单源/2源/≥3源），不得笼统写。"},
                    {"id":"GS-5","name":"证据四层命名贴合","criterion":"证据首层命名必须贴合疑点实际类型，不得全部套用货物流。"},
                    {"id":"GS-6","name":"税负能算出具体数字","criterion":"tax_impact和suggestion必须给出具体补税计算公式和金额区间。"},
                    {"id":"GS-7","name":"触发含三色预警","criterion":"触发指标须含黄/橙/红三色预警等级，按严重程度分档。"},
                    {"id":"GS-8","name":"正常解释有最常见标注","criterion":"合法解释中必须标注最常见解释作为优先核验素材。"},
                    {"id":"GS-9","name":"核查动作含现场核查","criterion":"稽查动作不少于3步且含1项现场核查，物理验证优于纸面比对。"},
                    {"id":"GS-10","name":"适用条件五维度","criterion":"适用条件须覆盖行业限制+纳税人资质+规模门槛+时间条件+金额门槛五个维度。"},
                    {"id":"GS-11","name":"关联网络图谱化","criterion":"涉及多主体交易的疑点须构建节点-边关系图，查六员重叠+地址电话重叠+资金闭环+购销闭环。单节点视角的'数据比对'无法发现环形矛盾——必须将交易对手连成网络才能暴露闭环结构。"},
                    {"id":"GS-12","name":"缺失因果链验证","criterion":"缺失型疑点必须写明'A存在→触发B义务→B=0'的因果链。穷举所有合法免履行义务情形，每种情形逐人/逐项提供证据。必须做双线并查。不为零值的偏差，而是法定义务的真空。"},
                    {"id":"GS-13","name":"时序集中度量化","criterion":"时序异常型疑点必须计算集中度指数=目标时段/全周期×100%。做三重交叉验证：客户下单时间+红冲关联度+物流收款时间差。时间异常不是笼统描述，是精确到日的时间分布统计+量化指标。"}
                ],
                "usage": "每轮精写完成: ①判断攻击角度(A/B/C/D/E/F) ②对照construction_methodology逐字段检查 ③对照cross_pollination_matrix检查是否缺少技术特征 ④对照checklist十三项打勾。十三勾全满=顶级精写。"
            },
            
            "rule_index": {
                "description": "1720条顶级精写规则全景索引——按攻击角度分类，每条标注矛盾核心、追问数量、标杆特征，供编制时选型参照。",
                "total": 30,
                "type_A_business_logic": {
                    "count": 5,
                    "desc": "A类（商业逻辑不可辩驳）——矛盾本身违反正常市场主体的经济理性，企业无法用'行业惯例'解释，开案成本最低，两证即可定案。",
                    "rules": [
                        {"id": 1717, "item": "预收账款挂账未申报", "score": 8, "contradiction": "已收款+未申报——五个字钉死", "drill_questions": "15问含仓库容积+仓储费物理追问", "signature": "所有标杆中矛盾最尖锐、辩解空间最小的规则。Q10'客户为什么愿意把钱放你这一年多'直接戳穿。推理5层到征管法第61720条。"},
                        {"id": 9, "item": "银行收款总额vs申报收入", "score": 9, "contradiction": "钱进了银行但报税时不见了——银行流水不可篡改", "drill_questions": "8问全渠道覆盖（对公+私户+第三方）", "signature": "隐匿收入的最核心信号。差额=未申报收入的最直接估算。私户收款追问是突破口。"},
                        {"id": 10, "item": "银行收款vs申报收入关联版", "score": 10, "contradiction": "同上+关联方占比>30%", "drill_questions": "6问含转移定价+无息拆借+境外付汇", "signature": "#9的关联升级版。收入隐匿+转移定价+资本弱化三线并查。"},
                        {"id": 1711, "item": "银行收款vs应税收入", "score": 9, "contradiction": "钱进了账但税表上找不到", "drill_questions": "7问", "signature": "#9的精简版，全渠道收款汇总+逐笔穿透分类。"},
                        {"id": 1715, "item": "平台收款vs申报收入", "score": 10, "contradiction": "9000万差额——代收代付还是自营隐匿？", "drill_questions": "8问含平台角色判定（代理vs转售商）", "signature": "平台经济最核心博弈。Q6'如果认定转售商就要按全额补税千万级'——给对方彻底算账。"}
                    ]
                },
                "type_B_physical_reality": {
                    "count": 4,
                    "desc": "B类（物理事实不可辩驳）——矛盾违反物理常识。不需要推理，只需要测量。追问用物理堵死辩解空间。",
                    "rules": [
                        {"id": 1716, "item": "大型器械货物流vs运输费", "score": 10, "contradiction": "有销售+有收款+无运费=物理上不可能", "drill_questions": "14问含装车能力/门卫放行/买方承运能力/仓库容积/仓储费五重物理验证", "signature": "物理追问的极致标杆。Q3'工厂有没有能吊装这个吨位设备的行车'——装车能力是物理事实，没有就是没有。"},
                        {"id": 20, "item": "存货周转率vs申报收入", "score": 9, "contradiction": "存货要卖3年但企业没停过工", "drill_questions": "9问含仓库容积实测+盘点差异", "signature": "周转率极值+持续经营=不可调和矛盾。Q2'仓库能装下账面这么多存货吗'——物理容量验证。"},
                        {"id": 4, "item": "无形资产摊销为零", "score": 9, "contradiction": "有专利无摊销——资产永远不过期在法律上不可能", "drill_questions": "8问含国家知识产权局官网查询+评估复核", "signature": "法律权利有保护期限，账面永远不摊=承认权利无限长——逻辑和物理双重不可能。"},
                        {"id": 19, "item": "其他应收款余额vs申报收入", "score": 9, "contradiction": "挂账是收入的2倍——钱被赶出了体外", "drill_questions": "9问含股东借款视同分红", "signature": "其他应收款是资产负债表最隐蔽的蓄水池——Q3'借300万2年不还要交60万个税'直接点出后果。"}
                    ]
                },
                "type_C_data_penetration": {
                    "count": 18,
                    "desc": "C类（数据异常需穿透）——矛盾是数据偏差而非必然违规。需要逐层排除合法解释后定性。数量最多，涵盖面最广。",
                    "rules": [
                        {"id": 1, "item": "资产负债表不匹配", "score": 9, "contradiction": "会计恒等式断裂——全世界会计系统基石", "drill_questions": "15问含追溯断裂时点+科目穿透+个人账户", "signature": "推理链最深的标杆之一——4层从会计等式推到账外循环认定偷税§63。"},
                        {"id": 2, "item": "月凭证量异常", "score": 8, "contradiction": "凭证量超同行2倍但收入没涨", "drill_questions": "10问含产能计算+人均消耗", "signature": "Q3'按产能1200张你做了3000张——多出来的谁做的'直接产能打脸。Q9'办公用品人均10倍'用常识戳穿。"},
                        {"id": 3, "item": "固定资产折旧不匹配", "score": 9, "contradiction": "有资产无折旧——不可能", "drill_questions": "8问含资产实物盘点", "signature": "折旧为零只有两种可能：资产不存在或故意不计提。百万级=第二种。"},
                        {"id": 5, "item": "进项发票四流不匹配", "score": 9, "contradiction": "发票合同付款物流——至少一个流对不上", "drill_questions": "15问含四流全溯源+供应商穿透", "signature": "极高风险score9。四流断裂是虚开发票最核心的数据特征。"},
                        {"id": 6, "item": "红冲与退款不匹配", "score": 8, "contradiction": "发票红冲了但钱没退——红冲假的", "drill_questions": "8问含客户外调确认", "signature": "三大操纵模式：跨期调节/虚假退货/恶意红冲。红冲金额-退款金额=未退款=红冲虚假金额。"},
                        {"id": 7, "item": "进项发票票面不规范", "score": 8, "contradiction": "票面不合规——法定抵扣凭证的有效性存疑", "drill_questions": "8问含换开能力评估", "signature": "瑕疵类vs否决类双路径。品名笼统+无物流=不仅是票面问题，是交易不真实。"},
                        {"id": 8, "item": "进项发票税号缺失", "score": 8, "contradiction": "无税号的发票已经在抵扣——在监管盲区中", "drill_questions": "9问含供应商换开能力", "signature": "偶发vs系统双路径。供应商注销+税号缺失=虚开高概率。"},
                        {"id": 11, "item": "进项品名与主营业务不匹配", "score": 10, "contradiction": "软件公司买钢材——做的和买的毫无关系", "drill_questions": "13问含供应商经营能力核查", "signature": "score10极高风险。Q2-Q3连续追问证据突脸让对方措手不及。"},
                        {"id": 12, "item": "发票销售方与付款方不一致", "score": 8, "contradiction": "开票是A公司但付款给了个人B——三流断裂", "drill_questions": "9问含资金流向追踪", "signature": "三流断裂是虚开的直接证据。对公付对私+无委托协议=资金流向极度可疑。"},
                        {"id": 13, "item": "毛利率与行业基准偏离", "score": 9, "contradiction": "毛利率只有同行三分之一——不赚钱还干了两年", "drill_questions": "9问含关联转移定价", "signature": "持续亏损持续经营=有体外资金维持。Q3'为什么没关停——钱从哪来'一语道破。"},
                        {"id": 14, "item": "申报毛利率vs行业基准", "score": 9, "contradiction": "赚钱能力比同行差一半——原因不可验证", "drill_questions": "8问", "signature": "#13的姊妹规则，聚焦申报vs基准的系统性偏差。"},
                        {"id": 15, "item": "费用增速与收入增速不匹配", "score": 8, "contradiction": "费用涨50%收入不动——费用在虚列或收入在藏", "drill_questions": "10问含软科目穿透+供应商人头双验证", "signature": "四大软科目差旅会议咨询招待是虚列主战场。Q8'200万口头咨询谁批准的'无法回应。"},
                        {"id": 16, "item": "业务招待费申报限额", "score": 8, "contradiction": "扣了185万多出法定上限——明眼人一看就是少缴45万", "drill_questions": "8问含双重限额计算+虚列发票核查", "signature": "直接算数打脸：按孰低15万多扣185万=少缴46万。数字碾压。"},
                        {"id": 17, "item": "三表数据不一致", "score": 9, "contradiction": "会计说3500万、申报说3000万——500万去哪了", "drill_questions": "8问含三表逐月比对", "signature": "三表差异方向导航：会计>申报=隐匿；会计<申报=虚开；走势各异=两套账。"},
                        {"id": 18, "item": "收入与成本增速不匹配", "score": 6, "contradiction": "收入涨30%成本只涨5%——弹性系数<0.2", "drill_questions": "8问含弹性系数检测", "signature": "中风险但弹性系数检测技术极强。弹性<0.2→收入虚增；>5→成本虚列。"},
                        {"id": 1712, "item": "外币合同汇率折算偏差", "score": 10, "contradiction": "选择了最有利于少缴税的汇率——人为选择", "drill_questions": "8问含双端操纵判定", "signature": "score10极高风险。汇率偏差×巨大交易额=巨大税收漏洞。Q3精确算账500万美元差0.4=200万没确认。"},
                        {"id": 1713, "item": "银行流水偏差10万精简版", "score": 10, "contradiction": "10万门槛精准触发——银行收到但税没报", "drill_questions": "6问精简版", "signature": "#9的小额精简版。10万门槛确保小额不误伤。6问快问快答。"},
                        {"id": 1714, "item": "平台保证金vs申报收入", "score": 9, "contradiction": "保证金没收了200万——没收就是收入没报就是隐匿", "drill_questions": "7问", "signature": "平台经济隐性收入——保证金没收=价外费用=增值税纳税义务已触发。"}
                    ]
                },
                "type_D_network_graph": {
                    "count": 1,
                    "desc": "D类（关系网络不可辩驳）——矛盾不在单点数值之间，而在多个节点的环形关系之中。单节点视角一切正常，但构建交易网络图谱后环形闭环暴露。",
                    "rules": [
                        {"id": 1718, "item": "进销交易闭环+资金回流", "score": 10, "contradiction": "A→B→C→A环形闭环——单节点看不出、连成网络全暴露", "drill_questions": "14问三组（关系穿透→资金追踪→交易实质）", "signature": "精写标准v4升级的标志性突破。首创网络图谱型疑点：六员重叠+地址电话重叠+资金回流闭环+购销闭环四维判定。"}
                    ]
                },
                "type_E_missing": {
                    "count": 1,
                    "desc": "E类（法律义务真空）——矛盾不在数值之间，而在于一个法定义务的完全缺失。A存在→B必须存在但B=0。不是偏差，是真空。",
                    "rules": [
                        {"id": 1719, "item": "代发工资vs社保人数缺口", "score": 10, "contradiction": "有工资=有员工=必须参保=参保为零——法定义务真空", "drill_questions": "14问三组（基数确认→缺口归因→后果算账）", "signature": "缺失型母体规则。首创双线并查（人数缺口+基数缺口），逐人归因五种合法免参保情形，每个缺口=每天万分之五滞纳金。"}
                    ]
                },
                "type_F_temporal": {
                    "count": 1,
                    "desc": "F类（时序节奏异常）——矛盾不在数值而在时间节奏。数值正确但集中发生=人为操纵的痕迹。",
                    "rules": [
                ...
                "cross_reference": "所有1720条规则均按本标准（preamble哲学+attack_angles选型+23_fields逐字段+checklist十三勾验收）编制。每条规则的详细内容见 static/tax_risk_rules_local_export.json。这1720条既是本标准的产出物，也是标准有效性的验证集——如果一条新规则无法达到同等深度，说明标准本身还需要完善。"
            },
            ...
            "standard_upgrade_criteria": {
                ...
                "description": "精写编制标准升级判定规则——双层自举系统。第一层：本规则用于判定精写编制标准何时需要升级。第二层：本规则自身也随精写编制标准一起升级迭代。每一次精写都是对精写编制标准的压力测试——如果精写过程中发现现有标准无法指导某些部分的编制，说明标准需要升级。",
                "core_principle": "发现标准=元标准。精写编制标准告诉你怎么写疑点；发现标准告诉你在写疑点时怎么判断精写编制标准是否需要升级。发现标准升级后能识别更多类型缺口→精写编制标准更完善→发现标准继续进化——形成永续正反馈闭环。",
                "double_bootstrap": {
                    "layer_1": {"name": "标准升级层", "flow": "精写每条疑点→逐维判定缺口→触发精写编制标准升级"},
                    "layer_2": {"name": "自举层", "flow": "精写编制标准升级后→审查发现标准是否有新盲区→发现标准自身升级"},
                    "positive_feedback": "发现标准升级→能识别更多缺口类型→精写编制标准更完善→暴露更细粒度的判定盲区→发现标准继续进化。两个版本号独立演进（精写编制标准v6/v7...，发现标准v1.0/v1.1...），升级链路解耦。"
                },
                "discovery_dimensions": [
                    {
                        "id": "DIM-1",
                        "name": "攻击角度覆盖缺口",
                        "question": "新规则的核心矛盾能否归入现有六类攻击角度（A-F）的marker？",
                        "trigger": "核心矛盾不匹配任何现有marker→需要新增攻击角度或扩展现有类型的marker列表",
                        "examples": [
                            "v5→v6: #1720月末集中开票——矛盾不在数值而在时间节奏→新增F类时序异常型",
                            "v6潜在缺口: #4无形资产摊销——核心矛盾是法律保护期届满而非物理事实不可辩驳→B类marker需扩展'法律事实不可辩驳'子类型"
                        ]
                    },
                    {
                        "id": "DIM-2",
                        "name": "GS自查项覆盖缺口",
                        "question": "精写过程中是否有某个GS自查项无法对当前规则执行有效检查？即GS项的要求对当前规则不适用、语义歧义或无法操作化",
                        "trigger": "任一GS项对当前规则无效→GS项需细化或增加例外条款",
                        "examples": [
                            "v6潜在缺口: GS-3要求'必须包含物理常识类追问'——对纯财务规则（资产负债表不平）、法律资产规则（无形资产摊销）、发票流规则（四流不匹配）均无法构造物理常识类追问→GS-3需增加'非实物类规则可用不可辩驳的非物理事实替代'的例外条款"
                        ]
                    },
                    {
                        "id": "DIM-3",
                        "name": "字段格式指导缺口",
                        "question": "精写某个字段时，现有格式描述是否足以指导编制？是否存在格式要求与实际规则特性冲突的情况？",
                        "trigger": "编制者按格式描述填写后仍不确定是否符合标准，或格式描述与规则特性冲突→需细化格式描述或增加规则类型特化指导",
                        "examples": [
                            "v6潜在缺口: threshold前置条件四维度（行业+资质+数据+时间）——对发票类规则（#5四流不匹配）天然适用'供应商+交易+资金+发票'四维度更精准→需增加'阈值维度可按规则类型特化'的指导",
                            "v6潜在缺口: direction格式要求'所有类型必须包含追溯首次异常时点层'——GS-1~13中无任何项检查此层是否存在→需新增GS-14或纳入现有GS项"
                        ]
                    },
                    {
                        "id": "DIM-4",
                        "name": "特殊类型编制指导缺口",
                        "question": "精写过程中是否出现了现有标准未定义的编制技术或验证手段？该技术/手段是否具有跨规则的通用性？",
                        "trigger": "编制者使用了标准未收录的技术/手段，且该技术/手段可推广到同类规则→需将其纳入标准作为新的编制技术",
                        "examples": [
                            "v6潜在缺口: #2月凭证量异常Q2使用'人力产能极限验证'（按人均产能计算理论最大凭证量）——这既不是B类的物理测量也不是C类的纯数据分析→需在B类marker中增加'人力产能与产出量不匹配'",
                            "v6潜在缺口: #4无形资产摊销的验证手段是'国家知识产权局官网查询'——这是法律数据库验证而非物理测量→需在标准中定义法律验证类核查动作指导"
                        ]
                    },
                    {
                        "id": "DIM-5",
                        "name": "跨类型适配缺口",
                        "question": "一条规则是否跨越了多个攻击角度类型的边界？现有标准是否提供了混合类型规则的编制指导？",
                        "trigger": "规则的核心矛盾涉及两个或以上攻击角度，但标准未提供混合类型的衔接指导→需增加跨类型编制指导",
                        "examples": [
                            "v6潜在缺口: #3固定资产折旧不匹配——C类（数据异常）+ B类元素（资产实物盘点=物理验证）→标准未定义C类含B类元素时的编制优先级"
                        ]
                    },
                    {
                        "id": "DIM-6",
                        "name": "边界定义缺口",
                        "question": "精写过程中是否出现了标准定义模糊的情况——某个字段该写什么/不该写什么、某条规则该算哪类/不该算哪类？",
                        "trigger": "多个规则在同一判定点上出现不一致的处理方式→边界需要明确化",
                        "examples": [
                            "v6潜在缺口: '正常业务解释'穷举到0种时是否合格？标准说'确实无合法解释可写0-3种'但未规定0种时的判定依据——什么异常有0种合法解释是合理的？什么异常0种解释说明定义有漏洞？"
                        ]
                    },
                    {
                        "id": "DIM-7",
                        "name": "发现标准自身盲区",
                        "question": "本次标准升级后，是否有新的缺口类型被创造出来而发现标准尚未覆盖？是否有DIM-1~6无法捕获的缺口在实际精写中出现？",
                        "trigger": "精写中发现缺口但无法归入DIM-1~6任一维度→发现标准自身需要扩展新维度",
                        "examples": [
                            "发现标准v1.0初始7维——未来精写中可能出现第8维"
                        ]
                    }
                ],
                "discovery_process": {
                    "description": "每条规则精写完成后的标准升级判定流程。不是'感觉有缺口'，而是逐维过审——每条规则单独过审，跨条交叉比对，跨批去重。",
                    "steps": [
                        {"step": 1, "name": "精写完成", "action": "规则23字段完整精写，通过提交前自检清单全部23项"},
                        {"step": 2, "name": "逐维过审", "action": "按DIM-1~7顺序逐维判定——对每个维度回答'精写过程中该维度是否暴露了缺口'。每条规则独立过审，不与其他规则混审"},
                        {"step": 3, "name": "捕捉缺口", "action": "任一维度回答'是'→记录缺口详情（涉及规则ID、具体字段、缺口描述、建议方案）。每条规则维护独立的缺口清单"},
                        {"step": 4, "name": "聚合归类", "action": "累计精写N条规则后（N=5起步，可调整），执行两层聚合：层1·批内交叉比较→层2·跨批去重比较。详见 cross_batch_dedup 规则"},
                    ...
                    "batch_size_note": "N=5为初始值。精写经验积累后，可由发现标准自身调整（如改为'每精写11720条审一次'或'发现缺口即审'）。此调整记录写入发现标准的version_history。"
                },
                "cross_batch_dedup": {
                    "description": "跨批去重比较规则——核心原则：不缺不重。同一个缺口不会因被多条规则触发而重复升级；新缺口不会因只被一条规则触发而漏掉。",
                    "two_layer_aggregation": {
                        "layer_1": {
                            "name": "批内交叉比较",
                            "description": "当前批次内部（如#6~#10共1720条）各自捕获的缺口放在一起，看哪些缺口是同一种类型被多条规则触发",
                            "rules": [
                                "同一DIM维度下，多条规则触发了同一类缺口（如#7和#9都触发DIM-3的threshold维度替换问题）→合并为一个共性缺口，记录所有触发规则ID",
                                "同一DIM维度下，多条规则触发了不同类缺口（如#6触发DIM-1法律事实，#8触发DIM-1人力产能）→各自独立，不合并",
                                "不同DIM维度触发的缺口永远不合并（DIM-1和DIM-3是不同维度）"
                            ]
                        },
                        "layer_2": {
                            "name": "跨批去重比较",
                            "description": "当前批次的缺口与历史批次（如#1~#5）已检出的缺口比对——新缺口升级，已有缺口追加记录不重复升级",
                            "rules": [
                                {
                                    "scenario": "同一缺口·跨批匹配",
                                    "condition": "#6~#10中触发的缺口与#1~#5中已发现的缺口完全相同（同一DIM+同一缺口类型+同一标准字段/章节），如#4已触发DIM-1法律事实子类型，#8又触发同一DIM-1同缺口",
                                    "action": "已有缺口，不重复计入升级计数。追加触发记录（追加规则ID到该缺口的触发规则列表），供后续审查时参考缺口影响面"
                                },
                                {
                                    "scenario": "新缺口·跨批新增",
                                    "condition": "#6~#10中触发的缺口在#1~#5中完全没有出现过（该DIM+缺口类型首次出现），如#1~#5的DIM-2只捕获了GS-3例外条款，而#8触发了DIM-2的GS-5新问题",
                                    "action": "新缺口，计入升级计数。如该缺口在当前批次内被多条规则触发→按层1合并为共性缺口"
                                },
                                {
                                    "scenario": "纯个案·单条独有",
                                    "condition": "某缺口只被一条规则触发，且在所有批次中都只出现过这一次，其他规则无同类问题",
                                    "action": "个案，不升级。记录到gap_log但标注为'个案-待观察'，如果后续批次再次出现→自动升级为共性缺口"
                                }
                            ]
                        }
                    },
                    "execution_example": {
                        "description": "精写#1~#5后发现5项缺口→聚合→5项均为共性缺口→触发升级。继续精写#6~#10→逐条过审→每完成一条记录新缺口→#6~#10全部精写完毕后→执行两层聚合",
                        "scenario_1": "#6~#10内部#8和#9都触发了DIM-1法律事实子类型→批内合并为一个缺口→跨批比对：该缺口与#4已发现的DIM-1法律事实子类型完全相同→已有缺口，不重复升级，追加触发记录（#4,#8,#9）",
                        "scenario_2": "#6~#10中#7触发了DIM-2的GS-5新问题→批内只有#7触发→跨批比对：#1~#5的DIM-2只捕获了GS-3例外条款，GS-5问题首次出现→新缺口。但因为批内只有#7触发→暂计为'个案-待观察'，不立即升级",
                        "scenario_3": "#6~#10中#6触发了DIM-6（边界定义缺口，0种解释判定依据）→批内只有#6触发→跨批比对：与#1~#5的DIM-6记录（未触发但已定义）→新缺口但批内单条触发→'个案-待观察'。后续#11~#15中再次出现DIM-6同类型缺口→自动升级为共性缺口",
                        "result": "#6~#10本轮：无新共性缺口需要立即升级（#1~#5的5项缺口已被覆盖，新出现的都是个案→记录为'待观察'）。升级计数=0，不触发精写编制标准升级"
                    }
                },
                "self_upgrade": {
                    "description": "发现标准的自举规则——它如何升级自己。",
                    "rules": [
                        {"id": "SU-1", "name": "新维度追加", "trigger": "精写中出现的缺口无法归入DIM-1~7→新增DIM-N维度", "action": "定义新维度的判定问题、触发条件、示例，追加到discovery_dimensions；version_history记录'vX.Y: DIM-N新增（触发规则#N）'"},
                        {"id": "SU-2", "name": "现有维度细化", "trigger": "某维度捕获了缺口但判断不够精确（范围过宽/过窄/歧义）→细化该维度的判定问题或触发条件", "action": "修订对应DIM的question和trigger文本；version_history记录修改内容"},
                        {"id": "SU-3", "name": "流程参数调整", "trigger": "发现标准运转效率不理想（太频繁/太迟钝/N值不合适）→调整discovery_process参数", "action": "修订steps或batch_size_note；version_history记录调整原因和新值"},
                        {"id": "SU-4", "name": "版本跟踪", "trigger": "发现标准自身任何修改→必须更新version字段和version_history", "action": "version字段更新为'vX.Y · YYYY-MM-DD'；version_history追加条目（见下方格式）"}
                    ],
                    "version_history": [
                        {"version": "v1.0", "date": "2026-07-22", "trigger": "精写#1~#5过程中发现现有前端面板的简版升级判定规则（单一攻击角度维度）无法捕获GS项/字段格式/特殊类型等缺口，系统性缺陷暴露——需建立完整的多维发现标准并将其纳入引擎记忆", "changes": "新建standard_upgrade_criteria章节：7个判定维度(DIM-1~7)、6步判定流程、1720条自举规则(SU-1~4)；首次定义了'发现标准'为独立的元标准章节，与精写编制标准并行演进"}
                    ]
                },
                "usage_example": {
                    "title": "精写#1~#5后的发现标准运行示例（v1.0首次实战）",
                    "rules": ["#1资产负债表不匹配", "#2月凭证量异常", "#3固定资产折旧不匹配", "#4无形资产摊销为零", "#5进项发票四流不匹配"],
                    "results": {
                        "DIM-1_触发": {"缺口": "B类marker未覆盖法律事实不可辩驳", "涉及规则": "#4", "是否需要升级": "是——B类marker需扩展"},
                        "DIM-2_触发": {"缺口": "GS-3物理常识类追问对非物理规则不适用", "涉及规则": "#1/#4/#5", "是否需要升级": "是——GS-3需增加例外条款"},
                        "DIM-3_触发": [{"缺口": "threshold前置条件四维度缺乏规则类型特化指导", "涉及规则": "#5"}, {"缺口": "direction'追溯首次异常时点'层无GS项检查", "涉及规则": "#2/#3/#4/#5"}],
                        "DIM-4_触发": {"缺口": "人力产能验证技术未分类", "涉及规则": "#2"},
                        "DIM-5_触发": {"缺口": "C类含B类元素时的编制优先级未定义", "涉及规则": "#3"},
                        "DIM-6_不触发": "正常业务解释0种边界暂未暴露",
                        "DIM-7_不触发": "DIM-1~6已覆盖全部5项缺口"
                    },
                    "conclusion": "精写1720条规则→触发DIM-1/2/3/4/5共5项缺口→5项中全部为共性缺口→触发精写编制标准升级。同时DIM-7未触发新盲区→发现标准v1.0暂不升级。"
                }
            },
            "execution_guide": {
                "version": "v1.0 · 2026-07-14",
                "purpose": "精写编制说明（配套执行指引）。本指引不重复定义23项字段的格式与内容，旨在解决编制人员在实务精写过程中反复出现的执行偏差问题。编制标准规定应当编制何种内容，本指引规定如何避免编制错误。本指引每项防错提示均对应系统的自动校验点（命令行自检工具、验证接口、系统启动加载校验三处同源执行），编制失误将被实时拦截——本指引所列标准即为系统校验契约，非纸面建议。",
                "id_prefix_map": {
                    "description": "目标编码体系（暂缓实施）：{类型前缀}-{三位序号}。现行主键为全库唯一数字id——链引用/身份锁/触发溯源全部基于数字id，切换须全库同步改造。",
                    "AN": "隐匿收入", "VC": "虚列成本", "VI": "虚开发票", "ST": "少缴税款", "OT": "其他违规"
                },
                "common_errors": [
                    {"id": 1, "error": "id体系混用或重复", "correct": "现行主键=全库唯一数字id——跨域链rule_refs、身份锁、触发溯源均以数字id串联，任何流程不得改动或复用已删除id；{类型前缀}-{三位序号}(AN/VC/VI/ST/OT)为目标编码体系，迁移须与全部链引用同步改造后方可切换", "field": "①", "engine_check": "校验器查id非空+全库唯一；身份锁保护id不被LLM改动；删规则时链引用替换/摘除流程强制执行"},
                    {"id": 2, "error": "分类字段乱写或漏打监控维度", "correct": "现行主分类=monitor_category（13大监控维度，权威源monitor_point_taxonomy），每条必打且禁止自创类名；category保留业务分类兼容历史，五类定性(隐匿收入/虚列成本/虚开发票/少缴税款/其他违规)为定性环节口径。双定性允许双类别(用/分隔)不超过两个", "field": "③", "engine_check": "monitor_category注入发现并进报告维度汇总；身份锁保护category/monitor_category不被LLM改动"},
                    {"id": 3, "error": "level写法不在受控词表", "correct": "现行受控词表：极高风险/高风险/中风险/低风险/良好（自动发现规则另有信息档）——校验器与全库系统逻辑均按此词表执行，禁止其他变体", "field": "④", "engine_check": "启动加载校验VALID_LEVELS词表；score-level锚点一致性检查（8+分须高/极高，6-7分须中/高）；重罪词条目强制≥高风险"},
                    {"id": 4, "error": "追问数量未达穷举就提交", "correct": "以穷举完成判定标准的三项自检逐一核验，全部通过方可提交", "field": "⑬", "engine_check": "穿透追问格式校验：题号、潜台词标注、应答话术三要素齐全，事实、证据、逻辑三组顺序正确，不少于3问"},
                    {"id": 5, "error": "推理链未标注依赖证据", "correct": "每层须标注依赖证据类型及具体内容，漏标将导致系统无法验证推理链完整性", "field": "⑫", "engine_check": "校验direction每层含「依赖证据」标注+复杂度标记+推理层格式，缺一报错"},
                    {"id": 6, "error": "正常解释证据要求写'提供相关证明'", "correct": "禁止笼统，必须具体到文件类型", "field": "⑯", "engine_check": "校验normal_reason至少4种、每种含「——需提供」+具体文件类型、附穷举说明；系统将其注入finding._escape_hatch供稽查员验证排除"},
                    {"id": 7, "error": "证据第一层全部套用'货物流'", "correct": "按疑点类型选择贴合的名称，不强制套用", "field": "⑲", "engine_check": "命名贴合软校验：申报流/账表勾稽/社保个税/合规程序类疑点的evidence首层若写「货物流」→报错（应按第一层命名指引取贴合名称）"},
                    {"id": 8, "error": "附加税费影响标为'间接'", "correct": "附加税费随增值税必然联动，须标注为次要影响。间接影响仅适用于可能存在但非必然的情形", "field": "⑱", "engine_check": "校验risk_table中「附加税费」若标注「间接」→报错，必须标「次要」（随增值税必然联动）"},
                    {"id": 9, "error": "定性路径只有一条", "correct": "三路径必须全写(无法证明→线索/部分证明→强证据/完整证明→铁证)", "field": "⑰", "engine_check": "校验determination三路径（线索/强证据/铁证）+应对总原则齐全；系统按独立来源域数为每条发现执行三档定级(_evidence_grade)"},
                    {"id": 10, "error": "policy_ref引用已废止法规", "correct": "法规引用须强制标注核验日期（格式：法规现行性核验:YYYY-MM-DD），并通过法律时效性核查程序验证", "field": "⑦", "engine_check": "law_validity_checker接入审计：自动识别废止法规并要求替换现行法；校验核验日期标注"},
                    {"id": 11, "error": "定性路径只有三路径，没有写阈值以下怎么处理", "correct": "设有量化阈值的规则必须补全【阈值以下处理】分支——未达到触发门槛时以合规提示形式记录，不纳入正式发现，不影响合规度评分", "field": "⑰", "engine_check": "校验有量化阈值的规则determination必须含【阈值以下处理】分支；系统按该分支将未达门槛情形记为合规提示不入稽查程序"},
                    {"id": 12, "error": "定性路径用了JSON dict/键值对格式", "correct": "determination必须为平铺文本段落——用自然段写路径一·路径二·路径三，严禁{'路径一': {...}}这种JSON格式", "field": "⑰", "engine_check": "校验determination首字符非{非[——JSON格式直接报错"},
                    {"id": 13, "error": "正常解释没有标注'最常见解释'", "correct": "所列合法解释中须至少有一项标注为最常见解释，作为稽查人员优先核验的素材。目前该标注遗漏率超过95%，须重点整改", "field": "⑯", "engine_check": "校验normal_reason中是否存在「最常见解释」字样——缺失则报错；系统将此标记注入finding以提示稽查员优先验证该解释"}
                ],
                "scoring_anchors": {
                    "description": "风险评分锚点（⑤score）。评分执行规则：评分与等级锚点一致性校验——8分及以上须为高风险或极高风险，6至7分须为中风险或高风险；重罪词条目（骗税、伪造、虚开、两套账、洗钱等19个关键词）强制不低于8分且不低于高风险等级；主动扫描生成的疑点提示评分上限为5分，待人工核实后按锚点标准升级",
                    "levels": [
                ...
                "impact_levels": {
                    "description": "影响程度判定锚点（⑱risk_table）。判定顺序：首先锁定核心影响，其次判断必然联动的次要影响，最后列明可能的间接影响，以实际涉及的税种为准编制。系统执行：校验risk_table必须包含核心影响标注项；附加税费标注为间接影响时将直接报错（附加税费随增值税必然联动，应标为次要影响）；报告补税测算按核心与次要税种范围计算",
                    "核心": "直接导致补税或触犯法条构成违法",
                    "次要": "随核心影响必然联动，非独立存在",
                    "间接": "可能存在但非必然，需额外证据支持"
                },
                "evidence_priority": {
                    "description": "证据优先级（⑲evidence）。系统执行：校验evidence须三级优先级（必须获取、应当获取、可以获取）齐全，且金额分级（大额、中额、小额）完整；必须获取级证据缺失的，对应发现仅能停留在线索档，不得升级为强证据或铁证级",
                    "必须获取": "四流合一直接证据，缺1项闭环不成立→入线索等级",
                    "应当获取": "业务合理性佐证，增强但非必需",
                    "可以获取": "排雷辅助证据，补强或排除作用，不获取不影响闭环"
                },
                "evidence_layer_naming": {
                    "description": "证据第一层命名指引（⑲evidence）。不强制使用货物流命名，应根据疑点实际类型准确描述核心验证内容。系统执行：命名贴合度校验——监控维度属于申报流监控、账表质量与勾稽、社保与个税交叉、税务合规与程序的疑点规则，其证据首层若仍标注为货物流则报错（无实物流转的疑点套用货物流命名即证据框架错位）",
                    "映射": {
                        "实物交易类": "货物流",
                        "账务断裂类": "账实核对层",
                        "纯申报类": "申报数据层",
                ...
                "quality_benchmarks": {
                    "description": "品质标杆对照说明：比对的核心不是数量多寡，而是每个字段是否达到同等的穷举深度。系统执行：标杆条目与规则库内条目保持实时一致（数字ID现行编码体系），验证接口可逐条执行36项全量复检",
                    "复杂异常": {"id": "#1", "item": "借贷不平衡", "layers": 4, "questions": 13, "normal_reasons": "4种已穷举", "note": "复杂度：复杂；含阈值以下处理分支；36项自检0错误"},
                    "中等异常": {"id": "#1813", "item": "预收账款长期挂账不转收入", "layers": 4, "questions": 13, "normal_reasons": "4种已穷举", "note": "canonical_example；四层证据框架+AB场景+金额分级"},
                    "简单异常": {"id": "原则", "item": "简单疑点如实写", "layers": "2-3", "questions": "4-6", "normal_reasons": "穷举完即合格", "note": "穷举完成但数量较少仍为合格编制，不将复杂情形的标杆数量作为所有规则的数量下限——按实际复杂程度编制"}
                },
                "submission_checklist": {
                    "description": "提交前自检清单（6组23项，每项均有对应系统校验点，命令行自检工具、验证接口、系统启动加载三处同源执行）",
                    ...
                    "整体自洽": ["⑫推理结论与⑰定性一致", "⑮舞弊手法与⑬追问形成策略→执行转化"],
                        "本体论与库权": ["铁律0：疑点名称、现象描述、触发指标、详细内容须包含数据矛盾特征，无法表述为X与Y对比关系的条目不构成疑点", "知识边界：立案标准/的认定/的区分/税务处理等知识型措辞禁入疑点库（存稽查知识库）", "通病禁行业：通病词+行业词同现即报错，行业差异写threshold行业调整", "重罪下限：19重罪词条目≥高风险且≥8分", "锚点一致：score与level档位互相锁定", "身份锁：id/item/category/monitor_category不被任何自动流程改动"]
                }
            }
        },
}
