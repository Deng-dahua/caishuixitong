...
═════ 推理引擎仪表盘（2026-06-30 补录）═════
  ...
  【#2 规则库】1740条税务合规指令按分类浏览/搜索/详情查看
...
═════ 系统文件关联清单（2026-06-30）═════
  ...
  static/tax_risk_rules_local_export.json（1740条税务合规指令）
...
TAX_BURDEN_RULES = {
    ...
    # 权威源：1740条规则全部带 monitor_category 字段，取值必须来自以下13类。
    ...
    "monitor_point_taxonomy": {
        ...
        "categories": [
            ...
            {"name": "申报流监控", "count": 439, "logic": "增值税、所得税、财报三表比对，税种间勾稽。两税收入不一致/零申报/税负率偏低/留抵异常/预缴不足。含66行业税负率区间规则。"},
            {"name": "社保与个税交叉", "count": 178, "logic": "工资、社保、个税三数勾稽。三源不一致/有薪无保/全员零个税/劳务报酬/股东借款视同分红/年终奖拆分。"},
            {"name": "经营实质穿透", "count": 195, "logic": "用物理世界数据（电、水、场地、人工、产能）验证账面真实性。电耗产量/仓储库存/运费发货/设备产能/物耗能耗产能三线归一。"},
            {"name": "关联交易与利益输送", "count": 53, "logic": "穿透隐藏关联关系，检验交易独立公允性。六员重叠/转移定价/资本弱化/无息拆借/购销闭环。"},
            {"name": "虚开发票专项", "count": 91, "logic": "资金回流、四流不一、闭环开票、短期激增。最高风险级：变名/挂靠/富余票/走逃失联/空壳开票。"},
            ...
            {"name": "行业专项监控", "count": 196, "logic": "制造/建筑/房地产/商贸/餐饮/电商/医药/教育/灵活用工/再生资源等行业特定风险。"},
            {"name": "外部数据比对", "count": 24, "logic": "工商、银行、海关、电力、人社等第三方数据与申报数据比对。"},
            {"name": "账表质量与勾稽", "count": 136, "logic": "报表自身平衡、账表一致、跨期逻辑合理。恒等式断裂/科目勾稽/往来挂账/折旧摊销/凭证质量。"},
            {"name": "税务合规与程序", "count": 55, "logic": "备案、申报期限、优惠资格、历史处罚、合同管理、发票程序等合规性监控。"},
        ],
