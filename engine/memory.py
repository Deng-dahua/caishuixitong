...
═════ 跨域协商引擎（2026-06-29 新增）═════
  ...
  【四类协商场景 — 15条协商规则】
  一、行业闸门消解（NEG-001~005，5条）
    触发：行业判定域得出"服务行业"结论
    消解：自动标记/删除不适用的制造业域发现
    NEG-001: 进销存匹配异常 → 消解（服务行业无实物商品）
    NEG-002: 存货积压预警 → 消解（无库存概念）
    NEG-003: BOM物料消耗不符合行业标准 → 消解（无物料清单）
    NEG-004: 进销比行业对标异常 → 消解（无进销比概念）
    NEG-005: 毛利率行业对标异常 → 降为提示级（不可比但仍参考）

  二、资料驱动的跨域标记（NEG-010~040，4条）
    触发：资料完备度域检测到某类文件缺失
    标记：通知所有依赖该文件的域降低结论证明力
    NEG-010: 缺合同 → 合同分层/合同比对 → 降为提示级
    NEG-020: 缺进项发票 → 进销匹配 → 降低置信度
    NEG-030: 缺银行流水 → 资金流向 → 标注"基于有限数据"
    NEG-040: 缺申报表 → 税务比对 → 标注"无法核查申报数据"

  三、证据矛盾消解（NEG-020~030，3条）
    触发：域A正面证据推翻域B负面结论
    消解：正面证据优先于基于间接信号推导的负面结论
    NEG-020: 社保人数差异 → 人员分类显示合法不参保 → 消解少缴发现
    NEG-025: 法代资金往来 → 凭证摘要显示"归还代垫款" → 消解利益输送标记
    NEG-030: 红冲率异常 → 退货单/折让协议确认真实 → 降为提示级

  四、联合增强（NEG-AUG-001~003，3条）
    触发：多个域异常信号同时触发
    增强：合成更高级别新发现（证明力提升一级）
    NEG-AUG-001: 资金流+发票流+工资域 → 三域联合 → "疑似系统性隐匿收入"
    NEG-AUG-002: 进项+地理+工商 → 三域联合 → "税收优惠地新设咨询公司"
    NEG-AUG-003: 银行+发票+凭证 → 三域联合 → "疑似资金过账/洗钱"

  执行时序：all_findings 生成 → run_negotiation() → 消解/降级/标记 → 过滤管线。
