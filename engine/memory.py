    ...
    # ═══════════════════════════════════════════════════════
        "rule_precise_writing": {
            ...
            "iron_rules": [
                ...
                "【铁律2·深度分级】推理链分三级(复杂≥4层/中等≥3层/简单≥2层)；穿透追问≥6条(3组×2条)；正常业务解释≥4种。简单异常不强凑。",
            ...
            "23_fields": {
                ...
                "deep": {
                    ...
                    "fields": {
                        "direction": {
                            "name": "推理链",
                            "format": "分级下限: 复杂异常≥4层(跨税种/关联方/多笔交易)，中等异常≥3层，简单异常≥2层(单税种/不跨关联方)。每层格式: 【推理第N层: XX法则】依赖证据: XX → 结论: XX。第一层必须是现象描述，最后一层必须是定性结论(指向⑰某条路径)。每层50-80字。",
                            "flexible": "简单异常判定标准: 影响仅限单一税种、不涉及多笔交易、无需穿透关联方。达不到层级如实注明原因。",
                            "example": "【推理第一层: 时间性差异法则】依赖证据: 预收账款明细账+合同交货条款 → 预收账款账龄>365天且无合理延期理由"
                        },
                        "drill_questions": {
                            "name": "穿透追问",
                            "format": "至少3组，每组至少2条，总计≥6条。确实简单可减至4条并注明原因。三组固定方向: 第一组【事实层】问发生了什么(潜台词→交易是否真实发生); 第二组【证据层】问凭什么(潜台词→四流是否合一); 第三组【逻辑层】问为什么和常理不符(潜台词→商业合理性)。格式 Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。每组递进排列，第一组不攻破不进入第二组。",
                            "flexible": "简单异常(如印花税漏缴)如实写4-6条，注明原因。不强凑8条。"
                        },
                        "phenomena": {
                            "name": "现象描述",
