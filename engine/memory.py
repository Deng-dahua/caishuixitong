    ...
    # ═══════════════════════════════════════════════════════
        "rule_precise_writing": {
            ...
            "execution_guide": {
                ...
                "purpose": "精写编制说明（v3配套执行指引）。不重复定义23字段格式和内容，解决编写者在实际精写过程中反复出现的执行偏差问题。标准管「写什么」，本说明管「怎么写才不会写错」。本说明每一条防错都对应系统校验点（三处校验器同源：命令行自检validate_rule_v3.py、验证API、启动加载校验），写错即被拦截——说明即校验契约，不是纸面建议。",
                "id_prefix_map": {
                ...
                "common_errors": [
                    ...
                    {"id": 4, "error": "追问数量未达穷举就提交", "correct": "用穷举完成判定标准三问自检，全部答'是'才提交", "field": "⑬", "engine_check": "穿透追问格式校验：Q{N}/→潜台词/A三件套齐全、事实→证据→逻辑三组顺序正确、至少3问"},
                    {"id": 5, "error": "推理链未标注依赖证据", "correct": "每层必须写'依赖证据:XX'，漏标=系统无法验证", "field": "⑫", "engine_check": "校验direction每层含「依赖证据」标注+复杂度标记+推理层格式，缺一报错"},
                    ...
                    {"id": 8, "error": "附加税费影响标为'间接'", "correct": "附加税费随增值税必然联动→标'次要'。间接=可能存在但非必然", "field": "⑱", "engine_check": "校验risk_table中「附加税费」若标注「间接」→报错，必须标「次要」（随增值税必然联动）"},
                    ...
                    {"id": 10, "error": "policy_ref引用已废止法规", "correct": "强制标注核验日期(格式:法规现行性核验:YYYY-MM-DD)，重跑law_validity_checker", "field": "⑦", "engine_check": "law_validity_checker接入审计：自动识别废止法规并要求替换现行法；校验核验日期标注"},
                    {"id": 11, "error": "定性路径只有三路径，没有写阈值以下怎么处理", "correct": "有量化阈值(㉑)的规则必须补全'阈值以下处理'分支——不生成正式发现但需以合规提示形式记录，不影响合规度评分", "field": "⑰", "engine_check": "校验有量化阈值的规则determination必须含「阈值以下处理」分支；系统按该分支将未达门槛情形记为合规提示不入稽查程序"},
                    ...
                    {"id": 13, "error": "正常解释没有标注'最常见解释'", "correct": "4种合法解释中必须有一项标注'最常见解释'作为红队攻击一优先素材——全员漏标率超过95%", "field": "⑯", "engine_check": "校验normal_reason中是否存在「最常见解释」字样——缺失则报错；系统将此标记注入finding以提示稽查员优先验证该解释"}
                ],
                "scoring_anchors": {
                    "description": "风险评分锚点（⑤score）。系统执行：score-level锚点一致性硬校验——8分以上必须高/极高风险、6-7分必须中/高风险；重罪词条目（骗税/伪造/虚开/两套账/洗钱等19词）强制≥8分且≥高风险；threshold扫描生成的疑点提示score上限5分待人工核实后按锚点升级",
                    "levels": [
                ...
                "impact_levels": {
                    "description": "影响程度判定锚点（⑱risk_table）。先锁定核心影响，再判断必然联动的次要影响，最后列出可能的间接影响。不凑数不强编。。系统执行：校验risk_table必须含「核心」影响标注；附加税费标「间接」直接报错（必然联动应标次要）；报告补税测算按核心/次要税种范围计算",
                    "核心": "直接导致补税或触犯法条构成违法",
                    "次要": "随核心影响必然联动，非独立存在",
                    "间接": "可能存在但非必然，需额外证据支持"
                },
                "evidence_priority": {
                    "description": "证据优先级（⑲evidence）。系统执行：校验evidence必须三级优先级（必须获取/应当获取/可以获取）齐全+金额分级（大额/中额/小额）；「必须获取」缺失对应发现只能停留线索档，不得升强证据/铁证",
                    "必须获取": "四流合一直接证据，缺1项闭环不成立→入线索等级",
                    "应当获取": "业务合理性佐证，增强但非必需",
                    "可以获取": "排雷辅助证据，补强或排除作用，不获取不影响闭环"
                },
                "evidence_layer_naming": {
                    "description": "第一层命名指引（⑲evidence）。不强制套用「货物流」，准确描述核心验证内容。系统执行：命名贴合软校验——监控维度属申报流监控/账表质量与勾稽/社保与个税交叉/税务合规与程序的疑点，evidence首层若仍写「货物流」即报错（无实物流转的疑点套用货物流=证据框架错位）",
                    "映射": {
                        "实物交易类": "货物流",
                        "账务断裂类": "账实核对层",
                        "纯申报类": "申报数据层",
                ...
                "quality_benchmarks": {
                    "description": "品质标杆对照：不是比数量，是比每个字段是否达到同等穷举深度。系统执行：标杆条目与库内实时一致（数字id现行体系），验证API可逐条复检36项",
                    ...
                    "简单异常": {"id": "原则", "item": "简单疑点如实写", "layers": "2-3", "questions": "4-6", "normal_reasons": "穷举完即合格", "note": "穷举但数量少也是合格的，不把复杂标杆数量当所有规则下限——一病一方"}
                },
                "submission_checklist": {
                    "description": "提交前自检（6组23项·每项均有系统校验点，三处校验器同源执行）",
                    ...
                    "整体自洽": ["⑫推理结论与⑰定性一致", "⑮舞弊手法与⑬追问形成策略→执行转化"],
                        "本体论与库权": ["铁律0：名称+现象+阈值+详情须含数据矛盾特征，写不出X vs Y不是疑点", "知识边界：立案标准/的认定/的区分/税务处理等知识型措辞禁入疑点库（存稽查知识库）", "通病禁行业：通病词+行业词同现即报错，行业差异写threshold行业调整", "重罪下限：19重罪词条目≥高风险且≥8分", "锚点一致：score与level档位互相锁定", "身份锁：id/item/category/monitor_category不被任何自动流程改动"]
                }
            }
        },
}
