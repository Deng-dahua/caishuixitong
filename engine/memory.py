    ...
    # ═══════════════════════════════════════════════════════
        "rule_precise_writing": {
            ...
            "gold_standard_decoded": {
                "description": "顶级精写方法论——从四条标杆规则（#1借贷不平/#2凭证量异常/#1716运输费缺失/#1717预收挂账）的反复打磨中提炼的编制体系。本章不是检查清单，而是完整的编制方法论——告诉你为什么这样写、怎么写、从其他规则可以借什么。",
                "attack_angles": {
                ...
                "cross_pollination_method": {
                    "description": "四条标杆的互相学习记录。编制任何规则时，先判断攻击角度（A/B/C），然后找到对应类型的标杆规则，从中借结构、借攻击力、借格式。",
                    "matrix": {
                ...
                "usage": "每轮精写完成: ①判断攻击角度(A/B/C) ②对照construction_methodology逐字段检查 ③对照cross_pollination_matrix检查是否缺少技术特征 ④对照checklist十项打勾。十勾全满=顶级精写。"
            },
            
            "rule_index": {
                "description": "27条顶级精写规则全景索引——按攻击角度分类，每条标注矛盾核心、追问数量、标杆特征，供编制时选型参照。",
                "total": 27,
                ...
                "cross_reference": "所有27条规则均按本标准（preamble哲学+attack_angles选型+23_fields逐字段+checklist十勾验收）编制。每条规则的详细内容见 static/tax_risk_rules_local_export.json。这27条既是本标准的产出物，也是标准有效性的验证集——如果一条新规则无法达到同等深度，说明标准本身还需要完善。"
            },
