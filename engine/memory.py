...
═════ 推理引擎仪表盘（2026-06-30 补录）═════
  ...
  【#2 规则库】1756条税务合规指令按分类浏览/搜索/详情查看
...
═════ 系统文件关联清单（2026-06-30）═════
  ...
  static/tax_risk_rules_local_export.json（1756条税务合规指令）
...
TAX_BURDEN_RULES = {
    ...
    # 权威源：1756条规则全部带 monitor_category 字段，取值必须来自以下13类。
    ...
    "monitor_point_taxonomy": {
        ...
        "categories": [
            {"name": "资金流监控", "count": 72, "logic": "银行流水是外部基准，比对申报、发票、账载，揭示隐匿收入、账外循环。私户收款/资金回流/公转私/现金异常/账外支付。"},
            {"name": "发票流监控", "count": 158, "logic": "进销匹配、品名关联、开票行为、发票合规。品名背离/数量不匹配/顶额开票/作废红冲/连号/夜间开票/滞留票。"},
            {"name": "申报流监控", "count": 442, "logic": "增值税、所得税、财报三表比对，税种间勾稽。两税收入不一致/零申报/税负率偏低/留抵异常/预缴不足。含66行业税负率区间规则。"},
            ...
            {"name": "行业专项监控", "count": 205, "logic": "制造/建筑/房地产/商贸/餐饮/电商/医药/教育/灵活用工/再生资源等行业特定风险。"},
            {"name": "外部数据比对", "count": 24, "logic": "工商、银行、海关、电力、人社等第三方数据与申报数据比对。"},
            {"name": "账表质量与勾稽", "count": 137, "logic": "报表自身平衡、账表一致、跨期逻辑合理。恒等式断裂/科目勾稽/往来挂账/折旧摊销/凭证质量。"},
