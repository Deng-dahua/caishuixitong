...
=== 政策引用标准（铁律） ===
税务稽查以现行有效法律法规为唯一引用依据。以下政策映射为系统强制规则：
...
# ══════════════════════════════════════════════════════════════
TAX_BURDEN_RULES = {
    "title": "税负模拟三原则——稽查报告必须做精准呈现，不可预估",
    ...
    "rule_2_vat": {
        ...
        "why": "稽查报告是专业严谨的报告。预估13%这个动作本身就说明'我其实不知道实际税额是多少'。专票有明确税额数据，普票根本不能抵扣——两者混为一谈用13%预估是专业错误。",
    ...
    "rule_3_income_tax": {
        ...
        "rule": "企业所得税率不是固定25%。小微企业优惠5-10%、高新技术企业15%、一般企业25%。必须根据企业类型选择正确税率。",
        "why": "税法规定企业所得税有多种优惠税率。不管企业类型一律按25%算，对小微企业来说严重失实。",
        "how": "检查target_entity.enterprise_type → 小微→5%/10%，高新→15%，一般→25%。同时在报告标题中标注所用税率。",
        "code": "main.py get_report_intelligence(): 按enterprise_type选inc_rate",
    },
