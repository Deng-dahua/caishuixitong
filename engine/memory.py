...
TAX_BURDEN_RULES = {
    ...
    # ═══════════════════════════════════════════════════════
        "rule_precise_writing": {
            "version": "2026-07-12 · 23字段修订版",
            "principle": "精写标准=23字段完整框架 + 分级深度  + 证据映射。每个字段的下限为最低要求，以异常点实际出发；确实达不到下限如实注明原因。框架是地图，不是尺子。",
            "iron_rules": [
                "【铁律1·字段齐全】23字段一个不能缺。基础字段9项自动填充，深度字段12项须精写。",
                "【铁律2·穷举至尽头】穿透追问问到无话可问为止（三组递进：事实→证据→逻辑）；推理链推到逻辑尽头为止（最后一层落地定性）。追问数量和推理层数是写完后的自然结果，不是写之前的硬性指标。简单异常自然少，复杂异常自然多。",
            ...
            "23_fields": {
                "basic": {
                    ...
                    "fields": {
                        ...
                        "policy_ref": {"name": "法律依据", "format": "法条编号+条文，必须引用现行有效版本。附法规最后校验日期", "example": "《增值税暂行条例》第19条(2024-07校验)"},
                ...
                "deep": {
                    ...
                    "fields": {
                        "direction": {
                            "name": "推理链",
                            "format": "推理到逻辑尽头，不设层数下限。从现象出发，每一层追问\"为什么会出现这个现象\"，直到无法再追问为止。最后一层必须落地到定性结论(指向⑰某条路径)。每层格式: 【推理第N层: XX法则】依赖证据: XX → 结论: XX。层数写完后的自然结果(复杂异常自然多，简单异常自然少)，不是写之前的硬性目标。",
                            "flexible": "三层结束条件(满足任一): ①定性落地——最后一层已明确指向定性结论; ②证据尽头——下一层需要的证据当前无法获取，标注断点并暂停; ③逻辑闭环——最后一层回到第一层前提，无新因果环节。",
                            "example": "【推理第一层: 时间性差异法则】依赖证据: 预收账款明细账+合同交货条款 → 预收账款账龄>365天且无合理延期理由"
                        },
                        "drill_questions": {
                            "name": "穿透追问",
                            "format": "穷举到无话可问，不设数量下限。三组递进方向(事实/证据/逻辑)为框架，每组追问到穷尽为止。只有穷尽第一组才能进入第二组。格式 Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。追问数量是写完后的自然结果，不是写之前的硬性指标。",
                            "flexible": "有效追问三标准: ①指向新信息，不重复已问过角度; ②指向可验证事实，不指向主观判断; ③与潜台词对应，不空泛。三组结束条件(每组满足其一): 事实穷尽(交易各要素全部覆盖)/证据穷尽(四流各环节均已追问对应证据)/逻辑穷尽(所有合理商业解释已排除)。"
                        },
                        "phenomena": {
                            "name": "现象描述",
                        ...
                        "normal_reason": {
                            "name": "正常业务解释",
                            "format": "至少4种情形，格式 {情形}——需提供{具体证据(文件类型)}。禁用'提供相关证明'须具体到文件。标记'最常见解释'作为红队攻击一优先素材。确无合法情形时如实写0-3种并注明原因，不允许编造。",
                            "flexible": "至少4种。无法穷举到4种的如实注明已穷举全部合理情形。",
                            "example": "①季节性旺季——提供过去三年同月销售数据对比表 [最常见解释]"
                        },
                        "determination": {
                            "name": "定性路径",
                            "format": "三路径必须对应证据链三档定级: ①无法证明→线索等级(单源数据触发)→定性'存疑，建议补充调查'+不入正式结论; ②部分证明→强证据(2独立来源验证)→定性'涉嫌XX'+可入正式结论但标注'证据部分闭环'; ③完整证明→铁证(≥3独立来源闭环)→定性'认定XX'+入正式结论可作处罚依据。每路径定义进入条件。结尾附应对总原则。",
                            "flexible": "至少2-3条路径。应对总原则示例: 能完整证明走铁证路径; 能部分证明走强证据路径补证据后重判; 无法证明入线索池不入正式结论。"
                        },
                        "risk_table": {
                            "name": "风险表格",
                        ...
                        "action": {
                            "name": "稽查动作",
                            "format": "至少5步。每步格式: 动作类型(现场核查/纸面比对/外调走访/联网核查)+具体操作+预期产出。确实不存在现场核查可能性的(如纯申报类异常)，注明原因后补充替代性核查手段(交叉比对第三方数据/函调/联网核查)，不强制虚构现场核查。",
                            "flexible": "至少5步。不可行时注明并补充替代手段。",
