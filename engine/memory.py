...
TAX_BURDEN_RULES = {
    ...
    "rule_precise_writing": {
        "version": "2026-07-11 · 23字段完整版",
        "principle": "精写标准=23字段完整框架 + 灵活深度。框架规定的字段一个不能少，但每个字段的具体深度（层数/条数/字数）以异常点实际出发。",
        "iron_rules": [
            "【铁律1·字段齐全】23字段一个不能缺。即使是'该异常点无合法情形'也要写明原因，不能留空。",
            "【铁律2·深度灵活】每个字段的下限标准是'最低要求'——异常点确实需要更多就写更多，确实达不到下限就如实写明原因并注明。不凑数、不硬编。",
            "【铁律3·异常点驱动】每条规则以该异常点的具体特征为出发点——虚开发票类追问要多推理要深，税率错误类追问集中推理直白。一病一方。",
        ],
        "23_fields": {
            "basic": {
                "count": 9,
                "description": "基础字段——每条必填",
                "fields": {
                    "id": "异常编号（全局唯一自增）",
                    "item": "异常名称（[行业] 异常标题 格式）",
                    "category": "所属类别",
                    "level": "风险等级（极高/高/中/低/良好）",
                    "score": "风险评分（1-10分）",
                    "check_frequency": "稽查频率（高频/中频/低频）",
                    "policy_ref": "法律依据（具体法条编号和条文）",
                    "tax_impact": "税务影响（涉及税种+补税区间）",
                    "applicable_condition": "适用条件（什么类型/规模/行业触发）"
                }
            },
            "source": {
                "count": 2,
                "description": "来源标记",
                "fields": {
                    "source": "来源（空=人工精写/系统发现）",
                    "auto_type": "自动发现类型（系统发现时填写）"
                }
            },
            "deep": {
                "count": 12,
                "description": "深度字段——精写的核心",
                "fields": {
                    "direction": {
                        "name": "推理链",
                        "format": "【推理第一层：XX法则】→【推理第二层：XX】→...，每层50-80字，至少4层，最后一层必须落地到定性",
                        "flexible": "复杂异常可5-6层，简单异常（如单纯税率错误）如实写2-3层但需注明为何不需要4层"
                    },
                    "drill_questions": {
                        "name": "穿透追问",
                        "format": "至少8条分3组，每组标注组名。每条格式为 Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}",
                        "flexible": "稽查角度超过8个就写更多；如果该异常点的所有逃税可能只需4条就能穷尽，写4条但注明原因"
                    },
                    "phenomena": {
                        "name": "现象描述",
                        "format": "异常定义 + 5种常见表现形式（用①②③④⑤列举）+ 适用范围兜底条款",
                        "flexible": "如该异常点只有2-3种表现，如实写并注明"
                    },
                    "focus": {
                        "name": "稽查重点",
                        "format": "列明该异常最常见的舞弊手法，用①②③④逐条标注",
                        "flexible": "至少有①②③，多则不限"
                    },
                    "normal_reason": {
                        "name": "正常业务解释",
                        "format": "每种格式为 {情形}——需提供{具体证据}，证据必须可核验",
                        "flexible": "至少4种情形。如该异常点确实没有那么多合法情形（如虚开发票），如实写0-3种并注明原因，不强编。降低下限避免注水凑数。"
                    },
                    "determination": {
                        "name": "定性路径",
                        "format": "三条推理路径：无法证明→定性+后果；部分证明→分类处理；完整证明→排除风险。结尾附应对总原则",
                        "flexible": "至少2-3条路径"
                    },
                    "risk_table": {
                        "name": "风险表格",
                        "format": "覆盖实际涉及税种/维度，不设数量下限。每行格式为 税种:具体风险描述。部分疑点只影响单一税种（如印花税漏缴），强制多税种会注水",
                        "flexible": "只列实际涉及的税种。简单异常涉及的税种少就少列，每行描述必须具体到该异常点的处理方式"
                    },
                    "evidence": {
                        "name": "证据清单",
                        "format": "四层框架：货物流 + 合同资金流 + 业务合理性 + 排雷。大额业务区分AB场景（自提vs直运）",
                        "flexible": "根据异常点性质调整框架。如不涉及货物流的异常（如税率错误），以其他维度替代"
                    },
                    "threshold": {
                        "name": "触发指标",
                        "format": "必须有量化阈值 + 前置条件（如销售额门槛）",
                        "flexible": "二元异常（有/无）写 =是即触发；连续异常给具体门槛和预警等级（黄/橙/红）"
                    },
                    "action": {
                        "name": "稽查动作",
                        "format": "至少5步具体核查步骤，其中必须包含现场核查手段（实地查看/调取记录/外调走访），不能全是纸面比对",
                        "flexible": "根据核查复杂度增减步骤数"
                    },
                    "suggestion": {
                        "name": "稽查处理",
                        "format": "明确查实后的处置方式（补税/罚款/移送），分税种说明",
                        "flexible": "无"
                    },
                    "remedy": {
                        "name": "整改建议",
                        "format": "三阶段：自查(稽查前) → 应对(稽查中·含话术策略) → 制度(长期)",
                        "flexible": "根本性异常（虚开发票）与轻微异常（申报遗漏）的整改内容完全不同"
                    }
                }
            }
        },
        "summary": "23字段框架 = 必须覆盖的维度。每个字段的具体深度 = 以异常点实际出发。框架是地图，不是尺子。"
    },
}
