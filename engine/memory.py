...
TAX_BURDEN_RULES = {
    ...
    "rule_precise_writing": {
        "principle": "精写是指导框架，不是硬编码模板。所有数量/字数均为参考下限，编写者根据异常点的实际情况灵活发挥。",
        "iron_rules": [
            "【铁律1·内容优先】规则的质量取决于内容深度和逻辑严密性，不取决于字段数量或字数。如果3层推理已说清问题，不需要硬凑4层。如果该异常点只有3种正常情形，不强编5种。",
            "【铁律2·异常点驱动】每一条规则的编写必须以该异常点的具体特征为出发点——不同异常点的推理深度、追问方向、正常情形数量天然不同。虚开发票类规则追问要多、推理要深；税率错误类规则追问集中、推理直白——不能套同一个模板。",
            "【铁律3·可突破标准】以下各字段的参考标准不是上限。如果某异常点需要更多追问才能穷举稽查角度，就写更多。如果需要正常情形之外的特殊证据，就加进去。标准是用来保证最低质量的，不是限制最大深度的。",
        ],
        "fields": {
            "direction": {
                "name": "推理链",
                "reference": "通常2-4层，从表面现象逐层深入到法律定性。每层回答一个'为什么这有问题'的子问题。编写原则：以说清楚为优先级，不以层数为目标。简单异常点（如税率错误）可能2层就够；复杂异常点（如虚开）可能需要5-6层。",
            },
            "drill_questions": {
                "name": "穿透式追问",
                "reference": "通常分2-3组，每组2-4条。每条追问:Q+问题→潜台词:稽查的真实意图。A:假设回答。编写原则：追问数量=该异常点稽查角度的数量。追问必须直击要害、不留退路。攻击性>温和性。如果该异常点只需要4条追问就已穷举所有逃税可能，就写4条。",
            },
            "normal_reason": {
                "name": "正常合规情形",
                "reference": "列举该异常点可能的合法合理解释，每种附对应的证据要求。编写原则：只列举真实存在的合法情形。如果该异常点几乎没有正常情形（如'无真实交易的虚开发票'），如实写0-1种并注明'极少有合法情形'。不能为了凑数编造不存在的正常情形。",
            },
            "determination": {
                "name": "定性路径",
                "reference": "至少1条，通常2-3条。每条给出：条件→定性结论→税务处理→法律后果。编写原则：路径数量=该异常点可能的定性方向数量。区分'高度疑似→核查取证后定性'和'有证据直接定性'的递进关系。疑点≠结论。",
            },
            "risk_table": {
                "name": "风险表格",
                "reference": "列出该异常点涉及的全部税种和法律风险。编写原则：只列实际涉及的税种。一个简单的增值税税率错误不需要列5个税种。每个税种的描述必须具体到'该异常点在这个税种下会怎么被处理'。",
            },
            "action": {
                "name": "稽查动作",
                "reference": "①-⑤步骤。编写原则：步骤必须是可执行的、有先后顺序的、针对该异常点的。不同异常点的核查步骤可能完全不同。",
            },
            "evidence": {
                "name": "证据清单",
                "reference": "该异常点核查所需的全部证据文件名称。编写原则：具体到文件名层级（如'购销合同的价格条款'而非'合同'）。不同异常点需要不同的证据组合。",
            },
            "threshold": {
                "name": "量化阈值",
                "reference": "触发该规则的量化条件+预警等级（黄/橙/红）。编写原则：阈值必须基于税法规定或行业数据。如果该异常点是二元的（有/没有），阈值就是'=是即触发'。如果是连续的，给出具体的数字门槛。",
            },
            "remedy": {
                "name": "整改方案",
                "reference": "三阶段：自查→应对→制度。编写原则：自查是内部能做的事，应对是被查到的处理，制度是防止再次发生。根本性异常（如虚开发票）的整改和轻微异常（如申报遗漏）的整改内容完全不同。",
            },
        },
        "summary": "精写标准=必须覆盖的维度（方向），不是必须填满的数字（数量）。每个异常点的特点是决定内容深度和广度的唯一依据。",
    },
}
