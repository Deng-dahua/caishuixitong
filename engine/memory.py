    ...
    # ═══════════════════════════════════════════════════════
        "rule_precise_writing": {
            ...
            "gold_standard_decoded": {
                "description": "顶级精写方法论——从六条标杆规则（#1借贷不平/#2凭证量异常/#1716运输费缺失/#1717预收挂账/#1718资金闭环回流/#1719社保人数缺口）的反复打磨中提炼的编制体系。v5升级: 新增缺失型维度——从'A!=B'和'A→B→C→A'升级到'A存在→B必须存在但B=0'的法律义务真空思维。",
                "attack_angles": {
                    "description": "疑点的五种攻击角度。编制疑点的第一步是判断它属于哪种攻击角度——这决定了推理链的穿透深度、追问的攻击力和辩解空间的大小。",
                ...
                "construction_methodology": {
                    "description": "从六条标杆提炼的逐字段编制方法论。每个字段说明为何这样写、标杆案例、常见错误。",
                    "phenomena": {
                ...
                "checklist": [
                    ...
                    {"id":"GS-12","name":"缺失因果链验证","criterion":"缺失型疑点必须写明'A存在→触发B义务→B=0'的因果链。穷举所有合法免履行义务情形，每种情形逐人/逐项提供证据。必须做双线并查（如人数缺口+基数缺口）。不为零值的偏差，而是法定义务的真空。"}
                ],
                "usage": "每轮精写完成: ①判断攻击角度(A/B/C/D/E) ②对照construction_methodology逐字段检查 ③对照cross_pollination_matrix检查是否缺少技术特征 ④对照checklist十二项打勾。十二勾全满=顶级精写。"
            },
            
            "rule_index": {
                "description": "29条顶级精写规则全景索引——按攻击角度分类，每条标注矛盾核心、追问数量、标杆特征，供编制时选型参照。",
                "total": 29,
                ...
                "cross_reference": "所有29条规则均按本标准（preamble哲学+attack_angles选型+23_fields逐字段+checklist十二勾验收）编制。每条规则的详细内容见 static/tax_risk_rules_local_export.json。这29条既是本标准的产出物，也是标准有效性的验证集——如果一条新规则无法达到同等深度，说明标准本身还需要完善。"
            },
