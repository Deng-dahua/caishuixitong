            ...
            "23_fields": {
                ...
                "deep": {
                    ...
                    "fields": {
                        ...
                        "drill_questions": {
                            "name": "穿透追问",
                            "format": "穷举至稽查终点——问题间环环相扣、因果递进。三组递进方向(事实→证据→逻辑)为追问框架，每组穷举至无法提出新的有效追问。每组攻击要点: 事实层——交易六要素全覆盖（谁/什么/什么时候/在哪/怎么办的/谁参与的），任一空白=追问未穷举; 证据层——四流全追问（合同流/货物流/资金流/发票流），每流必须追到具体证据类型; 逻辑层——所有合理商业解释全排除（合同特殊条款/行业惯例/交易对手特殊情况/关联方关系/不可抗力），无法提出新解释=穷举完成。必须包含至少1条物理常识类追问——用不可辩驳的物理事实堵死辩解空间（如'工厂有没有能吊装这个吨位设备的行车''仓库能不能放得下全部挂账对应货物''仓储费为什么没有同步增加'）。追问在两种终点之一停止：①证实违法（铁证闭环）②排除违法（自证清白）。格式: Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。追问末尾必须包含穷举完成判定——'事实层六要素全覆盖/证据层四流全追问/逻辑层所有解释全排除→三问全答是→穷举完成'。",
                            "flexible": "追问数量由三维覆盖度决定，不设上下限: 事实层（交易六要素全覆盖，至少5问）+ 证据层（四流全追问，至少4问）+ 逻辑层（所有解释全排除，至少3问）。含至少1条物理类追问。数量是穷举完后的自然结果。标杆参照: #1716(15问含3条物理追问)、#1717(15问含2条物理追问)、#9(16问含全渠道收款追问)。",
            ...
            "gold_standard_decoded": {
                ...
                "construction_methodology": {
                    ...
                    "drill_questions": {
                        ...
                        "physical_block": "必须包含至少1条物理常识类问题——用不可辩驳的事实堵死辩解空间。如#1717吸收的Q14'仓库能不能放得下'、Q15'仓储费为什么没有同步增加'。",
