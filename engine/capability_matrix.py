...
CAPABILITY_MATRIX = [
    ...
    # ── ★★★★ 新增四星能力（2026-06-25）──
    {
        ...
        "name": "财务报表税务稽查",
        "stars": 4,
        "core": "四层财税钩稽：表内平衡→跨表钩稽→指标趋势→发票vs报表对比；往来款项深度税务稽查(预收/预付/其他应收-个人/存货/应付职工薪酬)",
...
QUALITY_SYSTEM_LAYERS = {
    "核心数据资产": {
        "规则引擎": "tax_risk.py + risk_rules → 1514条稽查指令",
        "线索链系统": "main.py → 391条线索链 + 三级触发",
        "证据链系统": "main.py → 740条证据链 + 闭环检测",
        "跨域分析链": "main.py → 资金流/票据流/业务流三维验证",
    },
    "方法论体系": {
        "稽查方法论29条": "main.py → 全部代码化，每条可追溯代码位置",
        "四步稽查分析法": "detect→verify→diagnose→report 统一框架",
