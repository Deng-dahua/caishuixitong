...
设计目标：每次稽查结束后，强制产出"三段式覆盖清单"，让稽查员一眼看清
本次稽查是否存在盲区，做到"不留暗区、每个盲区都有交代"：

  1. 已覆盖(EXECUTED)：本次实际运行且已注册的规则，及其命中情况
  2. 未触发(NO_HIT)：已注册但本次数据未命中的规则（仍属已覆盖能力）
  3. 盲区(GAP)：经识别的稽查风险域，但当前系统尚无对应可执行规则，
     须明确标注原因（数据缺失 / 本质不可数字化 / 待实现）

此外枚举"中国主要税种稽查风险域全景"，与现有规则做映射，输出
coverage_matrix，使稽查员能断言本次稽查在规则维度"无遗漏"。
"""

from __future__ import annotations

# ── 中国主要税种 × 稽查风险域全景（用于覆盖度对标）────────────────────
...
RISK_DOMAIN_PANORAMA = [
    ...
    # 本质不可数字化（须人工/外部）
    ("稽查本质盲区", "账外经营/私户收款", False, "", "本质不可数字化，须资金穿透与举报线索+人工"),
    ("稽查本质盲区", "实物资产盘点", False, "", "须现场盘点，系统无法覆盖"),
    ("稽查本质盲区", "业务真实性主观定性", False, "", "须合同/物流/资金三流合一人工研判"),
    ("稽查本质盲区", "跨境交易穿透", False, "", "须境外税收居民与CRS数据，待外部接入"),
]
...
def format_coverage_text(report):
    """生成人类可读的覆盖度报告文本（用于稽查结论附件）。"""
    s = report["summary"]
    lines = []
    lines.append("【稽查覆盖度自检报告】")
    lines.append(
