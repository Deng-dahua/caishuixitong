...
同步修复 `tax-doc-analysis.js:344` 税务合规期间回退值、`tax-doc-analysis.js:413` 收款方关键词。
...
| 重复内容 | 重复次数 | 统一方案 |
|---------|---------|---------|
| 税务合规方法论㉖条 | 4处 | 提取为独立函数 `renderMethodologyBrief()` |
...
| 质量保障体系 | 税务合规指令/线索链/证据链 | 每个组件附"查看详情"链接 |
| 域分析 | 税务合规指令 | 每个域附"查看相关规则"链接 |
| 税务合规指令 | 域分析 | 每条规则附"查看触发发现"链接 |
| 线索链 | 证据链 | "关联证据链"链接 |
