"""
财税系统 - 存勤法税智能体对话引擎
支持：税务政策深度问答、账务处理指导、法律合规咨询、风险预警提示
LLM 驱动 + 本地规则引擎双模
"""
...
你的专业领域：
1. **税务政策（深度解读）**：增值税、企业所得税、个人所得税、印花税、城建税及附加、房产税、土地使用税等——引用具体法条文号
2. **账务处理（实务操作）**：会计分录编制、凭证处理、科目运用、期末结转、报表编制——给出具体分录示例
3. **法律合规（税务争议/税务合规应对）**：税务税务合规应对、税务行政复议、税务行政诉讼、税务争议解决路径
4. **风险提示（税务合规级）**：税务税务合规高风险行为、发票合规风险、经营实质风险、税负异常预警、跨区域经营合规
5. **税收优惠（适用条件/申报流程）**：高新技术企业、研发费用加计扣除、小规模纳税人优惠、农产品自产自销免税等

回答风格与要求（非常重要，请严格遵守）：
- 【结论先行】第一段用 1-2 句话给出明确结论，不要铺垫
- 【法规依据】必须引用具体法条/文号，格式：「根据《XX法》第X条」或「根据国家税务总局公告XXXX年第XX号」
- 【结构化】使用多级标题（## / ###）、表格、有序/无序列表，层次清晰
- 【深度】复杂问题回答不少于 500 字，逐层展开，不遗漏关键细节
- 【证据链】涉及税务争议/税务合规应对的问题，必须说明需要准备的证据材料清单
- 【救济程序】涉及税务争议的问题，必须说明法律救济路径（复议/诉讼的时限、条件、顺序）
- 【风险提示】用 ⚠️ 标注高风险点，用 💡 标注操作建议
- 【表格】对比类问题必须用表格呈现（如税率表、扣除标准表、期限表）
- 【分步骤】操作流程类问题必须分步骤说明，每步说明「谁来做、做什么、提交什么材料、时限要求」
- 【不瞎编】不确定的政策点数、比例、时限，明确说明「建议咨询当地税务机关或查阅最新公告」，不要给出可能错误的数字
- 【语言】用中文回答，专业术语首次出现时附简要解释

重要纪律（不可违反）：
- 不做税务筹划建议（如「如何避税」），只做政策解读和合规风险提示
- 不替用户做决策，只提供专业分析和可选项
- 涉及具体企业数据的问题，先说明「根据你提供的信息分析」
- 如用户上传了文件（Excel/PDF），先分析文件内容再回答，不要忽略文件"""
...
def _chat_tax_qa(user_message: str, company_id: int, db: Session) -> str:
    ...
    tax_qa = {
        ...
        r"个税.*(计算|多少|怎么|算)|个人所得税.*(怎么|算|多少)|工资.*个税": (
            "📋 **工资薪金个人所得税（累计预扣法）**\n\n"
        ...
        r"折旧.*年限|固定资产.*折旧|怎么折旧|买车.*入账|车辆.*折旧": (
            "📋 **固定资产折旧与入账全指引**\n\n"
            "## 1. 入账分录\n"
            "购入固定资产（以车辆为例）：\n"
            "```\n"
            ...
            "  贷：银行存款 / 应付账款    （价税合计）\n"
            "```\n"
            "⚠️ 取得增值税专用发票的，进项税额可全额抵扣（用于生产经营）。\n\n"
            "## 2. 残值率\n"
            "• 企业会计准则：合理确定预计净残值，通常按原值的 **3%-5%**\n"
            "• 税法规定：内资企业残值比例统一为原价的 **5%** 以内\n"
            "• 可在 0-5% 范围内自主确定，一经确定不得随意变更\n\n"
            "## 3. 折旧最低年限\n"
            "| 资产类别 | 最低年限 |\n"
            "|----------|----------|\n"
            "| 房屋、建筑物 | 20年 |\n"
            ...
            "| **飞机火车轮船以外的运输工具** | **4年** ✅ ← 汽车 |\n"
            "| 电子设备 | 3年 |\n\n"
            "💡 以8万元购车为例（假设不含税7.08万，税额0.92万，残值率5%）：\n"
            "  月折旧额 = 70,800 × (1-5%) ÷ 48个月 ≈ 1,401 元/月\n\n"
            "## 4. 每月折旧分录\n"
            "```\n"
            "借：管理费用 / 销售费用 — 折旧费    1,401\n"
            "  贷：累计折旧                     1,401\n"
            "```\n\n"
            "## 5. 税务注意事项\n"
            "⚠️ 二手车也需按最低年限折旧，不得因已使用而缩短\n"
            "⚠️ 折旧方法可选直线法（推荐）或加速折旧法，选定后不得随意变更\n"
            "⚠️ 折旧费税前扣除需取得合法凭证（发票+付款记录）\n"
...
def intent_from_text(msg: str) -> str:
    ...
    if best_score == 0:
        ...
        tax_keywords += r"优惠政策|税收优惠|减免税|税务合规|合规|风险|预警|"
        ...
        if re.search(tax_keywords, msg_lower):
            return None  # 返回 None 表示进入 LLM 存勤法税智能体
