...
function renderTaxRiskRules(container, isAuto) {
  ...
  h += '<details id="rr-standard" style="margin-bottom:16px;background:#fafbfc;border:1px solid #eef2f6;border-radius:8px;padding:12px 16px;font-size:11px;line-height:2;color:#5b6675"><summary style="font-weight:700;color:#16233a;cursor:pointer;font-size:12px">📐 精写编制标准（23字段完整版）</summary>'
    ...
    + '<b>一、推理链：</b>direction字段推理到逻辑尽头。每层追问"为什么"，直到无法再追问。三层结束条件: 定性落地/证据尽头/逻辑闭环。格式: 依赖证据→结论。层数自然呈现。<br>'
    + '<b>二、穿透追问：</b>drill_questions字段穷举到无话可问。三组递进(事实→证据→逻辑)，每组追问到穷尽。格式Q→潜台词。数量是写完后的自然结果。<br>'
    + '<b>三、现象描述：</b>phenomena字段典型表现枚举(非穷举)，至少5种+兜底条款+排除条件(类似表现不属于本异常的情况)。<br>'
    + '<b>四、稽查重点：</b>focus字段策略层——舞弊手法预判。格式:手法名称:操作方式→识别要点。与穿透追问分工:focus=策略层(预判),drill=执行层(提问)。<br>'
    + '<b>五、正常业务解释：</b>normal_reason字段至少4种情形，格式"{情形}——需提供{具体证据(文件类型)}"。标记最常见解释。不强编不注水。<br>'
    + '<b>六、定性路径：</b>determination字段三路径对应三档定级——无法证明→线索(不入结论)；部分证明→强证据(2源)；完整证明→铁证(≥3源)。每路径定义进入条件。<br>'
    + '<b>七、风险表格：</b>risk_table字段覆盖实际涉及税种，不设数量下限。每行格式:税种:风险描述 | 影响程度:核心/次要/间接。<br>'
    + '<b>八、证据清单：</b>evidence字段四层框架+金额分级+优先级。大额(>10万)分AB场景；中额(1-10万)≥2维度；小额(<1万)抽样。每层标注必须/应当/可以。<br>'
    + '<b>九、触发指标：</b>threshold字段量化阈值+前置条件(行业+资质+数据+时间)+行业差异(通用阈值+行业调整)。二元异常写=是即触发。<br>'
    + '<b>十、稽查动作：</b>action字段至少5步，每步标注类型+预期产出。纯申报类异常允许注明现场核查不可行并补充替代手段。<br>'
    + '<b>十一、稽查处理：</b>suggestion字段稽查局视角。固定格式:定性→补税(分税种+金额)→滞纳金→罚款(条款+区间)→移送标准。<br>'
    + '<b>十二、整改建议：</b>remedy字段企业视角。三阶段含时间:自查(通知前)→应对(稽查中+话术)→制度(长期建设)。与suggestion分工明确。<br>'
    + '<em style="color:#64748b">* 追问穷举到无话可问，推理到逻辑尽头。数量是自然结果，不是硬性指标。不凑数、不强编、一病一方。</em>'
    + '</details>';
  }
