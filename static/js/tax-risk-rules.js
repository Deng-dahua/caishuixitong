...
function renderTaxRiskRules(container) {
  ...
  h += '<details id="rr-standard" style="margin-bottom:10px;background:#fafbfc;border:1px solid #eef2f6;border-radius:8px;padding:10px;font-size:10px;line-height:20px;color:#334155"><summary style="font-weight:700;color:#16233a;cursor:pointer;font-size:10px">📐 精写编制标准（23字段完整版 · v3穷举至稽查终点）</summary>'
    ...
    + '<p><b>① id（异常编号）</b>——{类型前缀}-{三位序号}，类型: AN=隐匿收入/VC=虚列成本/VI=虚开发票/ST=少缴税款/OT=其他。引擎执行：发现溯源锚点。一键分析中 finding._rule_id 以此关联规则与发现；跨域线索链/证据链/分析链的 rule_refs 以此串联规则；删改规则时引擎按 id 同步全部链引用，防幽灵引用。</p>'
    + '<p><b>② item（异常名称）</b>——受控词表统一命名，同义不同名禁止。引擎执行：规则匹配主键。一键分析三级消费：①发现type与item精确匹配→注入深度字段；②精确失败走bigram语义匹配（阈值0.55宁缺勿滥）；③threshold主动扫描按item防重复生成。受身份锁保护，任何自动流程不得改名。</p>'
    + '<p><b>③ category（所属类别）</b>——五类之一: 隐匿收入/虚列成本/虚开发票/少缴税款/其他违规。引擎执行：定性分类载体。报告按类别汇总违规性质，LLM推理上下文携带。受身份锁保护。</p>'
    + '<p><b>④ level（风险等级）</b>——极高/高/中/低/良好。与合规度对应: 极高<40分/高40-60/中60-80/低80-90/良好>90。引擎执行：发现风险等级来源。threshold扫描触发的发现按「疑点非结论」铁律统一降为中风险待人工核实；校验器强制重罪条目≥高风险、score-level锚点一致。</p>'
    + '<p><b>⑤ score（风险评分）</b>——1-10分。锚点: 10=系统性造假/金额巨大/主观故意; 5=中等风险/需补充证据; 1=低风险/小额/偶发。引擎执行：发现评分与排序依据。报告按分值降序展示；threshold扫描注入的发现score上限5分（金额维度已验证、业务属性未核实）；8分以上进入高风险清单。</p>'
    + '<p><b>⑥ check_frequency（稽查频率）</b>——高频=每户必查; 中频=行业匹配时查; 低频=特定条件触发时查。引擎执行：扫描调度档位（一键分析执行）。高频/中频规则参与threshold常规扫描（触发置信度门槛0.85）；低频=特定条件触发时查——仅在结构化条件强触发（置信度≥0.95）时生成发现，防低频规则全量扫描误报。</p>'
    + '<p><b>⑦ policy_ref（法律依据）</b>——引用现行有效的法律法规，统一冠以“现行有效的《XX法》(版本)第X条“，保留条号保证稽查精确性。法律不写死、不当永久真理：每条引用须过法律时效性核查程序（engine/law_validity_checker，已接入审计）验证，末尾强制附核验日期。法律变动时核查程序自动识别废止法规并替换为现行法，重跑即可自动更新，无需人工维护废止对照表。引擎执行：发现的法律依据，直接注入发现与报告。law_validity_checker接入审计：自动核验法规现行性、废止法规自动替换、强制核验日期。</p>'
    + '<p><b>⑧ tax_impact（税务影响）</b>——分最低影响和典型影响: 税种(最低补税X万，典型补税Y万)。引擎执行：报告税负测算与处置建议段落的数据源，随发现注入。</p>'
    + '<p><b>⑨ applicable_condition（适用条件）</b>——五维度结构化: 行业限制+纳税人资质+规模门槛+时间条件+金额门槛。非全部必填。引擎执行：触发前置闸门（一键分析执行）。threshold扫描前引擎解析五维度中当前可判定的维度：行业限制与企业行业（加权投票识别结果）比对，限定行业不符直接拦截不触发，防止跨行业误报；资质/规模/时间/金额维度随对应数据源接入逐步启用硬校验。</p>'
    + '<p><b>二、来源标记（2项）</b></p>'
    + '<p><b>⑩ source（来源）</b>——空=人工精写 / 系统发现 / LLM生成。引擎执行：来源标记与身份锁保护对象。自动发现规则须经人工确认转正；报告区分人工精写/系统发现/LLM生成三种来源的可信度。</p>'
    + '<p><b>⑪ auto_type（自动发现类型）</b>——行业基准校准/购销倒挂/毛利为负/缺失数据/综合异常/未知模式。引擎执行：自动发现分类标记。自动规则面板分组展示与转正审核参考；通病类信号已在生成端黑名单拦截。</p>'
    + '<p><b>三、深度字段（12项·精写核心）</b></p>'
    + '<p><b>一、推理链（direction）</b>——推理至稽查终点：证实违法或排除违法。层与层之间因果递进、环环相扣。每层格式: 【推理第N层: XX法则】依赖证据: XX → 结论: XX。三种结束条件: ①定性落地（最后一层已明确指向证实违法或排除违法）；②证据尽头（下一层需要的证据无法获取，标注断点）；③逻辑闭环（最后一层回到第一层前提）。引擎执行：推理骨架（一键分析执行）。命中后注入 finding._rule_direction，作为LLM推理上下文与报告「为何成为疑点」段落的推理链来源；跨域分析链的推理路径与其衔接。</p>'
    + '<p><b>二、穿透追问（drill_questions）</b>——穷举至稽查终点：问题间环环相扣、因果递进，每个答案自然引出下一个问题。三组递进方向(事实→证据→逻辑)为追问框架。有效追问三标准: ①答案必然引出下一个问题（非孤立提问）；②答案可验证（指向具体证据）；③答案推动稽查进程（要么加深嫌疑，要么排除嫌疑）。追问在证实违法或排除违法时停止。格式: Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。引擎执行：稽查执行层素材。命中后随规则上下文注入LLM与报告稽查建议段，为现场讯问提供环环相扣的追问路径。</p>'
    + '<p><b>三、现象描述（phenomena）</b>——典型表现枚举(非穷举)，每种格式: 表现描述+典型行业/场景。至少5种典型表现。兜底条款: 其他同类或类似表现。增加排除条件: 什么情况下的类似表现不属于本异常。引擎执行：铁律0载体（一键分析执行）。引擎提取「数据矛盾点：」首句注入 finding._contradiction，报告直接展示该发现的数据矛盾本质；全文参与语义匹配辅助命中。</p>'
    + '<p><b>四、稽查重点（focus）</b>——策略层：舞弊手法预判，不直接用于提问。格式: 舞弊手法名称: 具体操作方式 → 识别要点。用①②③④逐条标注。与穿透追问分工: focus=策略层(预判)，drill_questions=执行层(将预判转化为讯问问题)。引擎执行：策略层预判。命中后注入LLM上下文，指导报告舞弊手法提示段；与drill_questions策略→执行分工。</p>'
    + '<p><b>五、正常业务解释（normal_reason）</b>——至少4种情形，格式 {情形}——需提供{具体证据(文件类型)}。禁用“提供相关证明“须具体到文件。标记“最常见解释“作为红队攻击一优先素材。确无合法情形时如实写0-3种并注明原因，不允许编造。引擎执行：逃生舱执行（一键分析执行）。引擎将合法解释摘要注入 finding._escape_hatch，报告提示稽查员先验证排除项后再定性，防误伤合规企业——本体论三问之③的运行时落地。</p>'
    + '<p><b>六、定性路径（determination）</b>——三路径必须对应证据链三档定级: ①无法证明→线索等级(单源数据触发)→定性“存疑，建议补充调查“+不入正式结论；②部分证明→强证据(2独立来源验证)→定性“涉嫌XX“+可入正式结论但标注“证据部分闭环“；③完整证明→铁证(≥3独立来源闭环)→定性“认定XX“+入正式结论可作处罚依据。每路径定义进入条件。设置了量化阈值(㉑)的规则，必须增加“阈值以下处理“分支——门槛未达到时以合规提示形式记录，不入稽查程序、不影响合规度评分。结尾附应对总原则。引擎执行：定性分级标尺（一键分析执行）。引擎按发现的独立来源域数量自动映射三档定级写入 finding._evidence_grade：1域=线索/2域=强证据/≥3域=铁证，与三路径定级一一对应；阈值以下处理分支约束合规提示不入稽查程序。</p>'
    + '<p><b>七、风险表格（risk_table）</b>——覆盖实际涉及税种/维度，不设数量下限。跨税种影响≥2个的逐税种列明，仅涉及单一税种的不强制跨税种。每行格式: 税种:具体风险描述 | 影响程度:核心/次要/间接。引擎执行：报告跨税种影响段落数据源，核心/次要/间接标注决定补税测算的税种范围。</p>'
    + '<p><b>八、证据清单（evidence）</b>——四层框架: 货物流+合同资金流+业务合理性+排雷。交易金额分级: 大额(>10万)分AB场景(自提vs直运); 中额(1-10万)至少取2个维度证据; 小额(<1万)抽样核验。每层证据标注优先级: 必须获取/应当获取/可以获取。排雷层与正常业务解释联动——列出合法情形，排雷层验证这些情形的证据是否真实存在。引擎执行：取证指引。四层框架与必须/应当/可以优先级随发现注入报告稽查建议，决定证据调取顺序与金额分级核验深度。</p>'
    + '<p><b>九、触发指标（threshold）</b>——必须有量化阈值+前置条件(行业+资质+数据+时间四维度)。阈值增加行业差异: 通用阈值+行业调整。二元异常写“=是即触发“。预警等级: 黄/橙/红。前置条件四个维度各自独立，全满足才触发。引擎执行：主动扫描执行核心（一键分析执行）。rule_consumer 结构化解析量化条件后对账扫描（银行流水/发票/工资/社保/凭证），条件满足即主动生成发现——触发门槛按check_frequency分档（常规0.85/低频0.95）；无法结构化解析的threshold降级为匹配辅助文本，不阻断分析。</p>'
    + '<p><b>十、稽查动作（action）</b>——从纸面比对到现场核查穷举全部核查步骤，下限至少3步含1项现场核查、上限穷举完毕。每步格式: 动作类型(现场核查/纸面比对/外调走访/联网核查)+具体操作+预期产出。确实不存在现场核查可能性的(如纯申报类异常)，注明原因后补充替代性核查手段(交叉比对第三方数据/函调/联网核查)，不强制虚构现场核查。引擎执行：核查方案来源。纸面比对→现场核查步骤随发现注入报告核查方案段。</p>'
    + '<p><b>十一、稽查处理（suggestion）</b>——稽查局视角。固定格式: 定性→补税(分税种+金额)→滞纳金(日万分之五+起止日期)→罚款(征管法条款+区间)→移送标准(金额/情节门槛)。与定性路径结论一致。引擎执行：稽查处置建议，直接进入发现与报告处置段落（稽查局视角）。</p>'
    + '<p><b>十二、整改建议（remedy）</b>——企业视角。三阶段含时间维度: 自查阶段(收到稽查通知前·主动补报可减轻处罚)→应对阶段(稽查进行中·含话术策略:如何配合/提供哪些材料/如何说明)→制度阶段(稽查结束后长期建设·内控制度/发票管理/合同规范)。与稽查处理分工: suggestion=稽查局视角(处罚)，remedy=企业视角(合规)。引擎执行：整改建议，进入报告企业视角段落，与suggestion分视角呈现。</p>'
