...
function renderTaxRiskRules(container) {
  ...
  h += '<details id="rr-standard" style="margin-bottom:10px;background:transparent;border:none;border-top:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0;padding:10px 0;font-size:10px;line-height:20px;color:#334155"><summary style="font-weight:700;color:#16233a;cursor:pointer;font-size:10px">📐 精写编制标准（23字段完整版 · v6六类攻击角度）</summary>'
    ...
    + '<p><b>二、穿透追问（drill_questions）</b>——穷举至稽查终点。三组递进（事实→证据→逻辑），每组穷举至无新有效追问。必须包含至少2条物理常识类追问——用不可辩驳的物理事实堵死辩解空间（如\"仓库能不能装下\"\"仓储费为什么没增加\"）。格式: Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。追问末尾必须包含穷举完成判定——\"事实六要素全覆盖/证据四流全追问/逻辑所有解释全排除→三问全答是→穷举完成\"。标杆: #1716(14问含5条物理追问)、#1717(15问含仓库容积+仓储费双验证)。</p>'
    ...
    + '<p style="margin-top:20px;padding-top:10px;border-top:2px solid #e2e8f0"><b>📊 2条深度精写规则全景索引</b></p>'
    ...
    + '<p>D类(2条)：#1718资金闭环回流(14问三组：关系穿透→资金追踪→交易实质)</p>'
    + '<p>E类(2条)：#1719社保人数缺口(14问三组：基数确认→缺口归因→后果算账)</p>'
    + '<p>F类(2条)：#1720月末集中开票(14问三组：时间确认→原因追问→后果算账)</p>'
