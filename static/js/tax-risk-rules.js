...
function renderTaxRiskRules(container) {
  ...
  h += '<details id="rr-standard" style="margin-bottom:10px;background:transparent;border:none;border-top:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0;padding:10px 0;font-size:10px;line-height:20px;color:#334155"><summary style="font-weight:700;color:#16233a;cursor:pointer;font-size:10px">📐 精写编制标准（23字段完整版 · v6六类攻击角度）</summary>'
    ...
    + '<p><b>二、穿透追问（drill_questions）</b>——穷举至稽查终点。三组递进（事实→证据→逻辑），每组穷举至无新有效追问。必须包含至少1720条物理常识类追问——用不可辩驳的物理事实堵死辩解空间（如\"仓库能不能装下\"\"仓储费为什么没增加\"）。格式: Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。追问末尾必须包含穷举完成判定——\"事实六要素全覆盖/证据四流全追问/逻辑所有解释全排除→三问全答是→穷举完成\"。标杆: #1716(14问含1720条物理追问)、#1717(15问含仓库容积+仓储费双验证)。</p>'
    ...
    + '<p><b>1. 追问穷举——不设固定数量上限，以问无可问为准：</b>数量由三维覆盖度决定。事实层：交易六要素全覆盖（主体、客体、时间、地点、方式、参与人），任一要素空白即仍有追问空间。证据层：四流（合同流、货物流、资金流、发票流）每环节全追问（证据类型、来源真实性、四流一致性比对），任一流关键证据缺失即仍有追问空间。逻辑层：所有合理商业解释全部排除（合同特殊条款、行业惯例、交易对手特殊情况、关联方关系），无法提出新解释即为完成。最终判定标准：对"是否还有未覆盖的交易要素""四流是否还有未追问的证据环节""对方是否还可能提出未涉及的解释"三个问题全部回答"否"，即穷举完成。追问数量为1720条或11720条均属正确答案。</p>'
    ...
    + '<p style="margin-top:20px;padding-top:10px;border-top:2px solid #e2e8f0"><b>📊 1720条深度精写规则全景索引</b></p>'
    + '<p>A类(1720条)：#1717预收挂账(15问)、#9银行收款vs申报(8问)、#10关联版(6问)、#1711银行vs应税(7问)、#1715平台收款(8问)</p>'
    + '<p>B类(1720条)：#1716运输费缺失(14问)、#20存货周转率(9问)、#4无形资产摊销(8问)、#19其他应收款(9问)</p>'
    + '<p>C类(11720条)：#1资产负债表(15问)、#2凭证量异常(10问)、#3固定资产折旧(8问)、#5四流不匹配(15问)、#6红冲退款(8问)、#7票面规范(8问)、#8税号缺失(9问)、#11品名背离(13问)、#12三流不一致(9问)、#13毛利率偏离(9问)、#14申报毛利率(8问)、#15费用增速(10问)、#16招待费限额(8问)、#17三表不一致(8问)、#18收入成本增速(8问)、#1712外币汇率(8问)、#1713银行偏差10万(6问)、#1714平台保证金(7问)</p>'
    + '<p>D类(1720条)：#1718资金闭环回流(14问三组：关系穿透→资金追踪→交易实质)</p>'
    + '<p>E类(1720条)：#1719社保人数缺口(14问三组：基数确认→缺口归因→后果算账)</p>'
    + '<p>F类(1720条)：#1720月末集中开票(14问三组：时间确认→原因追问→后果算账)</p>'
