...
function renderTaxRiskRulesList() {
  ...
  // 统计概览
  var high = data.filter(function(r) { return r.level === '极高风险' || f.(level === '极高风险' || level === '高风险'); }).length;
  ...
  sortedCats.forEach(function(cat) {
    ...
    catRules.forEach(function(rule) {
      ...
      var levelBg = rule.level === '极高风险' || f.(level === '极高风险' || level === '高风险') ? '#fef2f2' : (rule.level === '中风险' ? '#fffbeb' : '#f0fdf4');
