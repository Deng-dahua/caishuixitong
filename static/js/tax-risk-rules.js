...
function renderTaxRiskRules(container) {
  ...
  h += '<details id="rr-standard" style="margin-bottom:10px;background:#fafbfc;border:1px solid #eef2f6;border-radius:8px;padding:10px;font-size:10px;line-height:20px;color:#334155" open><summary style="font-weight:700;color:#16233a;cursor:pointer;font-size:10px">📐 精写编制标准（23字段完整版 · v3穷举至稽查终点）</summary>'
    ...
    + '<p><b>铁律4·角色分明</b>——稽查重点=策略层（舞弊手法预判）；穿透追问=执行层（讯问问题）。现象描述=现象层；风险表格=影响层。稽查处理=稽查局视角；整改建议=企业视角。</p>'
    + '<p><b>铁律5·证据可校验</b>——推理链每层标注依赖证据类型；证据清单标注优先级（必须/应当/可以）；触发指标含行业差异阈值。</p>'
    + '<p><b>一、基础字段（9项·每条必填）</b></p>'
    + '<p><b>① id（异常编号）</b>——{类型前缀}-{三位序号}，类型: AN=隐匿收入/VC=虚列成本/VI=虚开发票/ST=少缴税款/OT=其他。示例: AN-001。</p>'
    + '<p><b>② item（异常名称）</b>——受控词表统一命名，同义不同名禁止。示例: 预收账款长期挂账不转收入。</p>'
    + '<p><b>③ category（所属类别）</b>——五类之一: 隐匿收入/虚列成本/虚开发票/少缴税款/其他违规。示例: 隐匿收入。</p>'
    + '<p><b>④ level（风险等级）</b>——极高/高/中/低/良好。与合规度对应: 极高<40分/高40-60/中60-80/低80-90/良好>90。示例: 高。</p>'
    + '<p><b>⑤ score（风险评分）</b>——1-10分。锚点: 10=系统性造假/金额巨大/主观故意; 5=中等风险/需补充证据; 1=低风险/小额/偶发。示例: 8。</p>'
    + '<p><b>⑥ check_frequency（稽查频率）</b>——高频=每户必查; 中频=行业匹配时查; 低频=特定条件触发时查。示例: 高频。</p>'
    + '<p><b>⑦ policy_ref（法律依据）</b>——引用现行有效的法律法规，统一冠以“现行有效的《XX法》(版本)第X条“，保留条号保证稽查精确性。法律不写死、不当永久真理：每条引用须过法律时效性核查程序（engine/law_validity_checker，已接入审计）验证，末尾强制附核验日期。法律变动时核查程序自动识别废止法规并替换为现行法，重跑即可自动更新，无需人工维护废止对照表。示例: 现行有效的《增值税法》(2026-01-01施行)第十七条（法规现行性核验：2026-07-13）。</p>'
    + '<p><b>⑧ tax_impact（税务影响）</b>——分最低影响和典型影响: 税种(最低补税X万，典型补税Y万)。示例: 增值税(最低5万，典型15万)+企业所得税(最低12.5万，典型37.5万)。</p>'
    + '<p><b>⑨ applicable_condition（适用条件）</b>——五维度结构化: 行业限制+纳税人资质+规模门槛+时间条件+金额门槛。非全部必填。示例: 行业=不限; 资质=一般纳税人; 时间=账款账龄>365天; 金额=单笔>10万。</p>'
    + '<p><b>二、来源标记（2项）</b></p>'
    + '<p><b>⑩ source（来源）</b>——空=人工精写 / 系统发现 / LLM生成。示例: 系统发现。</p>'
    + '<p><b>⑪ auto_type（自动发现类型）</b>——行业基准校准/购销倒挂/毛利为负/缺失数据/综合异常/未知模式。</p>'
    + '<p><b>三、深度字段（12项·精写核心）</b></p>'
    + '<p><b>一、推理链（direction）</b>——推理至稽查终点：证实违法或排除违法。层与层之间因果递进、环环相扣。每层格式: 【推理第N层: XX法则】依赖证据: XX → 结论: XX。三种结束条件: ①定性落地（最后一层已明确指向证实违法或排除违法）；②证据尽头（下一层需要的证据无法获取，标注断点）；③逻辑闭环（最后一层回到第一层前提）。</p>'
    + '<p><b>二、穿透追问（drill_questions）</b>——穷举至稽查终点：问题间环环相扣、因果递进，每个答案自然引出下一个问题。三组递进方向(事实→证据→逻辑)为追问框架。有效追问三标准: ①答案必然引出下一个问题（非孤立提问）；②答案可验证（指向具体证据）；③答案推动稽查进程（要么加深嫌疑，要么排除嫌疑）。追问在证实违法或排除违法时停止。格式: Q{N}:{问题}→潜台词:{稽查真实意图}。A:{应对话术}。</p>'
    + '<p><b>三、现象描述（phenomena）</b>——典型表现枚举(非穷举)，每种格式: 表现描述+典型行业/场景。至少5种典型表现。兜底条款: 其他同类或类似表现。增加排除条件: 什么情况下的类似表现不属于本异常。</p>'
    + '<p><b>四、稽查重点（focus）</b>——策略层：舞弊手法预判，不直接用于提问。格式: 舞弊手法名称: 具体操作方式 → 识别要点。用①②③④逐条标注。与穿透追问分工: focus=策略层(预判)，drill_questions=执行层(将预判转化为讯问问题)。</p>'
    + '<p><b>五、正常业务解释（normal_reason）</b>——至少4种情形，格式 {情形}——需提供{具体证据(文件类型)}。禁用“提供相关证明“须具体到文件。标记“最常见解释“作为红队攻击一优先素材。确无合法情形时如实写0-3种并注明原因，不允许编造。</p>'
    + '<p><b>六、定性路径（determination）</b>——三路径必须对应证据链三档定级: ①无法证明→线索等级(单源数据触发)→定性“存疑，建议补充调查“+不入正式结论；②部分证明→强证据(2独立来源验证)→定性“涉嫌XX“+可入正式结论但标注“证据部分闭环“；③完整证明→铁证(≥3独立来源闭环)→定性“认定XX“+入正式结论可作处罚依据。每路径定义进入条件。设置了量化阈值(㉑)的规则，必须增加“阈值以下处理“分支——门槛未达到时以合规提示形式记录，不入稽查程序、不影响合规度评分。结尾附应对总原则。</p>'
    + '<p><b>七、风险表格（risk_table）</b>——覆盖实际涉及税种/维度，不设数量下限。跨税种影响≥2个的逐税种列明，仅涉及单一税种的不强制跨税种。每行格式: 税种:具体风险描述 | 影响程度:核心/次要/间接。</p>'
    + '<p><b>八、证据清单（evidence）</b>——四层框架: 货物流+合同资金流+业务合理性+排雷。交易金额分级: 大额(>10万)分AB场景(自提vs直运); 中额(1-10万)至少取2个维度证据; 小额(<1万)抽样核验。每层证据标注优先级: 必须获取/应当获取/可以获取。排雷层与正常业务解释联动——列出合法情形，排雷层验证这些情形的证据是否真实存在。</p>'
    + '<p><b>九、触发指标（threshold）</b>——必须有量化阈值+前置条件(行业+资质+数据+时间四维度)。阈值增加行业差异: 通用阈值+行业调整。二元异常写“=是即触发“。预警等级: 黄/橙/红。前置条件四个维度各自独立，全满足才触发。</p>'
    + '<p><b>十、稽查动作（action）</b>——从纸面比对到现场核查穷举全部核查步骤，下限至少3步含1项现场核查、上限穷举完毕。每步格式: 动作类型(现场核查/纸面比对/外调走访/联网核查)+具体操作+预期产出。确实不存在现场核查可能性的(如纯申报类异常)，注明原因后补充替代性核查手段(交叉比对第三方数据/函调/联网核查)，不强制虚构现场核查。</p>'
    + '<p><b>十一、稽查处理（suggestion）</b>——稽查局视角。固定格式: 定性→补税(分税种+金额)→滞纳金(日万分之五+起止日期)→罚款(征管法条款+区间)→移送标准(金额/情节门槛)。与定性路径结论一致。示例: 定性:涉嫌隐匿收入→补税:增值税13万+企业所得税32.5万→滞纳金:自税款所属期至缴清日→罚款:征管法§63税款50%-5倍→移送:金额>10万且主观故意。</p>'
    + '<p><b>十二、整改建议（remedy）</b>——企业视角。三阶段含时间维度: 自查阶段(收到稽查通知前·主动补报可减轻处罚)→应对阶段(稽查进行中·含话术策略:如何配合/提供哪些材料/如何说明)→制度阶段(稽查结束后长期建设·内控制度/发票管理/合同规范)。与稽查处理分工: suggestion=稽查局视角(处罚)，remedy=企业视角(合规)。</p>'
    + '<p><b>四、穷举完成判定标准（数量由业务复杂度决定，不是模板规定）</b></p>'
    + '<p><b>核心原则：</b>决定数量的不是模板，是业务本身的复杂程度。复杂疑点追问可能二十条、推理可能五层；简单疑点追问四五条、推理两层就到底。数量是写完之后的自然结果，不是写之前的硬性规定——不凑数、不强编、一病一方。</p>'
    + '<p><b>1. 追问穷举——不是13条，是问无可问：</b>数量由三维覆盖度决定。事实层：交易六要素全覆盖（谁/什么/什么时候/在哪里/怎么做的/谁参与的），任一空白即还有追问空间。证据层：四流（合同流/货物流/资金流/发票流）每环节全追问（证据类型/来源真实性/四流一致性比对），任一流关键证据缺失即还有追问空间。逻辑层：所有合理商业解释全排除（合同特殊条款/行业惯例/买方特殊需求/关联方关系），想不出新解释即完成。最终判定：对“还有未覆盖的交易要素吗/四流还有未追问的证据环节吗/对方还可能提哪些没问到的解释“三问全答“否“→穷举完成。13条还是3条都是正确答案。</p>'
    + '<p><b>2. 推理链层数——不是4层，是推无可推：</b>由因果链条自然长度决定。结束条件满足任一即停：①定性落地（指向偷税/少缴/虚开/不违规，无法再问“然后呢“）；②证据尽头（下一层证据无法获取，标注“证据断点“）；③逻辑闭环（回到第一层前提）。复杂异常自然4-5层、中等3-4层、简单2-3层，再加一层就是注水。</p>'
    + '<p><b>3. 正常业务解释——不是5种，是穷举完毕：</b>“异常“本身意味着不符合正常商业逻辑，合法解释太多它就不叫异常了。五个自问全答“否“即完成（合同条款/行业惯例/交易对手特殊情况/税收政策特殊规定/不可抗力）。仅0-3种时注明“已穷举全部合法情形“；5种以上需自问该异常是否真构成异常。</p>'
    + '<p><b>4. 风险表格——不是6税种，是覆盖实际涉及：</b>影响几个税种写几个，不设下限。跨税种≥2个逐税种列明并区分核心/次要/间接；仅单一税种注明“仅影响XX税不涉及其他“，不强制跨税种注水。</p>'
    + '<p><b>字段数量决定表：</b>⑫推理链——因果链条自然长度，下限2层(简单如实写)，无上限(推导到定性落地)。⑬穿透追问——交易要素+四流+解释覆盖度，不设下限(穷举完就停)，问无可问为止。⑯正常业务解释——实际合法情形数量，下限0(确实无就如实注明)，穷举完毕为止。⑱风险表格——实际涉及税种数，下限1(单税种就写单)，全部涉及就全部列。⑲证据清单——四流各环节证据数，每层至少1项必须获取，穷举完毕为止。㉑稽查动作——纸面到现场步骤数，至少3步含1项现场核查，穷举完毕为止。</p>'
    + '<p><b>📌 品质标杆（canonical_example: #1813 预收账款长期挂账）</b>——穿透追问: 13条，三组递进（事实Q1-Q5/证据Q6-Q9/逻辑Q10-Q13），每组穷举至无新有效追问。推理链: 4层，自然结果非硬性目标（账龄异常→纳税义务→未申报固定→隐匿定性）。正常解释: 4种已穷举，注明穷举原因。风险表格: 5税种含核心/次要/间接标注。证据清单: 四层框架完整，AB场景+金额分级+优先级，排雷与⑯联动。阈值: 四行业差异调整。</p>'
    + '<p>追问穷举至稽查终点（证实违法或排除违法），问题间环环相扣、因果递进。数量是因果链条的自然长度，不是硬性指标。不凑数、不强编、一病一方。</p>'
    + '</div>'
    + '</details>';
...
window._rrFilter = function() {
  ...
  (Array.isArray(rl.risk_table) ? rl.risk_table.map(function(rr){
    ...
    return tax + ':' + desc;
          ...
          // 传统字段（兼容未升级的规则）
          card += (rl.action ? '<div style="font-size:10px;color:#3a4048;margin:2px 0 4px;white-space:pre-wrap">🔍 核查动作：' + escHtml(rl.action) + '</div>' : '')
...
function renderTaxRiskRulesList() {
  ...
  sortedData.forEach(function(rule, _ri) {
      ...
      html += '<div data-rule-id="' + rid + '" data-level="' + (levelName || '') + '" data-triggered="' + (isTriggered ? '1' : '0') + '" data-category="' + (rule.category || '') + '" data-type="' + (isAutoRule ? 'auto' : 'manual') + '"'
        ...
        + (isAutoRule ? '<span style="margin-left:6px;font-size:10px;font-weight:400;color:#64748b">[' + escHtml(rule.industry || '') + ']</span>' : '')
        + (isTriggered ? '<span style="margin-left:8px;font-size:10px;padding:2px 8px;border-radius:4px;background:#fef2f2;color:#dc2626;font-weight:600">✅ 本次触发(' + triggered.length + ')</span>' : '')
        + '</div>'
        + '<div style="display:flex;gap:8px;align-items:center;flex-shrink:0;margin-left:16px">'
        + (isAutoRule 
            ? '<span style="font-size:10px;padding:2px 8px;border-radius:4px;background:#eff6ff;color:#2563eb;font-weight:600">🤖 自动发现</span>'
            : '<span style="font-size:10px;padding:2px 8px;border-radius:4px;background:' + color + '15;color:' + color + ';font-weight:600">' + icon + ' ' + (levelName || '') + '</span>')
        ...
        + (isAutoRule 
            ? '<span style="font-size:10px;color:#94a3b8">置信度 ' + (rule.confidence !== undefined ? Math.round(rule.confidence * 100) + '%' : '-') + '</span>'
            ...
            : '<span style="font-size:10px;color:#94a3b8">评分 ' + scoreVal + '</span>')
        ...
        // 触发溯源
        + (isTriggered ? '<div style="margin-bottom:6px;padding:8px 12px;background:#fef2f2;border-radius:4px;font-size:10px;line-height:20px">'
        ...
        // 详细内容 —— 自动发现规则也展示7段式字段
        + (rule.phenomena ? '<div style="font-size:10px;color:#475569;line-height:20px;margin-bottom:4px"><b>现象：</b>' + escHtml(rule.phenomena) + '</div>' : '')
        + (rule.direction ? '<div style="font-size:10px;color:#475569;line-height:20px;margin-bottom:4px"><b>逻辑：</b>' + escHtml(rule.direction) + '</div>' : '')
        + (rule.focus && rule.focus !== '待明确重点' ? '<div style="font-size:10px;color:#dc2626;line-height:20px;margin-bottom:4px"><b>重点：</b>' + escHtml(rule.focus) + '</div>' : '')
        + (rule.drill_questions ? '<div style="font-size:10px;color:#475569;line-height:20px;margin-bottom:4px"><b>追问：</b>' + escHtml(rule.drill_questions.replace(/\n/g,'<br>')) + '</div>' : '')
        + (detailText ? '<div style="font-size:10px;color:#475569;line-height:20px;margin-bottom:6px">' + escHtml(detailText) + '</div>' : '')
        + (rule.normal_reason && rule.normal_reason.length > 20 ? '<div style="font-size:10px;color:#059669;line-height:20px;margin-bottom:4px"><b>正常解释：</b>' + escHtml(rule.normal_reason) + '</div>' : '')
        + (rule.risk_table ? '<div style="font-size:10px;color:#dc2626;line-height:20px;margin-bottom:4px"><b>风险：</b>' + escHtml(rule.risk_table).replace(/\n/g,'<br>') + '</div>' : '')

        // 建议 + 佐证
        + (suggestText ? '<div style="font-size:10px;color:#334155;line-height:20px;margin-bottom:4px"><span style="font-weight:600;color:#0f172a">' + (isAutoRule ? '系统建议：' : '税务合规建议：') + '</span>' + escHtml(suggestText) + '</div>' : '')
        + (evidenceText ? '<div style="font-size:10px;color:#334155;line-height:20px;margin-bottom:4px"><span style="font-weight:600;color:#0f172a">' + (isAutoRule ? '发现依据：' : '所需佐证：') + '</span>' + escHtml(evidenceText) + '</div>' : '')

        // 自动发现额外信息
        + (isAutoRule ? '<div style="font-size:10px;color:#334155;line-height:20px;margin-bottom:4px"><span style="font-weight:600;color:#0f172a">信号出现率：</span>' + escHtml(rule.prevalence || '') + '</div>' : '')
        + (isAutoRule && rule.auto_discovered_at ? '<div style="font-size:10px;color:#334155;line-height:20px;margin-bottom:4px"><span style="font-weight:600;color:#0f172a">自动发现时间：</span>' + escHtml(rule.auto_discovered_at.substring(0, 19)) + '</div>' : '')
