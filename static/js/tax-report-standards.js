...
function renderReportStandards(container) {
  ...
  var h = '<style>'
    + '.rpt-layout{max-width:1140px;margin:0 auto;padding:40px 46px;background:#fff;color:#3a4048;font-size:10px;line-height:2.0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"PingFang SC","Microsoft YaHei",sans-serif}'
    + '.rpt-h1{font-size:10px;font-weight:800;color:#16233a;margin:0 0 12px;letter-spacing:-.02em;line-height:1.3}'
    + '.rpt-sub{display:inline-block;font-size:10px;color:#9a1f2b;border:1px solid #f4c2c7;background:#fef8f8;border-radius:20px;padding:4px 14px;margin:0 0 30px}'
    + '.rpt-chapter{margin:38px 0 22px;padding:0 0 10px;border-bottom:2px solid #16233a}'
    + '.rpt-ch-title{font-size:10px;font-weight:800;color:#16233a;margin:0 0 10px;line-height:1.35;display:flex;align-items:center;gap:10px}'
    + '.rpt-ch-title .cn{display:inline-flex;align-items:center;justify-content:center;min-width:28px;height:28px;background:#16233a;color:#fff;border-radius:6px;font-size:10px;font-weight:800}'
    + '.rpt-block{background:#fafbfc;border:1px solid #eff2f6;border-radius:8px;padding:13px 15px;margin:4px 0 14px;font-size:10px;line-height:2.0;color:#3a4048}'
    + '.rpt-block h4{font-size:10px;font-weight:700;color:#16233a;margin:18px 0 8px}'
    + '.rpt-block h4:first-child{margin-top:0}'
    + '.rpt-block strong{color:#1f2d3d;font-weight:600}'
    + '.rpt-em{font-style:normal;color:#9a1f2b;font-weight:600}'
    + '.rpt-bad{display:inline-block;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;margin-right:4px;vertical-align:2px;white-space:nowrap}'
    ...
    + '.rpt-flow{display:flex;flex-wrap:nowrap;gap:6px;align-items:center;margin:12px 0;font-size:10px;overflow-x:auto}'
    + '.rpt-flow span{display:inline-block;padding:6px 14px;background:#f1f5f9;border-radius:6px;font-weight:600;color:#475569;white-space:nowrap}'
    + '.rpt-flow span.act{background:#16233a;color:#fff;white-space:nowrap}'
    + '.rpt-flow .arr{color:#94a3b8;font-size:10px;margin:0 2px}'
    + '.rpt-table{width:100%;border-collapse:collapse;font-size:10px;margin:10px 0}'
    + '.rpt-table td{padding:8px 12px;border-bottom:1px solid #eff2f6;vertical-align:top;font-size:10px;color:#3a4048;line-height:1.85}'
    + '.rpt-table td:first-child{width:30px;white-space:nowrap;text-align:center}'
    + '.rpt-table td:nth-child(2),.rpt-table td:nth-child(3){white-space:nowrap;width:1%}'
    + '.rpt-table th{padding:8px 12px;background:#fafbfc;text-align:left;font-weight:600;color:#64748b;font-size:10px;border-bottom:2px solid #16233a;white-space:nowrap}'
    + '.rpt-rule{display:flex;gap:10px;align-items:flex-start;margin:6px 0;padding:10px 14px;background:#fafbfc;border-radius:6px;border-left:3px solid #e2e8f0;font-size:10px;line-height:1.95;color:#3a4048}'
    + '.rpt-rule .rn{font-size:10px;font-weight:800;color:#16233a;min-width:20px}'
    + '.rpt-rule .rc{font-size:10px;color:#3a4048;line-height:1.95}'
    + '.rpt-rule.fatal{border-left-color:#9a1f2b;background:#fef8f8}'
    + '.rpt-rule.high{border-left-color:#d97706;background:#fffbeb}'
    + '</style>';
  ...
  h += '<span class="rpt-em">三性校验不通过的分类处理（区分性质和严重程度）：</span><br>';
  h += '① <b>真实性不通过</b> → <span class="rpt-bad rpt-bad-r">直接排除</span>，不入任何结论。真实性有问题多半是数据源出错，不可修复。<br>';
  h += '② <b>关联性标注为"间接"或"参考"</b> → 可保留但需注明关联强度，<span class="rpt-bad rpt-bad-g">不降级</span>。间接证据仍有佐证价值。<br>';
  h += '③ <b>合法性不通过</b> → <span class="rpt-bad rpt-bad-r">排除</span>，并记录违规取证路径作为自省层的系统检查项。程序瑕疵不可容忍。';
  ...
  h += '<span class="rpt-em">反证排除声明（强制附加，含行业动态补充）</span> — 在总体结论后，必须附加反证排除声明。基础三段式（适用于所有行业）：<br>';
  h += '"本结论排除了以下合理可能：①企业能提供合理解释证明交易真实性；②第三方证据（如运输单据、对方确认函）可证实货物/服务已交付；③金额差异由计算口径或时间性差异导致且企业已提供说明。"<br>';
  h += '<span class="rpt-em">行业动态补充排除项：</span>根据企业行业特征和分析实际发现，自动追加行业特化排除项（不是模板套话，是针对本案特点的实质排除）：<br>';
  h += '\u2463 <b>贸易企业</b> → "货物已由供应商直发客户，被查单位未实际经手仓储，交易真实性以物流凭证为准。"<br>';
  h += '\u2463 <b>服务企业</b> → "服务成果已交付但未签署书面验收确认，以双方往来邮件或对公收款记录为佐证。"<br>';
  h += '\u2463 <b>制造企业</b> → "进销品名差异由生产工艺或BOM替代料导致，以生产领料记录和委外加工合同为佐证。"<br>';
  h += '\u2463 <b>建筑企业</b> → "工程进度与收入确认差异由甲方监理报告确认的完工百分比所致，以监理签章进度表为佐证。"<br>';
  ...
  h += '<span class="rpt-em">退修类型标签（每条退修记录至少打一个）：</span><br>';
  h += '<span class="rpt-bad rpt-bad-r">证据不足</span> <span class="rpt-bad rpt-bad-y">程序违法</span> <span class="rpt-bad rpt-bad-r">定性错误</span> <span class="rpt-bad rpt-bad-y">法律适用不当</span> <span class="rpt-bad rpt-bad-b">表述不合规</span> <span class="rpt-bad rpt-bad-g">计算错误</span><br>';
  h += '学习引擎按类型聚类消费退修数据——是定性能力弱（定性错误标签占比高）还是表述不规范（表述不合规标签占比高）？聚类统计趋势揭示引擎的薄弱环节，自动反馈到自省层触发定向能力增强。<br>';  
