...
function renderReportStandards(container) {
  ...
  var S12 = [
    ...
    { id:7, name:'方法在前→过程在后', severity:'低',
      requirement:'detail/how_found应先声明稽查方法（如"采用银行流水与发票逐票核对法"），再展示具体发现结果。',
    ...
    { id:8, name:'反模板句', severity:'高',
      requirement:'禁止出现："是税务稽查重点方向""需逐笔核实""请核实并提供相关佐证材料""申报不合规是税务行政处罚的常见案由"等。',
    ...
    { id:12, name:'法律条款号', severity:'高',
      ...
      example:'《增值税暂行条例》第九条 / 《发票管理办法》第二十二条' },
  ];
  ...
  h += '<a href="#rs-narrative">4 稽查叙事规范</a>';
  ...
  h += '<p style="font-size:13px;color:#94a3b8;margin:0 0 28px">根据《税务稽查工作规程》及稽查实务标准编制。系统内置12项硬性质量检查指标，质量保障管线自动执行，跨域协商和审核反馈的结果按规范展示。</p>';
  ...
  h += '<div class="rs-card">';
  h += '<p style="font-size:13px;color:#475569;line-height:2;margin:0 0 16px">报告生成后按四阶段流水线自动执行质量保障。12项标准为<strong>检测+标记</strong>模式（非强制阻断）——不通过的发现不会被删除，仅在正文底部标注⚠质量标注供审理环节参考。文本净化和建议增强在标准检查之前运行，能修复大部分常见问题，所以最终报告中真正出现质量标注的情况很少。</p>';
  ...
  h += '</div>';
  h += '<div class="rs-detail"><b>①文本净化：</b>自动清除模板句（如"是税务稽查重点方向"）、空描述（type字段或detail为空）、重复句（同一发现内连续出现相同内容的句子）、空占位符（如"()""如：()"等自动填充失效残留）。<br><b>②12项质量标准检查：</b>逐条对照12项标准检测每条发现的合规性。每项标准含：检查方法+正确范例+错误范例。不通过的在发现底部附加⚠标记。<br><b>③建议质量增强：</b>对suggestion字段执行11条增强规则——补充具体操作路径、添加法律依据引用、优化紧迫感表述。确保每条建议含：查什么→怎么查→正常怎么办→异常怎么办。<br><b>④文本二次净化：</b>再次执行文本净化，清除建议增强过程中可能产生的新模板句或格式残留。确保报告最终交付前内容纯净。</div>';
  h += '</div></div>';

  // ═══ 2. 报告7章结构 ═══
  h += '<div id="rs-structure" class="rs-sec"><div class="rs-sec-title"><span class="n">2</span>报告7章结构</div>';
  h += '<div class="rs-detail">正式税务稽查报告须含封面+7章正文+附件清单，严格遵循《税务稽查工作规程》第42条规定的10项内容。封面的编号格式、日期精确到日；第一章至第七章按以下要求逐章编写；附件中发票明细表按11列标准全量展示。</div>';
  var chaps = [
    ...
    {label:'第一章',title:'案件来源及稽查对象基本情况',body:'必须包含8项基本信息，以表格形式呈现：①案件来源——交代稽查启动原因（系统分析推送/举报/转办/协查）；②被查单位——企业全称须与工商登记完全一致；③统一社会信用代码——18位，精确到每一位；④法定代表人——姓名+必要信息；⑤企业类型——有限责任公司/股份有限公司/个体工商户等；⑥行业分类——须呈现三层穿透结论（工商登记X/发票推断Y/实质经营Z→综合判断）；⑦稽查期间——起止年月精确到月份；⑧稽查范围——本次检查的税种+资料范围+分析维度。行业分类须注明推断依据和联网核查数据来源。'},
    {label:'第二章',title:'稽查实施情况',body:'必须包含7个执行段落，每段200-400字，整体篇幅2000字以上。以稽查过程叙事形式展开：①资料审阅与类型识别——四方交叉验证过程详述，文件识别结果表格；②公司身份锚定与发票方向判定——逐行扫描比对逻辑和三类判定规则；③行业判定与服务闸门验证——金税编码统计/三层验证机制/跳过域清单；④资金流与发票流双向核对——收款端+付款端双向详述；⑤穿透分析与知识图谱——四项穿透（供应商/客户/人员/关联方）的执行方法和检测结果；⑥行业对标——适用指标和对比结果，服务行业声明跳过原因；⑦综合分析与结论形成——全链路分析管线的执行顺序。所有数据从report对象实时提取，使用客观第三人称叙事。'},
    {label:'第三章',title:'稽查发现问题及事实认定',body:'每条发现按六要素格式独立呈现，高风险优先排列：①稽查性质——发现类型标题；②稽查事实——具体描述+明细数据（须含供应商名称/金额/发票号/品名/数量/日期，禁止泛泛地说"存在XX问题"）；③证据材料——逐笔列示的证据明细表（items数组，不截断不缺斤短两）；④证据来源——规则ID（可点击溯源）+线索链编号+查证方式；⑤法律依据——完整法条名称+具体条款号，不得笼统引用；⑥处理建议——具体消除路径（"提供XX资料→如果A则做XX→无法做到的后果是XX"格式）。已审核的发现展示绿色审核横幅，不影响原始等级。跨域协商结果以彩色横幅（⛔消解/🔄调整/ℹ️标记）展示在发现标题下方。'},
    {label:'第四章',title:'稽查结论',body:'包含5个结论段落：①推理引擎综合结论卡片——综合评分+风险等级+完整结论叙述；②风险分布表——四级风险等级（极高/高/中/低）各列出数量/占总发现百分比/代表性事项举例，以表格形式呈现；③证据链完整性——跨36域分析的覆盖范围、多源数据交叉验证构成的核心证据闭环、每条发现的证据追溯能力（规则ID→线索链→证据链→原始数据行）；④稽查局限性声明——如实列出因资料缺失无法进一步确认的事项，缺什么资料就报什么局限性；⑤定调性总体结论——按风险等级自适应表述：高风险→"建议启动立案程序"；中风险→"建议限期自查整改"；低风险→"建议持续规范完善"。结论应定调性而非定论性。'},
    ...
    {label:'第七章',title:'稽查人员签字',body:'必须包含：①稽查执行人亲笔签名+执法证件号——不得代签/打印；②审理人亲笔签名+执法证件号——审理岗位人员签名；③稽查部门盖章——税务机关公章；④报告日期——系统时间自动获取，精确到日；⑤存档说明——"本报告一式三份：稽查部门留存一份，被查单位一份，报送上一级税务机关备案一份"。系统自动预留签名栏位及执法证件号栏位，正式税务文书需人工手签和盖章。'},
    {label:'附件',title:'证据清单',body:'必须包含以下附件：①销项发票全量明细——11列格式（购买方/品名/规格/单位/数量/金额/税额/价税合计/日期/票种/发票号）；②进项发票全量明细——同11列格式（销售方/品名/规格/单位/数量/金额/税额/价税合计/日期/票种/发票号）；③主营业务成本发票明细——核心成本供应商+品名+金额；④重大费用发票明细——费用类供应商+品名+金额；⑤银行流水汇总——收付款总额+收款方TOP5+付款方TOP5；⑥各资料文件清单——文件名+类型+有效记录数；⑦12项质量标准自检结果——通过数/未通过数/各标准问题统计。发票明细表上限200条，超出部分以电子附件形式另行提供，不截断不简化。'},
  ];
  ...
  h += '<div class="rs-card"><div class="rs-card-title">稽查报告用语规范</div>';
  h += '<div class="rs-detail"><b>核心原则：</b>稽查报告处于稽查发现阶段，尚未进入法律裁决程序——报告用语必须体现"发现"而非"定性"的立场。<br><b>正确用语：</b>稽查性质、稽查事实、稽查发现、稽查结论、涉嫌、存疑、提示风险、可能存在、疑似。<br><b>禁止用语：</b>违法性质、违法事实、违法行为、违法认定、确定、认定、已查明、经核实确认。<br><b>原因：</b>违法认定属于税务机关审理裁决后的法律定性。稽查报告阶段的发现尚需被查单位陈述申辩、税务机关审理复核——在报告中预判法律结论属于程序违法，审理环节会退回重写。</div></div>';
  h += '<div class="rs-card"><div class="rs-card-title">报告机密保护规则</div>';
  h += '<div class="rs-detail"><b>禁止出现在正式报告中的系统内部信息：</b><br>①引擎执行流程（阶段数量/模块名称/管线结构）——暴露系统架构<br>②规则/线索链/证据链的数量统计——暴露系统能力参数<br>③全链路闭环状态（规则ID追溯/证据链闭环等打勾清单）——系统内部质量自检<br>④系统自诊与自我修正报告——引擎内部工作日志<br>⑤审核审查面板（审查面板折叠于报告之上，不入报告正文）<br>⑥稽查行为准则/稽查方法论演进文字——系统内部文档<br>⑦内部技术标签（Synthesis:/Causal:/[AGI]/[Phase]等前缀）——这些是引擎内部使用的分类标签<br><b>原则：</b>报告是给被查单位和税务机关看的，只能呈现稽查结论和依据，不能暴露"系统是怎么做到的"内部实现细节。凡是标注"⚠仅供分析参考，不入正式报告"的信息，报告生成时必须移除。</div></div></div>';

  // ═══ 4. 稽查叙事规范 ═══
  h += '<div id="rs-narrative" class="rs-sec"><div class="rs-sec-title"><span class="n">4</span>稽查过程叙事规范</div>';
  h += '<div class="rs-card"><div class="rs-detail">每条发现按六要素格式呈现：稽查性质→稽查事实→证据材料→证据来源→法律依据→处理建议。已审核的发现额外展示绿色横幅，跨域协商的发现展示彩色横幅。语言风格：像老稽查员在书写正式文书——客观第三人称、数据详实、逻辑严密、法律引用准确。叙事方法遵循"方法在前→过程在后→数据支撑→后果推导→建议可执行"的递进结构。</div></div></div>';
  ...
  h += '<div class="rs-card"><div class="rs-detail"><b>功能：</b>全文播报——报告顶部固定播报控制条，一键从封面播至附件结束；点击播报——点击报告任意段落从该处开始播报至报告结束；播放控制——暂停/继续/停止，语速0.85x-1.3x可调；视觉跟随——当前播报段落橙色底纹高亮(#fef3c7)，自动滚动至视野中央。<br><b>音色标准：</b>中文男声(zh-CN male)，低沉严肃的中年稽查员声线。使用浏览器内置SpeechSynthesis API，不依赖外部服务。降级策略：zh-CN male→zh-CN non-Tingting→zh任意。<br><b>6档语调：</b>章节标题0.65音调/0.7x语速庄严缓慢有力；小节标题0.72/0.8x沉稳；高风险内容0.68/0.75x严肃凝重；法律条文0.70/0.72x清晰郑重；处理建议0.80/0.85x清晰有力；普通叙述0.78/0.88x新闻联播标准。</div></div></div>';
  ...
  h += '<div class="rs-card"><div class="rs-detail"><b>审核按钮：</b>每条发现右侧提供"🔍审核"按钮。弹窗中按审核内容模板（判断结论+具体问题+正确逻辑+需要证据+法律依据）填写意见。<br><b>审核后的展示规则：</b>①审核不改变发现的风险等级——审核是提升系统判断精度，不是消除发现；②已审核的发现展示绿色横幅"✅已审核：审核意见内容"；③审核数据存入correction_rules.json，下次分析时四级回退匹配自动应用；④审核记录不入正式报告正文——这是系统自学习数据；⑤审核机制本质：人工判断→系统学习→1次纠正→自动规则→累积进化。<br><b>纠正规则查看：</b>推理引擎仪表盘→智能大脑→纠正规则库。展示指纹/置信度/纠正次数/是否自动应用。</div></div></div>';
