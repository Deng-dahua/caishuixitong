...
function renderReportStandards(container) {
  ...
  var chaps = [
    ...
    {label:'第一章',title:'案件来源及稽查对象基本情况',body:'必须包含8项基本信息，以表格形式呈现：①案件来源——交代稽查启动原因（系统分析推送/举报/转办/协查）；②被查单位——企业全称须与工商登记完全一致；③统一社会信用代码——18位，精确到每一位；④法定代表人——姓名+必要信息；⑤企业类型——有限责任公司/股份有限公司/个体工商户等；⑥行业分类——须呈现三层穿透结论（工商登记X/发票推断Y/实质经营Z→综合判断）；⑦稽查期间——起止年月精确到月份；⑧稽查范围——本次检查的税种+资料范围+分析维度。行业分类须注明推断依据和联网核查数据来源。缺失的工商数据标注"待联网核查补充"而非"未获取"。'},
  ...
  h += '<div class="rs-card"><div class="rs-detail">每条发现必须遵循完整的六要素叙事框架——稽查性质→稽查事实→证据材料→证据来源→法律依据→处理建议。<b>叙事语言风格：</b>客观第三人称——所有how_found/description/tax_impact字段使用"经查""该企业""被查单位"表述，禁止出现第一人称"我""我们"或第二人称"你""你们"。<b>叙事递进结构：</b>方法在前→过程在后→数据支撑→后果推导→建议可执行——不是"发现XX问题建议XX"，而是"通过XX方法→核查了XX数据→发现XX异常→导致XX后果→建议从XX方面处理"。<b>叙事颗粒度：</b>每条发现必须含至少1个具体数值（金额/百分比/数量）和1个时间维度（期间/日期）。纯定性的描述如"存在较大偏差"不生硬——必须写"偏差52%""涉及金额423,605.31元""涉及32笔交易"。"较大""严重""异常多"等定性词必须有紧随其后的具体数值支撑。<br><b>已删除的五段叙事：</b>此前的"发现要点→线索获取→分析过程→证据组织→通俗理解"五段叙事已从报告中移除——其内容与六要素高度重叠，保留造成冗余。当前报告仅保留六要素作为发现的核心呈现格式。<br><b>已审核的发现额外展示绿色横幅"✅已审核：审核意见摘要"，跨域协商的发现按第10节规范展示彩色横幅（⛔消解/🔄调整/ℹ️标记）。</div></div></div>';
  ...
  h += '<b>对报告质量的意义：</b>四触发机制确保无论通过哪个入口启动系统、无论代码和数据如何变化，报告生成前所有模块的数据状态是一致的——用户在任何位置看到的同一个信息永远是同一个值。报告中不会出现"这里说33条方法论，那里说32条"的矛盾。</div></div></div>';
  ...
  h += '<b>阶段二：规则存储（学习记录）：</b>POST /api/feedback→record_correction()→生成指纹→存入correction_rules.json→累加计数→累计1次即升级为自动规则（auto_apply=true）。不同发现类型/不同行业的审核生成独立指纹分别存储。<br>';
  ...
  h += '<b>步骤1——域分析独立运行：</b>36个域分析函数各自独立产出发现。此时各域之间没有通信——域7说"收款vs开票偏差52%→高风险"，域15说"行业判定=服务行业"——两个结论在各自的域内都是正确的，但存在潜在矛盾。<br>';
