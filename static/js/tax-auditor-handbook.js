...
function renderAuditorHandbook(container) {
  ...
  html += '<tr><td class="hb-td-num">三</td><td>稽查发现问题及事实认定</td><td>五段叙事+六要素+证据链。同类风险(同type)自动合并为一条，子项逐条列示，杜绝报告冗长重复</td></tr>';
  ...
  html += '<p class="hb-method-principle">分类完成后系统必须自我反思。三信息综合分析法——同时检查对方户名、摘要、交易附言三个字段，不能只看单一字段。检查规则：①个人款中极小金额(<20元)且有零有整(如9.62元)→常识判断："没人会转9.62元"→银行自动结息；②付款方为空+摘要含银行词→银行费用；③金额为精确整数且对方为企业名→企业客户款无误。发现误分类自动修正并记录。</p>';
  html += '</div>';
