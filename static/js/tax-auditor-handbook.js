...
function renderAuditorHandbook(container) {
  ...
  h += '<div class="hb-card"><strong>🧠 有记忆</strong><p>每次分析自动提取指纹（行业+模式+信号+评分）存入audit_memory.json。上限501条，12维度加权相似度检索——行业(×3)>经营模式(×2)>信号类型(×2)>风险等级(×1.5)。后续分析自动检索相似案例，输出行业对标校准和常见信号预警。</p></div>';
...
// ═══ 第六章 ═══
  ...
  [['一键分析','第21-45条(检查)','_run_analyze自动执行全部分析域+四步核查法+链驱动引擎+协商引擎+方法论语料对账。一次点击=完整模拟税务合规检查环节——从文件上传到报告输出，全部自动化。'],
   ...
   ['报告生成','第51条(报告格式)','自动生成封面+7章+附件：案件来源→基本情况→实施情况→发现问题→税务合规结论→处理建议→告知权利→签字→证据清单。完全符合规程第42条规定的10项内容格式要求。'],
   ['合规门禁','第1条(程序合法)','12项质量标准(模板句清除/重复句合并/空描述删除/人性化表述/六要素完整/法律引用准确/具体数值/因果链/可执行建议/条款号/反跨复制/空占位符清除)+16项自省检查。全通过→绿色交付。'],
  ...
  h += '<tr><td class="lbl">代码层7函数</td><td class="val" style="font-size:12px">save_analysis_memory()保存分析指纹→audit_memory.json(501条)/ query_similar_cases()12维加权检索/ record_correction()审核→指纹→user_corrections.json/ apply_correction_rules()四级回退匹配/ record_user_feedback()用户反馈记录/ _adjust_signal_weights_from_feedback()信号权重调整/ get_adaptive_signal_weights()自适应权重获取</td></tr>';
