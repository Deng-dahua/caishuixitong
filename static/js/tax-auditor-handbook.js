...
function renderAuditorHandbook(container) {
  ...
  ['系统数据概览','税务合规工作流程','14类必查资料','税务合规方法论33条','税务合规判定规则','报告编制规范','关键法律条文','系统与规程映射','全链路质量保障','跨域协商引擎','数据一致性自检','审核反馈闭环','引擎记忆体系','引擎铁律编号','系统文件关联'].forEach(function(t,i){
  ...
  h += '<div class="hb-card"><strong>🧠 有记忆</strong><p>每次分析自动提取指纹（行业+模式+信号+评分）存入audit_memory.json。上限502条，12维度加权相似度检索——行业(×3)>经营模式(×2)>信号类型(×2)>风险等级(×1.5)。后续分析自动检索相似案例，输出行业对标校准和常见信号预警。</p></div>';
  ...
  h += '<div id="hb-s3" class="hb-sec"><div class="hb-sec-title"><span class="num">3</span>税务合规方法论33条</div>';
  h += '<div class="hb-detail">每条方法论均来自审计准则和税务合规实战。方法本身全行业适用——仅数据不同，逻辑通用。33条按处理阶段分为五层：<b>文件识别层</b>（①②）→<b>数据提取层</b>（③④⑤⑥）→<b>分析推理层</b>（⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱⑲⑳㉑㉒㉓㉔）→<b>结论输出层</b>（㉕㉖㉗㉘㉙）→<b>质量保障层</b>（㉚㉛㉜㉝）。<a href="/static/methodology_items.json" target="_blank" style="color:#2563eb;font-size:12px;margin-left:8px">📋 查看JSON源文件</a></div>';
...
// ═══ 第六章 ═══
  ...
  [['一键分析','第21-45条(检查)','_run_analyze自动执行全部分析域+四步核查法+链驱动引擎+协商引擎+方法论语料对账。一次点击=完整模拟税务合规检查环节——从文件上传到报告输出，全部自动化。'],
   ...
   ['分析链','第46条(审理审核)','19条分析链→reasoning_path[]多步推理→从证据→结论的综合判定。模拟审理部门的逐项审核——检查对象的准确性/事实证据的充分性/法律适用的正确性→0-7维异常评分→定案。'],
   ...
   ['风险评分','第49条(审理意见)','综合评分(76/100)→四级风险等级→P0/P1/P2策略→因果叙事链→证据闭环→形成税务合规结论。完全对应审理环节的"审理意见"——对检查结果的综合判断和定性建议。'],
   ['报告生成','第52条(报告格式)','自动生成封面+7章+附件：案件来源→基本情况→实施情况→发现问题→税务合规结论→处理建议→告知权利→签字→证据清单。完全符合规程第42条规定的10项内容格式要求。'],
   ['合规门禁','第2条(程序合法)','12项质量标准(模板句清除/重复句合并/空描述删除/人性化表述/六要素完整/法律引用准确/具体数值/因果链/可执行建议/条款号/反跨复制/空占位符清除)+16项自省检查。全通过→绿色交付。'],
  ...
  [['数据质量保障','文件解析前验证数据有效性/结构完整性/字段合法性。自动过滤空白行/汇总行/无效行。含82+列名映射和四方交叉验证，确保解析正确。文件格式识别失败→降级到数据推断兜底模式。'],
   ...
   ['证据质量保障','≥2数据域交叉触发+≥3条规则支持+≥60%触发率→有效证据链。12条跨域证据链从不同数据源独立收集证据。SHA256哈希存证保证证据不可篡改。证据链完整度在报告中量化展示（X/Y条链形成闭环）。'],
  ...
  h += '<table class="hb-tbl">';
  h += '<tr><td class="lbl">规则篇9章</td><td class="val" style="font-size:12px">行业推断铁律/系统税务合规判定规则33条/缺失的关键信息处理/收款分类规则12条/账务处理引擎铁律6条/引擎核心铁律5条/报告呈现规则/报告后四章规则/审核反馈闭环规则</td></tr>';
  ...
  h += '<tr><td class="lbl">代码层7函数</td><td class="val" style="font-size:12px">save_analysis_memory()保存分析指纹→audit_memory.json(502条)/ query_similar_cases()12维加权检索/ record_correction()审核→指纹→user_corrections.json/ apply_correction_rules()四级回退匹配/ record_user_feedback()用户反馈记录/ _adjust_signal_weights_from_feedback()信号权重调整/ get_adaptive_signal_weights()自适应权重获取</td></tr>';
  ...
  h += '<div class="hb-detail">2026-06-29重新划清边界：引擎铁律（engine/memory.py规则篇）= 系统硬逻辑，不可违反，每条在代码中有对应的实现或检测机制。AI行为准则（前端AI行为准则页面）= 智哥编码规范，约束写代码的行为方式和质量流程。共12条引擎铁律 + 7条AI准则，编号互不重叠，各自独立维护。</div>';
  h += '<table class="hb-tbl">';
  h += '<tr><td class="lbl" colspan="2" style="font-weight:700;color:#dc2626;text-align:center">引擎铁律（engine/memory.py 规则篇 · 12条 · 系统硬逻辑）</td></tr>';
