...
function renderAuditorHandbook(container) {
  ...
  h += '<div class="hb-detail">正式稽查报告须含封面+7章正文+附件清单。报告的每一条结论都可以通过规则ID→线索链ID→证据来源→原始数据行的六步溯源路径反向验证。<br><br><a href="?page=report-standards" style="display:inline-block;padding:10px 20px;background:#2563eb;color:#fff;border-radius:6px;text-decoration:none;font-weight:600">📋 查看 报告编制要求 → 第二节 报告完整结构</a></div>';
  h += '</div>';
...
// ═══ 第六章 ═══
  ...
  [['一键分析','第21-45条(检查)','_run_analyze自动执行全部分析域+四步核查法+链驱动引擎+协商引擎+方法论语料对账。一次点击=完整模拟稽查检查环节——从文件上传到报告输出，全部自动化。'],
   ...
   ['线索链','第22条(取证逻辑)','437条线索链(全部可执行),管理于cross_domain_clues.json。每条含触发关键词+investigation_path调查步骤+rule_refs关联规则ID+风险等级+建议+法条引用。每条线索链=一个稽查员的调查思路——\"从这里开始查，每一步查什么，查到了怎么办\"。'],
