...
async function renderAgiDashboard(container) {
  ...
  try {
    ...
    h += '<p style="font-size:13px;color:#94a3b8;margin:0 0 16px">存勤法税·智能大脑 — 36域分析 · 1514规则 · 1266条链 · 29条协商规则 · ' + pipe.total_events + '条学习事件</p>';
    ...
    h += _agiEngineCard('📐','税务合规方法论','MethodologyEngine','<code>methodology_loader.py</code>','33条税务合规方法论（全部代码化）+14类必查资料+12条核心法律条文。引擎按分析域的domain_key自动匹配适用的方法论——收款分析域→加载方法论②③④，进销存域→加载方法论⑥⑦⑧。根据实际数据特征动态选择组合，非写死checklist。','紫');
    ...
    h += '<div class="hb-info-card hb-info-red"><strong>📋 税务合规规则</strong><p><code>len(tax_risk_rules_local_export.json)</code> → <strong>1717条</strong></p><p>20个分类：发票匹配184+申报合规142+行业专项133+个税125+资产负债121+企业所得107+成本费用106+发票合规104+增值税101+经营实质98等</p></div>';
    h += '<div class="hb-info-card hb-info-purple"><strong>🔗 线索/证据链</strong><p><code>cross_domain_clues.json</code> → <strong>437条</strong>(41可执行+1174方法论)</p><p>可执行链含触发关键词/调查步骤/关联规则ID；方法链含完整税务合规步骤和法规引用</p></div>';
