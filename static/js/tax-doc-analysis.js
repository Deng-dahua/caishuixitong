...
function renderAnalyzeHeader(report) {
  ...
  var steps = [
    ...
    { title: '第四阶段：Phase3 交叉验证——冲突消解与证据闭环', desc: '㉑ 冲突消解引擎（信号互斥检测→自动降级/升级） → ㉒ 规则引擎（1824条逐条匹配） → ㉓ 线索链驱动（437条链驱动发现） → ㉔ 证据链匹配（22条跨域证据闭环） → ㉕ 轻量跨结论串联 → ㉖ 证伪检查（30+规则覆盖） → ㉗ 联网核查（DB缓存→API→搜索引擎三层降级） → ㉘ 经营实质五步核查法 → ㉙ 知识图谱（49实体/5异常关系检测）' },
...
function _renderReportFallback(r, allF) {
  ...
  h += '<h4>' + '最终产出' + '</h4><p class="i2">' + '经上述五环节全链路处理后，形成完整的税务合规结论。本次分析启用了全部核心引擎：文件识别引擎（四方交叉验证）、身份锚定引擎（逐行比对）、规则引擎（1824条）、线索链驱动（396条）、证据链闭环（745条）、因果叙事链、Benford检验、方法论过滤器、合规门禁、知识图谱构建、六员跨企业比对、收款分类自适应纠错。服务行业专属分析域（人均产值/经营费用完整性/工资社保合规）全量执行，进销存实物分析域自动跳过。每条结论均可通过规则ID→线索链ID→证据链ID→原始数据位置进行全链路追溯。' + '</p>';
