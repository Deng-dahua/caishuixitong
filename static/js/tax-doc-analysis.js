...
function renderAnalyzeHeader(report) {
  ...
  var steps = [
    ...
    { title: '第四阶段：Phase3 交叉验证——冲突消解与证据闭环', desc: '㉑ 冲突消解引擎（信号互斥检测→自动降级/升级） → ㉒ 规则引擎（1608条逐条匹配） → ㉓ 线索链驱动（1215条链驱动发现） → ㉔ 证据链匹配（22条跨域证据闭环） → ㉕ 轻量跨结论串联 → ㉖ 证伪检查（30+规则覆盖） → ㉗ 联网核查（DB缓存→API→搜索引擎三层降级） → ㉘ 经营实质五步核查法 → ㉙ 知识图谱（49实体/5异常关系检测）' },
...
function _renderReportFallback(r, allF) {
  ...
  h += '<p class="i2"><strong>判定一（进项判定）——</strong>若发票的购买方名称或购买方纳税人识别号与本公司法定名称/USCC匹配，则判定为进项发票。此时本公司为采购方，该发票由供应商向本公司开具，用于记录本公司的采购成本和进项税额。</p>';
  h += '<p class="i2"><strong>判定二（销项判定）——</strong>若发票的销售方名称或销售方纳税人识别号与本公司法定名称/USCC匹配，则判定为销项发票。此时本公司为销售方，该发票由本公司向客户开具，用于记录本公司的销售收入和销项税额。</p>';
  ...
  h += '<p class="i2"><strong>第二环节·链驱动跨域推理——</strong>触发规则后的风险项，由1215条线索链（41条可执行+1250条方法链）驱动跨域推理——线索链将分散在不同域中的碎片化信号串联起来，形成跨域的关联线索。再由22条跨域证据链执行多源交叉验证——证据链要求每条结论必须至少有2个以上独立数据源相互印证，单一数据源的孤立信号不构成可报告的发现。</p>';
