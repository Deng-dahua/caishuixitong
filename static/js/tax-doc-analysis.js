...
function _renderReportFallback(r, allF) {
  ...
  if (r.file_results && r.file_results.length) {
    ...
    h += '<p class="i2">稽查启动后，首先对被查单位提交的' + (r.files_count || 0) + '份经营资料进行逐一审阅和类型识别。识别过程采用"四方交叉验证"法：第一方——扫描文件名中的业务关键词（如"工资薪金""抵扣""开票""取票""社保""公积金""银行"等），形成初步类型假设；第二方——解析Excel表头结构，提取列名中的字段指纹（如"有效抵扣税额→进项认证抵扣""缴存基数→公积金""征收项目→个税申报"），进一步锁定文件类型；第三方——扫描数据内容，收集所有购买方/销售方名称及税号信息，为身份比对做准备；第四方——将收集到的买卖方信息与当前账套公司「' + (te.name || '') + '」（信用代码' + (te.uscc || '') + '）进行身份匹配。</p>';
