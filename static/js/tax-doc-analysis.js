...
function renderAnalyzeHeader(report) {
  ...
  var steps = [
    ...
    { title: '第四阶段：Phase3 交叉验证——冲突消解与证据闭环', desc: '㉑ 冲突消解引擎（信号互斥检测→自动降级/升级） → ㉒ 规则引擎（1720条逐条匹配） → ㉓ 线索链驱动（1条链驱动发现） → ㉔ 证据链匹配（22条跨域证据闭环） → ㉕ 轻量跨结论串联 → ㉖ 证伪检查（30+规则覆盖） → ㉗ 联网核查（DB缓存→API→搜索引擎三层降级） → ㉘ 经营实质五步核查法 → ㉙ 知识图谱（49实体/5异常关系检测）' },


    { title: '第五阶段：方法论过滤——噪声剔除97%', desc: '㉚ 禁止词硬删除（40+） → ㉛ 无资料条件过滤 → ㉜ 行业不匹配过滤 → ㉝ 服务行业进销存过滤（三层闸门） → ㉞ 重复发现去重 → ㉟ 正常结论排除 → ㊱ 1条→24条，剔除36条噪声' },


    { title: '第六阶段：Phase4 综合定性——AI推理与因果叙事', desc: '㊲ 风险综合评分 → ㊳ 因果叙事链（5条因果规则推理） → ㊴ 缺失后果自动触发（14类资料缺失→9条风险结论） → ㊵ 贝叶斯因果推理 → ㊶ 矛盾检测（11条逻辑冲突） → ㊷ 回溯引擎定位根因 → ㊸ 四步税务合规分析法（detect→verify→diagnose→report）' },
...
function _renderReportFallback(r, allF) {
  ...
  h += '<h4>' + '第二环节·链驱动跨域推理' + '</h4><p class="i2">' + '触发规则后的风险项，由{{clue_chains}}条线索链（41条可执行+1266条方法链）驱动跨域推理——线索链将分散在不同域中的碎片化信号串联起来，形成跨域的关联线索。再由22条跨域证据链执行多源交叉验证——证据链要求每条结论必须至少有2个以上独立数据源相互印证，单一数据源的孤立信号不构成可报告的发现。' + '</p>';
