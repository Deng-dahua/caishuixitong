...
function renderAnalyzeHeader(report) {
  ...
  var steps = [
    ...
    { title: '第四阶段：Phase3 交叉验证——冲突消解与证据闭环', desc: '㉑ 冲突消解引擎（信号互斥检测→自动降级/升级） → ㉒ 规则引擎（1720条逐条匹配） → ㉓ 线索链驱动（2条链驱动发现） → ㉔ 证据链匹配（22条跨域证据闭环） → ㉕ 轻量跨结论串联 → ㉖ 证伪检查（30+规则覆盖） → ㉗ 联网核查（DB缓存→API→搜索引擎三层降级） → ㉘ 经营实质五步核查法 → ㉙ 知识图谱（49实体/5异常关系检测）' },


    { title: '第五阶段：方法论过滤——噪声剔除97%', desc: '㉚ 禁止词硬删除（40+） → ㉛ 无资料条件过滤 → ㉜ 行业不匹配过滤 → ㉝ 服务行业进销存过滤（三层闸门） → ㉞ 重复发现去重 → ㉟ 正常结论排除 → ㊱ 2条→24条，剔除36条噪声' },


    { title: '第六阶段：Phase4 综合定性——AI推理与因果叙事', desc: '㊲ 风险综合评分 → ㊳ 因果叙事链（5条因果规则推理） → ㊴ 缺失后果自动触发（14类资料缺失→9条风险结论） → ㊵ 贝叶斯因果推理 → ㊶ 矛盾检测（12条逻辑冲突） → ㊷ 回溯引擎定位根因 → ㊸ 四步税务合规分析法（detect→verify→diagnose→report）' },


    { title: '第七阶段：质量保障——三层门禁', desc: '㊹ 文本净化（剔除模板句/重复句/空描述） → ㊺ 建议质量增强（12条补充操作路径） → ㊻ 12项质量标准检测（5/32项通过·15.62%） → ㊼ 合规门禁（178项检测+自动修复+质量标记） → ㊽ Provenance溯源链注入（27条） → ㊾ Benford数字检验 → ㊿ EMA自学习（58样本）' },


    { title: '第八阶段：持续学习——智能体反思与记忆积累', desc: '⓫ AGI法律推理 → ⓬ AGI跨企业关联 → ⓭ AGI趋势追踪 → ⓮ 自动规则发现（2条新信号） → ⓯ 审计策略推荐（5条·P0×2） → ⓰ 分析记忆保存（59条积累） → ⓱ 行业基准更新 → ⓲ 智能体反思与学习闭环' },
...
function _renderReportFallback(r, allF) {
  ...
  h += '<p class="i2">根据《税务合规工作规程》第二十一条之规定，本系统在对账套内' + (r.files_count || 0) + '份经营资料执行涉税风险自动预审时，检出多项涉税风险指标异常，触发税务合规预审程序。预审程序启动后，系统依法对被查单位提交的全部经营资料进行了系统性综合判定。以下为被查单位的基本情况及本税务合规事项的立案依据。</p>';
  ...
  h += '<p class="i2">根据《税务合规工作规程》第二十二条至第四十五条关于检查程序的规定，本次税务合规实施对被查单位提交的' + (r.files_count || 0) + '份经营资料执行了全面、系统的综合判定和深度交叉分析。实施过程中，税务合规工作覆盖了资料审阅与类型识别、公司身份锚定与发票方向判定、行业判定与服务闸门验证、资金流与发票流双向核对、穿透分析与知识图谱构建、行业对标、综合分析与结论形成共七个维度，全部分析过程由系统自动执行并记录于税务合规工作底稿，每项结论均可通过规则ID、线索链ID、证据链ID逐级追溯至原始数据。具体实施过程如下：</p>';
  ...
  if (r.file_results && r.file_results.length) {
    ...
    h += '<h4>' + '第二方（Excel表头结构解析）' + '</h4><p class="i2">' + '解析文件表头列名，提取字段指纹进行特征匹配。例如：检测到"有效抵扣税额"或"勾选状态"列→判定为进项抵扣认证文件；检测到"缴存基数"或"单位缴存额"列→判定为公积金缴存文件；检测到"征收项目"或"累计应扣缴税额"列→判定为个税申报文件。通过字段指纹进一步锁定文件类型。' + '</p>';


    h += '<h4>' + '第三方（数据内容扫描）' + '</h4><p class="i2">' + '扫描文件的数据行，收集所有购买方/销售方名称及纳税人识别号信息。统计有效数据行数（自动过滤空白行、小计行、合计行等无效数据），评估文件的完整性和可用性。同时为第四方身份比对准备完整的交易对手信息。' + '</p>';


    h += '<h4>' + '第四方（公司身份匹配）' + '</h4><p class="i2">' + "将第三方收集到的买卖方信息与当前账套公司「' + (te.name || '') + '」（信用代码' + (te.uscc || '') + '）进行身份匹配。若某文件的购买方信息与本公司完全匹配→判定为本公司作为采购方的进项发票；若销售方信息与本公司完全匹配→判定为本公司作为销售的销项发票；若买卖双方信息均存在但都不匹配本公司→标记为存疑，排除出后续分析。" + '</p>';
  ...
  h += '<h4>' + '判定一（进项判定）' + '</h4><p class="i2">' + '若发票的购买方名称或购买方纳税人识别号与本公司法定名称/统一社会信用代码匹配，则判定为进项发票。此时本公司为采购方，该发票由供应商向本公司开具，用于记录本公司的采购成本和进项税额。' + '</p>';


  h += '<h4>' + '判定二（销项判定）' + '</h4><p class="i2">' + '若发票的销售方名称或销售方纳税人识别号与本公司法定名称/统一社会信用代码匹配，则判定为销项发票。此时本公司为销售方，该发票由本公司向客户开具，用于记录本公司的销售收入和销项税额。' + '</p>';


  h += '<h4>' + '判定三（存疑排除）' + '</h4><p class="i2">' + '若买卖双方信息均存在但均与本公司身份不匹配，则标记为存疑发票，立即排除出全部后续分析流程。此举从根本上杜绝了A公司账套中混入B公司发票资料的数据污染风险。' + '</p>';
  ...
  h += '<h4>' + '第一级（抵扣用途分类）' + '</h4><p class="i2">' + '对进项发票的表头列名进行检测：含"有效抵扣税额"或"勾选状态"列的，识别为进项抵扣认证发票（用于增值税进项税额抵扣申报）；无上述列的，识别为普通进项发票（用于记账凭证编制和成本费用核算）。两类发票的用途和后续处理流程不同，需严格区分。' + '</p>';
  ...
  h += '<h4>' + '跳过进销存台账比对' + '</h4><p class="i2">' + '服务行业以人力、知识、创意为核心生产要素，不存在制造业的"原材料采购→生产加工→产成品销售"的实物转换过程，因此无需建立进销存台账来进行进销数量匹配。' + '</p>';
  ...
  h += '<h4>①收款端核查</h4><p class="i2">汇总银行账户全部贷方（收入）发生额，累计收款' + (bankTotalIn > 0 ? (bankTotalIn/10000).toFixed(2) + '万元' : 'N/A') + '。逐笔提取收款对方户名，将对方户名与销项发票的购买方名称做交叉比对。';
  ...
  h += '<h4>' + '①供应商穿透' + '</h4><p class="i2">' + '统计全部进项发票的供应商分布，计算全部供应商各自的采购金额及占比，逐户评估供应商集中度风险。同时按供应商注册城市进行地理聚类分析，检测同一城市是否存在大量供应商群集（如同一城市出现5家以上供应商，且注册地址相近——可能为同一控制人分散注册的壳公司群，用于虚构采购交易、虚抵进项税额）。' + '</p>';


  h += '<h4>' + '②客户穿透' + '</h4><p class="i2">' + '统计全部销项发票的客户分布，计算全部客户各自的销售金额及占比，逐户评估客户集中度风险。重点检测客户与供应商是否重叠——同一企业的名称同时出现在供应商名单和客户名单中，既有采购又有销售（可能涉及对倒开票、虚增交易流水、人为做大经营规模以骗取贷款或政府补贴）。' + '</p>';


  h += '<h4>' + '③人员穿透' + '</h4><p class="i2">' + '将工资表的在职人员名单与银行流水的收付款方姓名做交叉比对。检测以下异常模式：员工姓名出现在收款方名单中（员工同时从公司收取款项——可能涉及代收款、利益输送、账外工资）、员工姓名出现在付款方名单中（员工向公司付款——可能涉及个人卡归集收入后转回公户的隐匿收入模式）。' + '</p>';


  h += '<h4>' + '④关联方穿透' + '</h4><p class="i2">' + '将工商登记信息中的法定代表人、股东、高管姓名及关联企业名单，与供应商/客户名单做交叉比对。检测以下关联交易风险：法定代表人/股东名下的其他企业与本公司存在购销交易（关联交易未披露）、供应商/客户的法定代表人同时出现在本公司员工名单中（人员混同，可能涉及关联方资金占用或利润转移）。' + '</p>';


  h += '<h4>' + '⑤六员跨企业比对' + '</h4><p class="i2">' + '提取被查单位的六员名册（法定代表人、董事、监事、财务负责人、股东、经理），与全部供应商和客户的工商信息做逐名交叉比对。首先执行"一人多角"检测——同一人是否在本企业兼任多个角色；其次执行"跨企业重叠检测"——本企业六员是否同时出现在供应商或客户的任职名单中。六员重叠+购销交易→关联交易嫌疑→购销闭环→可能涉及虚开发票。联网查询失败时从本地数据库回退读取。' + '</p>';
  ...
  h += '<p class="i2">将被查单位的核心经营指标与' + (te.industry || '广告传媒') + '行业的{{industries}}行业基准值进行系统性对比。鉴于被查单位经金税编码判定为服务行业，毛利率、进销比等基于实物采购成本与销售收入比例关系的指标不适用于服务行业的成本结构特征——服务行业的成本主要由人力成本、创意制作成本、渠道推广成本构成，而非原材料采购成本。因此系统自动跳过了毛利率对标和进销比对标。</p>';
  ...
  h += '<h4>' + '第一环节·规则引擎匹配' + '</h4><p class="i2">' + '将18个域分析产出的初步发现（涵盖资金追踪、发票审计、经营实质、工资社保、资料完备度、多源交叉验证等领域）统一导入规则引擎。引擎内置{{rules_count}}条税务合规规则逐条扫描、逐条匹配，判定每条初步发现是否触发了已知的风险特征模式。触发规则的发现获得规则ID标识，进入下一环节。' + '</p>';


  h += '<h4>' + '第二环节·链驱动跨域推理' + '</h4><p class="i2">' + '触发规则后的风险项，由{{clue_chains}}条线索链（42条可执行+1266条方法链）驱动跨域推理——线索链将分散在不同域中的碎片化信号串联起来，形成跨域的关联线索。再由22条跨域证据链执行多源交叉验证——证据链要求每条结论必须至少有2个以上独立数据源相互印证，单一数据源的孤立信号不构成可报告的发现。' + '</p>';


  h += '<h4>' + '第三环节·因果叙事链推导' + '</h4><p class="i2">' + '将多个独立信号叠加推演为因果链条。例如"银行收款与开票金额偏差+个人账户收款集中+法人账户出现"三个独立信号叠加，引擎推导出"私户收款→隐匿收入→未申报纳税"的因果叙事。因果叙事链需至少2个必要信号同时触发，置信度≥75%才纳入报告。' + '</p>';


  h += '<h4>' + '第四环节·Benford检验与方法论过滤' + '</h4><p class="i2">' + '运行Benford数字分布检验，检测财务数据的自然分布规律是否被人为破坏（如发票金额首位数分布偏离本福特定律→可能存在人为凑整、虚构发票）。同时启动方法论过滤器四道工序：禁止词过滤（剔除40+类模板句和空洞表述）、条件过滤（无资料支撑的发现全删）、行业匹配过滤（非本行业的发现排除）、去重过滤（相同结论合并）。经此环节后，海量初步发现的噪声剔除率超过96%。' + '</p>';
  ...
  h += '<h4>' + '最终产出' + '</h4><p class="i2">' + '经上述五环节全链路处理后，形成完整的税务合规结论。本次分析启用了全部核心引擎：文件识别引擎（四方交叉验证）、身份锚定引擎（逐行比对）、规则引擎（1720条）、线索链驱动（396条）、证据链闭环（745条）、因果叙事链、Benford检验、方法论过滤器、合规门禁、知识图谱构建、六员跨企业比对、收款分类自适应纠错。服务行业专属分析域（人均产值/经营费用完整性/工资社保合规）全量执行，进销存实物分析域自动跳过。每条结论均可通过规则ID→线索链ID→证据链ID→原始数据位置进行全链路追溯。' + '</p>';
  ...
  for (var fi = 0; fi < allSorted.length; fi++) {
    ...
    h += '<p style="text-indent:2em;margin:8px 0;text-align:justify"><strong>⑤ 法律依据：</strong>' + (f.policy_ref || '《税收征收管理法》及《税务合规工作规程》相关规定') + '</p>';
  ...
  h += '<p class="i2">根据《中华人民共和国税收征收管理法》及《税务合规工作规程》，被查单位「' + (te.name || te.company_name || '') + '」在本次税务合规过程中依法享有以下权利：</p>';
  ...
  h += '<p class="i2">被查单位对税务合规认定的事实、依据和处理建议，有权进行陈述和申辩。税务合规部门应当充分听取被查单位的意见，对其提出的事实、理由和证据进行复核。被查单位提出的事实、理由或者证据成立的，税务合规部门应当采纳。陈述申辩应当在收到《税务合规报告》后<strong>7日内</strong>以书面形式提交。</p>';
