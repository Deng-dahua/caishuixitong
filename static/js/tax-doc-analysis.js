...
function _renderReportFallback(r, allF) {
  ...
  h += '<p class="i2"><strong>第二环节·链驱动跨域推理——</strong>触发规则后的风险项，由1215条线索链（41条可执行+1174条方法链）驱动跨域推理——线索链将分散在不同域中的碎片化信号串联起来，形成跨域的关联线索。再由22条跨域证据链执行多源交叉验证——证据链要求每条结论必须至少有2个以上独立数据源相互印证，单一数据源的孤立信号不构成可报告的发现。</p>';
