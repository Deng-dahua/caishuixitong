...
new_steps = [
    ...
    {'n':'④', 'title':'规则引擎与链驱动检查', 'icon':'⚙️',
     'desc':'' + '1505' + '条风险检查指令逐条与域分析发现做匹配。' + '391' + '条线索链引擎：每链多个调查步骤，通过定量/定性/缺失三类数据验证后触发，'
    ...
    {'n':'⑤', 'title':'方法论噪声过滤器', 'icon':'🎯',
     'desc':'方法论过滤器是确保报告质量的最后关口。HARD_BAN（硬删除）：23类禁止词绝对不允许出现在输出中——'
        ...
        + '风险检查重点发现（level_fixed=True）不参与任何过滤。典型效果：1638条→过滤后36条。'},
    ...
    {'n':'⑦', 'title':'正式风险检查报告输出', 'icon':'📝',
     'desc':'综合所有域分析发现、链驱动发现、证据闭环发现，经过方法论过滤器和建议增强后，生成结构化风险检查报告。'
        + '报告含：风险检查概况、企业工商信息（联网核查）、高风险/中风险/低风险发现（按优先级排序）、'
