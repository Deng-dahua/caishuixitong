  ...
  var layers = [
    { id:1, name:'核心数据资产', icon:'🗄️', color:'#2563eb',
      ...
      items:[
        ...
        {name:'线索链系统',source:'static/cross_domain_clues.json → engine/pipeline.py',desc:'1215条线索链（41条可执行链+1174条方法链），每条可执行链含1-15个调查步骤。三类验证触发链驱动发现：①定量阈值触发——数值超过预设阈值自动启动链（如偏差率>20%、集中度>70%）②定性模式触发——识别特定的数据模式启动链（如同一付款方向多个个人账户转账）③缺失数据触发——关键资料缺失时启动替代验证链（如缺合同→通过品名+金额+支付频率推断业务实质）。每条调查步骤含：step叙数、domain名称、action描述、data_required数据来源。链的触发率=实际触发的步骤数/链的总步骤数，触发率纳入证据闭环计算。代码实现：pipeline.py _run_analyze()中调用_domain_cross_domain_clues并传入all_findings。可执行链通过keyword匹配findings自动触发，旧方法链仅用于UI展示。'},
        {name:'证据链系统',source:'static/cross_domain_evidence.json + engine/pipeline.py',desc:'22条跨域证据链。证据闭环双重门禁——①维度门禁：每个证据链定义dimensions[]数组，各维度kws匹配findings→达到min_evidence阈值（≥2维）→触发②跨域门禁：至少2个不同数据源的维度支撑（银行流水+发票、工资+社保等），单域数据不构成闭环。全部达标→形成有效证据闭环→进入正式报告结论。每条证据链含：关联的rule_refs、最低证据数（min_evidence）、涉及的维度数组（dimensions）。证据链的收集过程全程可追溯——规则ID→线索链→证据链→原始数据行。代码实现：_domain_cross_domain_reasoning在all_findings构建后运行。'},
        {name:'跨域分析链',source:'engine/domain_analysis.py + engine/pipeline.py',desc:'13条跨域分析链，多源数据综合推理引擎。不同于单域分析（只在银行流水域内分析收款模式），跨域分析将多个证据链的结论进行综合推理判定：reasoning_path[]定义多步推理路径→从证据到结论的因果推断。典型分析链如\"七维系统性造假综合判定模型\"——经营实质×供应商×资金流×三流合一×跨税种×关联交易×综合，7维中0-2维低风险、3-4维中风险、5-6维高风险、7维全异常→系统性造假立案。跨域分析链在Phase3交叉验证阶段集中执行，输出含score/level/triggered_dimensions的综合判定发现。'},
      ]},
