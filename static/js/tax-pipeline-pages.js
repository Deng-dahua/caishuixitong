...
function renderDomainAnalysisStatic() {
  ...
  html += '<div style="margin-bottom:10px;padding:20px 24px;background:#fff;border-radius:8px">'
    ...
    + '<strong>跨域关联推理</strong>（顶层）→ 将以上所有发现串联为12条跨域证据链，形成最终税务合规结论。'
    + '</div>'
    + '</div>';
...
function renderFilterResult(report) {
  ...
  var rules = [
    {title:'① 税务合规重点保护', icon:'🛡️', color:'#2563eb', badge:'12类', desc:'执行顺序：第一步（先于所有过滤规则）。12类税务合规重点发现（虚开发票/骗取出口退税/隐匿收入/账外经营行为/阴阳合同/资金回流/关联交易转移利润/虚假申报/骗取税收优惠/恶意注销/走逃失联/暴力抗税）在过滤器启动前即被标记为level_fixed=true，此后所有过滤操作都跳过这些发现。三层保护：后端修正→过滤器绕过→前端标记。设计哲学：宁可12条假阳性进入报告，也不能让2条真阳性被过滤掉。'},
...
// ══════════════════════════════════════════════════════════════
//  智哥行为准则页面 —— 7条编码行为规范
//  系统铁律(12条)已迁至 engine/memory.py，本页面仅保留约束智哥写代码的行为规范
// ══════════════════════════════════════════════════════════════
...
function renderAiRules(container) {
  ...
  var categories = [
    ...
    {name:'质量保障准则', icon:'✅', color:'#dc2626', id:'quality', desc:'确保代码质量和正确性的行为准则。违反任何一条都可能导致系统崩溃、数据错误或用户看到的页面与实际代码不一致。这四条准则覆盖了从"写完代码"到"用户看到"中间的每一个质量关口——自检→验证→影响分析→输出检查。每一条背后都有至少一次因违反该准则导致的严重事故。', rules:[
      ...
      {id:16, name:'交付前输出自检', level:'铁律', date:'2026-06-21',
       desc:'每写一段面向用户的输出文本（报告/页面/提示），写完必须逐句读一遍——以用户的视角，假装自己是第一次看到这段文字。四项检查：①变量语义正确——company_type是"企业类型"不是"行业"，不要把工商登记的企业类型标签当行业来用 ②分支穷举自洽——如果系统检测到"无BOM数据"，报告中就不能出现"建议补全BOM表"这样的建议，因为无信号不应输出对应建议 ③数值边界处理——2条发现不写"存在0类风险"（0就是没有，不应该出现在统计报告中），超出上限不截断（发票明细202条上限→"另行提供"而非直接丢弃）④句意通顺不自相矛盾——前面说"数据完整"后面又说"缺失N类资料"是不行的。读完确认无误再提交。这条准则定义了输出的"可读性"——代码对不代表用户看得懂、读得通。',
  ...
  html += '<p style="font-size:10px;color:#64748b;margin:0 0 10px;line-height:2">共'+totalRules+'条（'+tieLvCount+'铁律+'+zhunZeCount+'准则）· 2大分类 · 仅约束智哥写代码的行为规范。系统自身的12条铁律已迁至 engine/memory.py。</p>';
  ...
  html += '<b>系统铁律12条</b>（已迁至 <code>engine/memory.py</code>）：定义系统应该如何工作——科目name必须查DB、三号必须合并、ref_id必须精确匹配等。这些是"系统怎么做"的规范。<br><br>';
  ...
  html += '系统铁律编号（共12条）：铁律一~六（账务处理·engine/memory.py §06）+ 铁律七~十一（核心铁律·engine/memory.py §07）+ 铁律十二（跨模块内容一致性·engine/memory.py §08）。<br><br>';
  ...
  var layers = [
    { id:1, name:'核心数据资产', icon:'🗄️', color:'#2563eb',
      ...
      items:[
        ...
        {name:'跨域分析链',source:'调度中枢',desc:'19条跨域分析链，多源数据综合推理系统。不同于单域分析（只在银行流水域内分析收款模式），跨域分析将多个证据链的结论进行综合推理判定：reasoning_path[]定义多步推理路径→从证据到结论的因果推断。典型分析链如\"七维系统性造假综合判定模型\"——经营实质×供应商×资金流×三流合一×跨税种×关联交易×综合，7维中0-2维低风险、3-4维中风险、5-6维高风险、7维全异常→系统性造假立案。跨域分析链在Phase3交叉验证阶段集中执行，输出含score/level/triggered_dimensions的综合判定发现。'},
      ]},
...
function renderTaxWorkflow(container) {
  ...
  var stages = [
    { n:'①', title:'选案环节（第14-22条）', icon:'🎯', color:'#7c3aed',
    ...
    { n:'③', title:'审理环节（第46-2条）', icon:'⚖️', color:'#059669',
    ...
    { n:'④', title:'执行环节', icon:'💵', color:'#dc2626',
      ...
      points: [
        ...
        ['法律依据', '《征管法》第32条(滞纳金)/第42条(强制执行)/第88条(复议前置)']
      ]
    },
...
function renderDataAssets(container) {
  ...
  var assets = [
    ...
    { n:'②', name:'线索/证据链', icon:'🔗', color:'#7c3aed',
      desc:'线索链定义从风险到发现的调查路径（42条精写线索链）。证据链定义多源验证的闭环条件（22条精写证据链，≥2个独立数据源交叉印证）。',
...
function renderJudgmentRules(container) {
  ...
  h += '<p style="margin:0">每条判定规则均由代码层（phase1_triage.py / pipeline.py / cross_domain_negotiation.py）独立实现，并通过<strong>系统记忆（engine/memory.py）</strong>记载规则定义与执行约束。证据闭环要求：≥60%触发率 + ≥3条规则触发 + ≥2个数据域交叉验证，三重门禁全部通过才形成有效证据闭环。</p>';
  h += '</div>';
...
function renderLegalRefs(container) {
  ...
  fetch('/static/legal_refs.json')
    ...
    .then(function(data) {
      ...
      h += '<div style="font-size:10px;color:#64748b">手册第6章 · 12条核心法条 · 法律推理系统自动匹配</div>';
      h += '</div>';
      ...
      // ── 段落说明 ──
      h += '<div style="font-size:10px;color:#3a4048;line-height:20px;margin-bottom:10px">';
      h += '<p style="margin:0 0 10px">以下<strong>12条法律条文</strong>为税务合规中最常引用的核心依据。税务合规报告的每项发现必须引用具体法条——笼统引用"相关税收法规"的表述在审理环节会被退回重写。条文的适用场景和处罚标准直接写入报告的法律依据字段，由<strong>法律推理系统（legal_reasoner.py）</strong>自动匹配。</p>';
      h += '<p style="margin:0">覆盖<strong>征管法6条</strong>（第32/35/54/60/63/64条）+ <strong>刑法2条</strong>（第201/205条）+ <strong>其他4条</strong>（发票管理办法第22条/增值税条例第19条/企业所得税法第42条/规程第42条），形成从行政处罚到刑事追诉的完整法律覆盖层次。</p>';
      h += '</div>';
...
function renderProcedureMapping(container) {
  ...
  h += '<p style="margin:0 0 10px">系统每一个功能模块都对应《税务合规工作规程》（国税发[2009]157号）的具体条款要求——确保系统产出<strong>不是凭空制造的</strong>，每一项分析、每一条结论都有法定的规程依据。12个功能模块完整覆盖从案源筛选到报告输出的<strong>全税务合规流程</strong>：第21-45条（检查实施）→第46条（审理审核）→第49条（审理意见）→第52条（报告格式）→第2条（程序合法）。</p>';
  h += '<p style="margin:0">这个映射表回答了"凭什么"的问题：凭什么一键稽查就是完整的检查环节？凭什么证据闭环就意味着证据充分？每一项能力的背后都有规程条款作为法律依据。</p>';
  h += '</div>';
  
  // 12项功能模块卡片
  var modules = [
    {n:'①',name:'一键稽查',clause:'第21-45条（检查）',desc:'_run_analyze自动执行全部分析域+四步核查法+链驱动系统+协商系统+方法论过滤器。一次点击=完整模拟税务合规检查环节——从文件上传到报告输出全部自动化。',color:'#2563eb'},
    ...
    {n:'⑤',name:'分析链',clause:'第46条（审理审核）',desc:'19条分析链，含推理路径多步推理，从证据到结论的综合判定。模拟审理部门的逐项审核——检查对象准确性/事实证据充分性/法律适用正确性→0-7维异常评分→定案。',color:'#dc2626'},
    ...
    {n:'⑨',name:'报告生成',clause:'第52条（报告格式）',desc:'自动生成封面+7章+附件。完全符合规程第42条规定的10项内容格式要求：案件来源→基本情况→实施情况→发现问题→税务合规结论→处理建议→告知权利→签字→证据清单。',color:'#db2777'},
    {n:'⑩',name:'合规门禁',clause:'第2条（程序合法）',desc:'质量标准（模板句清除/重复句合并/空描述删除/人性化表述/六要素完整/法律引用准确/具体数值/因果链/可执行建议/条款号/反跨复制/空占位符清除）+16项自省检查。全通过→绿色交付。',color:'#4f46e5'},
...
function renderCrossDomainNego() {
  ...
  "title": "跨域协商系统",
  "icon": "🤝",
  "subtitle": "{{domain_functions}}个域分析函数各自独立产出发现后，协商系统自动执行跨域对话——一个域的结论影响其他域的判定。29条协商规则覆盖四类场景：消解、标记、矛盾、增强。",
  "stats": [
    [
      "29条",
      "协商规则",
...
function renderIronLaws() {
  ...
  "title": "系统铁律编号体系",
  "icon": "⚖️",
  "subtitle": "系统铁律（engine/memory.py规则篇）=系统硬逻辑，不可违反。AI行为准则（前端页面）=编码规范。共12条系统铁律+7条AI准则，编号互不重叠，各司其职。",
  "stats": [
    [
      "12条",
      "系统铁律",
  ...
  "desc": "<p style=\"margin:0 0 10px\">系统铁律（12条）存储在engine/memory.py规则篇，是系统的硬逻辑，不可违反。AI行为准则（7条）在侧边栏AI行为准则页面展示。</p><p style=\"margin:0\">铁律一~六（账务处理）：科目name铁律、三号合并铁律、审计铁律、ref_id去重铁律、普票税额并入成本铁律、7分类禁止兜底铁律。铁律七~十一（核心规范）：规则=代码铁律、代码即承诺铁律、全行业适用铁律、主动关联更新铁律、方法论先行铁律。</p>",
  "cards": [
    [
      "铁律一~六（账务处理）",
...
function renderDADomains() {
  ...
  "desc": "<p style=\"margin:0 0 10px\">7条判定规则前置检查（域分析执行前必须先通过）：公司身份锚定、发票方向判定、进项再分类、服务行业闸门、品名级精准过滤、综合判断四方交叉验证、存疑排除。</p><p style=\"margin:0\">13大分类：①资金流分析（4域）②进销存分析（4域）③供应商与客户分析（4域）④多源交叉验证（5域）⑤经营实质分析（3域）⑥资料完备度与情报（2域）⑦发票深度分析（3域）⑧合同与凭证（2域）⑨税务与社保（3域）⑩资产与关联交易（2域）⑪行业对标与规则系统（4域）⑫跨域分析链（1域）⑬补充税种检查（3域）。</p>",
  "cards": [
    [
      "资金流+进销存（8域）",
...
function renderUnifiedDomainPanel(container) {
  ...
  h += '<p><b>资金流分析</b></p>';
  h += '<p><b>_domain_cash_flow_tracking</b>（资金全链路追踪）——输入：银行流水（必须）；最低：≥12条；输出finding.type：收款方异常、大额支付、频繁支付。</p>';
  h += '<p><b>_domain_fund_flow</b>（资金流向追踪）——输入：银行流水（必须）；最低：≥12条；输出finding.type：资金流向异常、单边大额资金。</p>';
  h += '<p><b>_domain_abnormal_transaction_time</b>（异常交易时间）——输入：银行流水（必须）；最低：≥12条；输出finding.type：非营业时间交易、节假日大额交易。</p>';
  h += '<p><b>_domain_personal_txn_risk</b>（个人交易风险）——输入：银行流水（必须）；最低：≥12条；输出finding.type：个人账户收款、对私转账高频。</p>';
  h += '<p><b>进销存分析</b></p>';
  ...
  h += '<p><b>_domain_profit_cashflow_contradiction</b>（利润现金流矛盾）——输入：科目余额表+银行流水（必须）；最低：各≥12条；输出finding.type：账面盈利现金枯竭、应收异常膨胀。</p>';
  h += '<p><b>_domain_revenue_time_investigation</b>（收入时间线调查）——输入：销项发票+银行流水（必须）；最低：各≥12条；输出finding.type：季末突击开票、收入集中化。</p>';
  h += '<p><b>_domain_extended_audit_rules</b>（扩展审查规则）——输入：原始数据（可选）；最低：无最低要求；输出finding.type：自定义信号。</p>';
  h += '<p><b>经营实质分析</b></p>';
  h += '<p><b>_domain_business_substance</b>（经营实质分析）——输入：银行流水+发票+工资+社保（应当）；至少3源（必须）；最低：各源≥12条；输出finding.type：人员不匹配、场地缺失、设备与产能不符。</p>';
  h += '<p><b>_domain_business_geography</b>（经营地理分析）——输入：发票+银行流水（必须）；工商登记地址（应当）；最低：≥12条；输出finding.type：注册地与经营地不一致、地址分散。</p>';
  ...
  h += '<p><b>_domain_voucher_account_anomaly</b>（凭证科目异常）——输入：会计凭证（必须）；最低：≥12条；输出finding.type：科目使用错误、摘要与科目不匹配。</p>';
  h += '<p><b>税务与社保</b></p>';
  ...
  h += '<p><b>行业对标与规则</b></p>';
  h += '<p><b>_domain_industry_benchmark</b>（行业基准对标）——输入：发票+科目余额表（必须）；最低：≥12条；输出finding.type：偏离行业基准。</p>';
  h += '<p><b>_domain_rule_engine_match</b>（规则系统匹配）——输入：所有数据（必须）；最低：无最低要求；输出finding.type：规则触发信号。</p>';
  h += '<p><b>_domain_tax_burden_analysis</b>（税负率分析）——输入：纳税申报表+发票（应当）；最低：≥12条；输出finding.type：税负率低于行业P25。</p>';
  h += '<p><b>_domain_trend_analysis</b>（趋势分析）——输入：科目余额表+发票（必须）；最低：≥12条；输出finding.type：指标异常波动。</p>';
  h += '<p><b>跨域分析链</b></p>';
  h += '<p><b>_domain_cross_analysis_chains</b>（跨域分析链）——输入：所有域发现（必须）；最低：域发现≥2条；输出finding.type：线索链+证据链并行配合→分析链跨域串联。</p>';
  h += '<p><b>补充税种</b></p>';
  h += '<p><b>_domain_stamp_duty_check</b>（印花税）——输入：合同+发票+银行流水（应当）；至少1源满足最低条数（必须）；最低：≥5条；输出finding.type：印花税漏缴。</p>';
  h += '<p><b>_domain_cit_reconciliation</b>（企业所得税汇算）——输入：科目余额表（必须）；凭证（应当）；最低：≥12条；输出finding.type：应纳税所得额调整。</p>';
  ...
  h += '<h2>五、跨域综合推理链（共19条，编号1-19连续）</h2>';
  h += '<p>经甄别，原48条分析链中以下19条属于真正的跨域综合推理链——每条均产生跨域数据分析结果并输出综合结论。已重新编号为1-19连续编号（原编号见分析链名称，对应关系可查cross_domain_analysis.json）。其余29条已分别归入前置判定规则、数据预处理模块、质量保障准则或系统铁律。</p>';
  h += '<p><b>1. 收款来源不匹配→隐匿收入推理链</b>——从单一的收款来源不匹配信号出发，逐层扩展分析范围。第一步确认不匹配的规模，第二步核实付款方身份（排除法人/股东的打款），第三步判断资金性质（经营收入还是非经营资金），第四步对无法解释的收款做出隐匿收入的判断。每一步都有回退路径——只要企业能提供合理解释，风险就会降级或消除。推理路径：从资金流分析到发票比对：逐名交叉比对全部收款方与销项发票购方名称；从发票比对到付款方身份核实：联网查询法定代表人/股东名单，比对未匹配的付款方；从付款方身份核实到资金性质判断：区分注资/借款/往来款/未开票收入四种可能；从资金性质判断到综合结论：无法说明来源的收款推定为未开票经营收入→隐匿收入风险。关联规则（共14条）：44、44、175、175、44、303、399、372、401、347、690、127、540、399。</p>';
  h += '<p><b>2. 加工费来自外地→全链条经营异常推理链（点→面推理核心）</b>——从加工费这一个单点异常出发，扩展到面的推理。第一步发现加工费在外地，第二步判断商业合理性（本地是否有集群），第三步扩展到三组地址对比，第四步检测运输成本缺失，第五步得出物理不可能的结论。这是点→面推理的标准案例——单点异常可解释，但四点叠加无法解释——换由另一名稽查人员使用相同资料独立复核，亦会得出相同结论。推理路径：从发票分析到产业集群判断：分析企业所在地是否有同类加工产业集群；从产业集群判断到商业合理性判断：外地加工增加了运输成本和周期，舍近求远→商业逻辑存疑；从商业合理性判断到全链条地理分析：提取三组地址做城市交叉比对；从全链条地理分析到物理合理性判断：货物在N个城市间反复运输但无运输费→在物理上不具备可行性；从物理合理性判断到综合结论：全链条经营实质存疑→所有跨省交易的真实性需要逐笔验证。关联规则（共9条）：44、44、44、1317、326、1267、1269、372、1059。</p>';
  h += '<p><b>3. 进销品名不匹配→企业类型重判推理链（三层穿透）</b>——不从单一来源判定企业类型，而是通过三层递进穿透：工商登记看法律形式、发票数据看经营实质、加工信号看业务模式。三者不一致时以实质重于形式为原则。风险不是消除了，而是从品名问题转移到了加工真实性问题——此属于风险转移，并非风险消除。推理路径：从工商查询到发票推断：从全部发票品名关键词做66行业加权投票，得出发票推断行业；从发票推断到加工信号检测：筛查进项发票中是否含加工费+是否存在原材料采购；从加工信号检测到三层综合判断：工商登记=批发业(形式)+发票推断=纺织制造(实质)+加工信号=制造业(模式)；从三层综合判断到风险重定向：风险从「有进无销=隐匿收入」转移至「加工链条是否真实」。关联规则（共9条）：127、347、372、372、775、891、44、1325、1052。</p>';
  h += '<p><b>4. 有进无销→隐匿收入vs制造业加工推理链</b>——有进无销是税务合规中最常见的信号，但统一标准处理即认定为高风险是不准确的。系统先判断企业类型——纯贸易企业有进无销确实是隐匿收入信号，但制造业企业采购原料加工成成品后品名不同，进销品名不匹配是正常的。诊断后风险焦点从「隐匿收入」转移至「加工链条是否真实」。推理路径：从进销存比对到行业模式判断：检查企业是纯贸易还是制造业——加工费+原材料信号；从行业模式判断到制造业诊断(如有加工信号)：有加工费+有原材料→制造业加工链条→风险焦点转移；从制造业诊断到BOM验证：要求企业提供BOM表验证原材料投入→加工→成品产出的逻辑；从BOM验证到综合结论：无法提供BOM→加工链条无法验证→有进无销仍可能是隐匿收入→回到高风险。关联规则（共15条）：20、21、22、23、44、44、44、44、44、44、127、513、540、1302、1267。</p>';
  h += '<p><b>5. 合同缺失→交易真实性存疑推理链（四层分层）</b>——合同缺失不是统一标准处理的问题——不是所有交易都需要合同。系统通过四层自动分层，将缺失的影响精准定位到必签和应签两类，避免把所有供应商都视为「需要合同」（那会要求连加油站、酒店都签合同——不符合实务要求）。分层判断使风险判定的准确性及可执行性均得到提升。推理路径：从资料完备度检测到合同需求分层：对每个供应商按品名/金额/类型四层自动分类：必签/应签/可免/小额；从合同需求分层到交易真实性验证：四流合一缺了合同流→这些交易的商业实质无法通过书面证据验证；从交易真实性验证到分层风险判断：必签类缺合同=高风险；应签类缺合同=中风险；可免类无合同=正常；从分层风险判断到综合结论：缺失合同→四流不合一→税务合规可逐笔质疑交易真实性→虚开发票嫌疑→印花税漏缴。关联规则（共12条）：14、15、16、17、44、44、44、44、127、175、691、372。</p>';
  h += '<p><b>6. 重物跨省缺运输→货物流断裂推理链</b>——从地址分布出发，结合行业重物属性和运输成本缺失，形成货物流物证链断裂的推理。关键在于行业的重物判断——不是所有行业都需要运输成本（如软件/咨询/设计等服务业），但纺织/建材/机械/食品等重物产业必须有。系统根据行业自适应地判断重物属性，不搞统一标准处理。推理路径：从地址分析到行业重物判断：根据行业分类判断产品是否为需运输的重物；从行业重物判断到运输成本检测：检索全部发票和银行流水中的运输/物流/快递/货运关键词；从运输成本检测到货物流判断：重物跨省运输必有运费（通常占货值3%-8%），零运费=货物流物证链断裂；从货物流判断到综合结论：货物流无法验证→三流合一缺了货物流→交易真实性存疑→进项税额+成本均面临否定风险。关联规则（共12条）：10、44、44、44、44、44、44、44、175、127、1317、513。</p>';
  h += '<p><b>7. 费用发票占比异常→虚增成本推理链</b>——从费用结构异常出发，逐层深挖费用的真实性和关联性。关键判断在于费用与经营是否相关——招待费/差旅费可能有经营关联，但旅游/购物/生活用品等明显与经营无关。系统通过联网查询股东身份，将个人消费入账的风险精准定位。推理路径：从费用结构分析到费用明细核查：逐类筛查费用发票的品名/金额/时间/对方；从费用明细核查到股东关联判断：联网查询股东和法定代表人，判断费用是否与股东个人消费相关；从股东关联判断到税前扣除判断：与生产经营无关的支出不得税前扣除→需调增应纳税所得额；从税前扣除判断到综合结论：虚增成本费用→利润虚减→企业所得税少缴+个人所得税漏缴。关联规则（共12条）：127、127、44、44、127、303、347、376、399、347、891、691。</p>';
  h += '<p><b>8. 进销比异常+零申报→账外经营行为推理链</b>——进销比异常是最强的税务合规信号之一——采购量远超销售量的本质是「购入货物后未通过正常开票渠道售出」。但不是一看到进销比异常就定「账外经营行为」——需要先排除存货积压的可能。如果存货没有增加、采购款已付、但销项极少——那就是典型的账外经营行为信号。结合收入端（收款远超开票），形成进销两端闭合的证据链。推理路径：从进销比分析到存货分析：检查存货周转率——如果存货也没有对应增加则货物去向不明；从存货分析到资金流追踪：追踪银行付款去向——采购款付了但货去哪了？；从资金流追踪到收入端对比：付款正常+进货正常+销项极少→货物已售出但未开票→账外经营行为；从收入端对比到综合结论：进销比异常+零申报+收款开票偏差→完整的账外经营行为链条→严重隐匿收入。关联规则（共12条）：19、44、44、336、127、541、303、44、1173、399。</p>';
  h += '<p><b>9. 供应链六员交叉→关联交易推断链（人员→发票→资金三段式跨域）</b>——通过人员信息→发票数据→资金流的三段式跨域推理：从发票中定位供应商/客户→联网核查其六员→与本企业六员交叉比对→发现重叠后核查资金流→确认关联交易并评估转移定价风险。推理路径：联网核查被查单位+供应商/客户六员；提取进销发票中的供应商/客户名称+金额；逐名交叉比对→发现重叠；核查重叠企业间资金往来。关联规则（共7条）：44、44、44、175、303、1269、1059。</p>';
  h += '<p><b>10. 购销闭环→虚开发票推理链（发票→进销存→资金三段式跨域）</b>——从发票数据发现供应商=客户→形成购销闭环→核查进销商品流转逻辑是否合理→核查资金流是否形成回流→判定虚开发票风险等级。推理路径：提取进项销售方+销项购买方；交叉比对：供应商∩客户；核查闭环企业间的商品流转逻辑；核查闭环企业间资金流向。关联规则（共8条）：127、44、127、127、175、1269、1059、127。</p>';
  h += '<p><b>11. 仓库→产能→能耗→物流全链条经营实质分析</b>——单一维度的矛盾可能属于偶发现象，两个维度的矛盾则高度可疑，三个及以上维度的矛盾通常足以形成认定结论。仓储容量不足、设备产能无法支撑、能耗数据不匹配、物流费用缺失，以上多项异常叠加表明企业账面反映的经营状况与实际严重不符。推理路径：从租金发票推算仓储面积→对比库存体积→仓容矛盾？；从设备参数推算理论产能→对比账面产量→产能矛盾？；从电费/水费推算能耗→对比行业单耗标准→能耗矛盾？；从运输距离推算运费→对比账面运费→物流矛盾？；四个维度交叉验证→几个维度出现矛盾？→矛盾越多置信度越高→从单点怀疑升级为系统性造假判定。关联规则（共2条）：44、1317。</p>';
  h += '<p><b>12. 系统性造假综合判定模型(七维交叉)</b>——七维交叉验证模型——从经营实质、供应商穿透、资金流、三流合一、跨税种、关联交易七个维度综合评估企业造假可能性。单维异常是线索，多维异常是定案。七维全异常=企业账务大概率全面造假。推理路径：库存体积vs仓容/设备产能vs产量/用电量vs生产规模/人员vs产出→几维矛盾？；供应商五维评分(资质/时间/规模/价格/资金)→几个维度不通过？；收付款匹配度/个人收款占比/资金回流迹象/过桥账户→资金流异常度；发票/资金/物流/合同四流一致性→几流断裂？；增值税vs所得税/个税vs社保/印花税vs购销→几税申报不一致？；关联方交易占比/转移定价/资本弱化/费用分摊→关联交易风险度；七维综合评分→0-2维异常=低风险→3-4维=中风险→5-6维=高风险→7维=系统性造假→触发全面税务合规立案。关联规则（共12条）：44、44、44、175、44、44、127、326、303、303、347、1052。</p>';
  h += '<p><b>13. 隐匿收入综合判定模型</b>——三线归一法：资金偏差线(收款-开票)=来源分类线(未开票收入)=消费验证线(超申报消费)。三条线独立推算的隐匿收入金额应当大致相等。金额越接近，证据强度越高，结论越经得起检验。推理路径：计算收款-开票偏差的绝对金额和相对比例→分月度/季度看趋势；对未匹配收款逐笔分类→借款/注资/代收/未开票/其他→计算各类占比；追踪个人账户收款和异常消费→推算最低隐匿收入规模；三指标交叉验证→偏差金额≈未开票收入≈隐匿消费→一致即定案。关联规则（共15条）：20、21、22、23、44、401、127、513、303、44、540、541、376、585、401。</p>';
  ...
  h += '<p><b>18. 全税种综合判定模型</b>——多税种交叉验证。单一税种异常可能是疏忽，3+税种同时异常=系统性偷逃税。综合判定降低误报。推理路径：收集各税种触发发现；逐税种比对申报vs实际；≥3税种异常→系统性偷逃税；多税种累计偷税额→处罚档次。关联规则（共4条）：44、44、376、1264。</p>';
  h += '<p><b>19. 物耗能耗产能三线归一分析</b>——产量可以从产能、电耗、物耗三个独立维度推算。三维交叉印证→单一维偏差可能是统计误差，三维全偏=造假。推理路径：设备铭牌×工时→理论最大产量；电费÷行业单位电耗→推算产量；原材料投入÷行业投入产出比→推算产量；三维推算→中位数vs账面产量；三维全偏低且偏差>30%→系统性造假。关联规则（共2条）：44、401。</p>';
...
function renderAGICore() {
  ...
  "desc": "<p style=\"margin:0 0 10px\">6个核心智能系统：①自我反思器 SelfReflector——14维反向假设验证，对每条高风险发现生成竞争假设并逐条验证 ②洞见总结器 InsightSynthesizer——从all_findings自动组织为五段式结构化报告 ③跨分析学习器 CrossAnalysisLearner——跨企业分析经验积累，12维度加权相似度检索 ④税务合规方法论 MethodologyEngine——33条方法论+14类必查资料+12条法律条文 ⑤规则发现 RuleDiscovery——三层递进归纳系统 ⑥自动巡逻 PatrolEngine——定期重分析，对比前后两次报告差异。</p>",
  "cards": [
    [
      "自我反思器",
...
function renderAGICausal() {
  ...
  "title": "因果推理层",
  "icon": "📊",
  "subtitle": "4个因果推理系统：SCM因果推理（do-干预/反事实/混淆/因果链）、元认知系统（四维推理质量评估）、法律三段论（12条结构化规则）、因果网络（条件概率+多信号联合预测）。",
  "stats": [
    [
      "4个",
      "因果系统",
  ...
  "desc": "<p style=\"margin:0 0 10px\">4个因果推理系统：①SCM因果推理 SCMReasoner——从条件概率升级为结构化因果推理，四种推理（do-干预/反事实/混淆因子检测/因果链查询），预置9条税务因果先验 ②元认知系统 Metacognition——四维推理质量评估（因果链完整性/证据充分性/法律依据/可操作性）→质量分→不确定性检测→信息缺口识别→行动建议 ③法律三段论 LegalReasoner——12条结构化法律规则，大前提(法条)+小前提(本案事实)→结论(法律定性) ④因果网络 CausalNetwork——条件概率矩阵+多信号联合预测，置信度=P(结论|信号) x log(lift+1)。</p>",
  "cards": [
    [
      "SCM因果推理",
    ...
    [
      "法律三段论",
      "12条结构化法律规则。大前提(法条)+小前提(本案事实)→结论(法律定性)。含征管法第63条、发票管理办法第22条、刑法第205条等。",
      "#d97706"
    ],
    [
      "因果网络",
...
function renderAGIKnowledge() {
  ...
  "cards": [
    [
      "统一知识库",
      "9域知识：政策库、因果网络、信号模式、语义词典（14类同义词）、风险同义词、行业画像、自愈规则、经验教训、分析历史（最近102条）。线程安全/全局单例/JSON持久化。",
      "#2563eb"
    ],
    [
      "自学习系统",
...
function renderAGISpecial() {
  ...
  "desc": "<p style=\"margin:0 0 10px\">7个专项系统：①语义推理器——14类品名同义词库，两层匹配（子字符串+编辑距离Levenshtein≤2），创造性假设系统（Jaccard类比推理）②未知模式检测——规则覆盖度检查+7种异常检测器（结构化转账/幽灵供应商/价格异常/数量尖峰/月末突击/个人大额转账/营收平滑）③假设验证系统——每条重要发现生成2-3条互斥竞争假设→贝叶斯更新后验概率 ④跨企业关系网——一人多角检测+连锁稽查点（A→B→C→A闭环虚开）⑤税收优惠分析——9类优惠，联网核查三步法（搜索→抓取→提取），90天缓存 ⑥跨域协商系统——29条协商规则 ⑦数据一致性系统——双维度自检。</p>",
  "cards": [
    [
      "语义推理+未知检测",
...
function renderAGIKnowledgeConfig() {
  ...
  "desc": "<p style=\"margin:0 0 10px\">9类知识库：①政策库policies（9条税收优惠）②因果网络causal_edges（信号→结论因果边）③信号模式signal_patterns（多信号组合）④语义词典semantic_dict（14类同义词）⑤行业画像industry_profiles（8大行业）⑥自愈规则healing_rules（错误→修正）⑦经验教训lessons（跨分析积累）⑧分析历史analysis_history（最近102条）⑨巡逻快照patrol_snapshots（巡逻基线）。</p>",
  "cards": [
    [
      "知识库（5类）",
    ...
    [
      "知识库（4类）",
      "自愈规则（动态活跃规则数，同类错误≥2生成规则）、经验教训（动态经验条数，跨分析积累）、分析历史（最近102条）、巡逻快照（动态企业数，巡逻基线）",
      "#7c3aed"
    ],
    [
      "核心配置参数（5项）",
    ...
    [
      "核心配置参数（5项）",
      "SCM因果：do-干预/反事实/混淆检测/因果链/9条先验；知识库：线程安全/单例/102条历史/JSON持久化；联网核查：三步法/90天缓存/chinatax.gov.cn；并行加速：多模块并行/DAG依赖图/可开关/提升30-50%；事件总线：pub/sub松耦合/14种事件/502条日志/自动持久化",
      "#d97706"
    ]
  ]
});
...
function renderMethodologyPart2(){
  var h='';
  // ═══ 第三层·布网 ═══
  h+='<h2>第三层 · 分析布网 —— 全面启动，四链齐发</h2><p>情报池就绪。系统全线激活规则系统驱动→线索链调查+证据链闭环（并行配合）→分析链跨域串联。{{rules_count}}条指令+{{clue_chains}}条线索+{{evidence_chains}}条证据+19条跨域分析链+{{domain_functions}}个域分析函数——同时启动、四层协作。这是系统的全面分析。</p>';
  ...
  h+='<p><b>Phase4  综合定性·跨域分析链判定（引用分税种为计算依据）</b>：19条跨域分析链做多源综合推理（每条含多步推理路径，每步带回退分支——能进能退）。如"七维系统性造假判定模型"：经营实质×供应商×资金流×四流合一×跨税种×关联交易×综合——七维命中越多风险越高，全异常则系统性造假立案。本阶段的<strong>税款测算</strong>依据来源于下节分税种杀手锏——每条分析链按税种规则计算补税额、滞纳金、罚款区间。</p></section>';
  ...
  h+='<details style="margin-bottom:10px"><summary style="font-size:10px;font-weight:700;color:#16233a;cursor:pointer;padding:10px 0">🔗 线索链（42条精写·调查路径）</summary><div class="live"><div id="au-chains-data"></div></div></details>';
  h+='<details style="margin-bottom:10px"><summary style="font-size:10px;font-weight:700;color:#16233a;cursor:pointer;padding:10px 0">🔒 证据链（22条精写·验证标准）</summary><div class="live"><div id="au-evidence-data"></div></div></details>';
  ...
  // ═══ 第四层·过滤 ═══
  h+='<h2>第四层 · 疑点过滤 —— 把102条信号淬成3条铁证</h2><p>布网阶段{{domain_functions}}个域同时发动，会产生大量粗糙信号。把粗糙信号淬成铁证，靠的是三道过滤器。</p>';
