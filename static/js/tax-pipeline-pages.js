...
function renderUnifiedDomainPanel(container) {
  ...
  h += '<p><b>4. 有进无销→隐匿收入vs制造业加工推理链</b>——有进无销是税务合规中最常见的信号，但一刀切「高风险」是错误的。系统先判断企业类型——纯贸易企业有进无销确实是隐匿收入信号，但制造业企业采购原料加工成成品后品名不同，进销品名不匹配是正常的。诊断后风险焦点从「隐匿收入」转移至「加工链条是否真实」。推理路径：从进销存比对到行业模式判断：检查企业是纯贸易还是制造业——加工费+原材料信号；从行业模式判断到制造业诊断(如有加工信号)：有加工费+有原材料→制造业加工链条→风险焦点转移；从制造业诊断到BOM验证：要求企业提供BOM表验证原材料投入→加工→成品产出的逻辑；从BOM验证到综合结论：无法提供BOM→加工链条无法验证→有进无销仍可能是隐匿收入→回到高风险。关联规则（共15条）：20、21、22、23、55、63、64、80、210、216、338、537、721、1358、1502。</p>';
  h += '<p><b>5. 合同缺失→交易真实性存疑推理链（四层分层）</b>——合同缺失不是一刀切的问题——不是所有交易都需要合同。系统通过四层自动分层，将缺失的影响精准定位到必签和应签两类，避免把所有供应商都视为「需要合同」（那会要求连加油站、酒店都签合同——不切实际）。分层判断使风险结论更精准、更可操作。推理路径：从资料完备度检测到合同需求分层：对每个供应商按品名/金额/类型四层自动分类：必签/应签/可免/小额；从合同需求分层到交易真实性验证：四流合一缺了合同流→这些交易的商业实质无法通过书面证据验证；从交易真实性验证到分层风险判断：必签类缺合同=高风险；应签类缺合同=中风险；可免类无合同=正常；从分层风险判断到综合结论：缺失合同→四流不合一→税务合规可逐笔质疑交易真实性→虚开发票嫌疑→印花税漏缴。关联规则（共13条）：14、15、16、17、63、64、72、216、310、313、712、1775、1505。</p>';
