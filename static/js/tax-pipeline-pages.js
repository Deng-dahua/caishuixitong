...
function renderUnifiedDomainPanel(container) {
  ...
  var h = '';
  h += '<style>'
    + '.udp{max-width:1140px;margin:0 auto;padding:0;font-size:12.5px;line-height:2.0;color:#3a4048;font-family:-apple-system,"PingFang SC","Microsoft YaHei",sans-serif}'
    + '.udp h2{font-size:17px;font-weight:800;color:#16233a;margin:32px 0 12px;padding-bottom:10px;border-bottom:2px solid #16233a}'
    ...
    + '.udp .dom-table{width:100%;border-collapse:collapse;font-size:11px;margin:10px 0}'
    + '.udp .dom-table td{padding:6px 10px;border-bottom:1px solid #eff2f6;vertical-align:top;font-size:11.5px;line-height:1.8}'
    ...
    + '.udp .dom-table .ghead td{background:#fafbfc;padding:10px 10px;font-size:12px;border-bottom:1px solid #e2e8f0}'
    + '.udp .udp-block{background:#fafbfc;border:1px solid #eff2f6;border-radius:8px;padding:13px 15px;margin:4px 0 14px;font-size:12.5px;line-height:2.0}'
    ...
    + '.udp .udp-stat .val{font-size:22px;font-weight:800;color:#16233a;display:block;line-height:1.2}'
    + '.udp .udp-stat .lbl{font-size:11px;color:#64748b;display:block;margin-top:4px}'
    + '.udp .udp-rule{display:flex;gap:10px;align-items:flex-start;margin:6px 0;padding:10px 14px;background:#fafbfc;border-radius:6px;border-left:3px solid #e2e8f0;font-size:12.5px;line-height:1.95}'
    + '.udp .udp-rule .rn{font-size:12px;font-weight:800;color:#16233a;min-width:22px;text-align:center;flex-shrink:0}'
    + '.udp .udp-rule .rc{font-size:12.5px;line-height:1.95}'
    ...
    + '.udp .udp-chain{padding:14px 18px;margin-bottom:10px;background:#fff;border:1px solid #eff2f6;border-radius:8px;font-size:12.5px;line-height:1.9}'
    + '.udp .udp-step{padding:8px 14px;margin:4px 0;background:#f8fafc;border-radius:6px;font-size:12.5px;line-height:1.9;border-left:3px solid #e2e8f0}'
    + '.udp .udp-step .sn{display:inline-flex;align-items:center;justify-content:center;min-width:22px;height:22px;border-radius:50%;background:#16233a;color:#fff;font-size:11px;font-weight:700;margin-right:10px;flex-shrink:0}'
    + '.udp details{margin-bottom:8px}'
    + '.udp summary{font-size:13px;font-weight:700;color:#16233a;cursor:pointer;padding:6px 0}'
    + '</style>';
  h += '<div class="udp">';
  ...
  h += '<p style="font-size:11px;color:#94a3b8">数据要求标注：<b>必须</b>=缺则跳过该域；<b>应当</b>=缺则降级；<b>可选</b>=有则用。以finding.type为纽带，与线索链trigger_keywords和证据链dimensions的匹配关键词严格对齐。完整finding.type受控词表见附录。</p>';
  ...
  h += '<tr class="ghead"><td colspan="5"><b>资金流分析</b>（4域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_cash_flow_tracking</td><td>资金全链路追踪</td><td style="font-size:11px">银行流水（必须）</td><td style="font-size:11px;white-space:nowrap">≥10条</td><td style="font-size:11px">收款方异常、大额支付、频繁支付</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_fund_flow</td><td>资金流向追踪</td><td style="font-size:11px">银行流水（必须）</td><td style="font-size:11px;white-space:nowrap">≥10条</td><td style="font-size:11px">资金流向异常、单边大额资金</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_abnormal_transaction_time</td><td>异常交易时间</td><td style="font-size:11px">银行流水（必须）</td><td style="font-size:11px;white-space:nowrap">≥10条</td><td style="font-size:11px">非营业时间交易、节假日大额交易</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_personal_txn_risk</td><td>个人交易风险</td><td style="font-size:11px">银行流水（必须）</td><td style="font-size:11px;white-space:nowrap">≥10条</td><td style="font-size:11px">个人账户收款、对私转账高频</td></tr><tr class="ghead"><td colspan="5"><b>进销存分析</b>（4域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_purchase_sales_gross_margin</td><td>进销毛利率</td><td style="font-size:11px">销项发票+进项发票（必须）；科目余额表（应当）</td><td style="font-size:11px;white-space:nowrap">发票≥10张</td><td style="font-size:11px">毛利率偏离行业基准、进销倒挂</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_invoice_substantive_audit</td><td>发票实质性审计</td><td style="font-size:11px">发票数据（必须）</td><td style="font-size:11px;white-space:nowrap">≥10张</td><td style="font-size:11px">发票号码异常、税率异常、品名异常</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_inventory_turnover</td><td>存货周转预警</td><td style="font-size:11px">库存明细（必须）；发票数据（应当）</td><td style="font-size:11px;white-space:nowrap">库存记录≥5条</td><td style="font-size:11px">周转率异常、库存积压、快进快出</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_invoice_inventory_payment_triangle</td><td>三角验证</td><td style="font-size:11px">发票+入库单+付款单（必须）</td><td style="font-size:11px;white-space:nowrap">三单各≥3条</td><td style="font-size:11px">三单不一致、付款与入库不匹配</td></tr><tr class="ghead"><td colspan="5"><b>供应商与客户</b>（4域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_supplier_penetration</td><td>供应商穿透</td><td style="font-size:11px">进项发票（必须）</td><td style="font-size:11px;white-space:nowrap">≥5条</td><td style="font-size:11px">对私供应商、小规模供应商、新设公司</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_supplier_profile</td><td>供应商画像</td><td style="font-size:11px">进项发票+工商信息（必须）</td><td style="font-size:11px;white-space:nowrap">≥5条</td><td style="font-size:11px">供应商经营异常、失联供应商、风险供应商</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_upstream_downstream</td><td>上下游穿透</td><td style="font-size:11px">进项发票+销项发票（必须）</td><td style="font-size:11px;white-space:nowrap">各≥5条</td><td style="font-size:11px">供应商即客户、关联方重合</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_customer_three_source</td><td>客户三源穿透</td><td style="font-size:11px">销项发票（必须）</td><td style="font-size:11px;white-space:nowrap">≥5条</td><td style="font-size:11px">客户信息不一致、虚假客户</td></tr><tr class="ghead"><td colspan="5"><b>多源交叉验证</b>（5域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_multi_source_cross_validation</td><td>多源交叉验证</td><td style="font-size:11px">银行流水+发票+账载凭证+申报表（应当）；至少2源（必须）</td><td style="font-size:11px;white-space:nowrap">各源≥5条</td><td style="font-size:11px">四源偏差、单源异常</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_voucher_invoice_revenue_compare</td><td>凭证发票收入对比</td><td style="font-size:11px">会计凭证+销项发票（必须）</td><td style="font-size:11px;white-space:nowrap">各≥5条</td><td style="font-size:11px">凭证收入与发票不一致</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_profit_cashflow_contradiction</td><td>利润现金流矛盾</td><td style="font-size:11px">科目余额表+银行流水（必须）</td><td style="font-size:11px;white-space:nowrap">各≥10条</td><td style="font-size:11px">账面盈利现金枯竭、应收异常膨胀</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_revenue_time_investigation</td><td>收入时间线调查</td><td style="font-size:11px">销项发票+银行流水（必须）</td><td style="font-size:11px;white-space:nowrap">各≥10条</td><td style="font-size:11px">季末突击开票、收入集中化</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_extended_audit_rules</td><td>扩展审查规则</td><td style="font-size:11px">原始数据（可选）</td><td style="font-size:11px;white-space:nowrap">无最低要求</td><td style="font-size:11px">自定义信号</td></tr><tr class="ghead"><td colspan="5"><b>经营实质分析</b>（3域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_business_substance</td><td>经营实质分析</td><td style="font-size:11px">银行流水+发票+工资+社保（应当）；至少3源（必须）</td><td style="font-size:11px;white-space:nowrap">各源≥10条</td><td style="font-size:11px">人员不匹配、场地缺失、设备与产能不符</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_business_geography</td><td>经营地理分析</td><td style="font-size:11px">发票+银行流水（必须）；工商登记地址（应当）</td><td style="font-size:11px;white-space:nowrap">≥10条</td><td style="font-size:11px">注册地与经营地不一致、地址分散</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_personnel_business_matching</td><td>人员业务匹配</td><td style="font-size:11px">工资表+社保数据（必须）；个税申报（应当）</td><td style="font-size:11px;white-space:nowrap">≥5条</td><td style="font-size:11px">社保人数与业务规模不匹配、零参保</td></tr><tr class="ghead"><td colspan="5"><b>资料完备度</b>（2域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_material_completeness</td><td>资料完备度评估</td><td style="font-size:11px">所有上传文件（必须）</td><td style="font-size:11px;white-space:nowrap">无最低要求</td><td style="font-size:11px">资料缺失、资料不完备</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_material_intel</td><td>资料情报摘要</td><td style="font-size:11px">所有上传文件（必须）</td><td style="font-size:11px;white-space:nowrap">无最低要求</td><td style="font-size:11px">收购/重组/诉讼等关键情报</td></tr><tr class="ghead"><td colspan="5"><b>发票深度分析</b>（3域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_invoice_deep_features</td><td>发票深度特征</td><td style="font-size:11px">发票数据（必须）</td><td style="font-size:11px;white-space:nowrap">≥20张</td><td style="font-size:11px">连号开票、顶格开票、整数金额、夜间开票</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_invoice_lifecycle</td><td>发票生命周期</td><td style="font-size:11px">发票数据（必须）</td><td style="font-size:11px;white-space:nowrap">≥20张</td><td style="font-size:11px">领用与开具不符、异常作废</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_red_rush_invoice</td><td>红冲作废发票</td><td style="font-size:11px">发票数据（必须）</td><td style="font-size:11px;white-space:nowrap">≥20张</td><td style="font-size:11px">红冲频率过高、作废率大于15%</td></tr><tr class="ghead"><td colspan="5"><b>合同与凭证</b>（2域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_contract_comparison</td><td>合同比对</td><td style="font-size:11px">合同文件+发票（必须）</td><td style="font-size:11px;white-space:nowrap">合同≥1份，发票≥5张</td><td style="font-size:11px">合同金额与发票不符、合同缺失</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_voucher_account_anomaly</td><td>凭证科目异常</td><td style="font-size:11px">会计凭证（必须）</td><td style="font-size:11px;white-space:nowrap">≥10条</td><td style="font-size:11px">科目使用错误、摘要与科目不匹配</td></tr><tr class="ghead"><td colspan="5"><b>税务与社保</b>（3域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_tax_payment_consistency</td><td>税务缴纳一致性</td><td style="font-size:11px">纳税申报表（必须）；发票+银行流水（应当）</td><td style="font-size:11px;white-space:nowrap">申报表≥1份，其他≥5条</td><td style="font-size:11px">申报与缴纳不一致</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_vat_declaration_comparison</td><td>增值税申报比对</td><td style="font-size:11px">增值税申报表+发票（必须）</td><td style="font-size:11px;white-space:nowrap">≥5条</td><td style="font-size:11px">销项申报与发票不一致、进项申报异常</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_salary_social_security</td><td>工资社保比对</td><td style="font-size:11px">工资表+社保数据+个税申报（应当）；至少2源（必须）</td><td style="font-size:11px;white-space:nowrap">各≥5条</td><td style="font-size:11px">三数不一致</td></tr><tr class="ghead"><td colspan="5"><b>资产与关联交易</b>（2域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_asset_depreciation_amortization</td><td>资产折旧摊销</td><td style="font-size:11px">科目余额表（必须）；固定资产明细（应当）</td><td style="font-size:11px;white-space:nowrap">≥5条</td><td style="font-size:11px">折旧年限异常、摊销方法不合规</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_related_party_transaction</td><td>关联交易穿透</td><td style="font-size:11px">进项发票+销项发票+工商信息（必须）</td><td style="font-size:11px;white-space:nowrap">各≥5条</td><td style="font-size:11px">关联交易、转移定价、债资比超标</td></tr><tr class="ghead"><td colspan="5"><b>行业对标与规则</b>（4域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_industry_benchmark</td><td>行业基准对标</td><td style="font-size:11px">发票+科目余额表（必须）</td><td style="font-size:11px;white-space:nowrap">≥10条</td><td style="font-size:11px">偏离行业基准</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_rule_engine_match</td><td>规则引擎匹配</td><td style="font-size:11px">所有数据（必须）</td><td style="font-size:11px;white-space:nowrap">无最低要求</td><td style="font-size:11px">规则触发信号</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_tax_burden_analysis</td><td>税负率分析</td><td style="font-size:11px">纳税申报表+发票（应当）</td><td style="font-size:11px;white-space:nowrap">≥10条</td><td style="font-size:11px">税负率低于行业P25</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_trend_analysis</td><td>趋势分析</td><td style="font-size:11px">科目余额表+发票（必须）</td><td style="font-size:11px;white-space:nowrap">≥10条</td><td style="font-size:11px">指标异常波动</td></tr><tr class="ghead"><td colspan="5"><b>跨域分析链</b>（1域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_cross_analysis_chains</td><td>跨域分析链</td><td style="font-size:11px">所有域发现（必须）</td><td style="font-size:11px;white-space:nowrap">域发现≥1条</td><td style="font-size:11px">跨域线索→证据→分析三链贯通</td></tr><tr class="ghead"><td colspan="5"><b>补充税种</b>（3域）</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_stamp_duty_check</td><td>印花税</td><td style="font-size:11px">合同+发票+银行流水（应当）；至少1源满足最低条数（必须）</td><td style="font-size:11px;white-space:nowrap">≥5条</td><td style="font-size:11px">印花税漏缴</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_cit_reconciliation</td><td>企业所得税汇算</td><td style="font-size:11px">科目余额表（必须）；凭证（应当）</td><td style="font-size:11px;white-space:nowrap">≥10条</td><td style="font-size:11px">应纳税所得额调整</td></tr><tr><td style="font-size:11px;font-family:monospace;white-space:nowrap">_domain_export_vat_verification</td><td>出口退税核验</td><td style="font-size:11px">出口报关单+发票+收汇记录（必须）</td><td style="font-size:11px;white-space:nowrap">各≥3条</td><td style="font-size:11px">三单不匹配</td></tr>';
  h += '</table></div>';
  ...
  h += '<div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">1. 收款来源不匹配→隐匿收入推理链</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">从单一的收款来源不匹配信号出发，逐层扩展分析范围。第一步确认不匹配的规模，第二步核实付款方身份（排除法人/股东的打款），第三步判断资金性质（经营收入还是非经营资金），第四步对无法解释的收款做出隐匿收入的判断。每一步都有回退路径——只要企业能提供合理解释，风险就会降级或消除。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>资金流分析</b> 到 <b style="color:#0e7490">发票比对</b>：逐名交叉比对全部收款方与销项发票购方名称</div><div class="udp-step"><span class="sn">2</span>从 <b>发票比对</b> 到 <b style="color:#0e7490">付款方身份核实</b>：联网查询法定代表人/股东名单，比对未匹配的付款方</div><div class="udp-step"><span class="sn">3</span>从 <b>付款方身份核实</b> 到 <b style="color:#0e7490">资金性质判断</b>：区分注资/借款/往来款/未开票收入四种可能</div><div class="udp-step"><span class="sn">4</span>从 <b>资金性质判断</b> 到 <b style="color:#0e7490">综合结论</b>：无法说明来源的收款推定为未开票经营收入→隐匿收入风险</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：79、210、217、244、249、359、476、654、655、685...5条</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">2. 加工费来自外地→全链条经营异常推理链（点→面推理核心）</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">从加工费这一个单点异常出发，扩展到面的推理。第一步发现加工费在外地，第二步判断商业合理性（本地是否有集群），第三步扩展到三组地址对比，第四步检测运输成本缺失，第五步得出物理不可能的结论。这是点→面推理的标准案例——单点异常可解释，但四点叠加无法解释——换一个税务合规员拿同样资料，同样会得出这个结论。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（5步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>发票分析</b> 到 <b style="color:#0e7490">产业集群判断</b>：分析企业所在地是否有同类加工产业集群</div><div class="udp-step"><span class="sn">2</span>从 <b>产业集群判断</b> 到 <b style="color:#0e7490">商业合理性判断</b>：外地加工增加了运输成本和周期，舍近求远→商业逻辑存疑</div><div class="udp-step"><span class="sn">3</span>从 <b>商业合理性判断</b> 到 <b style="color:#0e7490">全链条地理分析</b>：提取三组地址做城市交叉比对</div><div class="udp-step"><span class="sn">4</span>从 <b>全链条地理分析</b> 到 <b style="color:#0e7490">物理合理性判断</b>：货物在N个城市间反复运输但无运输费→物理上不可能</div><div class="udp-step"><span class="sn">5</span>从 <b>物理合理性判断</b> 到 <b style="color:#0e7490">综合结论</b>：全链条经营实质存疑→所有跨省交易的真实性需要逐笔验证</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：55、80、216、1500、1501、1502、1504、1505、1510、999606...3条</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#d9770615;color:#d97706">中风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">3. 进销品名不匹配→企业类型重判推理链（三层穿透）</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">不从单一来源判定企业类型，而是通过三层递进穿透：工商登记看法律形式、发票数据看经营实质、加工信号看业务模式。三者不一致时以实质重于形式为原则。风险不是消除了，而是从品名问题转移到了加工真实性问题——这是风险转移而非风险消除。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>工商查询</b> 到 <b style="color:#0e7490">发票推断</b>：从全部发票品名关键词做66行业加权投票，得出发票推断行业</div><div class="udp-step"><span class="sn">2</span>从 <b>发票推断</b> 到 <b style="color:#0e7490">加工信号检测</b>：筛查进项发票中是否含加工费+是否存在原材料采购</div><div class="udp-step"><span class="sn">3</span>从 <b>加工信号检测</b> 到 <b style="color:#0e7490">三层综合判断</b>：工商登记=批发业(形式) + 发票推断=纺织制造(实质) + 加工信号=制造业(模式)</div><div class="udp-step"><span class="sn">4</span>从 <b>三层综合判断</b> 到 <b style="color:#0e7490">风险重定向</b>：风险从「有进无销=隐匿收入」转移至「加工链条是否真实」</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：218、368、390、694、1017、1018、1357、1444、1495、999501...5条</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">4. 有进无销→隐匿收入vs制造业加工推理链</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">有进无销是税务合规中最常见的信号，但一刀切「高 风险」是错误的。系统先判断企业类型——纯贸易企业有进无销确实是隐匿收入信号，但制造业企业采购原料加工成成品后品名不同，进销品名不匹配是正常的。诊断后风险焦点从「隐匿收入」转移至「加工链条是否真实」。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>进销存比对</b> 到 <b style="color:#0e7490">行业模式判断</b>：检查企业是纯贸易还是制造业——加工费+原材料信号</div><div class="udp-step"><span class="sn">2</span>从 <b>行业模式判断</b> 到 <b style="color:#0e7490">制造业诊断(如有加工信号)</b>：有加工费+有原材料→制造业加工链条→风险焦点转移</div><div class="udp-step"><span class="sn">3</span>从 <b>制造业诊断</b> 到 <b style="color:#0e7490">BOM验证</b>：要求企业提供BOM表验证原材料投入→加工→成品产出的逻辑</div><div class="udp-step"><span class="sn">4</span>从 <b>BOM验证</b> 到 <b style="color:#0e7490">综合结论</b>：无法提供BOM→加工链条无法验证→有进无销仍可能是隐匿收入→回到高风险</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：20、21、22、23、55、63、64、80、210、216...5条</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">5. 合同缺失→交易真实性存疑推理链（四层分层）</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">合同缺失不是一刀切的问题——不是所有交易都需要合同。系统通过四层自动分层，将缺失的影响精准定位到必签和应签两类，避免把所有供应商都视为「需要合同」（那会要求连加油站、酒店都签合同——不切实际）。分层判断使风险结论更精准、更可操作。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>资料完备度检测</b> 到 <b style="color:#0e7490">合同需求分层</b>：对每个供应商按品名/金额/类型四层自动分类：必签/应签/可免/小额</div><div class="udp-step"><span class="sn">2</span>从 <b>合同需求分层</b> 到 <b style="color:#0e7490">交易真实性验证</b>：四流合一缺了合同流→这些交易的商业实质无法通过书面证据验证</div><div class="udp-step"><span class="sn">3</span>从 <b>交易真实性验证</b> 到 <b style="color:#0e7490">分层风险判断</b>：必签类缺合同=高风险；应签类缺合同=中风险；可免类无合同=正常</div><div class="udp-step"><span class="sn">4</span>从 <b>分层风险判断</b> 到 <b style="color:#0e7490">综合结论</b>：缺失合同→四流不合一→税务合规可逐笔质疑交易真实性→虚开发票嫌疑→印花税漏缴</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：14、15、16、17、63、64、72、216、310、313...5条</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">6. 重物跨省缺运输→货物流断裂推理链</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">从地址分布出发，结合行业重物属性和运输成本缺失，形成货物流物证链断裂的推理。关键在于行业的重物判断——不是所有行业都需要运输成本（如软件/咨询/设计等服务业），但纺织/建材/机械/食品等重物产业必须有。系统根据行业自适应地判断重物属性，不搞一刀切。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>地址分析</b> 到 <b style="color:#0e7490">行业重物判断</b>：根据行业分类判断产品是否为需运输的重物</div><div class="udp-step"><span class="sn">2</span>从 <b>行业重物判断</b> 到 <b style="color:#0e7490">运输成本检测</b>：检索全部发票和银行流水中的运输/物流/快递/货运关键词</div><div class="udp-step"><span class="sn">3</span>从 <b>运输成本检测</b> 到 <b style="color:#0e7490">货物流判断</b>：重物跨省运输必有运费（通常占货值3%-8%），零运费=货物流物证链断裂</div><div class="udp-step"><span class="sn">4</span>从 <b>货物流判断</b> 到 <b style="color:#0e7490">综合结论</b>：货物流无法验证→三流合一缺了货物流→交易真实性存疑→进项税额+成本均面临否定风险</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：10、63、91、93、101、118、121、132、181、999...5条</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#d9770615;color:#d97706">中风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">7. 费用发票占比异常→虚增成本推理链</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">从费用结构异常出发，逐层深挖费用的真实性和关联性。关键判断在于费用与经营是否相关——招待费/差旅费可能有经营关联，但旅游/购物/生活用品等明显与经营无关。系统通过联网查询股东身份，将个人消费入账的风险精准定位。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>费用结构分析</b> 到 <b style="color:#0e7490">费用明细核查</b>：逐类筛查费用发票的品名/金额/时间/对方</div><div class="udp-step"><span class="sn">2</span>从 <b>费用明细核查</b> 到 <b style="color:#0e7490">股东关联判断</b>：联网查询股东和法定代表人，判断费用是否与股东个人消费相关</div><div class="udp-step"><span class="sn">3</span>从 <b>股东关联判断</b> 到 <b style="color:#0e7490">税前扣除判断</b>：与生产经营无关的支出不得税前扣除→需调增应纳税所得额</div><div class="udp-step"><span class="sn">4</span>从 <b>税前扣除判断</b> 到 <b style="color:#0e7490">综合结论</b>：虚增成本费用→利润虚减→企业所得税少缴+个人所得税漏缴</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：133、135、138、144、310、425、748、880、895、1117...5条</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">8. 进销比异常+零申报→账外经营推理链</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">进销比异常是最强的税务合规信号之一——采购量远超销售量的本质是「买了货但没有开发票卖出去」。但不是一看到进销比异常就定「账外经营」——需要先排除存货积压的可能。如果存货没有增加、采购款已付、但销项极少——那就是典型的账外经营信号。结合收入端（收款远超开票），形成进销两端闭合的证据链。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>进销比分析</b> 到 <b style="color:#0e7490">存货分析</b>：检查存货周转率——如果存货也没有对应增加则货物去向成谜</div><div class="udp-step"><span class="sn">2</span>从 <b>存货分析</b> 到 <b style="color:#0e7490">资金流追踪</b>：追踪银行付款去向——采购款付了但货去哪了？</div><div class="udp-step"><span class="sn">3</span>从 <b>资金流追踪</b> 到 <b style="color:#0e7490">收入端对比</b>：付款正常+进货正常+销项极少→货物已售出但未开票→账外经营</div><div class="udp-step"><span class="sn">4</span>从 <b>收入端对比</b> 到 <b style="color:#0e7490">综合结论</b>：进销比异常+零申报+收款开票偏差→完整的账外经营链条→严重隐匿收入</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：19、81、249、379、382、775、1144、1146、1424、1503...3条</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">9. 供应链六员交叉→关联交易推断链（人员→发票→资金三段式跨域）</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">通过人员信息→发票数据→资金流的三段式跨域推理：从发票中定位供应商/客户→联网核查其六员→与本企业六员交叉比对→发现重叠后核查资金流→确认关联交易并评估转移定价风险。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>联网核查被查单位+供应商/客户六员</div><div class="udp-step"><span class="sn">2</span>提取进销发票中的供应商/客户名称+金额</div><div class="udp-step"><span class="sn">3</span>逐名交叉比对→发现重叠</div><div class="udp-step"><span class="sn">4</span>核查重叠企业间资金往来</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：90、249、267、298、447、1504、1510、999606、999609</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">10. 购销闭环→虚开发票推理链（发票→进销存→资金三段式跨域）</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">从发票数据发现供应商=客户→形成购销闭环→核查进销商品流转逻辑是否合理→核查资金流是否形成回流→判定虚开发票风险等级。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>提取进项销售方+销项购买方</div><div class="udp-step"><span class="sn">2</span>交叉比对：供应商∩客户</div><div class="udp-step"><span class="sn">3</span>核查闭环企业间的商品流转逻辑</div><div class="udp-step"><span class="sn">4</span>核查闭环企业间资金流向</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：266、267、272、284、298、1504、1510、1511、999606、999609</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">11. 仓库→产能→能耗→物流全链条经营实质分析</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">单一矛盾可能是偶然→两个矛盾高度可疑→三个以上矛盾几乎可以定案。仓库装不下+设备产不出+电量不够用+物流费对不上=企业的账面经营纯粹是虚构的。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（5步）：</div><div class="udp-step"><span class="sn">1</span>从租金发票推算仓储面积→对比库存体积→仓容矛盾？</div><div class="udp-step"><span class="sn">2</span>从设备参数推算理论产能→对比账面产量→产能矛盾？</div><div class="udp-step"><span class="sn">3</span>从电费/水费推算能耗→对比行业单耗标准→能耗矛盾？</div><div class="udp-step"><span class="sn">4</span>从运输距离推算运费→对比账面运费→物流矛盾？</div><div class="udp-step"><span class="sn">5</span>四个维度交叉验证→几个维度出现矛盾？→矛盾越多置信度越高→从单点怀疑升级为系统性造假判定</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：267、1500、999501、999502、999503、999504、999605、999606</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">12. 系统性造假综合判定模型(七维交叉)</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">七维交叉验证模型——从经营实质、供应商穿透、资金流、三流合一、跨税种、关联交易七个维度综合评估企业造假可能性。单维异常是线索，多维异常是定案。七维全异常=企业账务大概率全面造假。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（7步）：</div><div class="udp-step"><span class="sn">1</span>库存体积vs仓容/设备产能vs产量/用电量vs生产规模/人员vs产出→几维矛盾？</div><div class="udp-step"><span class="sn">2</span>供应商五维评分(资质/时间/规模/价格/资金)→几个维度不通过？</div><div class="udp-step"><span class="sn">3</span>收付款匹配度/个人收款占比/资金回流迹象/过桥账户→资金流异常度</div><div class="udp-step"><span class="sn">4</span>发票/资金/物流/合同四流一致性→几流断裂？</div><div class="udp-step"><span class="sn">5</span>增值税vs所得税/个税vs社保/印花税vs购销→几税申报不一致？</div><div class="udp-step"><span class="sn">6</span>关联方交易占比/转移定价/资本弱化/费用分摊→关联交易风险度</div><div class="udp-step"><span class="sn">7</span>七维综合评分→0-2维异常=低风险→3-4维=中风险→5-6维=高风险→7维=系统性造假→触发全面税务合规立案</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：63、90、130、226、228、267、272、364、374、447...5条</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">13. 隐匿收入综合判定模型</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">三线归一法：资金偏差线(收款-开票)=来源分类线(未开票收入)=消费验证线(超申报消费)。三条线独立推算的隐匿收入金额应当大致相等。金额越接近→证据越强→结论越不可辩驳。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>计算收款-开票偏差的绝对金额和相对比例→分月度/季度看趋势</div><div class="udp-step"><span class="sn">2</span>对未匹配收款逐笔分类→借款/注资/代收/未开票/其他→计算各类占比</div><div class="udp-step"><span class="sn">3</span>追踪个人账户收款和异常消费→推算最低隐匿收入规模</div><div class="udp-step"><span class="sn">4</span>三指标交叉验证→偏差金额≈未开票收入≈隐匿消费→一致即定案</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：20、21、22、23、210、622、623、624、625、626...5条</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#d9770615;color:#d97706">中风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">14. 经营实质地理分析</span></div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>发票数据</b> 到 <b style="color:#0e7490">地址提取</b>：从发票提取购方/销方和交易地址</div><div class="udp-step"><span class="sn">2</span>从 <b>地址提取</b> 到 <b style="color:#0e7490">三方比对</b>：注册地、经营地、交易地城市级三方比对</div><div class="udp-step"><span class="sn">3</span>从 <b>三方比对</b> 到 <b style="color:#0e7490">运输成本检测</b>：检索银行流水和发票中的运输/物流/货运费用</div><div class="udp-step"><span class="sn">4</span>从 <b>运输成本检测</b> 到 <b style="color:#0e7490">综合结论</b>：三地不同+零运费=物理上不可能。全链条经营实质存疑</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#d9770615;color:#d97706">中风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">15. 税务合规六员跨企业比对</span></div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>工商信息</b> 到 <b style="color:#0e7490">六员提取</b>：提取被查企业及进销方TOP10的六员名单</div><div class="udp-step"><span class="sn">2</span>从 <b>六员提取</b> 到 <b style="color:#0e7490">交叉比对</b>：逐名交叉比对。同一人出现两家企业则标记</div><div class="udp-step"><span class="sn">3</span>从 <b>交叉比对</b> 到 <b style="color:#0e7490">关联方标记</b>：标记六员重叠的企业对为潜在关联方</div><div class="udp-step"><span class="sn">4</span>从 <b>关联方标记</b> 到 <b style="color:#0e7490">关联交易穿透</b>：执行价格公允性+资金占用+债资比检测</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#d9770615;color:#d97706">中风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">16. 供应链联网核查（全链条人员交叉比对）</span></div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>进销发票</b> 到 <b style="color:#0e7490">TOP10提取</b>：提取进项TOP10和销项TOP10的六员信息</div><div class="udp-step"><span class="sn">2</span>从 <b>TOP10提取</b> 到 <b style="color:#0e7490">全链条交叉</b>：将20家企业的六员做全排列交叉比对</div><div class="udp-step"><span class="sn">3</span>从 <b>全链条交叉</b> 到 <b style="color:#0e7490">闭环检测</b>：检测供应商A→客户B→供应商A的闭环模式</div><div class="udp-step"><span class="sn">4</span>从 <b>闭环检测</b> 到 <b style="color:#0e7490">综合结论</b>：闭环成立→供应链存在隐藏关联→利益输送风险</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#d9770615;color:#d97706">中风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">17. 资金回流检测法</span></div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>从 <b>银行流水</b> 到 <b style="color:#0e7490">对手方网络</b>：构建全部收付款方的关系网络图</div><div class="udp-step"><span class="sn">2</span>从 <b>对手方网络</b> 到 <b style="color:#0e7490">闭环检测</b>：检测A→B→A或A→B→C→A的闭环路径（时间差<90天，偏差<5%）</div><div class="udp-step"><span class="sn">3</span>从 <b>闭环检测</b> 到 <b style="color:#0e7490">发票验证</b>：核查闭环内各节点发票。有发票=过账，无发票=返还</div><div class="udp-step"><span class="sn">4</span>从 <b>发票验证</b> 到 <b style="color:#0e7490">综合结论</b>：回流成立→虚开发票/虚构交易。多笔闭环叠加则立案</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">18. 全税种综合判定模型</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">多税种交叉验证。单一税种异常可能是疏忽，3+税种同时异常=系统性偷逃税。综合判定降低误报。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（4步）：</div><div class="udp-step"><span class="sn">1</span>收集各税种触发发现</div><div class="udp-step"><span class="sn">2</span>逐税种比对申报vs实际</div><div class="udp-step"><span class="sn">3</span>≥3税种异常→系统性偷逃税</div><div class="udp-step"><span class="sn">4</span>多税种累计偷税额→处罚档次</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：210、228、880、1497、999631、999678</div></div><div class="udp-chain"><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:600;background:#dc262615;color:#dc2626">高风险</span><span style="font-size:14px;font-weight:700;color:#0f172a">19. 物耗能耗产能三线归一分析</span></div><div style="font-size:13px;color:#64748b;margin-bottom:6px;line-height:1.9">产量可以从产能、电耗、物耗三个独立维度推算。三维交叉印证→单一维偏差可能是统计误差，三维全偏=造假。</div><div style="font-size:12px;font-weight:600;color:#94a3b8;margin:8px 0 6px">推理路径（5步）：</div><div class="udp-step"><span class="sn">1</span>设备铭牌×工时→理论最大产量</div><div class="udp-step"><span class="sn">2</span>电费÷行业单位电耗→推算产量</div><div class="udp-step"><span class="sn">3</span>原材料投入÷行业投入产出比→推算产量</div><div class="udp-step"><span class="sn">4</span>三维推算→中位数 vs 账面产量</div><div class="udp-step"><span class="sn">5</span>三维全偏低且偏差>30%→系统性造假</div><div style="font-size:10px;color:#94a3b8;margin-top:6px">关联规则：258、445、999605</div></div>';
  ...
  h += '<div class="udp-block"><p style="font-size:11px;color:#94a3b8">域函数产出的finding.type必须从此表中取值。各finding.type的匹配关键词为独立集合，不同finding.type之间不共享匹配关键词——精确匹配，匹配不上就会漏触发。完整词表及唯一性约束见engine/memory.py。</p>';
...
function renderMethodologyContent(){
  ...
  h+='<div class="maxim"><b>四句铁律贯穿全部七层（后文不再重述）：</b>实质重于形式（登记不算、干的才算）；孤证不立（单源数据不定案）；疑点非结论（起点是疑点、落点是铁证）；宁存疑不错杀（说不清的标存疑、有铁证的才下定论）。</div></section>';

  // ═══ 第一层·启动 ═══
  h+='<div class="layer" id="au-L1"><div class="ln">第一层</div><div class="lt">启动 —— 资料包一打开，先认人再认门</div><div class="ld">引擎拿到企业上传的全部资料。此时不急着翻任何一份文件——先做三件事：锁定身份、穿透行业、立起标尺。</div></div>';
  ...
  h+='<section id="au-s3"><h2>三相符·四流合一：立起一切判断的标尺（术语唯一权威定义处）</h2><p>身份和行业定了，接下来引擎在心里立起两把尺。</p><div class="card"><div class="ct">三相符 —— 防的是"报得对不对"</div><div class="cx">账载、票载、申报三套数据<strong>同口径一致</strong>。银行流水是外部基准——收入端对四方（账载、银行、开票、申报），一处不符即为疑点。这条线贯穿后面五层中每一次比对。</div></div><div class="card"><div class="ct">四流合一（术语唯一权威定义处） —— 防的是"业务真不真"</div><div class="cx"><em>本节为"四流合一"的唯一权威定义处，后文战法④及跨域协商皆引用此定义，不再重述。</em>合同流、货物流、资金流、发票流指向同一主体、同一金额、同一时点。四流齐全为真实交易；一流游离即藏猫腻；两流以上对不上——警报拉满。引擎把这四个维度设为每条证据链的必查项。</div></div><p>两把尺交叉，引擎面对任何一笔交易、任何一个数字，都可以问出那两句："该报的报了吗？"和"该有的事情真有吗？"</p></section>';

  // ═══ 第二层·扫描 ═══
  h+='<div class="layer" id="au-L2"><div class="ln">第二层</div><div class="lt">扫描 —— 把散落文件变成结构化情报</div><div class="ld">身份已定、标尺已立。引擎现在翻开全部上传文件，逐份识别类型、逐行提取情报、锁定第一批疑点。</div></div>';
  ...
  h+='<section id="au-s6"><h2>三大突破口：情报提取完毕后，先攻最脆弱的三个点（四方比对唯一定义处）</h2><div class="step"><div class="sh"><span class="n">①</span>银行流水 · 命门</div><p>钱必须真金白银地走，最难造假。引擎在这一步做<strong>四方比对（本节为唯一定义处，后文不再重述）</strong>——把银行收款和账载收入、发票开具、纳税申报四套口径摊开，进账>开票→无票收入需核查；进账>账载→款项未入账直指隐匿。</p></div><div class="step"><div class="sh"><span class="n">②</span>发票 · 关卡</div><p>进销两端都要看。进项品名是否与实质行业匹配（做广告却进钢材→虚开信号）；销项品名与经营范围匹配性；红冲作废频率异常放大。</p></div><div class="step"><div class="sh"><span class="n">③</span>往来款 · 暗门</div><p>最容易埋东西的地方：预收账款长期挂账不转收入、应付账款异常膨胀——金额大+账龄长+对手可疑=必有蹊跷。股东借款跨年不还视同分红问题详见第三层战法⑥关联交易穿透。</p></div></section>';
  // 新增 s7 时间序列分析
  h+='<section id="au-s7"><h2>时间序列分析：跨期波动是隐匿的指纹</h2><p>引擎对关键财务指标做<strong>月份环比/季度同比/年度趋势</strong>三维度扫描，识别三类异常模式。</p>';
  h+='<div class="step"><div class="sh"><span class="n">①</span>收入季节性断裂</div><p>同行业正常波动范围内，年末/季末收入突增或骤降（偏离均值±2σ）→跨期调节信号。引擎自动对比相邻两期开票时点与收款时点，识别"开票集中化、收款均匀化"的时间错配。</p></div>';
  h+='<div class="step"><div class="sh"><span class="n">②</span>成本费用脉冲</div><p>大额费用集中在某月突发（如咨询费单月超全年50%）→可能虚列或跨期调节。引擎追踪费用的发票开具时间、付款时间、服务合同签署时间——三时点不一致即为疑点。</p></div>';
  h+='<div class="step"><div class="sh"><span class="n">③</span>往来款周期性波动</div><p>应收/应付/预收/预付的余额在特定月份规律性冲减→可能循环倒账。引擎绘制科目余额时间曲线，识别"月初大额挂账、月末集中冲平"的周期模式。</p></div>';
...
function renderMethodologyPart2(){
  var h='';
  // ═══ 第三层·布网 ═══
  h+='<div class="layer" id="au-L3"><div class="ln">第三层</div><div class="lt">布网 —— 火力全开，四链齐发</div><div class="ld">情报池就绪。引擎全线激活规则引擎→线索链调查→证据链闭环→分析链综合推理。{{rules_count}}条指令+{{clue_chains}}条线索+{{evidence_chains}}条证据+48条跨域分析链+{{domain_functions}}个域分析函数——同时启动、四阶段递进。这是引擎的全力一击。</div></div>';
  ...
  h+='<div class="step"><div class="sh"><span class="n">Phase2</span> 深挖·线索链逐条调查（引用六大战法为方法库）</div><p>Phase1的信号被投喂给{{clue_chains}}条线索链——每条是一条完整的调查路径（investigation_path，1-15步），从"这个信号该从哪查"到"查到了什么结果"。本阶段的<strong>调查方法</strong>来源于下节六大战法——每条线索链可调用战法①-⑥中的任一方法组合。三类触发：<strong>定量阈值</strong>（数值超限）、<strong>定性模式</strong>（特定关键词匹配）、<strong>缺失数据</strong>（资料缺口触发替代链）。</p></div>';
  ...
  h+='<div class="step"><div class="sh"><span class="n">Phase4</span> 综合定性·跨域分析链判定（引用分税种为计算依据）</div><p>48条跨域分析链做多源综合推理（每条含多步推理路径，每步带回退分支——能进能退）。如"七维系统性造假判定模型"：经营实质×供应商×资金流×四流合一×跨税种×关联交易×综合——七维命中越多风险越高，全异常则系统性造假立案。本阶段的<strong>税款测算</strong>依据来源于下节分税种杀手锏——每条分析链按税种规则计算补税额、滞纳金、罚款区间。</p></div></section>';

  h+='<section id="au-s9"><h2>六大战法：Phase2深挖的方法库</h2><p class="adv">{{domain_functions}}个域分析函数并行审视所有数据——同一笔银行流水，在资金流域看收款来源、在经营实质域看费用结构、在税务域看税费支出。以下是Phase2调查阶段可调用的核心战法（战法①资金流与第二层突破口①四方比对内容一致，仅作引用不重述）。</p>';
  h+='<div class="step"><div class="sh"><span class="n">①</span>资金流突破（引用第二层s6突破口①）</div><p><em>本战法与第二层s6突破口①四方比对内容一致，此处仅作引用。具体内容参见第二层s6。</em></p></div>';
  h+='<div class="step"><div class="sh"><span class="n">②</span>收入完整性（视同销售为唯一权威定义处）</div><p>三条线：<strong>完整性</strong>——所有渠道（对公、私户、现金、平台、微信支付宝）收款是否全部入账；<strong>及时性</strong>——纳税义务时点是否被人为推迟（预收挂账、跨期调节）；<strong>视同销售（唯一权威定义处，后文不再重述）</strong>——赠送/投资/职工福利/以物易物/抵债/非货币性资产交换——最容易遗漏，引擎单列排查。</p></div>';
  h+='<div class="step"><div class="sh"><span class="n">③</span>成本真实性（吸收识伪图谱增量）</div><p>引擎做<strong>进销匹配</strong>：进的料和销的品，在产品链上必须对得上——进钢材销服装且无加工=猫腻。同时做<strong>成本收入配比</strong>：收入没涨成本猛增→毛利断崖→虚增信号。<strong>凭证合规</strong>：大额费用须有合法有效税前扣除凭证，白条、顶替发票、无票列支不得扣除。特别盯紧<strong>虚列高发科目</strong>：咨询费/服务费/会议费/佣金——集中爆发无合同无成果交付=纯虚列。<strong>识伪图谱增量检测</strong>：无货虚增、虚构人员工资、费用化资本性支出。</p></div>';
  h+='<div class="step"><div class="sh"><span class="n">④</span>四流合一查虚开（引用第一层s3定义）</div><p><em>四流合一定义见第一层s3。本战法为应用层执行。</em>引擎把每笔交易的合同流、货物流、资金流、发票流在数据库里对齐比对：票款一致但货没动→无货虚开；货票匹配但款由第三方付且回流→资金过账；合同与发票品名金额不符→变名开票。四流查通，虚开现形。</p></div>';
  h+='<div class="step"><div class="sh"><span class="n">⑤</span>人场货查空壳（吸收两套账）</div><p>引擎做三向比对：<strong>人</strong>——社保参保数+个税申报数是否匹配业务规模（年入千万零参保→查哪来的人）。<strong>场</strong>——注册地是否有水电物业费、仓储物流记录（无办公痕迹=纸面企业）。<strong>货</strong>——购销品名与经营范围匹配性、物流凭证齐全度、进销存计量合理性。三者全空的，被标为"空壳嫌疑"。<strong>两套账与体外循环检测</strong>：内账（真实经营数据）与外账（报税数据）之间的差额即逃税证据。</p></div>';
  h+='<div class="step"><div class="sh"><span class="n">⑥</span>关联交易穿透（唯一权威定义处）</div><p><em>本战法为关联交易穿透的唯一权威定义处，后文不再独立设节。</em>引擎从股东/董监高/共同地址电话/资金往来维度自动绘制<strong>关联方网络图</strong>（含隐性关联）。然后四步穿透：Step1比对关联价格与独立第三方价→偏离度超30%触发预警；Step2追利润流向（高税负向低税负低卖高买→腾挪）；Step3查关联方无息大额资金占用→视同贷款利息收入；Step4关联债资比超2:1（金融5:1）触发资本弱化。股东借款跨年不还视同分红（20%）在此战法一并检测。</p></div></section>';
  ...
  h+='<div class="gi"><b>企业所得税</b>·收入与调整：政府补助计税、投资收益/债务重组/资产处置遗漏、视同销售、跨期收入。扣除项：<strong>招待费限额（唯一权威定义处）</strong>——发生额60%且不超过营收5‰，孰低原则；广宣费15%（化妆品/医药/饮料30%）；福利费14%；工资须实发且与社保个税勾稽；折旧摊销年限方法；研发加计归集合规性</div>';
  h+='<div class="gi"><b>个人所得税</b>·私户/现金发薪未扣缴、股东借款跨年视同分红(20%)、股权转让平价无理由、发票报销顶替工资、多处取酬未合并。三数勾稽：工资发放人数=社保参保=个税申报</div>';
  h+='<div class="gi"><b>社保</b>·缴费基数低于实发、试用期不参保、劳务派遣责任、参保人数与个税申报不一致</div>';
  h+='<div class="gi"><b>残保金</b>·未按在岗人数1.5%（各地比例）申报缴纳、减免政策适用错误</div>';
  h+='<div class="gi"><b>印花税</b>·合同台账逐笔对税目、实收资本+资本公积增加未缴、产权转移书据漏缴</div>';
  h+='<div class="gi"><b>房产税</b>·自用从价/出租从租之争、地下建筑未入原值</div>';
  h+='<div class="gi"><b>土地使用税</b>·面积不符、等级税率适用错误</div>';
  h+='<div class="gi"><b>资源税</b>·开采量与销售量比对、税率适用错误（从价/从量）、减免政策适用</div>';
  h+='<div class="gi"><b>契税</b>·房屋/土地权属转移未缴、计税依据错误、减免政策适用</div>';
  h+='<div class="gi"><b>土地增值税</b>·房地产企业清算条件、增值率计算、扣除项目归集、预征率与清算率差异</div>';
  h+='<div class="gi"><b>附加税费</b>·随增值税补缴：城建7%/5%/1%、教育附加3%、地方教育附加2%</div>';
  h+='</div></section>';
  ...
  h+='<div class="grid">';
  h+='<div class="gi"><b>制造业</b>·进销存匹配（原材料→产成品链）、加工费/委外加工检测、BOM分析、生产成本归集、存货周转异常</div>';
  h+='<div class="gi"><b>房地产</b>·预售收入确认、土地增值税清算、成本分摊、关联交易定价、车位/储藏室单独核算</div>';
  h+='<div class="gi"><b>建筑业</b>·工程进度确认、甲供材处理、劳务分包、跨区域涉税、预缴税款抵扣</div>';
  h+='<div class="gi"><b>金融业</b>·利息收入确认、手续费佣金、不良资产处置、同业业务、资管产品增值税</div>';
  h+='<div class="gi"><b>互联网平台</b>·平台服务费、用户补贴、流量变现、数据资产、跨境支付</div>';
  h+='<div class="gi"><b>商贸零售</b>·进销差价分析、库存周转、促销折扣、会员积分、团购/批零差异</div>';
  h+='<div class="gi"><b>餐饮住宿</b>·现金收入核查、食材成本率、会员储值卡、团购平台结算、外包服务</div>';
  h+='<div class="gi"><b>交通运输</b>·燃油成本占比、车辆租赁/自有、过路费、跨区域运营、物流代理</div>';
  h+='<div class="gi"><b>医药医疗</b>·研发费用加计、临床试验外包、医疗器械租赁、药品采购渠道、医保结算</div>';
  h+='<div class="gi"><b>教育培训</b>·预收款确认、课时消耗、教材收入、线上/线下差异、民办非企业税收</div>';
  h+='</div><p><em>行业包为增量检测规则——不替代通用规则，在通用规则基础上叠加行业特化维度。未匹配行业的，仅执行通用规则。</em></p></section>';
...
function renderMethodologyPart3(){
  var h='';
  // ═══ 第五层·定案 ═══
  h+='<div class="layer" id="au-L5"><div class="ln">第五层</div><div class="lt">定案 —— 从"可能有"到"就是有"</div><div class="ld">前四层产出了一批经得起推敲的发现。这一层是稽查的灵魂——把发现固定成推不翻的铁案。</div></div>';
  ...
  h+='<div class="card"><div class="ct">征管法§63 偷税（伪造/变造/隐匿/擅自销毁账簿，虚假申报）</div><div class="cx">罚款：<strong>税款50%至5倍</strong>。情节严重（金额巨大/屡查屡犯/对抗检查）从重；主动补缴从轻。</div></div>';
  h+='<div class="card"><div class="ct">征管法§64 少缴税款（不申报，但无伪造/变造/隐匿账册行为）</div><div class="cx">罚款：<strong>税款50%至5倍</strong>。与偷税差别在于主观故意证据——无账册造假/无资金回流→倾向于少缴。</div></div>';
  h+='<div class="card"><div class="ct">征管法§68 抗税（暴力/威胁方法拒不缴纳税款）</div><div class="cx">罚款：<strong>拒缴税款1倍至5倍</strong>。情节严重的，移送司法机关追究刑事责任。</div></div></section>';
  h+='<section id="au-s16"><h2>定性分寸：主观故意判定依据（征管法§63四种手段）</h2><p>引擎区分三个层次：</p>';
  h+='<div class="step"><div class="sh"><span class="n">①</span>偷税（征管法§63）</div><p>有<strong>四种法定手段之一</strong>且造成不缴/少缴结果：伪造/变造账簿记账凭证、在账簿上多列支出/不列/少列收入、隐匿/擅自销毁账簿记账凭证、虚假纳税申报。<strong>证据闭环要求</strong>：资金回流+内外账不符+反复操作+销毁隐匿账册——≥3项闭环。</p></div>';
  h+='<div class="step"><div class="sh"><span class="n">②</span>少缴税款（征管法§64）</div><p>有少申报事实但<strong>无主观故意证据</strong>（偶发性计算错误、政策理解偏差、未涉及账册造假）。罚款区间与偷税相同，但量刑轻重不同。</p></div>';
  h+='<div class="step"><div class="sh"><span class="n">③</span>虚开发票</div><p>有证据证明交易不存在或票面信息与实际交易不符。<em>定性准不准，是法律判决能不能立住的关键。够什么级别定什么级，不拔高不放过。</em></p></div></section>';
  h+='<section id="au-s17"><h2>对抗性自检：把案子反过来审</h2><p>稽查不是单向指控——每一条结论写完，引擎必须<strong>反过来</strong>做三件事，确保铁案推不翻。</p>';
  h+='<div class="step"><div class="sh"><span class="n">①</span>反向假设验证</div><p>假设发现的异常信号存在合法商业解释——是否能用正常业务逻辑自圆其说？如果能（如季节性/新开业/大单结算），降档；如果所有合理解释都不成立，强化的不是怀疑、是证据。</p></div>';
  h+='<div class="step"><div class="sh"><span class="n">②</span>对手方交叉比对</div><p>引擎搜索数据库中的交易对手方信息——检查对手方对该笔交易的记录是否一致（金额/时间/性质）。如果对手方已申报而本方未申报，隐匿证据成立。</p></div>';
  h+='<div class="step"><div class="sh"><span class="n">③</span>政策适用性复查</div><p>检查结论引用的法规是否现行有效、是否存在特殊优惠或过渡期政策覆盖该事项。以最新法规库为准，不引用失效条款。</p></div></section>';

  // ═══ 第六层·出鞘 ═══
  h+='<div class="layer" id="au-L6"><div class="ln">第六层</div><div class="lt">出鞘 —— 老稽查员交了卷</div><div class="ld">前五层打完，引擎手里攥着一批铁证。这一层是收口——把铁证写成铁案报告，交给稽查员过目。</div></div>';
  ...
  h+='<div class="card"><div class="ct">五维风险热力图</div><div class="cx"><strong>收入端</strong>（无票收入/私户收款/跨期调节权重30%）→<strong>成本端</strong>（虚列成本/无票列支权重20%）→<strong>税种合规</strong>（各税种穿透权重25%）→<strong>证据链强度</strong>（闭环数量权重15%）→<strong>行业对标</strong>（基准偏离度权重10%）。每维度0-100分，加权得出总分。</div></div>';
  ...
  // ═══ 第七层·进化 ═══
  h+='<div class="layer" id="au-L7"><div class="ln">第七层</div><div class="lt">进化 —— 引擎自己学会长大</div><div class="ld">前六层是"老稽查员教引擎做事"。这一层是"引擎自己学会长大"——从每一条发现、每一次判定中汲取经验，校准规则、发现新模式、同步政策变更。</div></div>';
  ...
  // 独立支线·税收优惠
  h+='<div class="layer" id="au-s24-layer"><div class="ln">独立支线</div><div class="lt">税收优惠扫描 —— 引擎唯一的正向分析</div><div class="ld">以上七层全是"找问题"，这一条支线是"找好处"——扫描企业应享未享的税收优惠，为纳税人省钱。</div></div>';
  ...
  h+='<div class="maxim"><b>七层毕、一卷成、引擎长。</b>启动定身份、扫描提情报、布网铺火力、过滤淬铁证、定案下结论、出鞘交铁卷、进化学新招。引擎就是这样被教会的——不是学了一堆散装规则，而是掌握了从进门到出鞘再到自我进化的完整功夫。按下一键分析，背后的是这七层递进的引擎；看完这份卷，背后的是五十年的稽查经验；而引擎的进化，来自每一次判定后的反思。</div>';
