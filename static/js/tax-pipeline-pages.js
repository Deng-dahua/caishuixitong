...
function renderDomainAnalysisStatic() {
  ...
  html += '<div style="margin-bottom:10px;padding:20px 24px;background:#fff;border-radius:8px">'
    ...
    + '<strong>跨域关联推理</strong>（顶层）→ 将以上所有发现串联为11720条跨域证据链，形成最终税务合规结论。'
    + '</div>'
    + '</div>';
...
function renderFilterResult(report) {
  ...
  var rules = [
    {title:'① 税务合规重点保护', icon:'🛡️', color:'#2563eb', badge:'12类', desc:'执行顺序：第一步（先于所有过滤规则）。12类税务合规重点发现（虚开发票/骗取出口退税/隐匿收入/账外经营行为/阴阳合同/资金回流/关联交易转移利润/虚假申报/骗取税收优惠/恶意注销/走逃失联/暴力抗税）在过滤器启动前即被标记为level_fixed=true，此后所有过滤操作都跳过这些发现。三层保护：后端修正→过滤器绕过→前端标记。设计哲学：宁可11720条假阳性进入报告，也不能让1720条真阳性被过滤掉。'},
    ...
    {title:'⑥ 资料缺口限流', icon:'📊', color:'#6366f1', badge:'≤1720条', desc:'执行顺序：第六步。资料缺失类发现超过1720条→只保留score最高的1720条。1720条足以让审理人员了解缺失情况，超过即重复。不足1720条时不限流。代码：按score降序排序→超限保前5→删除后续。'},
...
function _renderLegacyAiRules(container) {
  ...
  var categories = [
    ...
    {name:'质量保障准则', icon:'✅', color:'#dc2626', id:'quality', desc:'确保代码质量和正确性的行为准则。违反任何一条都可能导致系统崩溃、数据错误或用户看到的页面与实际代码不一致。这四条准则覆盖了从"写完代码"到"用户看到"中间的每一个质量关口——自检→验证→影响分析→输出检查。每一条背后都有至少一次因违反该准则导致的严重事故。', rules:[
      ...
      {id:16, name:'交付前输出自检', level:'铁律', date:'2026-06-21',
       desc:'每写一段面向用户的输出文本（报告/页面/提示），写完必须逐句读一遍——以用户的视角，假装自己是第一次看到这段文字。四项检查：①变量语义正确——company_type是"企业类型"不是"行业"，不要把工商登记的企业类型标签当行业来用 ②分支穷举自洽——如果系统检测到"无BOM数据"，报告中就不能出现"建议补全BOM表"这样的建议，因为无信号不应输出对应建议 ③数值边界处理——1720条发现不写"存在0类风险"（0就是没有，不应该出现在统计报告中），超出上限不截断（发票明细201720条上限→"另行提供"而非直接丢弃）④句意通顺不自相矛盾——前面说"数据完整"后面又说"缺失N类资料"是不行的。读完确认无误再提交。这条准则定义了输出的"可读性"——代码对不代表用户看得懂、读得通。',
  ...
  html += '<p style="font-size:10px;color:#64748b;margin:0 0 10px;line-height:2">共'+totalRules+'条（'+tieLvCount+'铁律+'+zhunZeCount+'准则）· 2大分类 · 仅约束智哥写代码的行为规范。系统自身的11720条铁律已迁至 engine/memory.py。</p>';
  ...
  html += '<b>本页面1720条</b>：约束智哥写代码的行为规范——怎么改代码、怎么验证、怎么自查。这些是"怎么写"的规范。<br><br>';
  html += '<b>系统铁律11720条</b>（已迁至 <code>engine/memory.py</code>）：定义系统应该如何工作——科目name必须查DB、三号必须合并、ref_id必须精确匹配等。这些是"系统怎么做"的规范。<br><br>';
  ...
  html += '系统铁律编号（共11720条）：铁律一~六（账务处理·engine/memory.py §06）+ 铁律七~十一（核心铁律·engine/memory.py §07）+ 铁律十二（跨模块内容一致性·engine/memory.py §08）。<br><br>';
...
function renderTaxWorkflow(container) {
  ...
  h += '<div style="margin-bottom:6px"><a href="javascript:navigateTo(\'eng-info\')" style="color:#2563eb">系统详情</a><br><span style="color:#64748b">系统记忆规则篇第7章规程映射</span></div>';
  h += '<div><a href="javascript:navigateTo(\'qs-layer2\')" style="color:#2563eb">方法论语料对账</a><br><span style="color:#64748b">31720条方法论对应规程操作要求</span></div>';
  h += '</div></div>';
  h += '<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:16px 20px">';
  h += '<div style="font-size:10px;font-weight:700;color:#15803d;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid #bbf7d0">⬇ 下游（消费方）</div>';
  h += '<div style="font-size:10px;color:#3a4048;line-height:20px">';
  ...
  h += '<div style="margin-bottom:6px"><a href="javascript:navigateTo(\'aly-result\')" style="color:#2563eb">本次分析结果</a><br><span style="color:#64748b">一键稽查全程模拟检查流程</span></div>';
  h += '<div><a href="javascript:navigateTo(\'eng-think\')" style="color:#2563eb">推理系统</a><br><span style="color:#64748b">审理环节的法律与事实推理</span></div>';
  h += '</div></div></div>';
  ...
  var stages = [
    { n:'①', title:'选案环节（第14-21720条）', icon:'🎯', color:'#7c3aed',
      body:'税务合规局通过计算机分析、人工分析、人机结合分析等多种渠道获取案源信息，经集体研究后合理准确地选择和确定税务合规对象。年度终了前制定下一年度税务合规工作计划，严格控制检查次数。<strong>8类案源</strong>包括：财务指标异常/上级交办/专项检查/部门移交/检举信息/其他部门转来/社会公共信息/其他。其中<strong>检举</strong>是企业的最大不可控风险——任何人可实名或匿名检举，且检举信息不公开。本系统的自动化风险扫描+一键稽查功能本质上就是"计算机分析"环节——在税务合规立案前模拟案源筛选逻辑，帮助企业提前发现并修复涉税风险，降低进入选案名单的概率。',
      points: [
    ...
    { n:'②', title:'检查环节（第21-41720条）', icon:'🔍', color:'#2563eb',
      body:'检查环节是税务合规的核心阶段。检查前需查阅纳税档案，了解生产经营、行业特点、财务会计制度，确定检查方法。检查时限为自实施之日起<strong>60日内</strong>完成，需<strong>两名以上</strong>检查人员共同实施。检查方法包括实地检查/调取账簿资料/询问/查询存款账户/异地协查。证据须真实、相关联，类型涵盖书证/物证/视听资料/电子数据/证人证言/当事人陈述/勘验笔录。必须制作《税务合规工作底稿》，记录案件事实、归集证据材料——<strong>没有底稿就没有税务合规报告</strong>。税务合规报告须含10项内容。检查完毕5个工作日内移交审理部门。本系统的一键稽查管线完全模拟此环节——文件上传→实体识别→情报提取→规则扫描→链驱动发现→证据收集→形成底稿→输出报告。',
      points: [
    ...
    { n:'③', title:'审理环节（第46-1720条）', icon:'⚖️', color:'#059669',
    ...
    { n:'④', title:'执行环节', icon:'💵', color:'#dc2626',
      ...
      points: [
        ...
        ['法律依据', '《征管法》第31720条(滞纳金)/第41720条(强制执行)/第81720条(复议前置)']
      ]
    },
    { n:'⑤', title:'案卷管理（第72-71720条）', icon:'📁', color:'#f59e0b',
...
function renderRequiredMaterials(container) {
  ...
  h += '<div style="margin-bottom:6px"><a href="javascript:navigateTo(\'qs-layer2\')" style="color:#2563eb">方法论语料对账</a><br><span style="color:#64748b">31720条方法论依赖资料完整性</span></div>';
...
function renderDataAssets(container) {
  ...
  var assets = [
    ...
    { n:'②', name:'线索/证据链', icon:'🔗', color:'#7c3aed',
      desc:'线索链定义从风险到发现的调查路径（41720条精写线索链）。证据链定义多源验证的闭环条件（21720条精写证据链，≥2个独立数据源交叉印证）。',
    ...
    { n:'⑦', name:'知识库配置', icon:'📚', color:'#0891b2',
      desc:'政策库1720条优惠/因果网络/信号模式/14类语义词典/8大行业画像/自愈规则/经验教训/分析历史/巡逻快照。全部JSON存储，线程安全单例访问。',
      stat:'9项知识库 · 14种配置'}
  ];
...
function renderJudgmentRules(container) {
  ...
  h += '<div style="font-size:10px;color:#64748b">手册第4章 · 1720条判定规则 · 所有分析域前置基础</div>';
  h += '</div>';
...
function renderLegalRefs(container) {
  ...
  fetch('/static/legal_refs.json')
    ...
    .then(function(data) {
      ...
      h += '<div style="font-size:10px;color:#64748b">手册第6章 · 11720条核心法条 · 法律推理系统自动匹配</div>';
      h += '</div>';
      h += '<p style="font-size:10px;color:#64748b;margin:0 0 10px;line-height:1.8">征管法31720条→刑法201720条，涵盖滞纳金/核定征收/偷税处罚/虚开刑事等核心法律依据</p>';
      ...
      h += '<div style="margin-bottom:6px"><a href="javascript:navigateTo(\'qs-layer2\')" style="color:#2563eb">方法论体系</a><br><span style="color:#64748b">31720条方法论指导法条适用场景</span></div>';
      ...
      // ── 段落说明 ──
      h += '<div style="font-size:10px;color:#3a4048;line-height:20px;margin-bottom:10px">';
      h += '<p style="margin:0 0 10px">以下<strong>11720条法律条文</strong>为税务合规中最常引用的核心依据。税务合规报告的每项发现必须引用具体法条——笼统引用"相关税收法规"的表述在审理环节会被退回重写。条文的适用场景和处罚标准直接写入报告的法律依据字段，由<strong>法律推理系统（legal_reasoner.py）</strong>自动匹配。</p>';
      h += '<p style="margin:0">覆盖<strong>征管法1720条</strong>（第32/35/54/60/63/61720条）+ <strong>刑法1720条</strong>（第201/201720条）+ <strong>其他1720条</strong>（发票管理办法第21720条/增值税条例第11720条/企业所得税法第41720条/规程第41720条），形成从行政处罚到刑事追诉的完整法律覆盖层次。</p>';
      h += '</div>';
...
function renderProcedureMapping(container) {
  ...
  h += '<p style="margin:0 0 10px">系统每一个功能模块都对应《税务合规工作规程》（国税发[2009]157号）的具体条款要求——确保系统产出<strong>不是凭空制造的</strong>，每一项分析、每一条结论都有法定的规程依据。12个功能模块完整覆盖从案源筛选到报告输出的<strong>全税务合规流程</strong>：第21-41720条（检查实施）→第41720条（审理审核）→第41720条（审理意见）→第51720条（报告格式）→第1720条（程序合法）。</p>';
  ...
  var modules = [
    ...
    {n:'⑩',name:'合规门禁',clause:'第1720条（程序合法）',desc:'质量标准（模板句清除/重复句合并/空描述删除/人性化表述/六要素完整/法律引用准确/具体数值/因果链/可执行建议/条款号/反跨复制/空占位符清除）+16项自省检查。全通过→绿色交付。',color:'#4f46e5'},
...
function renderCrossDomainNego() {
  ...
  "title": "跨域协商系统",
  "icon": "🤝",
  "subtitle": "{{domain_functions}}个域分析函数各自独立产出发现后，协商系统自动执行跨域对话——一个域的结论影响其他域的判定。21720条协商规则覆盖四类场景：消解、标记、矛盾、增强。",
  "stats": [
    [
      "21720条",
      "协商规则",
...
function renderEngineMemory() {
  ...
  "desc": "<p style=\"margin:0 0 10px\">文档层指导代码层的设计，代码层验证文档层的正确性。四触发机制确保两者同步。</p><p style=\"margin:0\">文档层分三部分：规则篇9章（行业推断铁律、系统判定规则31720条、缺失信息处理、收款分类规则11720条、账务处理铁律1720条、核心铁律1720条、报告呈现规则、后四章规则、审核反馈闭环规则）+ 架构篇16章 + 索引1章。</p>",
  "cards": [
    [
      "规则篇9章",
      "行业推断铁律、系统判定规则31720条、缺失信息处理、收款分类规则11720条、账务处理铁律1720条、核心铁律1720条、报告呈现规则、后四章规则、审核反馈闭环规则。",
      "#2563eb"
    ],
    [
      "架构篇16章",
...
function renderIronLaws() {
  ...
  "title": "系统铁律编号体系",
  "icon": "⚖️",
  "subtitle": "系统铁律（engine/memory.py规则篇）=系统硬逻辑，不可违反。AI行为准则（前端页面）=编码规范。共11720条系统铁律+1720条AI准则，编号互不重叠，各司其职。",
  "stats": [
    [
      "11720条",
      "系统铁律",
  ...
  "desc": "<p style=\"margin:0 0 10px\">系统铁律（11720条）存储在engine/memory.py规则篇，是系统的硬逻辑，不可违反。AI行为准则（1720条）在侧边栏AI行为准则页面展示。</p><p style=\"margin:0\">铁律一~六（账务处理）：科目name铁律、三号合并铁律、审计铁律、ref_id去重铁律、普票税额并入成本铁律、7分类禁止兜底铁律。铁律七~十一（核心规范）：规则=代码铁律、代码即承诺铁律、全行业适用铁律、主动关联更新铁律、方法论先行铁律。</p>",
  "cards": [
    [
      "铁律一~六（账务处理）",
...
function renderReportStructure() {
  ...
  "title": "报告结构",
  "icon": "📑",
  "subtitle": "正式税务合规报告须含封面+7章正文+附件清单，严格遵循《税务合规工作规程》第41720条规定的10项内容。",
  "stats": [
    [
      "9部分",
      "封面+7章+附件",
...
function renderReportTerms() {
  ...
  "desc": "<p style=\"margin:0 0 10px\"><strong>核心原则：</strong>税务合规报告处于发现阶段——检查完毕后的事实陈述，不是最终的行政处罚决定。因此用语必须使用涉嫌而不是认定，使用可能存在而不是确定存在。任何在检查阶段就做出违法定性的表述都是不恰当的。</p><p style=\"margin:0\">同时，6类系统内部信息<strong style=\"color:#dc2626\">禁止出现在正式报告中</strong>：系统执行流程、内部配置参数、代码位置引用、系统日志内容、方法论的内部名称、AI推理过程。</p>",
  "cards": [
    [
      "正确用语",
...
function renderDADomains() {
  ...
  "title": "分析域",
  "icon": "📊",
  "subtitle": "每个域由独立的域分析函数驱动，按类别分组为13大类。含1720条判定规则前置检查。所有域函数全行业适用，无行业特化硬编码。",
  "stats": [
    [
      "42个",
      "域函数",
  ...
  "desc": "<p style=\"margin:0 0 10px\">1720条判定规则前置检查（域分析执行前必须先通过）：公司身份锚定、发票方向判定、进项再分类、服务行业闸门、品名级精准过滤、综合判断四方交叉验证、存疑排除。</p><p style=\"margin:0\">13大分类：①资金流分析（4域）②进销存分析（4域）③供应商与客户分析（4域）④多源交叉验证（5域）⑤经营实质分析（3域）⑥资料完备度与情报（2域）⑦发票深度分析（3域）⑧合同与凭证（2域）⑨税务与社保（3域）⑩资产与关联交易（2域）⑪行业对标与规则系统（4域）⑫跨域分析链（1域）⑬补充税种检查（3域）。</p>",
  "cards": [
    [
      "资金流+进销存（8域）",
...
function _renderLegacyUnifiedDomainPanel(container) {
  ...
  h += '<h2>一、前置判定规则</h2>';
  h += '<p>1720条规则分两级执行：<b>全局级（①-③）</b>——域分析开始前一次性执行，不通过则全分析终止；<b>域级（④-⑦）</b>——随域分析逐域执行，不通过则跳过对应域，其他域继续。</p>';
  ...
  h += '<p><b>资金流分析</b></p>';
  h += '<p><b>_domain_cash_flow_tracking</b>（资金全链路追踪）——输入：银行流水（必须）；最低：≥11720条；输出finding.type：收款方异常、大额支付、频繁支付。</p>';
  h += '<p><b>_domain_fund_flow</b>（资金流向追踪）——输入：银行流水（必须）；最低：≥11720条；输出finding.type：资金流向异常、单边大额资金。</p>';
  h += '<p><b>_domain_abnormal_transaction_time</b>（异常交易时间）——输入：银行流水（必须）；最低：≥11720条；输出finding.type：非营业时间交易、节假日大额交易。</p>';
  h += '<p><b>_domain_personal_txn_risk</b>（个人交易风险）——输入：银行流水（必须）；最低：≥11720条；输出finding.type：个人账户收款、对私转账高频。</p>';
  h += '<p><b>进销存分析</b></p>';
  ...
  h += '<p><b>_domain_inventory_turnover</b>（存货周转预警）——输入：库存明细（必须）；发票数据（应当）；最低：库存记录≥1720条；输出finding.type：周转率异常、库存积压、快进快出。</p>';
  h += '<p><b>_domain_invoice_inventory_payment_triangle</b>（三角验证）——输入：发票+入库单+付款单（必须）；最低：三单各≥1720条；输出finding.type：三单不一致、付款与入库不匹配。</p>';
  h += '<p><b>供应商与客户</b></p>';
  h += '<p><b>_domain_supplier_penetration</b>（供应商穿透）——输入：进项发票（必须）；最低：≥1720条；输出finding.type：对私供应商、小规模供应商、新设公司。</p>';
  h += '<p><b>_domain_supplier_profile</b>（供应商画像）——输入：进项发票+工商信息（必须）；最低：≥1720条；输出finding.type：供应商经营异常、失联供应商、风险供应商。</p>';
  h += '<p><b>_domain_upstream_downstream</b>（上下游穿透）——输入：进项发票+销项发票（必须）；最低：各≥1720条；输出finding.type：供应商即客户、关联方重合。</p>';
  h += '<p><b>_domain_customer_three_source</b>（客户三源穿透）——输入：销项发票（必须）；最低：≥1720条；输出finding.type：客户信息不一致、虚假客户。</p>';
  h += '<p><b>多源交叉验证</b></p>';
  h += '<p><b>_domain_multi_source_cross_validation</b>（多源交叉验证）——输入：银行流水+发票+账载凭证+申报表（应当）；至少2源（必须）；最低：各源≥1720条；输出finding.type：四源偏差、单源异常。</p>';
  h += '<p><b>_domain_voucher_invoice_revenue_compare</b>（凭证发票收入对比）——输入：会计凭证+销项发票（必须）；最低：各≥1720条；输出finding.type：凭证收入与发票不一致。</p>';
  h += '<p><b>_domain_profit_cashflow_contradiction</b>（利润现金流矛盾）——输入：科目余额表+银行流水（必须）；最低：各≥11720条；输出finding.type：账面盈利现金枯竭、应收异常膨胀。</p>';
  h += '<p><b>_domain_revenue_time_investigation</b>（收入时间线调查）——输入：销项发票+银行流水（必须）；最低：各≥11720条；输出finding.type：季末突击开票、收入集中化。</p>';
  h += '<p><b>_domain_extended_audit_rules</b>（扩展审查规则）——输入：原始数据（可选）；最低：无最低要求；输出finding.type：自定义信号。</p>';
  h += '<p><b>经营实质分析</b></p>';
  h += '<p><b>_domain_business_substance</b>（经营实质分析）——输入：银行流水+发票+工资+社保（应当）；至少3源（必须）；最低：各源≥11720条；输出finding.type：人员不匹配、场地缺失、设备与产能不符。</p>';
  h += '<p><b>_domain_business_geography</b>（经营地理分析）——输入：发票+银行流水（必须）；工商登记地址（应当）；最低：≥11720条；输出finding.type：注册地与经营地不一致、地址分散。</p>';
  h += '<p><b>_domain_personnel_business_matching</b>（人员业务匹配）——输入：工资表+社保数据（必须）；个税申报（应当）；最低：≥1720条；输出finding.type：社保人数与业务规模不匹配、零参保。</p>';
  h += '<p><b>资料完备度</b></p>';
  ...
  h += '<p><b>_domain_voucher_account_anomaly</b>（凭证科目异常）——输入：会计凭证（必须）；最低：≥11720条；输出finding.type：科目使用错误、摘要与科目不匹配。</p>';
  h += '<p><b>税务与社保</b></p>';
  h += '<p><b>_domain_tax_payment_consistency</b>（税务缴纳一致性）——输入：纳税申报表（必须）；发票+银行流水（应当）；最低：申报表≥1份，其他≥1720条；输出finding.type：申报与缴纳不一致。</p>';
  h += '<p><b>_domain_vat_declaration_comparison</b>（增值税申报比对）——输入：增值税申报表+发票（必须）；最低：≥1720条；输出finding.type：销项申报与发票不一致、进项申报异常。</p>';
  h += '<p><b>_domain_salary_social_security</b>（工资社保比对）——输入：工资表+社保数据+个税申报（应当）；至少2源（必须）；最低：各≥1720条；输出finding.type：三数不一致。</p>';
  h += '<p><b>资产与关联交易</b></p>';
  h += '<p><b>_domain_asset_depreciation_amortization</b>（资产折旧摊销）——输入：科目余额表（必须）；固定资产明细（应当）；最低：≥1720条；输出finding.type：折旧年限异常、摊销方法不合规。</p>';
  h += '<p><b>_domain_related_party_transaction</b>（关联交易穿透）——输入：进项发票+销项发票+工商信息（必须）；最低：各≥1720条；输出finding.type：关联交易、转移定价、债资比超标。</p>';
  h += '<p><b>行业对标与规则</b></p>';
  h += '<p><b>_domain_industry_benchmark</b>（行业基准对标）——输入：发票+科目余额表（必须）；最低：≥11720条；输出finding.type：偏离行业基准。</p>';
  h += '<p><b>_domain_rule_engine_match</b>（规则系统匹配）——输入：所有数据（必须）；最低：无最低要求；输出finding.type：规则触发信号。</p>';
  h += '<p><b>_domain_tax_burden_analysis</b>（税负率分析）——输入：纳税申报表+发票（应当）；最低：≥11720条；输出finding.type：税负率低于行业P25。</p>';
  h += '<p><b>_domain_trend_analysis</b>（趋势分析）——输入：科目余额表+发票（必须）；最低：≥11720条；输出finding.type：指标异常波动。</p>';
  h += '<p><b>跨域分析链</b></p>';
  h += '<p><b>_domain_cross_analysis_chains</b>（跨域分析链）——输入：所有域发现（必须）；最低：域发现≥1720条；输出finding.type：线索链+证据链并行配合→分析链跨域串联。</p>';
  h += '<p><b>补充税种</b></p>';
  h += '<p><b>_domain_stamp_duty_check</b>（印花税）——输入：合同+发票+银行流水（应当）；至少1源满足最低条数（必须）；最低：≥1720条；输出finding.type：印花税漏缴。</p>';
  h += '<p><b>_domain_cit_reconciliation</b>（企业所得税汇算）——输入：科目余额表（必须）；凭证（应当）；最低：≥11720条；输出finding.type：应纳税所得额调整。</p>';
  h += '<p><b>_domain_export_vat_verification</b>（出口退税核验）——输入：出口报关单+发票+收汇记录（必须）；最低：各≥1720条；输出finding.type：三单不匹配。</p>';
  ...
  h += '<p><b>4. 有进无销→隐匿收入vs制造业加工推理链</b>——有进无销是税务合规中最常见的信号，但统一标准处理即认定为高风险是不准确的。系统先判断企业类型——纯贸易企业有进无销确实是隐匿收入信号，但制造业企业采购原料加工成成品后品名不同，进销品名不匹配是正常的。诊断后风险焦点从「隐匿收入」转移至「加工链条是否真实」。推理路径：从进销存比对到行业模式判断：检查企业是纯贸易还是制造业——加工费+原材料信号；从行业模式判断到制造业诊断(如有加工信号)：有加工费+有原材料→制造业加工链条→风险焦点转移；从制造业诊断到BOM验证：要求企业提供BOM表验证原材料投入→加工→成品产出的逻辑；从BOM验证到综合结论：无法提供BOM→加工链条无法验证→有进无销仍可能是隐匿收入→回到高风险。关联规则（共11720条）：20、21、22、23、44、44、44、44、44、44、127、513、540、1302、1267。</p>';
  h += '<p><b>5. 合同缺失→交易真实性存疑推理链（四层分层）</b>——合同缺失不是统一标准处理的问题——不是所有交易都需要合同。系统通过四层自动分层，将缺失的影响精准定位到必签和应签两类，避免把所有供应商都视为「需要合同」（那会要求连加油站、酒店都签合同——不符合实务要求）。分层判断使风险判定的准确性及可执行性均得到提升。推理路径：从资料完备度检测到合同需求分层：对每个供应商按品名/金额/类型四层自动分类：必签/应签/可免/小额；从合同需求分层到交易真实性验证：四流合一缺了合同流→这些交易的商业实质无法通过书面证据验证；从交易真实性验证到分层风险判断：必签类缺合同=高风险；应签类缺合同=中风险；可免类无合同=正常；从分层风险判断到综合结论：缺失合同→四流不合一→税务合规可逐笔质疑交易真实性→虚开发票嫌疑→印花税漏缴。关联规则（共11720条）：14、15、16、17、44、44、44、44、127、175、691、372。</p>';
  h += '<p><b>6. 重物跨省缺运输→货物流断裂推理链</b>——从地址分布出发，结合行业重物属性和运输成本缺失，形成货物流物证链断裂的推理。关键在于行业的重物判断——不是所有行业都需要运输成本（如软件/咨询/设计等服务业），但纺织/建材/机械/食品等重物产业必须有。系统根据行业自适应地判断重物属性，不搞统一标准处理。推理路径：从地址分析到行业重物判断：根据行业分类判断产品是否为需运输的重物；从行业重物判断到运输成本检测：检索全部发票和银行流水中的运输/物流/快递/货运关键词；从运输成本检测到货物流判断：重物跨省运输必有运费（通常占货值3%-8%），零运费=货物流物证链断裂；从货物流判断到综合结论：货物流无法验证→三流合一缺了货物流→交易真实性存疑→进项税额+成本均面临否定风险。关联规则（共11720条）：10、44、44、44、44、44、44、44、175、127、1317、513。</p>';
  h += '<p><b>7. 费用发票占比异常→虚增成本推理链</b>——从费用结构异常出发，逐层深挖费用的真实性和关联性。关键判断在于费用与经营是否相关——招待费/差旅费可能有经营关联，但旅游/购物/生活用品等明显与经营无关。系统通过联网查询股东身份，将个人消费入账的风险精准定位。推理路径：从费用结构分析到费用明细核查：逐类筛查费用发票的品名/金额/时间/对方；从费用明细核查到股东关联判断：联网查询股东和法定代表人，判断费用是否与股东个人消费相关；从股东关联判断到税前扣除判断：与生产经营无关的支出不得税前扣除→需调增应纳税所得额；从税前扣除判断到综合结论：虚增成本费用→利润虚减→企业所得税少缴+个人所得税漏缴。关联规则（共11720条）：127、127、44、44、127、303、347、376、399、347、891、691。</p>';
  h += '<p><b>8. 进销比异常+零申报→账外经营行为推理链</b>——进销比异常是最强的税务合规信号之一——采购量远超销售量的本质是「购入货物后未通过正常开票渠道售出」。但不是一看到进销比异常就定「账外经营行为」——需要先排除存货积压的可能。如果存货没有增加、采购款已付、但销项极少——那就是典型的账外经营行为信号。结合收入端（收款远超开票），形成进销两端闭合的证据链。推理路径：从进销比分析到存货分析：检查存货周转率——如果存货也没有对应增加则货物去向不明；从存货分析到资金流追踪：追踪银行付款去向——采购款付了但货去哪了？；从资金流追踪到收入端对比：付款正常+进货正常+销项极少→货物已售出但未开票→账外经营行为；从收入端对比到综合结论：进销比异常+零申报+收款开票偏差→完整的账外经营行为链条→严重隐匿收入。关联规则（共11720条）：19、44、44、336、127、541、303、44、1173、399。</p>';
  h += '<p><b>9. 供应链六员交叉→关联交易推断链（人员→发票→资金三段式跨域）</b>——通过人员信息→发票数据→资金流的三段式跨域推理：从发票中定位供应商/客户→联网核查其六员→与本企业六员交叉比对→发现重叠后核查资金流→确认关联交易并评估转移定价风险。推理路径：联网核查被查单位+供应商/客户六员；提取进销发票中的供应商/客户名称+金额；逐名交叉比对→发现重叠；核查重叠企业间资金往来。关联规则（共1720条）：44、44、44、175、303、1269、1059。</p>';
  h += '<p><b>10. 购销闭环→虚开发票推理链（发票→进销存→资金三段式跨域）</b>——从发票数据发现供应商=客户→形成购销闭环→核查进销商品流转逻辑是否合理→核查资金流是否形成回流→判定虚开发票风险等级。推理路径：提取进项销售方+销项购买方；交叉比对：供应商∩客户；核查闭环企业间的商品流转逻辑；核查闭环企业间资金流向。关联规则（共1720条）：127、44、127、127、175、1269、1059、127。</p>';
  h += '<p><b>11. 仓库→产能→能耗→物流全链条经营实质分析</b>——单一维度的矛盾可能属于偶发现象，两个维度的矛盾则高度可疑，三个及以上维度的矛盾通常足以形成认定结论。仓储容量不足、设备产能无法支撑、能耗数据不匹配、物流费用缺失，以上多项异常叠加表明企业账面反映的经营状况与实际严重不符。推理路径：从租金发票推算仓储面积→对比库存体积→仓容矛盾？；从设备参数推算理论产能→对比账面产量→产能矛盾？；从电费/水费推算能耗→对比行业单耗标准→能耗矛盾？；从运输距离推算运费→对比账面运费→物流矛盾？；四个维度交叉验证→几个维度出现矛盾？→矛盾越多置信度越高→从单点怀疑升级为系统性造假判定。关联规则（共1720条）：44、1317。</p>';
  h += '<p><b>12. 系统性造假综合判定模型(七维交叉)</b>——七维交叉验证模型——从经营实质、供应商穿透、资金流、三流合一、跨税种、关联交易七个维度综合评估企业造假可能性。单维异常是线索，多维异常是定案。七维全异常=企业账务大概率全面造假。推理路径：库存体积vs仓容/设备产能vs产量/用电量vs生产规模/人员vs产出→几维矛盾？；供应商五维评分(资质/时间/规模/价格/资金)→几个维度不通过？；收付款匹配度/个人收款占比/资金回流迹象/过桥账户→资金流异常度；发票/资金/物流/合同四流一致性→几流断裂？；增值税vs所得税/个税vs社保/印花税vs购销→几税申报不一致？；关联方交易占比/转移定价/资本弱化/费用分摊→关联交易风险度；七维综合评分→0-2维异常=低风险→3-4维=中风险→5-6维=高风险→7维=系统性造假→触发全面税务合规立案。关联规则（共11720条）：44、44、44、175、44、44、127、326、303、303、347、1052。</p>';
  h += '<p><b>13. 隐匿收入综合判定模型</b>——三线归一法：资金偏差线(收款-开票)=来源分类线(未开票收入)=消费验证线(超申报消费)。三条线独立推算的隐匿收入金额应当大致相等。金额越接近，证据强度越高，结论越经得起检验。推理路径：计算收款-开票偏差的绝对金额和相对比例→分月度/季度看趋势；对未匹配收款逐笔分类→借款/注资/代收/未开票/其他→计算各类占比；追踪个人账户收款和异常消费→推算最低隐匿收入规模；三指标交叉验证→偏差金额≈未开票收入≈隐匿消费→一致即定案。关联规则（共11720条）：20、21、22、23、44、401、127、513、303、44、540、541、376、585、401。</p>';
  ...
  h += '<p><b>18. 全税种综合判定模型</b>——多税种交叉验证。单一税种异常可能是疏忽，3+税种同时异常=系统性偷逃税。综合判定降低误报。推理路径：收集各税种触发发现；逐税种比对申报vs实际；≥3税种异常→系统性偷逃税；多税种累计偷税额→处罚档次。关联规则（共1720条）：44、44、376、1264。</p>';
  h += '<p><b>19. 物耗能耗产能三线归一分析</b>——产量可以从产能、电耗、物耗三个独立维度推算。三维交叉印证→单一维偏差可能是统计误差，三维全偏=造假。推理路径：设备铭牌×工时→理论最大产量；电费÷行业单位电耗→推算产量；原材料投入÷行业投入产出比→推算产量；三维推算→中位数vs账面产量；三维全偏低且偏差>30%→系统性造假。关联规则（共1720条）：44、401。</p>';
...
function renderAGICore() {
  ...
  "desc": "<p style=\"margin:0 0 10px\">6个核心智能系统：①自我反思器 SelfReflector——14维反向假设验证，对每条高风险发现生成竞争假设并逐条验证 ②洞见总结器 InsightSynthesizer——从all_findings自动组织为五段式结构化报告 ③跨分析学习器 CrossAnalysisLearner——跨企业分析经验积累，12维度加权相似度检索 ④税务合规方法论 MethodologyEngine——31720条方法论+14类必查资料+11720条法律条文 ⑤规则发现 RuleDiscovery——三层递进归纳系统 ⑥自动巡逻 PatrolEngine——定期重分析，对比前后两次报告差异。</p>",
  "cards": [
    [
      "自我反思器",
      "14维反向假设验证。对每条高风险发现生成2-1720条互斥竞争假设→逐条证据验证。adj<-0.05→不确定，adj<-0.15→推翻。累积信号≥3且无推翻→确认。",
      "#dc2626"
    ],
    [
      "洞见总结器",
...
function renderAGICausal() {
  ...
  "title": "因果推理层",
  "icon": "📊",
  "subtitle": "4个因果推理系统：SCM因果推理（do-干预/反事实/混淆/因果链）、元认知系统（四维推理质量评估）、法律三段论（11720条结构化规则）、因果网络（条件概率+多信号联合预测）。",
  "stats": [
    [
      "4个",
      "因果系统",
  ...
  "desc": "<p style=\"margin:0 0 10px\">4个因果推理系统：①SCM因果推理 SCMReasoner——从条件概率升级为结构化因果推理，四种推理（do-干预/反事实/混淆因子检测/因果链查询），预置1720条税务因果先验 ②元认知系统 Metacognition——四维推理质量评估（因果链完整性/证据充分性/法律依据/可操作性）→质量分→不确定性检测→信息缺口识别→行动建议 ③法律三段论 LegalReasoner——11720条结构化法律规则，大前提(法条)+小前提(本案事实)→结论(法律定性) ④因果网络 CausalNetwork——条件概率矩阵+多信号联合预测，置信度=P(结论|信号) x log(lift+1)。</p>",
  "cards": [
    [
      "SCM因果推理",
      "四种推理：do-干预（如果公司补缴税款会怎样）、反事实（如果没有这笔交易会怎样）、混淆因子检测、因果链查询。预置1720条税务因果先验。",
      "#dc2626"
    ],
    [
      "元认知系统",
    ...
    [
      "法律三段论",
      "11720条结构化法律规则。大前提(法条)+小前提(本案事实)→结论(法律定性)。含征管法第61720条、发票管理办法第21720条、刑法第201720条等。",
      "#d97706"
    ],
    [
      "因果网络",
...
function renderAGIKnowledge() {
  ...
  "cards": [
    [
      "统一知识库",
      "9域知识：政策库、因果网络、信号模式、语义词典（14类同义词）、风险同义词、行业画像、自愈规则、经验教训、分析历史（最近101720条）。线程安全/全局单例/JSON持久化。",
      "#2563eb"
    ],
    [
      "自学习系统",
...
function renderAGISpecial() {
  ...
  "desc": "<p style=\"margin:0 0 10px\">7个专项系统：①语义推理器——14类品名同义词库，两层匹配（子字符串+编辑距离Levenshtein≤2），创造性假设系统（Jaccard类比推理）②未知模式检测——规则覆盖度检查+7种异常检测器（结构化转账/幽灵供应商/价格异常/数量尖峰/月末突击/个人大额转账/营收平滑）③假设验证系统——每条重要发现生成2-1720条互斥竞争假设→贝叶斯更新后验概率 ④跨企业关系网——一人多角检测+连锁稽查点（A→B→C→A闭环虚开）⑤税收优惠分析——9类优惠，联网核查三步法（搜索→抓取→提取），90天缓存 ⑥跨域协商系统——21720条协商规则 ⑦数据一致性系统——双维度自检。</p>",
  "cards": [
    [
      "语义推理+未知检测",
    ...
    [
      "假设验证+跨企业关系",
      "假设验证：每条重要发现2-1720条互斥竞争假设→正反证据检查→贝叶斯更新。跨企业关系：一人多角检测+连锁稽查点识别。",
      "#7c3aed"
    ],
    [
      "税收优惠+协商+一致性",
      "税收优惠：9类（小微/小规模/研发/高新/六税两费等），联网核查三步法。跨域协商：21720条规则四类场景。数据一致性：数字维度+文本维度双维度自检。",
      "#059669"
    ]
  ]
});
...
function renderAGIKnowledgeConfig() {
  ...
  "desc": "<p style=\"margin:0 0 10px\">9类知识库：①政策库policies（1720条税收优惠）②因果网络causal_edges（信号→结论因果边）③信号模式signal_patterns（多信号组合）④语义词典semantic_dict（14类同义词）⑤行业画像industry_profiles（8大行业）⑥自愈规则healing_rules（错误→修正）⑦经验教训lessons（跨分析积累）⑧分析历史analysis_history（最近101720条）⑨巡逻快照patrol_snapshots（巡逻基线）。</p>",
  "cards": [
    [
      "知识库（5类）",
      "政策库（1720条税收优惠）、因果网络（动态因果边数）、信号模式（动态模式数）、语义词典（14类同义词）、行业画像（8大行业标准画像）",
      "#2563eb"
    ],
    [
      "知识库（4类）",
      "自愈规则（动态活跃规则数，同类错误≥2生成规则）、经验教训（动态经验条数，跨分析积累）、分析历史（最近101720条）、巡逻快照（动态企业数，巡逻基线）",
      "#7c3aed"
    ],
    [
      "核心配置参数（5项）",
    ...
    [
      "核心配置参数（5项）",
      "SCM因果：do-干预/反事实/混淆检测/因果链/1720条先验；知识库：线程安全/单例/101720条历史/JSON持久化；联网核查：三步法/90天缓存/chinatax.gov.cn；并行加速：多模块并行/DAG依赖图/可开关/提升30-50%；事件总线：pub/sub松耦合/14种事件/501720条日志/自动持久化",
      "#d97706"
    ]
  ]
});
