...
async function loadAnalyzeOverview() {
  ...
  html += '<div style="margin-bottom:48px;padding:24px;background:#f8fafc;border-radius:8px">'
    ...
    + '<div style="padding:12px 16px;margin-bottom:8px;background:#fff;border-radius:6px;border-left:3px solid #7c3aed"><strong>线索链追溯</strong> → 每条发现可追溯到 audit_chains.json 中的调查路径（' + pc('trailChains','391') + '条线索链）。触发率=已触发步骤/总步骤。</div>'
    + '<div style="padding:12px 16px;margin-bottom:8px;background:#fff;border-radius:6px;border-left:3px solid #059669"><strong>证据来源</strong> → how_found 字段记录稽查审计溯源（如"从发票数据中发现进项品名与销项品名不匹配"），换一个稽查员拿同样资料能得出同样结论。</div>'
    + '<div style="padding:12px 16px;margin-bottom:8px;background:#fff;border-radius:6px;border-left:3px solid #dc2626"><strong>一键分析</strong> → renderTaxDocReport 前端渲染时展示稽查溯源标记（▶ 规则追溯 / ▶ 线索追溯 / ▶ 证据来源）。</div>'
    + '<div style="padding:12px 16px;margin-bottom:8px;background:#fff;border-radius:6px;border-left:3px solid #f59e0b"><strong>证据链闭环</strong> → ≥60%触发率 + ≥3条规则 + ≥2数据域 → 闭环发现 → 强制升级高风险。234条证据链闭环触发→升级。</div>'
    + '<div style="padding:12px 16px;background:#fff;border-radius:6px;border-left:3px solid #0891b2"><strong>跨域证据链</strong> → 8条跨域证据链从 cross_domain_evidence.json 加载，多源数据交叉验证形成闭环。覆盖资金流+票据流+业务流三维验证。</div>'
    + '</div>'
    + '</div>';
