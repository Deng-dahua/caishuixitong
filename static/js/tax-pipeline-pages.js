...
function renderDomainAnalysisStatic() {
  ...
  html += '<div style="margin-bottom:10px;padding:20px 24px;background:#fff;border-radius:8px">'
    ...
    + '<strong>跨域关联推理</strong>（顶层）→ 将以上所有发现串联为11条跨域证据链，形成最终税务合规结论。'
    + '</div>'
    + '</div>';
...
function renderFilterResult(report) {
  ...
  var rules = [
    {title:'① 税务合规重点保护', icon:'🛡️', color:'#2563eb', badge:'12类', desc:'执行顺序：第一步（先于所有过滤规则）。12类税务合规重点发现（虚开发票/骗取出口退税/隐匿收入/账外经营行为/阴阳合同/资金回流/关联交易转移利润/虚假申报/骗取税收优惠/恶意注销/走逃失联/暴力抗税）在过滤器启动前即被标记为level_fixed=true，此后所有过滤操作都跳过这些发现。三层保护：后端修正→过滤器绕过→前端标记。设计哲学：宁可11条假阳性进入报告，也不能让1条真阳性被过滤掉。'},
...
// ══════════════════════════════════════════════════════════════
//  智哥行为准则页面 —— 7条编码行为规范
//  系统铁律(11条)已迁至 engine/memory.py，本页面仅保留约束智哥写代码的行为规范
// ══════════════════════════════════════════════════════════════
...
function renderAiRules(container) {
  ...
  var categories = [
    ...
    {name:'质量保障准则', icon:'✅', color:'#dc2626', id:'quality', desc:'确保代码质量和正确性的行为准则。违反任何一条都可能导致系统崩溃、数据错误或用户看到的页面与实际代码不一致。这四条准则覆盖了从"写完代码"到"用户看到"中间的每一个质量关口——自检→验证→影响分析→输出检查。每一条背后都有至少一次因违反该准则导致的严重事故。', rules:[
      ...
      {id:16, name:'交付前输出自检', level:'铁律', date:'2026-06-21',
       desc:'每写一段面向用户的输出文本（报告/页面/提示），写完必须逐句读一遍——以用户的视角，假装自己是第一次看到这段文字。四项检查：①变量语义正确——company_type是"企业类型"不是"行业"，不要把工商登记的企业类型标签当行业来用 ②分支穷举自洽——如果系统检测到"无BOM数据"，报告中就不能出现"建议补全BOM表"这样的建议，因为无信号不应输出对应建议 ③数值边界处理——1条发现不写"存在0类风险"（0就是没有，不应该出现在统计报告中），超出上限不截断（发票明细201条上限→"另行提供"而非直接丢弃）④句意通顺不自相矛盾——前面说"数据完整"后面又说"缺失N类资料"是不行的。读完确认无误再提交。这条准则定义了输出的"可读性"——代码对不代表用户看得懂、读得通。',
  ...
  html += '<p style="font-size:10px;color:#64748b;margin:0 0 10px;line-height:2">共'+totalRules+'条（'+tieLvCount+'铁律+'+zhunZeCount+'准则）· 2大分类 · 仅约束智哥写代码的行为规范。系统自身的11条铁律已迁至 engine/memory.py。</p>';
  ...
  html += '<b>系统铁律11条</b>（已迁至 <code>engine/memory.py</code>）：定义系统应该如何工作——科目name必须查DB、三号必须合并、ref_id必须精确匹配等。这些是"系统怎么做"的规范。<br><br>';
  ...
  html += '系统铁律编号（共11条）：铁律一~六（账务处理·engine/memory.py §06）+ 铁律七~十一（核心铁律·engine/memory.py §07）+ 铁律十二（跨模块内容一致性·engine/memory.py §08）。<br><br>';
...
function renderTaxWorkflow(container) {
  ...
  var stages = [
    { n:'①', title:'选案环节（第14-21条）', icon:'🎯', color:'#7c3aed',
    ...
    { n:'③', title:'审理环节（第46-1条）', icon:'⚖️', color:'#059669',
    ...
    { n:'④', title:'执行环节', icon:'💵', color:'#dc2626',
      ...
      points: [
        ...
        ['法律依据', '《征管法》第32条(滞纳金)/第41条(强制执行)/第88条(复议前置)']
      ]
    },
...
function renderDataAssets(container) {
  ...
  var assets = [
    ...
    { n:'②', name:'线索/证据链', icon:'🔗', color:'#7c3aed',
      desc:'线索链定义从风险到发现的调查路径（41条精写线索链）。证据链定义多源验证的闭环条件（21条精写证据链，≥2个独立数据源交叉印证）。',
...
function renderProcedureMapping(container) {
  ...
  h += '<p style="margin:0 0 10px">系统每一个功能模块都对应《税务合规工作规程》（国税发[2009]157号）的具体条款要求——确保系统产出<strong>不是凭空制造的</strong>，每一项分析、每一条结论都有法定的规程依据。12个功能模块完整覆盖从案源筛选到报告输出的<strong>全税务合规流程</strong>：第21-45条（检查实施）→第46条（审理审核）→第49条（审理意见）→第51条（报告格式）→第1条（程序合法）。</p>';
  ...
  var modules = [
    ...
    {n:'⑩',name:'合规门禁',clause:'第1条（程序合法）',desc:'质量标准（模板句清除/重复句合并/空描述删除/人性化表述/六要素完整/法律引用准确/具体数值/因果链/可执行建议/条款号/反跨复制/空占位符清除）+16项自省检查。全通过→绿色交付。',color:'#4f46e5'},
...
function renderIronLaws() {
  ...
  "title": "系统铁律编号体系",
  "icon": "⚖️",
  "subtitle": "系统铁律（engine/memory.py规则篇）=系统硬逻辑，不可违反。AI行为准则（前端页面）=编码规范。共11条系统铁律+7条AI准则，编号互不重叠，各司其职。",
  "stats": [
    [
      "11条",
      "系统铁律",
  ...
  "desc": "<p style=\"margin:0 0 10px\">系统铁律（11条）存储在engine/memory.py规则篇，是系统的硬逻辑，不可违反。AI行为准则（7条）在侧边栏AI行为准则页面展示。</p><p style=\"margin:0\">铁律一~六（账务处理）：科目name铁律、三号合并铁律、审计铁律、ref_id去重铁律、普票税额并入成本铁律、7分类禁止兜底铁律。铁律七~十一（核心规范）：规则=代码铁律、代码即承诺铁律、全行业适用铁律、主动关联更新铁律、方法论先行铁律。</p>",
  "cards": [
    [
      "铁律一~六（账务处理）",
...
function renderUnifiedDomainPanel(container) {
  ...
  h += '<p><b>资金流分析</b></p>';
  h += '<p><b>_domain_cash_flow_tracking</b>（资金全链路追踪）——输入：银行流水（必须）；最低：≥11条；输出finding.type：收款方异常、大额支付、频繁支付。</p>';
  h += '<p><b>_domain_fund_flow</b>（资金流向追踪）——输入：银行流水（必须）；最低：≥11条；输出finding.type：资金流向异常、单边大额资金。</p>';
  h += '<p><b>_domain_abnormal_transaction_time</b>（异常交易时间）——输入：银行流水（必须）；最低：≥11条；输出finding.type：非营业时间交易、节假日大额交易。</p>';
  h += '<p><b>_domain_personal_txn_risk</b>（个人交易风险）——输入：银行流水（必须）；最低：≥11条；输出finding.type：个人账户收款、对私转账高频。</p>';
  h += '<p><b>进销存分析</b></p>';
  ...
  h += '<p><b>_domain_profit_cashflow_contradiction</b>（利润现金流矛盾）——输入：科目余额表+银行流水（必须）；最低：各≥11条；输出finding.type：账面盈利现金枯竭、应收异常膨胀。</p>';
  h += '<p><b>_domain_revenue_time_investigation</b>（收入时间线调查）——输入：销项发票+银行流水（必须）；最低：各≥11条；输出finding.type：季末突击开票、收入集中化。</p>';
  h += '<p><b>_domain_extended_audit_rules</b>（扩展审查规则）——输入：原始数据（可选）；最低：无最低要求；输出finding.type：自定义信号。</p>';
  h += '<p><b>经营实质分析</b></p>';
  h += '<p><b>_domain_business_substance</b>（经营实质分析）——输入：银行流水+发票+工资+社保（应当）；至少3源（必须）；最低：各源≥11条；输出finding.type：人员不匹配、场地缺失、设备与产能不符。</p>';
  h += '<p><b>_domain_business_geography</b>（经营地理分析）——输入：发票+银行流水（必须）；工商登记地址（应当）；最低：≥11条；输出finding.type：注册地与经营地不一致、地址分散。</p>';
  ...
  h += '<p><b>_domain_voucher_account_anomaly</b>（凭证科目异常）——输入：会计凭证（必须）；最低：≥11条；输出finding.type：科目使用错误、摘要与科目不匹配。</p>';
  h += '<p><b>税务与社保</b></p>';
  ...
  h += '<p><b>行业对标与规则</b></p>';
  h += '<p><b>_domain_industry_benchmark</b>（行业基准对标）——输入：发票+科目余额表（必须）；最低：≥11条；输出finding.type：偏离行业基准。</p>';
  h += '<p><b>_domain_rule_engine_match</b>（规则系统匹配）——输入：所有数据（必须）；最低：无最低要求；输出finding.type：规则触发信号。</p>';
  h += '<p><b>_domain_tax_burden_analysis</b>（税负率分析）——输入：纳税申报表+发票（应当）；最低：≥11条；输出finding.type：税负率低于行业P25。</p>';
  h += '<p><b>_domain_trend_analysis</b>（趋势分析）——输入：科目余额表+发票（必须）；最低：≥11条；输出finding.type：指标异常波动。</p>';
  h += '<p><b>跨域分析链</b></p>';
  h += '<p><b>_domain_cross_analysis_chains</b>（跨域分析链）——输入：所有域发现（必须）；最低：域发现≥1条；输出finding.type：线索链+证据链并行配合→分析链跨域串联。</p>';
  h += '<p><b>补充税种</b></p>';
  h += '<p><b>_domain_stamp_duty_check</b>（印花税）——输入：合同+发票+银行流水（应当）；至少1源满足最低条数（必须）；最低：≥5条；输出finding.type：印花税漏缴。</p>';
  h += '<p><b>_domain_cit_reconciliation</b>（企业所得税汇算）——输入：科目余额表（必须）；凭证（应当）；最低：≥11条；输出finding.type：应纳税所得额调整。</p>';
...
function renderAGICausal() {
  ...
  "title": "因果推理层",
  "icon": "📊",
  "subtitle": "4个因果推理系统：SCM因果推理（do-干预/反事实/混淆/因果链）、元认知系统（四维推理质量评估）、法律三段论（11条结构化规则）、因果网络（条件概率+多信号联合预测）。",
  "stats": [
    [
      "4个",
      "因果系统",
...
function renderAGIKnowledge() {
  ...
  "cards": [
    [
      "统一知识库",
      "9域知识：政策库、因果网络、信号模式、语义词典（14类同义词）、风险同义词、行业画像、自愈规则、经验教训、分析历史（最近101条）。线程安全/全局单例/JSON持久化。",
      "#2563eb"
    ],
    [
      "自学习系统",
...
function renderAGIKnowledgeConfig() {
  ...
  "desc": "<p style=\"margin:0 0 10px\">9类知识库：①政策库policies（9条税收优惠）②因果网络causal_edges（信号→结论因果边）③信号模式signal_patterns（多信号组合）④语义词典semantic_dict（14类同义词）⑤行业画像industry_profiles（8大行业）⑥自愈规则healing_rules（错误→修正）⑦经验教训lessons（跨分析积累）⑧分析历史analysis_history（最近101条）⑨巡逻快照patrol_snapshots（巡逻基线）。</p>",
  "cards": [
    [
      "知识库（5类）",
    ...
    [
      "知识库（4类）",
      "自愈规则（动态活跃规则数，同类错误≥2生成规则）、经验教训（动态经验条数，跨分析积累）、分析历史（最近101条）、巡逻快照（动态企业数，巡逻基线）",
      "#7c3aed"
    ],
    [
      "核心配置参数（5项）",
    ...
    [
      "核心配置参数（5项）",
      "SCM因果：do-干预/反事实/混淆检测/因果链/9条先验；知识库：线程安全/单例/101条历史/JSON持久化；联网核查：三步法/90天缓存/chinatax.gov.cn；并行加速：多模块并行/DAG依赖图/可开关/提升30-50%；事件总线：pub/sub松耦合/14种事件/501条日志/自动持久化",
      "#d97706"
    ]
  ]
});
...
function renderMethodologyPart2(){
  ...
  h+='<details style="margin-bottom:10px"><summary style="font-size:10px;font-weight:700;color:#16233a;cursor:pointer;padding:10px 0">🔗 线索链（41条精写·调查路径）</summary><div class="live"><div id="au-chains-data"></div></div></details>';
  h+='<details style="margin-bottom:10px"><summary style="font-size:10px;font-weight:700;color:#16233a;cursor:pointer;padding:10px 0">🔒 证据链（21条精写·验证标准）</summary><div class="live"><div id="au-evidence-data"></div></div></details>';
  ...
  // ═══ 第四层·过滤 ═══
  h+='<h2>第四层 · 疑点过滤 —— 把101条信号淬成3条铁证</h2><p>布网阶段{{domain_functions}}个域同时发动，会产生大量粗糙信号。把粗糙信号淬成铁证，靠的是三道过滤器。</p>';
