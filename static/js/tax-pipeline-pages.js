...
function renderUnifiedDomainPanel(container) {
  ...
  h += '<p><b>1. 收款来源不匹配→隐匿收入推理链</b>——从单一的收款来源不匹配信号出发，逐层扩展分析范围。第一步确认不匹配的规模，第二步核实付款方身份（排除法人/股东的打款），第三步判断资金性质（经营收入还是非经营资金），第四步对无法解释的收款做出隐匿收入的判断。每一步都有回退路径——只要企业能提供合理解释，风险就会降级或消除。推理路径：从资金流分析到发票比对：逐名交叉比对全部收款方与销项发票购方名称；从发票比对到付款方身份核实：联网查询法定代表人/股东名单，比对未匹配的付款方；从付款方身份核实到资金性质判断：区分注资/借款/往来款/未开票收入四种可能；从资金性质判断到综合结论：无法说明来源的收款推定为未开票经营收入→隐匿收入风险。关联规则（共14条）：79、210、217、244、249、359、476、654、655、685、822、1226、1147、1503。</p>';
  h += '<p><b>2. 加工费来自外地→全链条经营异常推理链（点→面推理核心）</b>——从加工费这一个单点异常出发，扩展到面的推理。第一步发现加工费在外地，第二步判断商业合理性（本地是否有集群），第三步扩展到三组地址对比，第四步检测运输成本缺失，第五步得出物理不可能的结论。这是点→面推理的标准案例——单点异常可解释，但四点叠加无法解释——换一个税务合规员拿同样资料，同样会得出这个结论。推理路径：从发票分析到产业集群判断：分析企业所在地是否有同类加工产业集群；从产业集群判断到商业合理性判断：外地加工增加了运输成本和周期，舍近求远→商业逻辑存疑；从商业合理性判断到全链条地理分析：提取三组地址做城市交叉比对；从全链条地理分析到物理合理性判断：货物在N个城市间反复运输但无运输费→物理上不可能；从物理合理性判断到综合结论：全链条经营实质存疑→所有跨省交易的真实性需要逐笔验证。关联规则（共9条）：55、80、216、1500、1501、1502、1504、1505、1510。</p>';
  h += '<p><b>3. 进销品名不匹配→企业类型重判推理链（三层穿透）</b>——不从单一来源判定企业类型，而是通过三层递进穿透：工商登记看法律形式、发票数据看经营实质、加工信号看业务模式。三者不一致时以实质重于形式为原则。风险不是消除了，而是从品名问题转移到了加工真实性问题——这是风险转移而非风险消除。推理路径：从工商查询到发票推断：从全部发票品名关键词做66行业加权投票，得出发票推断行业；从发票推断到加工信号检测：筛查进项发票中是否含加工费+是否存在原材料采购；从加工信号检测到三层综合判断：工商登记=批发业(形式)+发票推断=纺织制造(实质)+加工信号=制造业(模式)；从三层综合判断到风险重定向：风险从「有进无销=隐匿收入」转移至「加工链条是否真实」。关联规则（共9条）：218、368、390、694、1017、1018、99、1444、1495。</p>';
  h += '<p><b>4. 有进无销→隐匿收入vs制造业加工推理链</b>——有进无销是税务合规中最常见的信号，但一刀切「高风险」是错误的。系统先判断企业类型——纯贸易企业有进无销确实是隐匿收入信号，但制造业企业采购原料加工成成品后品名不同，进销品名不匹配是正常的。诊断后风险焦点从「隐匿收入」转移至「加工链条是否真实」。推理路径：从进销存比对到行业模式判断：检查企业是纯贸易还是制造业——加工费+原材料信号；从行业模式判断到制造业诊断(如有加工信号)：有加工费+有原材料→制造业加工链条→风险焦点转移；从制造业诊断到BOM验证：要求企业提供BOM表验证原材料投入→加工→成品产出的逻辑；从BOM验证到综合结论：无法提供BOM→加工链条无法验证→有进无销仍可能是隐匿收入→回到高风险。关联规则（共15条）：20、21、22、23、55、63、64、80、210、216、338、537、1051、1358、1502。</p>';
  h += '<p><b>5. 合同缺失→交易真实性存疑推理链（四层分层）</b>——合同缺失不是一刀切的问题——不是所有交易都需要合同。系统通过四层自动分层，将缺失的影响精准定位到必签和应签两类，避免把所有供应商都视为「需要合同」（那会要求连加油站、酒店都签合同——不切实际）。分层判断使风险结论更精准、更可操作。推理路径：从资料完备度检测到合同需求分层：对每个供应商按品名/金额/类型四层自动分类：必签/应签/可免/小额；从合同需求分层到交易真实性验证：四流合一缺了合同流→这些交易的商业实质无法通过书面证据验证；从交易真实性验证到分层风险判断：必签类缺合同=高风险；应签类缺合同=中风险；可免类无合同=正常；从分层风险判断到综合结论：缺失合同→四流不合一→税务合规可逐笔质疑交易真实性→虚开发票嫌疑→印花税漏缴。关联规则（共13条）：14、15、16、17、63、64、72、216、310、313、712、1323、1505。</p>';
  h += '<p><b>6. 重物跨省缺运输→货物流断裂推理链</b>——从地址分布出发，结合行业重物属性和运输成本缺失，形成货物流物证链断裂的推理。关键在于行业的重物判断——不是所有行业都需要运输成本（如软件/咨询/设计等服务业），但纺织/建材/机械/食品等重物产业必须有。系统根据行业自适应地判断重物属性，不搞一刀切。推理路径：从地址分析到行业重物判断：根据行业分类判断产品是否为需运输的重物；从行业重物判断到运输成本检测：检索全部发票和银行流水中的运输/物流/快递/货运关键词；从运输成本检测到货物流判断：重物跨省运输必有运费（通常占货值3%-8%），零运费=货物流物证链断裂；从货物流判断到综合结论：货物流无法验证→三流合一缺了货物流→交易真实性存疑→进项税额+成本均面临否定风险。关联规则（共12条）：10、63、91、93、101、118、121、132、181、999、1500、1512。</p>';
  h += '<p><b>7. 费用发票占比异常→虚增成本推理链</b>——从费用结构异常出发，逐层深挖费用的真实性和关联性。关键判断在于费用与经营是否相关——招待费/差旅费可能有经营关联，但旅游/购物/生活用品等明显与经营无关。系统通过联网查询股东身份，将个人消费入账的风险精准定位。推理路径：从费用结构分析到费用明细核查：逐类筛查费用发票的品名/金额/时间/对方；从费用明细核查到股东关联判断：联网查询股东和法定代表人，判断费用是否与股东个人消费相关；从股东关联判断到税前扣除判断：与生产经营无关的支出不得税前扣除→需调增应纳税所得额；从税前扣除判断到综合结论：虚增成本费用→利润虚减→企业所得税少缴+个人所得税漏缴。关联规则（共12条）：133、135、138、144、310、425、748、880、895、1117、1170、1178。</p>';
  h += '<p><b>8. 进销比异常+零申报→账外经营推理链</b>——进销比异常是最强的税务合规信号之一——采购量远超销售量的本质是「买了货但没有开发票卖出去」。但不是一看到进销比异常就定「账外经营」——需要先排除存货积压的可能。如果存货没有增加、采购款已付、但销项极少——那就是典型的账外经营信号。结合收入端（收款远超开票），形成进销两端闭合的证据链。推理路径：从进销比分析到存货分析：检查存货周转率——如果存货也没有对应增加则货物去向成谜；从存货分析到资金流追踪：追踪银行付款去向——采购款付了但货去哪了？；从资金流追踪到收入端对比：付款正常+进货正常+销项极少→货物已售出但未开票→账外经营；从收入端对比到综合结论：进销比异常+零申报+收款开票偏差→完整的账外经营链条→严重隐匿收入。关联规则（共10条）：19、81、249、379、382、628、1144、1146、1424、1503。</p>';
  h += '<p><b>9. 供应链六员交叉→关联交易推断链（人员→发票→资金三段式跨域）</b>——通过人员信息→发票数据→资金流的三段式跨域推理：从发票中定位供应商/客户→联网核查其六员→与本企业六员交叉比对→发现重叠后核查资金流→确认关联交易并评估转移定价风险。推理路径：联网核查被查单位+供应商/客户六员；提取进销发票中的供应商/客户名称+金额；逐名交叉比对→发现重叠；核查重叠企业间资金往来。关联规则（共7条）：90、249、267、298、447、1504、1510。</p>';
  h += '<p><b>10. 购销闭环→虚开发票推理链（发票→进销存→资金三段式跨域）</b>——从发票数据发现供应商=客户→形成购销闭环→核查进销商品流转逻辑是否合理→核查资金流是否形成回流→判定虚开发票风险等级。推理路径：提取进项销售方+销项购买方；交叉比对：供应商∩客户；核查闭环企业间的商品流转逻辑；核查闭环企业间资金流向。关联规则（共8条）：266、267、272、284、298、1504、1510、1511。</p>';
  h += '<p><b>11. 仓库→产能→能耗→物流全链条经营实质分析</b>——单一矛盾可能是偶然→两个矛盾高度可疑→三个以上矛盾几乎可以定案。仓库装不下+设备产不出+电量不够用+物流费对不上=企业的账面经营纯粹是虚构的。推理路径：从租金发票推算仓储面积→对比库存体积→仓容矛盾？；从设备参数推算理论产能→对比账面产量→产能矛盾？；从电费/水费推算能耗→对比行业单耗标准→能耗矛盾？；从运输距离推算运费→对比账面运费→物流矛盾？；四个维度交叉验证→几个维度出现矛盾？→矛盾越多置信度越高→从单点怀疑升级为系统性造假判定。关联规则（共2条）：267、1500。</p>';
  h += '<p><b>12. 系统性造假综合判定模型(七维交叉)</b>——七维交叉验证模型——从经营实质、供应商穿透、资金流、三流合一、跨税种、关联交易七个维度综合评估企业造假可能性。单维异常是线索，多维异常是定案。七维全异常=企业账务大概率全面造假。推理路径：库存体积vs仓容/设备产能vs产量/用电量vs生产规模/人员vs产出→几维矛盾？；供应商五维评分(资质/时间/规模/价格/资金)→几个维度不通过？；收付款匹配度/个人收款占比/资金回流迹象/过桥账户→资金流异常度；发票/资金/物流/合同四流一致性→几流断裂？；增值税vs所得税/个税vs社保/印花税vs购销→几税申报不一致？；关联方交易占比/转移定价/资本弱化/费用分摊→关联交易风险度；七维综合评分→0-2维异常=低风险→3-4维=中风险→5-6维=高风险→7维=系统性造假→触发全面税务合规立案。关联规则（共12条）：63、90、130、226、228、267、272、364、374、447、450、1495。</p>';
  h += '<p><b>13. 隐匿收入综合判定模型</b>——三线归一法：资金偏差线(收款-开票)=来源分类线(未开票收入)=消费验证线(超申报消费)。三条线独立推算的隐匿收入金额应当大致相等。金额越接近→证据越强→结论越不可辩驳。推理路径：计算收款-开票偏差的绝对金额和相对比例→分月度/季度看趋势；对未匹配收款逐笔分类→借款/注资/代收/未开票/其他→计算各类占比；追踪个人账户收款和异常消费→推算最低隐匿收入规模；三指标交叉验证→偏差金额≈未开票收入≈隐匿消费→一致即定案。关联规则（共15条）：20、21、22、23、210、622、623、624、625、626、627、628、629、630、655。</p>';
  ...
  h += '<p><b>18. 全税种综合判定模型</b>——多税种交叉验证。单一税种异常可能是疏忽，3+税种同时异常=系统性偷逃税。综合判定降低误报。推理路径：收集各税种触发发现；逐税种比对申报vs实际；≥3税种异常→系统性偷逃税；多税种累计偷税额→处罚档次。关联规则（共4条）：210、228、880、1497。</p>';
  h += '<p><b>19. 物耗能耗产能三线归一分析</b>——产量可以从产能、电耗、物耗三个独立维度推算。三维交叉印证→单一维偏差可能是统计误差，三维全偏=造假。推理路径：设备铭牌×工时→理论最大产量；电费÷行业单位电耗→推算产量；原材料投入÷行业投入产出比→推算产量；三维推算→中位数vs账面产量；三维全偏低且偏差>30%→系统性造假。关联规则（共2条）：258、445。</p>';
