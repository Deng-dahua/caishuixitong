...
var MODULE_DEPS = {
  status: {
    ...
    downstream: [{name:'阶段4综合.py', desc:'读取状态生成综合报告'}, {name:'主程序.py', desc:'API返回引擎状态给前端'}, {name:'系统日志界面', desc:'展示执行记录'}]
  },
  rules: {
    upstream: [{name:'自学习.py', desc:'record_correction()记录用户纠正'}, {name:'人类学习.py', desc:'12项认知能力学习'}, {name:'用户报告页', desc:'编辑/审核/追问三通道'}],
    downstream: [{name:'管道.py', desc:'apply_correction_rules()下次自动应用'}, {name:'记忆.py', desc:'规则写入引擎记忆永久生效'}]
  },
  brain: {
    upstream: [{name:'AGI引擎.py', desc:'agi.answer()大模型智能推理'}, {name:'AGI核心.py', desc:'反事实/边界/泛化/单样本'}, {name:'AGI最终.py', desc:'工具/因果链/静默学习'}, {name:'调度器.py', desc:'get_director()智能调度'}, {name:'LLM客户端.py', desc:'云端/本地大模型API'}],
    downstream: [{name:'管道.py', desc:'AGI桥接参与资料分析'}, {name:'主程序.py', desc:'智能问答API服务前端追问'}]
  },
  quality: {
    upstream: [{name:'能力矩阵.py', desc:'check_quality_system()质量度量'}, {name:'自愈.py', desc:'auto_detect_inconsistencies()自动修复'}, {name:'自学习.py', desc:'run_compliance_gate()合规门禁12条'}, {name:'审计一致性.py', desc:'pre-commit钩子全量同步'}],
    downstream: [{name:'管道.py', desc:'综合报告质量评估'}, {name:'主程序.py', desc:'pre-commit钩子触发检查'}]
  },
  methods: {
    upstream: [{name:'AGI管道.py', desc:'create_pipeline()高级推理'}, {name:'语义推理器.py', desc:'SemanticReasoner语义匹配'}, {name:'因果网络.py', desc:'AutonomousReasoner因果推理'}, {name:'SCM推理器.py', desc:'结构因果模型推理'}, {name:'方法论加载器.py', desc:'方法库知识注入'}],
    downstream: [{name:'管道.py', desc:'高级分析阶段桥接AGI推理'}, {name:'AGI引擎.py', desc:'agi.answer()嵌入方法知识'}]
  },
  details: {
    upstream: [{name:'引擎/ 全部52个模块', desc:'所有引擎模块汇总统计'}, {name:'税务风险规则导出.json', desc:'1611条稽查指令'}, {name:'跨域线索.json + 跨域证据.json', desc:'437+781条线索和证据链'}],
    downstream: [{name:'运行仪表盘子模块', desc:'6个子模块共用数据源'}, {name:'系统日志界面', desc:'引擎运行记录追溯'}]
  }
};
...
function appendDependencyInfo(tabId) {
  ...
  h += '<div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:6px;padding:12px 16px">';
  h += '<div style="font-size:12px;font-weight:700;color:#0369a1;margin-bottom:8px">⬆ 上游（数据/功能的提供方）</div>';
  deps.upstream.forEach(function(u){
    h += '<div style="font-size:11px;color:#475569;line-height:1.8;margin-bottom:6px"><b style="color:#0f172a">' + u.name + '</b><br><span style="color:#94a3b8">' + u.desc + '</span></div>';
  });
  h += '</div>';
  h += '<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;padding:12px 16px">';
  h += '<div style="font-size:12px;font-weight:700;color:#15803d;margin-bottom:8px">⬇ 下游（数据/功能的消费方）</div>';
  deps.downstream.forEach(function(d){
    h += '<div style="font-size:11px;color:#475569;line-height:1.8;margin-bottom:6px"><b style="color:#0f172a">' + d.name + '</b><br><span style="color:#94a3b8">' + d.desc + '</span></div>';
  });
  h += '</div>';
  h += '</div>';
  area.innerHTML += h;
