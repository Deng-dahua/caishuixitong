...
function renderNegotiationTab() {
  ...
  var rules = [
    ...
    {id:'NEG-AUG-001',scene:'跨域联合核验',action:'生成核验任务',from:'经营费用缺口+运输缺口+场所信号',to:'经营实质联合核验',desc:'仅连接多个资料信号，优先核验经营场所、人员、履约和物流；不自动认定空壳或虚开。'},
    {id:'NEG-AUG-002',scene:'跨域联合核验',action:'生成核验任务',from:'个人收款+收款分类+发票交易',to:'个人收款性质核验',desc:'逐笔连接客户、合同、开票、入账和申报；个人账户信号本身不证明隐匿收入。'},
    {id:'NEG-AUG-003',scene:'跨域联合核验',action:'生成核验任务',from:'供应商结构+关联重叠+集中度',to:'供应商真实性与关联关系核验',desc:'核验工商、控制关系、合同、履约、物流和资金，不自动认定对倒或虚开。'},
    {id:'NEG-AUG-004',scene:'跨域联合核验',action:'生成核验任务',from:'红冲/作废+收款差异',to:'交易变化与红冲依据核验',desc:'核验退货、折让、退款、重开和申报调整，不自动认定虚假交易。'},
    {id:'NEG-AUG-005',scene:'跨域联合核验',action:'生成核验任务',from:'工资个税+社保口径',to:'人员与薪酬口径核验',desc:'核验人员身份、用工关系、工资、个税、社保和代发，不自动认定两套工资。'},
    {id:'NEG-AUG-006',scene:'跨域联合核验',action:'生成核验任务',from:'专票状态+进项税额',to:'进项与采购链核验',desc:'核验纳税人身份、抵扣状态、采购真实性和申报所属期，不推定隐匿采购。'},
    {id:'NEG-AUG-007',scene:'跨域联合核验',action:'生成核验任务',from:'个人收款+股东往来',to:'股东资金性质核验',desc:'核验经营收款、借款、注资、报销、分红等可能解释，不自动认定人格混同或偷逃税。'},
    {id:'NEG-AUG-008',scene:'跨域联合核验',action:'生成核验任务',from:'新办企业+大额开票',to:'经营能力与履约核验',desc:'核验人员、场所、采购、交付、资金和客户，不因新办或金额较大自动认定空壳。'},
    {id:'NEG-AUG-009',scene:'跨域联合核验',action:'生成核验任务',from:'劳务派遣+多处工资',to:'用工与扣缴口径核验',desc:'核验真实用工、派遣协议、人员服务和扣缴资料，不自动认定拆分工资。'},
    {id:'NEG-AUG-010',scene:'跨域联合核验',action:'生成核验任务',from:'境外付款+外汇信号',to:'跨境交易核验',desc:'核验交易性质、服务发生地、受益所有人、协定和代扣资料，再判断税务影响。'},
  ];

  var h = '';
  h += '<h3 style="font-size:18px;font-weight:700;color:#1a1a2e;margin:0 0 4px">🤝 跨域协商规则</h3>';
  h += '<p style="font-size:13px;color:#94a3b8;margin:0 0 20px">引擎在全部域分析完成后自动运行。具体条款待从官方有效文本核验协商规则：消解层具体条款待从官方有效文本核验 / 降级层具体条款待从官方有效文本核验 / 标记层具体条款待从官方有效文本核验 / 联合增强层具体条款待从官方有效文本核验。</p>';
  ...
  h += '<b>执行时序</b>：所有{{domain_functions}}个域分析函数独立完成→跨域协商引擎(run_negotiation)扫描all_findings→逐条匹配具体条款待从官方有效文本核验NEG规则→消解矛盾/降级不适/标记受限/增强多域→输出协商后findings→进入方法论过滤器→生成报告。协商引擎在Phase3交叉验证之后、方法论过滤器之前执行。<br><br>';
  h += '<b>代码位置</b>：<code>engine/cross_domain_negotiation.py</code>——具体条款待从官方有效文本核验协商规则以NEGOTIATION_RULES列表形式定义，每条规则含id/场景/动作/触发条件/执行逻辑五个字段。新增协商规则只需在列表中追加新条目，无需修改其他代码。<br><br>';
...
var MODULE_DEPS = {
  ...
  quality: {
    upstream: [{name:'能力矩阵.py',desc:'check_quality_system()质量度量'},{name:'自愈.py',desc:'auto_detect_inconsistencies()自动修复'},{name:'自学习.py',desc:'合规门禁具体条款待从官方有效文本核验铁律'},{name:'审计一致性.py',desc:'pre-commit全量同步'}],
...
async function renderQualityDashboard(container) {
  ...
  h += '<div class="qa-layer"><div class="num" style="background:#7c3aed">2</div><div class="body"><b>方法论体系</b><br>具体条款待从官方有效文本核验方法论约束分析逻辑边界，防止推断超出数据支撑范围。六大分析框架覆盖全流程。</div></div>';
...
function renderEngineThink(container) {
  ...
  h += '<div><a href="javascript:navigateTo(\'eng-qual\')" style="color:#2563eb">质量保障</a><br><span style="color:#94a3b8">具体条款待从官方有效文本核验方法论约束推理逻辑边界</span></div>';
  h += '</div></div>';
  h += '<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:16px 20px">';
  h += '<div style="font-size:12px;font-weight:700;color:#15803d;margin-bottom:12px;padding-bottom:6px;border-bottom:1px solid #bbf7d0">⬇ 下游（消费方）</div>';
  h += '<div style="font-size:11px;color:#475569;line-height:2.0">';
  ...
  h += '<p style="margin:0 0 16px">推理引擎由四个子引擎组成：<strong>语义推理器</strong>负责将自然语言描述的发现与规则库中的标准表述进行语义匹配；<strong>因果网络</strong>构建信号之间的条件概率关系，当多个异常信号同时出现时自动计算因果关联强度；<strong>SCM推理器</strong>使用结构因果模型进行do-干预和反事实推理；<strong>方法论增强器</strong>在推理过程中注入具体条款待从官方有效文本核验方法论知识，使推理遵循税务合规最佳实践。</p>';
